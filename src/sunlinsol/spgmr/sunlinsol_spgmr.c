                                 ...
                                 N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  if (preOnLeft) {
    ...
    SUNCheckCallLastErrNoRet(N_VScale(ONE, V[0], vtemp), sunctx);
  }
  ...
  *res_norm = r_norm = beta = SUNRsqrt(r_norm);
  ...
  for (ntries=0; ntries<=max_restarts; ntries++) {
    ...
    for (l=0; l<l_max; l++) {
      ...
      if (preOnRight) {
        SUNCheckCallLastErrNoRet(N_VScale(ONE, vtemp, V[l_plus_1]), sunctx);
      ...
      if (preOnLeft) {
        ...
        SUNCheckCallLastErrNoRet(N_VScale(ONE, V[l_plus_1], vtemp), sunctx);
      }

      /* Apply left scaling: V[l+1] = s1 P1_inv A P2_inv s2_inv V[l] */
      if (scale1) {
        SUNCheckCallLastErrNoRet(N_VProd(s1, vtemp, V[l_plus_1]), sunctx);
      } else {
        SUNCheckCallLastErrNoRet(N_VScale(ONE, vtemp, V[l_plus_1]), sunctx);
      }
      ...
      if (gstype == SUN_CLASSICAL_GS) {
        SUNCheckCallNoRet(SUNClassicalGS(V, Hes, l_plus_1, l_max,
                                    &(Hes[l_plus_1][l]), cv, Xv),
                     sunctx);
      } else {
        SUNCheckCallNoRet(SUNModifiedGS(V, Hes, l_plus_1, l_max, &(Hes[l_plus_1][l])),
                     sunctx);
      }
      ...
      *res_norm = rho = SUNRabs(rotation_product*r_norm);
      ...
      SUNCheckCallLastErrNoRet(N_VScale(ONE/Hes[l_plus_1][l], V[l_plus_1], V[l_plus_1]), sunctx);
    }
    ...
    if (converged) {
      ...
      if (scale2) {
        SUNCheckCallLastErrNoRet(N_VDiv(xcor, s2, xcor), sunctx);
      }
      ...
      if (preOnRight) {
        ...
        SUNCheckCallLastErrNoRet(N_VScale(ONE, xcor, vtemp), sunctx);
      }

      /* Add vtemp to initial x to get final solution x, and return */
      if (*zeroguess) {
        SUNCheckCallLastErrNoRet(N_VScale(ONE, vtemp, x), sunctx);
      } else {
        SUNCheckCallLastErrNoRet(N_VLinearSum(ONE, x, ONE, vtemp, x), sunctx);
      }
                                ...
                                long int* leniwLS)
{
  ...
  if (SPGMR_CONTENT(S)->vtemp->ops->nvspace) {
    SUNCheckCallLastErrNoRet(N_VSpace(SPGMR_CONTENT(S)->vtemp, &lrw1, &liw1), S->sunctx);
  } else {
    lrw1 = liw1 = 0;
  }

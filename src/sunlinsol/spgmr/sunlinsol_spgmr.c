                                 ...
                                 N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  if (preOnLeft) {
    ...
    SUNCheckCallLastErrNoRet(N_VScale(ONE, V[0], vtemp));
  }
  ...
  *res_norm = r_norm = beta = SUNRsqrt(r_norm);
  ...
  for (ntries=0; ntries<=max_restarts; ntries++) {
    ...
    for (l=0; l<l_max; l++) {
      ...
      if (preOnRight) {
        SUNCheckCallLastErrNoRet(N_VScale(ONE, vtemp, V[l_plus_1]));
      ...
      if (preOnLeft) {
        ...
        SUNCheckCallLastErrNoRet(N_VScale(ONE, V[l_plus_1], vtemp));
      }

      /* Apply left scaling: V[l+1] = s1 P1_inv A P2_inv s2_inv V[l] */
      if (scale1) {
        SUNCheckCallLastErrNoRet(N_VProd(s1, vtemp, V[l_plus_1]));
      } else {
        SUNCheckCallLastErrNoRet(N_VScale(ONE, vtemp, V[l_plus_1]));
      }
      ...
      *res_norm = rho = SUNRabs(rotation_product*r_norm);
      ...
      SUNCheckCallLastErrNoRet(N_VScale(ONE/Hes[l_plus_1][l], V[l_plus_1], V[l_plus_1]));
    }
    ...
    if (converged) {
      ...
      if (preOnRight) {
        ...
        SUNCheckCallLastErrNoRet(N_VScale(ONE, xcor, vtemp));
      }
      ...
      if (*zeroguess) {
        ...
        SUNCheckCallLastErrNoRet(N_VLinearSum(ONE, x, ONE, vtemp, x));
      }
                                ...
                                long int* leniwLS)
{
  ...
  if (SPGMR_CONTENT(S)->vtemp->ops->nvspace) {
    SUNCheckCallLastErrNoRet(N_VSpace(SPGMR_CONTENT(S)->vtemp, &lrw1, &liw1));
  } else {
    lrw1 = liw1 = 0;
  }

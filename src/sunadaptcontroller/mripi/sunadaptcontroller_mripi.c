...
SUNErrCode SUNAdaptController_EstimateMRISteps_MRIPI(
  ...
  sunrealtype dsm, sunrealtype* Hnew, sunrealtype* hnew)
{
  ...
  const sunrealtype es1 = ONE / SUNMAX(MRIPI_BIAS(C) * DSM, TINY);
  const sunrealtype es2 = ONE / MRIPI_ESP(C);
  const sunrealtype ef1 = ONE / SUNMAX(MRIPI_BIAS(C) * dsm, TINY);
  const sunrealtype ef2 = ONE / MRIPI_EFP(C);
...
SUNErrCode SUNAdaptController_SetErrorBias_MRIPI(SUNAdaptController C,
                                                 sunrealtype bias)
{
  SUNFunctionBegin(C->sunctx);
...
SUNErrCode SUNAdaptController_UpdateMRIH_MRIPI(SUNAdaptController C,
...
SUNErrCode SUNAdaptController_Space_MRIPI(SUNAdaptController C, long int* lenrw,
                                          long int* leniw)
{
  SUNFunctionBegin(C->sunctx);

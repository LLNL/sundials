...
void N_VSpace_Petsc(N_Vector v, indextype *lrw, indextype *liw)
{
  MPI_Comm comm;
  int npes;
...
{
  indextype i;
  indextype N        = NV_LOCLENGTH_PTC(x);
  indextype N_global = NV_GLOBLENGTH_PTC(x);
  MPI_Comm comm     = NV_COMM_PTC(x);
  Vec *xv = NV_PVEC_PTC(x);
...
realtype N_VMinQuotient_Petsc(N_Vector num, N_Vector denom)
{
  ...
  indextype i; 
  indextype N    = NV_LOCLENGTH_PTC(num);

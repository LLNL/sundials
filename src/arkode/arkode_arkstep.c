                  ...
                  realtype t0, ARKVecResizeFn resize, void *resize_data)
{
  ...
  if (step_mem->NLS)  {
    ...
    ier = ARKStepSetNonlinearSolver(ark_mem, NLS);
    if (ier != ARK_SUCCESS) {
      arkProcessError(ark_mem, ARK_MEM_FAIL, "ARKode::ARKStep",
...
int arkStep_Init(void* arkode_mem, int init_type)
{
  ...
  if (step_mem->mass_mem != NULL) {
    ...
    if (step_mem->msetup != NULL) {
      ier = step_mem->msetup((void *) ark_mem, ark_mem->tempv1,
  ...
  /* Check that lsolve function exists (if needed) */
  ...
  /* Initialize the nonlinear solver object (if supplied) */
  if (step_mem->NLS) {
    ier = arkStep_NlsInit(ark_mem);
    if (ier != ARK_SUCCESS) {
      arkProcessError(ark_mem, ARK_NLS_INIT_FAIL, "ARKode::ARKStep", "arkStep_Init",
...
/*---------------------------------------------------------------
 ...
 If present, it calls the supplied SUNNonlinearSolver object to
 perform the solve.  Otherwise, it calls one of
 arkStep_NlsAccelFP, arkStep_Ls or arkStep_NlsNewton to do the
 work.
...
int arkStep_Nls(ARKodeMem ark_mem, int nflag)
{
  ...
  /* call the appropriate solver: if a NLS module is supplied,
  ...
  if (step_mem->NLS) {
    ...
    if (step_mem->linear) {
      zcor0 = ark_mem->tempv2;
      N_VConst(ZERO, zcor0);
      retval = arkStep_Ls(ark_mem, nflag);
      N_VLinearSum(ONE, step_mem->zpred, ONE, ark_mem->tempv2, ark_mem->ycur);
      return(retval);
    }
    ...
    zcor0 = ark_mem->tempv2;
    N_VConst(ZERO, zcor0);
    retval = arkStep_NlsNewton(ark_mem, nflag);
    N_VLinearSum(ONE, step_mem->zpred, ONE, ark_mem->tempv2, ark_mem->ycur);
    return(retval);
  }
...
int arkStep_NlsNewton(ARKodeMem ark_mem, int nflag)
{
  ...
  z0   = step_mem->zpred;  /* rename zpred as z0 for readability */
  is   = step_mem->istage;
  ...
  /* Looping point for attempts at solution of the nonlinear system:
     ...
     Copy the prediction (z0) into the output (z).
  ...
  for(;;) {
    ...
    /* compute nonlinear residual, and put into b */
    retval = arkStep_NlsResidual(zcor, b, ark_mem);
    if (retval != SUN_NLS_SUCCESS) break;

    /* update system matrix/factorization if necessary */
    if (callLSetup) {
      ier = arkStep_NlsLSetup(zcor, b, jbad, &(step_mem->jcur), ark_mem);
      if (ier != 0) break;
    }
    ...
    /***********************************
     ...
     - Upon entry, the predicted solution is held in z0.
    ...
    */
    ...
    for(;;) {
      ...
      /* if successful update Jacobian status, copy solution into ycur, and return */
      if (retval == ARK_SUCCESS) {
      ...
      /* compute the nonlinear residual, and put into b */
      retval = arkStep_NlsResidual(zcor, b, ark_mem);
      if (retval != SUN_NLS_SUCCESS) break;

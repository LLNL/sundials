                    ...
                    SUNContext sunctx)
{
  ...
  if (fe == NULL && fi == NULL)
  {
    arkProcessError(NULL, ARK_ILL_INPUT, "ARKODE::ARKStep", "ARKStepCreate",
                    MSG_ARK_NULL_F);
    return (NULL);
  }
  ...
  if (ark_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "ARKStepCreate",
                    MSG_ARK_NO_MEM);
    return (NULL);
  }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int ARKStepResize(void* arkode_mem, N_Vector y0, realtype hscale, realtype t0,
    ...
    for (i = 0; i < step_mem->stages; i++)
    {
                        ...
                        &step_mem->Fe[i]))
      {
        arkProcessError(ark_mem, ARK_MEM_FAIL, "ARKODE::ARKStep",
                        "ARKStepResize", "Unable to resize vector");
        return (ARK_MEM_FAIL);
      }
    }
  }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int ARKStepReInit(void* arkode_mem, ARKRhsFn fe, ARKRhsFn fi, realtype t0,
                  N_Vector y0)
{
  ARKodeMem ark_mem;
  ARKodeARKStepMem step_mem;
  ...
  {
    arkProcessError(ark_mem, ARK_NO_MALLOC, "ARKODE::ARKStep", "ARKStepReInit",
                    MSG_ARK_NO_MALLOC);
  ...
  if (fe == NULL && fi == NULL)
  {
    arkProcessError(ark_mem, ARK_ILL_INPUT, "ARKODE::ARKStep", "ARKStepReInit",
                    MSG_ARK_NULL_F);
...
int ARKStepRootInit(void* arkode_mem, int nrtfn, ARKRootFn g)
{
  ...
  if (arkode_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "ARKStepRootInit",
                    MSG_ARK_NO_MEM);
    return (ARK_MEM_NULL);
  }
  ark_mem = (ARKodeMem)arkode_mem;
                  ...
                  realtype* tret, int itask)
{
  ...
  if (arkode_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "ARKStepEvolve",
                    MSG_ARK_NO_MEM);
    return (ARK_MEM_NULL);
  }
  ark_mem = (ARKodeMem)arkode_mem;
...
int ARKStepGetDky(void* arkode_mem, realtype t, int k, N_Vector dky)
{
  ...
  if (arkode_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "ARKStepGetDky",
                    MSG_ARK_NO_MEM);
    return (ARK_MEM_NULL);
  }
  ark_mem = (ARKodeMem)arkode_mem;
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int arkStep_GetGammas(void* arkode_mem, realtype* gamma, realtype* gamrat,
                      booleantype** jcur, booleantype* dgamma_fail)
{
  ARKodeMem ark_mem;
  ARKodeARKStepMem step_mem;
  int retval;
...
int arkStep_Init(void* arkode_mem, int init_type)
{
  ...
  if (init_type == FIRST_INIT)
  {
    ...
    /* Ensure that if adaptivity is enabled, then method includes embedding
     * coefficients */
    if (!ark_mem->fixedstep && (step_mem->p == 0))
    {
      arkProcessError(ark_mem, ARK_ILL_INPUT, "ARKODE::ARKStep",
                      "arkStep_Init", "Adaptive timestepping cannot be performed without embedding coefficients");
      return (ARK_ILL_INPUT);
    }
    ...
    /* If the bootstrap predictor is enabled, signal to shared arkode module
       that fullrhs is required after each step */
  ...
  if (step_mem->NLS)
  {
    ...
    if (retval != ARK_SUCCESS)
    {
      arkProcessError(ark_mem, ARK_NLS_INIT_FAIL, "ARKODE::ARKStep",
                      "arkStep_Init",
...
int arkStep_FullRHS(void* arkode_mem, realtype t, N_Vector y, N_Vector f, int mode)
{
  ...
  case ARK_FULLRHS_END:
    ...
    if (step_mem->explicit &&
        (SUNRabs(step_mem->Be->c[step_mem->stages - 1] - ONE) > TINY))
    {
      recomputeRHS = SUNTRUE;
    }
    if (step_mem->implicit &&
        (SUNRabs(step_mem->Bi->c[step_mem->stages - 1] - ONE) > TINY))
    {
      recomputeRHS = SUNTRUE;
    }
...
int arkStep_TakeStep_Z(void* arkode_mem, realtype* dsmPtr, int* nflagPtr)
{
  ...
  /* if problem will involve no algebraic solvers, initialize nflagPtr to
   * success */
  ...
  if (step_mem->NLS)
  {
    if ((step_mem->NLS)->ops->setup)
    {
      ...
      N_VConst(ZERO, zcor0); /* set guess to all 0 (since using
  ...
  for (is = 0; is < step_mem->stages; is++)
  {
    ...
    /* NOTE: with internally inconsistent IMEX methods (c_i^E != c_i^I) the
       value of tcur corresponds to the stage time from the implicit table
       (c_i^I). */
    ...
    if (step_mem->explicit)
    {
      ...
      if (step_mem->expforcing)
      {
        ...
        nvec     = 1;
        arkStep_ApplyForcing(step_mem,
                             ark_mem->tn + step_mem->Be->c[is] * ark_mem->h,
                             ONE, &nvec);
        N_VLinearCombination(nvec, cvals, Xvecs, step_mem->Fe[is]);
      }
    }

    /* if using a time-dependent mass matrix, update Fe[is] and/or Fi[is] with
     * M(t)^{-1} */
  ...
  else { retval = arkStep_ComputeSolutions(ark_mem, dsmPtr); }
...
#if SUNDIALS_LOGGING_LEVEL >= SUNDIALS_LOGGING_INFO
  SUNLogger_QueueMsg(ARK_LOGGER, SUN_LOGLEVEL_INFO, "ARKODE::arkStep_TakeStep_Z",
                          ...
                          ARKodeMem* ark_mem, ARKodeARKStepMem* step_mem)
{
  ...
  if (arkode_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", fname, MSG_ARK_NO_MEM);
    return (ARK_MEM_NULL);
  }
  *ark_mem = (ARKodeMem)arkode_mem;
  if ((*ark_mem)->step_mem == NULL)
  {
    arkProcessError(*ark_mem, ARK_MEM_NULL, "ARKODE::ARKStep", fname,
                    MSG_ARKSTEP_NO_MEM);
    return (ARK_MEM_NULL);
  }
  *step_mem = (ARKodeARKStepMem)(*ark_mem)->step_mem;
...
int arkStep_CheckButcherTables(ARKodeMem ark_mem)
{
  ...
  if (step_mem->explicit && step_mem->Be == NULL)
  {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                    "arkStep_CheckButcherTables", "explicit table is NULL!");
  ...
  {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                    "arkStep_CheckButcherTables", "implicit table is NULL!");
    return (ARK_INVALID_TABLE);
  }
  ...
  {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                    "arkStep_CheckButcherTables", "stages < 1!");
    return (ARK_INVALID_TABLE);
  }
  ...
  {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                    "arkStep_CheckButcherTables", "method order < 1!");
    return (ARK_INVALID_TABLE);
  }
  ...
  {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                    "arkStep_CheckButcherTables", "embedding order < 1!");
    return (ARK_INVALID_TABLE);
  }
  ...
  if ((step_mem->p > 0) && (!ark_mem->fixedstep))
  {
    if (step_mem->implicit)
    {
      ...
      {
        arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                        "arkStep_CheckButcherTables", "no implicit embedding!");
        return (ARK_INVALID_TABLE);
      }
    }
    if (step_mem->explicit)
    {
      ...
      {
        arkProcessError(ark_mem, ARK_INVALID_TABLE, "ARKODE::ARKStep",
                        "arkStep_CheckButcherTables", "no explicit embedding!");
        return (ARK_INVALID_TABLE);
      }
    }
  }
...
int arkStep_Predict(ARKodeMem ark_mem, int istage, N_Vector yguess)
{
  ...
  if (ark_mem->step_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "arkStep_Predict",
                    MSG_ARKSTEP_NO_MEM);
    return (ARK_MEM_NULL);
  }
  step_mem = (ARKodeARKStepMem)ark_mem->step_mem;
  ...
  {
    arkProcessError(ark_mem, ARK_MEM_NULL, "ARKODE::ARKStep", "arkStep_Predict",
  ...
  case 2:
    ...
    /***** Interpolatory Predictor 2 -- decrease order w/ increasing level of
     * extrapolation *****/
...
int arkStep_StageSetup(ARKodeMem ark_mem, booleantype implicit)
{
  ...
  if (ark_mem->step_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE::ARKStep", "arkStep_StageSetup",
                    MSG_ARKSTEP_NO_MEM);
    return (ARK_MEM_NULL);
  }
  step_mem = (ARKodeARKStepMem)ark_mem->step_mem;
  ...
  if (implicit)
  {
    ...
    step_mem->gamrat = (ark_mem->firststage)
                         ? ONE
                         : step_mem->gamma / step_mem->gammap; /* protect x/x
                                                                  != 1.0 */
  }
  ...
  if (implicit && (step_mem->predictor == 5))
  {
    ...
    else { N_VConst(ZERO, step_mem->sdata); }
    return (ARK_SUCCESS);
  }
...
/*------------------------------------------------------------------------------
  ...
  arrays in the step_mem structure.  The dereferenced input *nvec should
  indicate the next available entry in the cvals/Xvecs arrays.  The input 's' is
  a scaling factor that should be applied to each of these coefficients.
  ----------------------------------------------------------------------------*/

void arkStep_ApplyForcing(ARKodeARKStepMem step_mem, realtype t, realtype s,
                          int* nvec)
{
  realtype tau, taui;
  int i;

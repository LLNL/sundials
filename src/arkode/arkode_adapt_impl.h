...
#define ETAMXF                                                \
  RCONST(0.3) /* step size reduction factor on multiple error \
                 test failures (multiple implies >= SMALL_NEF) */
#define ETAMIN                                                 \
  RCONST(0.1) /* smallest allowable step size reduction factor \
                 on an error test failure */
#define ETACF                                             \
  RCONST(0.25) /* step size reduction factor on nonlinear \
                  convergence failure */
#define SMALL_NEF                                       \
...
int arkAdapt(void* arkode_mem, ARKodeHAdaptMem hadapt_mem, N_Vector ycur,
             realtype tcur, realtype hcur, realtype dsm, long int nst);
int arkAdaptPID(ARKodeHAdaptMem hadapt_mem, int k, realtype hcur, realtype ecur,
                realtype* hnew);
int arkAdaptPI(ARKodeHAdaptMem hadapt_mem, int k, realtype hcur, realtype ecur,
               realtype* hnew);
int arkAdaptI(ARKodeHAdaptMem hadapt_mem, int k, realtype hcur, realtype ecur,
              realtype* hnew);

...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int ERKStepResize(void* arkode_mem, N_Vector y0, realtype hscale, realtype t0,
...
int erkStep_Init(void* arkode_mem, int init_type)
{
  ...
  if (init_type == RESIZE_INIT || init_type == RESET_INIT) return (ARK_SUCCESS);
...
int erkStep_TakeStep(void* arkode_mem, realtype* dsmPtr, int* nflagPtr)
{
  ...
  *nflagPtr = ARK_SUCCESS;
  ...
  SUNLogger_QueueMsg(ARK_LOGGER, SUN_LOGLEVEL_INFO, "ARKODE::erkStep_TakeStep",
                     "start-stage",

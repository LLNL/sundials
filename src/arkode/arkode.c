...
/*---------------------------------------------------------------
  ...
  *StepCreate.
  ...
  ---------------------------------------------------------------*/
int arkResize(ARKodeMem ark_mem, N_Vector y0, realtype hscale, realtype t0,
...
/*---------------------------------------------------------------
  ...
  *StepSetStopTime function), in which case if a solver step
  ...
  ---------------------------------------------------------------*/
int arkEvolve(ARKodeMem ark_mem, realtype tout, N_Vector yout, realtype* tret,
              int itask)
{
  long int nstloc;
  ...
  for (;;)
  {
    ...
    for (;;)
    {
      ...
      /* when fixed time-stepping is enabled, 'success' == successful stage
         solves (checked in previous block), so just enforce no step size change
       */
      if (ark_mem->fixedstep)
      {
        ark_mem->eta = ONE;
      ...
      /* if we've made it here then no nonrecoverable failures occurred; someone
         above has recommended an 'eta' value for the next step -- enforce
         bounds on that value and set upcoming step size */
    ...
    /* If step attempt loop succeeded, complete step (update current time,
       solution, error stepsize history arrays; call user-supplied step
       postprocessing function) (added stuff from arkStep_PrepareNextStep --
       revisit) */
...
int arkInit(ARKodeMem ark_mem, realtype t0, N_Vector y0, int init_type)
{
  ...
  if (init_type == FIRST_INIT)
  {
    ...
    /* Indicate that evaluation of the full RHS is not required after each step,
       this flag is updated to SUNTRUE by the interpolation module
       initialization function and/or the stepper initialization function in
       arkInitialSetup */
    ark_mem->call_fullrhs = SUNFALSE;
...
booleantype arkCheckTimestepper(ARKodeMem ark_mem)
{
  if ((ark_mem->step_init == NULL) || (ark_mem->step == NULL) ||
      (ark_mem->step_mem == NULL) || (ark_mem->step_fullrhs == NULL))
  {
    return (SUNFALSE);
  }
  return (SUNTRUE);
...
booleantype arkAllocVectors(ARKodeMem ark_mem, N_Vector tmpl)
{
  ...
  if (!arkAllocVec(ark_mem, tmpl, &ark_mem->fn)) { return (SUNFALSE); }
...
#if SUNDIALS_LOGGING_LEVEL >= SUNDIALS_LOGGING_INFO
  SUNLogger_QueueMsg(ARK_LOGGER, SUN_LOGLEVEL_INFO, "ARKODE::arkCompleteStep",
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int arkPredict_Bootstrap(ARKodeMem ark_mem, realtype hj, realtype tau, int nvec,
                         realtype* cvals, N_Vector* Xvecs, N_Vector yguess)
{
  realtype a0, a1, a2;
  int i, retval;

...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int mriStep_GetGammas(void* arkode_mem, realtype* gamma, realtype* gamrat,
...
int mriStep_Init(void* arkode_mem, int init_type)
{
  ...
  if (init_type == FIRST_INIT)
  {
    ...
    step_mem->Ae_row = (realtype*)calloc(step_mem->stages, sizeof(realtype));
    ark_mem->lrw += step_mem->stages;
    ...
    step_mem->Ai_row = (realtype*)calloc(step_mem->stages, sizeof(realtype));
    ark_mem->lrw += step_mem->stages;
...
int mriStep_TakeStep(void* arkode_mem, realtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  if (step_mem->NLS)
  {
    ...
    {
      N_VConst(ZERO, ark_mem->tempv3); /* set guess to 0 for predictor-corrector
                                          form */
...
#if SUNDIALS_LOGGING_LEVEL >= SUNDIALS_LOGGING_INFO
  SUNLogger_QueueMsg(ARK_LOGGER, SUN_LOGLEVEL_INFO, "ARKODE::mriStep_TakeStep",
...
int MRIStepInnerStepper_AddForcing(MRIStepInnerStepper stepper, realtype t,
                                   N_Vector f)
{
  realtype tau, taui;
  int i;
...
int mriStepInnerStepper_HasRequiredOps(MRIStepInnerStepper stepper)
{
  ...
  if (stepper->ops->evolve && stepper->ops->fullrhs) { return ARK_SUCCESS; }
  else { return ARK_ILL_INPUT; }
}

/* Evolve the inner (fast) ODE */
int mriStepInnerStepper_Evolve(MRIStepInnerStepper stepper, realtype t0,
...
int mriStepInnerStepper_FullRhs(MRIStepInnerStepper stepper, realtype t,
...
int mriStepInnerStepper_Reset(MRIStepInnerStepper stepper, realtype tR,
                              N_Vector yR)
{
  if (stepper == NULL) { return ARK_ILL_INPUT; }
  if (stepper->ops == NULL) { return ARK_ILL_INPUT; }

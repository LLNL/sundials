...
int mriStep_Init(void* arkode_mem, int init_type)
{
  ...
  if (init_type == FIRST_INIT) {
    ...
    if (retval != ARK_SUCCESS) {
      arkProcessError(ark_mem, ARK_ILL_INPUT, __LINE__, __func__, __FILE__, "Error in coupling table");
      return(ARK_ILL_INPUT);
    }
  ...
  *dsmPtr = ZERO;
  ...
  if (step_mem->explicit_rhs)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "Fse[0] =", "");
    SUNCheckCallLastErrNoRet(N_VPrintFile(step_mem->Fse[0], ark_mem->sunctx->logger->debug_fp));
  }
  if (step_mem->implicit_rhs)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "Fsi[0] =", "");
    SUNCheckCallLastErrNoRet(N_VPrintFile(step_mem->Fsi[0], ark_mem->sunctx->logger->debug_fp));
  }
#endif
    ...
    if (is < step_mem->stages - 1 && step_mem->stage_map[is] > -1) {
      ...
      if (step_mem->explicit_rhs) {
        ...
        N_VPrintFile(step_mem->Fse[step_mem->stage_map[is]],
                     ark_mem->sunctx->logger->debug_fp);
#endif
      }
                          ...
                          ARKodeMem *ark_mem, ARKodeMRIStepMem *step_mem)
{
  ...
  *ark_mem = (ARKodeMem) arkode_mem;
  if ((*ark_mem)->step_mem==NULL) {
    arkProcessError(*ark_mem, ARK_MEM_NULL, __LINE__, __func__, __FILE__, MSG_MRISTEP_NO_MEM);
    return(ARK_MEM_NULL);
  }
  *step_mem = (ARKodeMRIStepMem) (*ark_mem)->step_mem;
  return(ARK_SUCCESS);
...
int mriStep_CheckCoupling(ARKodeMem ark_mem)
{
  ...
  if (step_mem->MRIC->stages < 1) {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, __LINE__, __func__, __FILE__, "stages < 1!");
    return(ARK_INVALID_TABLE);
  }

  /* check that method order q > 0 */
  if (step_mem->MRIC->q < 1) {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, __LINE__, __func__, __FILE__, "method order < 1");
    return(ARK_INVALID_TABLE);
  }

  /* check that embedding order p > 0 (if adaptive) */
  if ((step_mem->MRIC->p < 1) && (!ark_mem->fixedstep)) {
    arkProcessError(ark_mem, ARK_INVALID_TABLE, __LINE__, __func__, __FILE__, "embedding order < 1");
    return(ARK_INVALID_TABLE);
  }
                                ...
                                int stage, realtype cdiff)
{
  ...
  for (k = 0; k < nmat; k++)
  {
    SUNLogger_QueueMsg(ark_mem->sunctx->logger, SUN_LOGLEVEL_DEBUG,
                       ...
                       "forcing[%i] =", k);
    SUNCheckCallLastErrNoRet(N_VPrintFile(step_mem->stepper->forcing[k], ark_mem->sunctx->logger->debug_fp));
  }
#endif
...
int mriStep_Predict(ARKodeMem ark_mem, int istage, N_Vector yguess)
{
  ...
  /* access ARKodeMRIStepMem structure */
  if (ark_mem->step_mem == NULL) {
    arkProcessError(NULL, ARK_MEM_NULL, __LINE__, __func__, __FILE__, MSG_MRISTEP_NO_MEM);
    return(ARK_MEM_NULL);
  }
  step_mem = (ARKodeMRIStepMem) ark_mem->step_mem;

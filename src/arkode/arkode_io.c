...
int arkSetDefaults(void* arkode_mem)
{
  ...
  ark_mem->hadapt_mem->small_nef = SMALL_NEF; /* num error fails before ETAMXF
                                                 enforced */
  ...
  ark_mem->hadapt_mem->lbound = HFIXED_LB; /* step adaptivity no-change lower
                                              bound */
  ark_mem->hadapt_mem->ubound = HFIXED_UB; /* step adaptivity no-change upper
                                              bound */
...
int arkSetDiagnostics(void* arkode_mem, FILE* diagfp)
{
  ...
  if (arkode_mem == NULL)
  {
    arkProcessError(NULL, ARK_MEM_NULL, "ARKODE", "arkSetDiagnostics",
                    MSG_ARK_NO_MEM);
    return (ARK_MEM_NULL);
  }
  ark_mem         = (ARKodeMem)arkode_mem;
  ark_mem->diagfp = diagfp;
  if (diagfp != NULL) { ark_mem->report = SUNTRUE; }
  else { ark_mem->report = SUNFALSE; }
...
int arkSetErrorBias(void* arkode_mem, realtype bias)
{
  ...
  if (bias < ONE) { hadapt_mem->bias = BIAS; }
  else { hadapt_mem->bias = bias; }
                           ...
                           realtype adapt_params[3])
{
  ...
  ARKodeHAdaptMem hadapt_mem;
  ARKodeMem ark_mem;
  retval = arkAccessHAdaptMem(arkode_mem, "arkSetAdaptivityMethod", &ark_mem,
                              &hadapt_mem);
  if (retval != ARK_SUCCESS) { return (retval); }
...
int arkSetAdaptivityFn(void* arkode_mem, ARKAdaptFn hfun, void* h_data)
{
  ...
  ARKodeHAdaptMem hadapt_mem;
  ARKodeMem ark_mem;
  retval = arkAccessHAdaptMem(arkode_mem, "arkSetAdaptivityFn", &ark_mem,
                              &hadapt_mem);
  if (retval != ARK_SUCCESS) { return (retval); }

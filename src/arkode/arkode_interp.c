...
int arkInterpEvaluate(void* arkode_mem, ARKInterp interp, realtype tau, int d,
                              ...
                              int d, int order, N_Vector yout)
{
  ...
  case (4): /* quartic interpolant */
    ...
    if (d == 0)
    {
      ...
      a[2] = h * FOURTH *
             (-FIVE * tau2 - RCONST(14.0) * tau3 - RCONST(9.0) * tau4);
      a[3] = h * (tau + TWO * tau2 + tau3);
  ...
  case (5): /* quintic interpolant */
    ...
    if (d == 0)
    {
      a[0] = RCONST(54.0) * tau5 + RCONST(135.0) * tau4 + RCONST(110.0) * tau3 +
             RCONST(30.0) * tau2;
      a[1] = ONE - a[0];
      a[2] = h / FOUR *
             (RCONST(27.0) * tau5 + RCONST(63.0) * tau4 + RCONST(49.0) * tau3 +
              RCONST(13.0) * tau2);
      a[3] = h / FOUR *
             (RCONST(27.0) * tau5 + RCONST(72.0) * tau4 + RCONST(67.0) * tau3 +
              RCONST(26.0) * tau2 + FOUR * tau);
      a[4] = h / FOUR *
             (RCONST(81.0) * tau5 + RCONST(189.0) * tau4 +
              RCONST(135.0) * tau3 + RCONST(27.0) * tau2);
      a[5] = h / FOUR *
             (RCONST(81.0) * tau5 + RCONST(216.0) * tau4 +
              RCONST(189.0) * tau3 + RCONST(54.0) * tau2);
    }
    else if (d == 1)
    {
      a[0] = (RCONST(270.0) * tau4 + RCONST(540.0) * tau3 +
              RCONST(330.0) * tau2 + RCONST(60.0) * tau) /
             h;
      a[1] = -a[0];
      a[2] = (RCONST(135.0) * tau4 + RCONST(252.0) * tau3 +
              RCONST(147.0) * tau2 + RCONST(26.0) * tau) /
             FOUR;
      a[3] = (RCONST(135.0) * tau4 + RCONST(288.0) * tau3 +
              RCONST(201.0) * tau2 + RCONST(52.0) * tau + FOUR) /
             FOUR;
      a[4] = (RCONST(405.0) * tau4 + RCONST(4.0) * 189 * tau3 +
              RCONST(405.0) * tau2 + RCONST(54.0) * tau) /
             FOUR;
      a[5] = (RCONST(405.0) * tau4 + RCONST(864.0) * tau3 +
              RCONST(567.0) * tau2 + RCONST(108.0) * tau) /
             FOUR;
    }
    else if (d == 2)
    {
      a[0] = (RCONST(1080.0) * tau3 + RCONST(1620.0) * tau2 +
              RCONST(660.0) * tau + RCONST(60.0)) /
             h2;
      a[1] = -a[0];
      a[2] = (RCONST(270.0) * tau3 + RCONST(378.0) * tau2 +
              RCONST(147.0) * tau + RCONST(13.0)) /
             (TWO * h);
      a[3] = (RCONST(270.0) * tau3 + RCONST(432.0) * tau2 +
              RCONST(201.0) * tau + RCONST(26.0)) /
             (TWO * h);
      a[4] = (RCONST(810.0) * tau3 + RCONST(1134.0) * tau2 +
              RCONST(405.0) * tau + RCONST(27.0)) /
             (TWO * h);
      a[5] = (RCONST(810.0) * tau3 + RCONST(1296.0) * tau2 +
              RCONST(567.0) * tau + RCONST(54.0)) /
             (TWO * h);
    }
    else if (d == 3)
    {
      ...
      a[1] = -a[0];
      a[2] = (RCONST(810.0) * tau2 + RCONST(756.0) * tau + RCONST(147.0)) /
             (TWO * h2);
      a[3] = (RCONST(810.0) * tau2 + RCONST(864.0) * tau + RCONST(201.0)) /
             (TWO * h2);
      a[4] = (RCONST(2430.0) * tau2 + RCONST(2268.0) * tau + RCONST(405.0)) /
             (TWO * h2);
      a[5] = (RCONST(2430.0) * tau2 + RCONST(2592.0) * tau + RCONST(567.0)) /
             (TWO * h2);
    }
    else if (d == 4)
    {
...
ARKInterp arkInterpCreate_Lagrange(void* arkode_mem, int degree)
{
  ...
  content->nmax = SUNMIN(degree + 1, ARK_INTERP_MAX_DEGREE + 1); /* respect
                                                                    maximum
                                                                    possible */
  content->nmaxalloc = 0;
  content->nhist     = 0;
...
/*---------------------------------------------------------------
  ...
  approximate).
  ---------------------------------------------------------------*/
int arkInterpEvaluate_Lagrange(void* arkode_mem, ARKInterp I, realtype tau,
                               int deriv, int degree, N_Vector yout)
{
  ...
  /* convert from tau back to t (both tnew and told are valid since q>0 =>
   * NHIST>1) */

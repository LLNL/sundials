                   ...
                   sunrealtype dqrely, ARKLocalFn gloc, ARKCommFn cfn)
{
  ...
  pdata->tmp1 = NULL;
  if (sunVec_Clone(ark_mem->tempv1, &(pdata->tmp1)))
  {
    N_VDestroy(pdata->zlocal);
  ...
  pdata->tmp2 = NULL;
  if (sunVec_Clone(ark_mem->tempv1, &(pdata->tmp2)))
  {
    (void)sunVec_Destroy(&(pdata->tmp1));
    N_VDestroy(pdata->zlocal);
    N_VDestroy(pdata->rlocal);
    SUNMatDestroy(pdata->savedP);
    SUNMatDestroy(pdata->savedJ);
  ...
  pdata->tmp3 = NULL;
  if (sunVec_Clone(ark_mem->tempv1, &(pdata->tmp3)))
  {
    (void)sunVec_Destroy(&(pdata->tmp1));
    (void)sunVec_Destroy(&(pdata->tmp2));
    N_VDestroy(pdata->zlocal);
    N_VDestroy(pdata->rlocal);
    SUNMatDestroy(pdata->savedP);
    SUNMatDestroy(pdata->savedJ);
  ...
  if (pdata->LS == NULL)
  {
    (void)sunVec_Destroy(&(pdata->tmp1));
    (void)sunVec_Destroy(&(pdata->tmp2));
    (void)sunVec_Destroy(&(pdata->tmp3));
    N_VDestroy(pdata->zlocal);
    N_VDestroy(pdata->rlocal);
    SUNMatDestroy(pdata->savedP);
    SUNMatDestroy(pdata->savedJ);

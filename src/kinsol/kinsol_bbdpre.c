                   ...
                   realtype dq_rel_uu, KINBBDLocalFn gloc, KINBBDCommFn gcomm)
{
  ...
  if (kinmem == NULL)
  {
    KINProcessError(NULL, KINLS_MEM_NULL, "KINBBDPRE", "KINBBDPrecInit",
                    MSGBBD_MEM_NULL);
  ...
  {
    KINProcessError(kin_mem, KINLS_LMEM_NULL, "KINBBDPRE", "KINBBDPrecInit",
                    MSGBBD_LMEM_NULL);
  ...
  /* Note: Do NOT need to check for N_VScale since has already been checked for
   * in KINSOL */
  if (kin_mem->kin_vtemp1->ops->nvgetarraypointer == NULL)
  {
    KINProcessError(kin_mem, KINLS_ILL_INPUT, "KINBBDPRE", "KINBBDPrecInit",
                    MSGBBD_BAD_NVECTOR);
  ...
  if (pdata == NULL)
  {
    KINProcessError(kin_mem, KINLS_MEM_FAIL, "KINBBDPRE", "KINBBDPrecInit",
                    MSGBBD_MEM_FAIL);
  ...
  pdata->rlocal = NULL;
  pdata->rlocal = N_VNewEmpty_Serial(Nlocal, kin_mem->kin_sunctx); /* empty
                                                                      vector */
  ...
  if (flag != SUNLS_SUCCESS)
  {
    ...
    free(pdata);
    pdata = NULL;
    KINProcessError(kin_mem, KINLS_SUNLS_FAIL, "KINBBDPRE", "KINBBDPrecInit",
                    MSGBBD_SUNLS_FAIL);
...
int KINBBDPrecGetWorkSpace(void* kinmem, long int* lenrwBBDP, long int* leniwBBDP)
{
  ...
  if (kinmem == NULL)
  {
    KINProcessError(NULL, KINLS_MEM_NULL, "KINBBDPRE", "KINBBDPrecGetWorkSpace",
                    MSGBBD_MEM_NULL);
    return (KINLS_MEM_NULL);
  }
  kin_mem = (KINMem)kinmem;

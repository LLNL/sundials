...
void *KINCreate(void)
{
  ...
  kin_mem->kin_adaptive_m_aa_factor = RCONST(1.0);
...
static int KINFP(KINMem kin_mem)
{
  ...
  while (ret == CONTINUE_ITERATIONS) {
    ...
    if (kin_mem->kin_fnorm <= (tolfac * kin_mem->kin_fnormtol) && !(kin_mem->kin_use_mxiter)) {
      ret = KIN_SUCCESS;
    }
                       ...
                       long int iter, realtype *R, realtype *gamma)
{
  ...
  if (kin_mem->kin_adaptive_m_aa)
  {
    ...
    long int m_aa_adapt = SUNMAX(0, floor(-log10(kin_mem->kin_adaptive_m_aa_factor * fv_norm)));

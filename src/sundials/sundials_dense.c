...
int DenseORMQR(SUNDlsMat A, realtype* beta, realtype* vn, realtype* vm,
               realtype* wrk)
{
  return (SUNDlsMat_denseORMQR(A->cols, A->M, A->N, beta, vn, vm, wrk));
}
 ...
 * upper triangular matrix R; the elements below the diagonal, with the array
 * beta, represent the orthogonal matrix Q as a product of elementary reflectors
 * .
 *
...
int SUNDlsMat_denseORMQR(realtype** a, sunindextype m, sunindextype n,
                         realtype* beta, realtype* vn, realtype* vm, realtype* v)
{
  realtype *col_j, s;
  sunindextype i, j;

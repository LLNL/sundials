               ...
               PSolveFn psolve, realtype *res_norm, int *nli, int *nps)
{
  ...
  *nli = *nps = 0;    /* Initialize counters */
  ...
  if (preOnLeft) {
    ...
    ier = psolve(P_data, V[0], vtemp, s1, delta, PREC_LEFT);
    (*nps)++;
    if (ier != 0)
  ...
  for (ntries = 0; ntries <= max_restarts; ntries++) {
    ...
    for (l = 0; l < l_max; l++) {
      ...
      if (preOnRight) {
        ...
        ier = psolve(P_data, V[l_plus_1], vtemp, s2, delta, PREC_RIGHT);
        (*nps)++;
        if (ier != 0)
          return((ier < 0) ? SPGMR_PSOLVE_FAIL_UNREC : SPGMR_PSOLVE_FAIL_REC);
      }
      ...
      if (preOnLeft) {
        ...
        ier = psolve(P_data, V[l_plus_1], vtemp, s1, delta, PREC_LEFT);
        (*nps)++;
        if (ier != 0)
          return((ier < 0) ? SPGMR_PSOLVE_FAIL_UNREC : SPGMR_PSOLVE_FAIL_REC);
      } else {

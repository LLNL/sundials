...
/* The atomic functions below are implemented using the atomic compare and swap
   ...
   (*address == assumed) ? (assumed + val) : *address. Since *address could
   change between when the value is loaded and the atomicCAS call the operation
   is repeated until *address does not change between the read and the compare
   and swap operation. */
...
/*
 * Compute the maximum of 2 double-precision floating point values using an
 * atomic operation "address" is the address of the reference value which might
 * get updated with the maximum "value" is the value that is compared to the
 * reference in order to determine the maximum
 */
...
/*
 * Compute the maximum of 2 single-precision floating point values using an
 * atomic operation "address" is the address of the reference value which might
 * get updated with the maximum "value" is the value that is compared to the
 * reference in order to determine the maximum
 */
...
/*
 * Compute the minimum of 2 double-precision floating point values using an
 * atomic operation "address" is the address of the reference value which might
 * get updated with the minimum "value" is the value that is compared to the
 * reference in order to determine the minimum
 */
...
/*
 * Compute the minimum of 2 single-precision floating point values using an
 * atomic operation "address" is the address of the reference value which might
 * get updated with the minimum "value" is the value that is compared to the
 * reference in order to determine the minimum
 */
...
struct GridReducerLDS
{
  __device__ __forceinline__ void operator()(T val, T identity, T* device_mem,
                                             unsigned int* device_count)
  {
    return impl::gridReduce<T, BinaryReductionOp>(val, identity, device_mem,
                                                  device_count);
  }
};
...
struct GridReducerAtomic
{
  __device__ __forceinline__ void operator()(T val, T identity, T* device_mem,
                                             unsigned int* device_count)
  {
    return impl::gridReduceAtomic<T, BinaryReductionOp>(val, identity,
                                                        device_mem);
  }
};

		 ...
		 realtype *res_norm, int *nli, int *nps)
{
  ...
  *nli = *nps = 0;    /* Initialize counters */
  ...
  if (preOnLeft) {
    ...
    ier = psolve(P_data, r_star, vtemp1, sb, delta, PREC_LEFT);
    (*nps)++;
    if (ier != 0)
  ...
  *res_norm = r_init_norm = SUNRsqrt(rho[0]);
  ...
  if (preOnRight) {
    ...
    ier = psolve(P_data, v_, vtemp1, sx, delta, PREC_RIGHT);
    (*nps)++;
  ...
  if (preOnLeft) {
    ...
    ier = psolve(P_data, v_, vtemp1, sb, delta, PREC_LEFT);
    (*nps)++;
    if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_REC);
  }
  ...
  for (n = 0; n < l_max; ++n) {
    ...
    if (preOnRight) {
      ...
      ier = psolve(P_data, vtemp1, r_[1], sx, delta, PREC_RIGHT);
      (*nps)++;
      if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_REC);
    }
    ...
    if (preOnLeft) {
      ...
      ier = psolve(P_data, vtemp1, r_[1], sb, delta, PREC_LEFT);
      (*nps)++;
      if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_REC);
    }
	...
	if (preOnRight) {
	  ...
	  ier = psolve(P_data, vtemp1, vtemp2, sx, delta, PREC_RIGHT);
	  (*nps)++;
	...
	if (preOnLeft) {
	  ...
	  ier = psolve(P_data, vtemp2, vtemp1, sx, delta, PREC_LEFT);
	  (*nps)++;
	  if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_REC);
	}
	...
	if (!b_ok) {
	  ...
	  if (preOnLeft) {
	    ...
	    ier = psolve(P_data, b, vtemp3, sb, delta, PREC_LEFT);
	    (*nps)++;
	    if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_REC);
	  }
  ...
  if ((converged == TRUE) || (r_curr_norm < r_init_norm)) {
    ...
    if (preOnRight) {
      ...
      ier = psolve(P_data, x, vtemp1, sx, delta, PREC_RIGHT);
      (*nps)++;
      if (ier != 0) return((ier < 0) ? SPTFQMR_PSOLVE_FAIL_UNREC : SPTFQMR_PSOLVE_FAIL_UNREC);

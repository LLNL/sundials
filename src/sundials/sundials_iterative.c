                         ...
                         sunrealtype* new_vk_norm)
{
  ...
  int i, k_minus_1, i0;
  sunrealtype new_norm_2, vk_norm, new_norm;
  ...
  *new_vk_norm = SUNRsqrt(SUN_REAL(temp));
  ...
  if (new_norm_2 != ZERO)
  {
    new_norm = SUNRSQR(*new_vk_norm) - new_norm_2;
    *new_vk_norm = (new_norm > ZERO) ? SUNRsqrt(new_norm) : ZERO;
  }
                          ...
                          N_Vector* vtemp)
{
  ...
  vk_norm = SUNRsqrt(SUN_REAL(stemp[k - i0]));
  for (i = k - i0 - 1; i >= 0; i--)
  {
...
int SUNQRfact(int n, sunscalartype** h, sunscalartype* q, int job)
{
  ...
  case 0:
    ...
    for (k = 0; k < n; k++)
    {
      ...
      else if (SUNabs(temp2) >= SUNabs(temp1))
      {
        temp3 = SUNSQR(temp1) / SUNSQR(temp2);
        c     = ONE / ((SUNabs(temp2)/SUNabs(temp1)) * SUNRsqrt(ONE + temp3));
        s     = -c * (temp1 * SUNCONJ(temp2)) / SUNSQR(temp1);
      }
      else
      {
        temp3 = SUNSQR(temp2) / SUNSQR(temp1);
        c     = ONE / SUNRsqrt(ONE + temp3);
        s     = -c * (temp1 * SUNCONJ(temp2)) / SUNSQR(temp1);
      }
      q[q_ptr]     = c;
      q[q_ptr + 1] = s;
...
SUNErrCode SUNQRAdd_ICWY_SB(N_Vector* Q, sunscalartype* R, N_Vector df, int m,
                            int mMax, void* QRdata)
{
  SUNFunctionBegin(Q[0]->sunctx);
  sunindextype j, k;

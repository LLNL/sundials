...
int SUNMemoryHelper_GetAllocStats(SUNMemoryHelper helper, SUNMemoryType mem_type,
                          ...
                          size_t mem_size, SUNMemoryType mem_type, void* queue)
{
  int ier;
  SUNDIALS_MARK_FUNCTION_BEGIN(getSUNProfiler(helper));
  if (helper->ops->alloc == NULL) { ier = -1; }
  else { ier = helper->ops->alloc(helper, memptr, mem_size, mem_type, queue); }
  SUNDIALS_MARK_FUNCTION_END(getSUNProfiler(helper));
  return (ier);
}
                         ...
                         size_t memory_size, void* queue)
{
  ...
  {
    SUNDIALS_DEBUG_PRINT(
      "ERROR in SUNMemoryHelper_Copy: function pointer is NULL\n");
    ier = -1;
  }
...
SUNMemoryHelper SUNMemoryHelper_Clone(SUNMemoryHelper helper)
{
  ...
  {
    if (helper->content != NULL) { return (NULL); }
    else
    {

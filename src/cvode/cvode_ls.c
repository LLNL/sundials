...
/*-----------------------------------------------------------------
  ...
  -----------------------------------------------------------------*/
int cvLsDenseDQJac(realtype t, N_Vector y, N_Vector fy, SUNMatrix Jac,
...
/*-----------------------------------------------------------------
  ...
  -----------------------------------------------------------------*/
int cvLsDQJtimes(N_Vector v, N_Vector Jv, realtype t, N_Vector y, N_Vector fy,
              ...
              N_Vector fnow)
{
  ...
  if (cvls_mem->LS->ops->setscalingvectors)
  {
    ...
    /* If solver is iterative and does not support scaling vectors, update the
       ...
       Hence (using the notation from sunlinsol_spgmr.h, with S = diag(w)),
         ...
         <=> || b - A x ||_2 < tol / w_mean
       So we compute w_mean = ||w||_RMS = ||w||_2 and scale the desired
  ...
  /* If using a direct or matrix-iterative solver, BDF method, and gamma has

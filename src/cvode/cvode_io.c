...
int CVodeSetNumStepsEtaMaxEarlyStep(void* cvode_mem, long int small_nst)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetNumStepsEtaMaxEarlyStep",
                   MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMax(void* cvode_mem, sunrealtype eta_max_gs)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMax",
                   MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMin(void* cvode_mem, sunrealtype eta_min)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMin",
                   MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
                                 ...
                                 sunbooleantype on_off)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetSingleNonlinSolvIter",
                   MSGCV_NO_MEM);
    return(CV_MEM_NULL);
  }
  ...
  /* Set option
   * true = pass convergence test after single iteration
   * false = (default) usual convergence test
   */
  cv_mem->cv_single_nls_iter = on_off;
...
int CVodeSetNextStep(void* cvode_mem, sunrealtype hnew)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetNextStep",
                   MSGCV_NO_MEM);
    return(CV_MEM_NULL);
  }

...
void* CVodeCreate(int lmm, SUNContext sunctx)
{
  ...
  {
    cvProcessError(NULL, 0, __LINE__, __func__, __FILE__, __FILE__);
    return (NULL);
  }
...
int CVodeInit(void* cvode_mem, CVRhsFn f, sunrealtype t0, N_Vector y0)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, __LINE__, __func__, __FILE__, __FILE__);
    return (CV_MEM_NULL);
  }
  ...
  if (y0 == NULL)
  {
    cvProcessError(cv_mem, CV_ILL_INPUT, __LINE__, __func__, __FILE__, __FILE__);
  ...
  if (!allocOK)
  {
    cvProcessError(cv_mem, CV_MEM_FAIL, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_MEM_FAIL);
  }
  ...
  {
    cvProcessError(cv_mem, retval, __LINE__, __func__, __FILE__, __FILE__);
    cvFreeVectors(cv_mem);
...
int CVodeReInit(void* cvode_mem, sunrealtype t0, N_Vector y0)
{
  ...
  {
    cvProcessError(cv_mem, CV_NO_MALLOC, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_NO_MALLOC);
  }
          ...
          int itask)
{
  ...
  for (;;)
  {
    ...
    if (cv_mem->cv_nst > 0)
    {
      ewtsetOK = cv_mem->cv_efun(cv_mem->cv_zn[0], cv_mem->cv_ewt,
                                 cv_mem->cv_e_data);
...
int CVodeGetDky(void* cvode_mem, sunrealtype t, int k, N_Vector dky)
{
  ...
  if (dky == NULL)
  {
    cvProcessError(cv_mem, CV_BAD_DKY, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_BAD_DKY);
  }
  ...
  {
    cvProcessError(cv_mem, CV_BAD_K, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_BAD_K);
  }
  ...
  if ((t - tp) * (t - tn1) > ZERO)
  {
    cvProcessError(cv_mem, CV_BAD_T, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_BAD_T);
  }
...
static int cvHandleFailure(CVodeMem cv_mem, int flag)
{
  ...
  */
  ...
  case CV_RTFUNC_FAIL:
    ...
    break;
  case CV_TOO_CLOSE:
    cvProcessError(cv_mem, CV_TOO_CLOSE, __LINE__, __func__, __FILE__, __FILE__);
    break;
  case CV_MEM_NULL:
    cvProcessError(NULL, CV_MEM_NULL, __LINE__, __func__, __FILE__, __FILE__);
    break;
  case SUN_NLS_MEM_NULL:
    cvProcessError(cv_mem, CV_MEM_NULL, __LINE__, __func__, __FILE__, __FILE__);
    break;
  ...
  case CV_CONSTR_FAIL:
    ...
    break;
  case CV_NLS_FAIL:
    cvProcessError(cv_mem, CV_NLS_FAIL, __LINE__, __func__, __FILE__, __FILE__);
    break;
...
static int cvEwtSetSS(CVodeMem cv_mem, N_Vector ycur, N_Vector weight)
{
  ...
  if (cv_mem->cv_usefused)
  {
    ...
    cvEwtSetSS_fused(cv_mem->cv_atolmin0, cv_mem->cv_reltol, cv_mem->cv_Sabstol,
                     ycur, cv_mem->cv_tempv, weight);
...
static int cvEwtSetSV(CVodeMem cv_mem, N_Vector ycur, N_Vector weight)
{
  ...
  if (cv_mem->cv_usefused)
  {
    /* We compute weight (inverse of tempv) regardless of the component test
       since it will be thrown away in this case anyways. */
    cvEwtSetSV_fused(cv_mem->cv_atolmin0, cv_mem->cv_reltol, cv_mem->cv_Vabstol,
                     ycur, cv_mem->cv_tempv, weight);
    if (cv_mem->cv_atolmin0)
    {
      if (N_VMin(cv_mem->cv_tempv) <= ZERO) return (-1);

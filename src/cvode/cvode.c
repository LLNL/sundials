...
int CVodeInit(void *cvode_mem, CVRhsFn f, realtype t0, N_Vector y0)
{
  ...
  NLS = SUNCheckCallLastErrNoRet(SUNNonlinSol_Newton(y0, CV_SUNCTX), CV_SUNCTX);
...
int CVodeGetDky(void *cvode_mem, realtype t, int k, N_Vector dky)
{
  ...
  ier = SUNCheckCallLastErrNoRet(N_VLinearCombination(nvec, cv_mem->cv_cvals, cv_mem->cv_Xvecs, dky), CV_SUNCTX);
  if (ier != CV_SUCCESS) {
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
...
static int cvEwtSetSS(CVodeMem cv_mem, N_Vector ycur, N_Vector weight)
{
  ...
  if (cv_mem->cv_usefused)
  {
    ...
    if (cv_mem->cv_atolmin0) {
      min = SUNCheckCallLastErrNoRet(N_VMin(cv_mem->cv_tempv), CV_SUNCTX);

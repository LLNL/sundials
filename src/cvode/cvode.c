 ...
 * nonlinear system (cvNls returned nflag == CONV_FAIL, CONSTR_RECVR,
 * or RHSFUNC_RECVR). In this case, if ncf is now equal to maxncf
 * or |h| = hmin, we return the value CV_CONV_FAILURE (if nflag=CONV_FAIL),
 * CV_CONSTR_FAIL (if nflag=CONSTR_RECVR), or CV_REPTD_RHSFUNC_ERR
 * (if nflag=RHSFUNC_RECVR). If not, we set *nflagPtr = PREV_CONV_FAIL
 * and return the value PREDICT_AGAIN, telling cvStep to reattempt the step.
 *
 */
                         ...
                         int *ncfPtr)
{
  ...
  /* At this point, nflag = CONV_FAIL, CONSTR_RECVR, or RHSFUNC_RECVR; increment ncf */

  (*ncfPtr)++;
  cv_mem->cv_etamax = ONE;

  /* If we had maxncf failures or |h| = hmin,
     return CV_CONV_FAILURE, CV_CONSTR_FAIL, or CV_REPTD_RHSFUNC_ERR. */

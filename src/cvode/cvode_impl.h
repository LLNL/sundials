 ...
 * --------------------------
 * ETA_MIN_FX_DEFAULT  if eta_min_fx < eta < eta_max_fx reject a change in step
 * size or order ETA_MAX_FX_DEFAULT ETA_MAX_FS_DEFAULT  -+ ETA_MAX_ES_DEFAULT |
 * ETA_MAX_GS_DEFAULT   |
 ...
 * SMALL_NEF_DEFAULT   if small_nef <= nef <= MXNEF1, then eta =  SUNMIN(eta,
 * eta_max_ef) ONEPSM (1+epsilon)  used in testing if the step size is below its
 * bound
 */
...
typedef struct CVodeMemRec
{
  ...
  booleantype cv_usefused; /* flag indicating if CVODE specific fused kernels
                              should be used */
...
int cvDoProjection(CVodeMem cv_mem, int* nflagPtr, realtype saved_t, int* npfPtr);

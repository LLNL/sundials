...
static int CVBoomerAMGSetup(realtype t, N_Vector y, N_Vector fy,
                            ...
                            realtype gamma, void* hypamg_data, N_Vector tmp1,
                            N_Vector tmp2, N_Vector tmp3);

static int CVBoomerAMGSolve(realtype t, N_Vector y, N_Vector fy, N_Vector r,
                            N_Vector z, realtype gamma, realtype delta, int lr,
                            void* hypamg_data, N_Vector tmp);

/* Prototype for CVBoomerAMGFree */
static void CVBoomerAMGFree(CVodeMem cv_mem);

static int CVParCsrCreateGammaIdentity(HYPRE_IJMatrix* id_mat, realtype gamma,
                                       sunindextype ilower, sunindextype iupper);
...
int CVBoomerAMGInit(void* cvode_mem, int ilower, int iupper, int jlower,
                    int jupper, int N)
{
  CVodeMem cv_mem;
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CVSPILS_MEM_NULL, "CVHYPRE_BOOMERAMG",
                   "CVBoomerAMGInit", MSGBBD_MEM_NULL);
  ...
  {
    cvProcessError(cv_mem, CVSPILS_LMEM_NULL, "CVHYPRE_BOOMERAMG",
                   "CVBoomerAMGInit", MSGBBD_LMEM_NULL);
  ...
  {
    cvProcessError(cv_mem, CVSPILS_ILL_INPUT, "CVHYPRE_BOOMERAMG",
                   "CVBoomerAMGInit", MSGBBD_BAD_NVECTOR);
  ...
  if (pdata == NULL)
  {
    cvProcessError(cv_mem, CVSPILS_MEM_FAIL, "CVHYPRE_BOOMERAMG",
                   "CVBoomerAMGInit", MSGBBD_MEM_FAIL);
  ...
  HYPRE_IJMatrixCreate(MPI_COMM_WORLD, ilower, iupper, jlower, jupper,
                       &(pdata->A));
  ...
  //  HYPRE_BoomerAMGSetPrintLevel((pdata->solver), );  /* print solve info +
  //  parameters */ HYPRE_BoomerAMGSetCoarsenType(pdata->solver, 6); /* Falgout
  //  coarsening */ HYPRE_BoomerAMGSetRelaxType(pdata->solver, 3);   /*
  //  G-S/Jacobi hybrid relaxation */ HYPRE_BoomerAMGSetNumSweeps(pdata->solver,
  //  1);   /* Sweeeps on each level */
  //  HYPRE_BoomerAMGSetMaxLevels(pdata->solver, 20);  /* maximum number of
  //  levels */ HYPRE_BoomerAMGSetTol(pdata->solver, 1e-7);      /* conv.
  //  tolerance */
 ...
 *         Currently this argument is superflous, since we cannot store a
 * previous J
 *
...
int CVBoomerAMGSetup(realtype t, N_Vector y, N_Vector fy, booleantype jok,
                     booleantype* jcurPtr, realtype gamma, void* hypamg_data,
                     N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  *jcurPtr = SUNTRUE;
  ...
  /*Consider recomputing P in some Solve iterations if BoomerAMGSetup is much
  more expensive than computing P*/
  pdata->jacfn(pdata->N, pdata->ilower, pdata->iupper, pdata->jlower,
               pdata->jupper, gamma, t, y, fy, &(pdata->A),
               (void*)(cv_mem->cv_user_data), tmp1, tmp2, tmp3);
  ...
  HYPRE_BoomerAMGSetup(pdata->solver, pdata->parcsr_A, pdata->par_b,
                       pdata->par_x);
...
int CVBoomerAMGSolve(realtype t, N_Vector y, N_Vector fy, N_Vector r,
                     N_Vector z, realtype gamma, realtype delta, int lr,
                     void* hypamg_data, N_Vector tmp)
{
  CVBoomerAMGData pdata;
  ...
  /*  jacfn(N, ilower, iupper, jlower, jupper, gamma, t, y, fy, &A, (void*)
   * jacobian_data, tmp, tmp, tmp);*/
  pdata->par_b = (HYPRE_ParVector)(NV_HYPRE_PARVEC_PH(r));
  pdata->par_x = (HYPRE_ParVector)(NV_HYPRE_PARVEC_PH(z));
  HYPRE_BoomerAMGSolve(pdata->solver, pdata->parcsr_A, pdata->par_b,
                       pdata->par_x);
...
static void CVBoomerAMGFree(CVodeMem cv_mem)
{
  ...
  /* Need to return solver function so that users can get diagnostics out before
  CVodeFree() Run info - needed logging turned on */
...
/*
int JacTimes(N_Vector v, N_Vector Jv, realtype t, N_Vector y, N_Vector fy, void
*user_data, N_Vector tmp)
{
  ...
  if (user_data->ode_mem == NULL) {
    cvProcessError(NULL, CVSPILS_MEM_NULL, "CVSPILS", "CVSpilsSetJacTimesVecFn",
MSGS_CVMEM_NULL); return(CVSPILS_MEM_NULL);
  }
  cv_mem = (CVodeMem) user_data->ode_mem;

  if (cv_mem->cv_lmem == NULL) {
    cvProcessError(cv_mem, CVSPILS_LMEM_NULL, "CVSPILS",
"CVSpilsSetJacTimesVecFn", MSGS_LMEM_NULL); return(CVSPILS_LMEM_NULL);
  }
  cvspils_mem = (CVSpilsMem) cv_mem->cv_lmem;
  pdata = (CVBoomerAMGData) cvspils_mem->s_P_data;
 hypre_ParCSRMatrixMatvec ( 1.0, pdata->parcsr_A, NV_HYPRE_PARVEC_PH(v), 1.0,
NV_HYPRE_PARVEC_PH(Jv));
}*/
/*
 * -----------------------------------------------------------------
...
int CVParCsrCreateGammaIdentity(HYPRE_IJMatrix* id_mat, realtype gamma,
                                sunindextype ilow, sunindextype iup)
{
  int nnz, i;
  double values[5];
  int cols[5];

...
int CVBand(void *cvode_mem, int N, int mupper, int mlower)
{
  ...
  if (cvode_mem == NULL) {
    CVProcessError(NULL, CVDIRECT_MEM_NULL, "CVBAND", "CVBand", MSGD_CVMEM_NULL);
  ...
  if (vec_tmpl->ops->nvgetarraypointer == NULL) {
    CVProcessError(cv_mem, CVDIRECT_ILL_INPUT, "CVBAND", "CVBand", MSGD_BAD_NVECTOR);
  ...
  if (cvdls_mem == NULL) {
    CVProcessError(cv_mem, CVDIRECT_MEM_FAIL, "CVBAND", "CVBand", MSGD_MEM_FAIL);
  ...
  if ((ml < 0) || (mu < 0) || (ml >= N) || (mu >= N)) {
    CVProcessError(cv_mem, CVDIRECT_ILL_INPUT, "CVBAND", "CVBand", MSGD_BAD_SIZES);
    return(CVDIRECT_ILL_INPUT);
  }
                       ...
                       N_Vector vtemp2, N_Vector vtemp3)
{
  ...
  if (jok) {
    ...
    *jcurPtr = TRUE;
    ...
    if (retval < 0) {
      CVProcessError(cv_mem, CVDIRECT_JACFUNC_UNRECVR, "CVBAND", "cvBandSetup", MSGD_JACFUNC_FAILED);

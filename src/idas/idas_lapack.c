...
int IDALapackDense(void *ida_mem, int N)
{
  ...
  if (ida_mem == NULL) {
    IDAProcessError(NULL, IDADIRECT_MEM_NULL, "IDASLAPACK", "IDALapackDense", MSGD_IDAMEM_NULL);
  ...
  if (tempv->ops->nvgetarraypointer == NULL ||
    ...
    IDAProcessError(IDA_mem, IDADIRECT_ILL_INPUT, "IDASLAPACK", "IDALapackDense", MSGD_BAD_NVECTOR);
  ...
  if (idadls_mem == NULL) {
    IDAProcessError(IDA_mem, IDADIRECT_MEM_FAIL, "IDASLAPACK", "IDALapackDense", MSGD_MEM_FAIL);
...
int IDALapackBand(void *ida_mem, int N, int mupper, int mlower)
{
  ...
  /* Return immediately if ida_mem is NULL */
  if (ida_mem == NULL) {
    IDAProcessError(NULL, IDADIRECT_MEM_NULL, "IDASLAPACK", "IDALapackBand", MSGD_IDAMEM_NULL);
    return(IDADIRECT_MEM_NULL);
  }
  IDA_mem = (IDAMem) ida_mem;
  ...
  if (tempv->ops->nvgetarraypointer == NULL) {
    IDAProcessError(IDA_mem, IDADIRECT_ILL_INPUT, "IDASLAPACK", "IDALapackBand", MSGD_BAD_NVECTOR);
    return(IDADIRECT_ILL_INPUT);
  }
  ...
  idadls_mem = (IDADlsMem) malloc(sizeof(IDADlsMemRec));
  if (idadls_mem == NULL) {
    IDAProcessError(IDA_mem, IDADIRECT_MEM_FAIL, "IDASLAPACK", "IDALapackBand", MSGD_MEM_FAIL);
    return(IDADIRECT_MEM_FAIL);
  }
  ...
  if ((ml < 0) || (mu < 0) || (ml >= N) || (mu >= N)) {
    IDAProcessError(IDA_mem, IDADIRECT_ILL_INPUT, "IDASLAPACK", "IDALapackBand", MSGD_BAD_SIZES);
    return(IDADIRECT_ILL_INPUT);
  }
                               ...
                               N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  if (retval < 0) {
    IDAProcessError(IDA_mem, IDADIRECT_JACFUNC_UNRECVR, "IDASLAPACK", "idaLapackDenseSetup", MSGD_JACFUNC_FAILED);
                              ...
                              N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  if (retval < 0) {
    IDAProcessError(IDA_mem, IDADIRECT_JACFUNC_UNRECVR, "IDASLAPACK", "idaLapackBandSetup", MSGD_JACFUNC_FAILED);
    last_flag = IDADIRECT_JACFUNC_UNRECVR;
    return(-1);
  } else if (retval > 0) {
    last_flag = IDADIRECT_JACFUNC_RECVR;

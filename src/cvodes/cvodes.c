...
int CVodeQuadReInit(void* cvode_mem, N_Vector yQ0)
{
  ...
  {
    cvProcessError(cv_mem, CV_NO_QUAD, __LINE__, __func__, __FILE__, __FILE__);
    return (CV_NO_QUAD);
  }
...
int CVodeSensReInit(void* cvode_mem, int ism, N_Vector* yS0)
{
  ...
  {
    cvProcessError(cv_mem, CV_NO_SENS, __LINE__, __func__, __FILE__, __FILE__);
    return (CV_NO_SENS);
  }
...
int CVodeGetSensDky1(void* cvode_mem, sunrealtype t, int k, int is, N_Vector dkyS)
{
  ...
  {
    cvProcessError(cv_mem, CV_BAD_IS, __LINE__, __func__, __FILE__, __FILE__);
    SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    return (CV_BAD_IS);
  }

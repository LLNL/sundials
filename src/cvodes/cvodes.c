          ...
          realtype *tret, int itask)
{
  ...
  if (nst > 0) {
    ...
    if ( itask == CV_ONE_STEP && SUN_ABS(tn - tretlast) > troundoff ) {
      tretlast = *tret = tn;
      N_VScale(ONE, zn[0], yout);
      return(CV_SUCCESS);
    }
  ...
  loop {
    ...
    if ( tstopset ) {
      troundoff = FUZZ_FACTOR*uround*(SUN_ABS(tn) + SUN_ABS(h));
      if ( SUN_ABS(tn - tstop) <= troundoff) tn = tstop;
    }
                       ...
                       realtype alpha0_hat, realtype xi_inv, realtype xistar_inv)
{
  ...
  A2 = ONE + q * A1;
  tq[2] = SUN_ABS(A1 / (alpha0 * A2));
  tq[5] = SUN_ABS(A2 * xistar_inv / (l[q] * xi_inv));
  if (qwait == 1) {
    if (q > 1) {
      C = xistar_inv / l[q];
      A3 = alpha0 + ONE / q;
    ...
    Cppinv = (ONE - A6 + A5) / A2;
    tq[3] = SUN_ABS(Cppinv / (xi_inv * (q+2) * A5));
  }
  tq[4] = nlscoef / tq[2];
}
...
static int cvNlsNewton(CVodeMem cv_mem, int nflag)
{
  ...
  if (setupNonNull) {      
    callSetup = (nflag == PREV_CONV_FAIL) || (nflag == PREV_ERR_FAIL) ||
      (nst == 0) || (nst >= nstlp + MSBP) || (SUN_ABS(gamrat-ONE) > DGMAX);
...
static int cvStgrNlsFunctional(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    if (m > 0) crateS = SUN_MAX(CRDOWN * crateS, Del / Delp);
    dcon = Del * SUN_MIN(ONE, crateS) / tq[4];
                         ...
                         int *nefPtr, long int *netfPtr, realtype *dsmPtr)
{
  ...
  *nflagPtr = PREV_ERR_FAIL;
  ...
  if ((SUN_ABS(h) <= hmin*ONEPSM) || (*nefPtr == maxnef)) return(CV_ERR_FAILURE);
...
static int cvSLdet(CVodeMem cv_mem)
{
  ...
  for (k=1; k<=3; k++) {
    ...
    rav[k] = FOURTH*sumrat;
    vrat[k] = SUN_ABS(FOURTH*sumrsq - rav[k]*rav[k]);
...
static int cvRcheck1(CVodeMem cv_mem)
{
  ...
  tlo = tn;
  ttol = (SUN_ABS(tn) + SUN_ABS(h))*uround*HUNDRED;
                         ...
                         N_Vector ytemp, N_Vector ftemp)
{
  ...
  rDeltay = SUN_MAX(norms, rdelta) / pbari;
  Deltay  = ONE/rDeltay;

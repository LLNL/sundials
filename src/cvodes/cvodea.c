...
static int CVAfindIndex(CVodeMem cv_mem, realtype t, long int* indx,
...
static int CVArhs(realtype t, N_Vector yB, N_Vector yBdot, void* cvode_mem);

static int CVArhsQ(realtype t, N_Vector yB, N_Vector qBdot, void* cvode_mem);
...
int CVodeF(void* cvode_mem, realtype tout, N_Vector yout, realtype* tret,
...
int CVodeInitB(void* cvode_mem, int which, CVRhsFnB fB, realtype tB0, N_Vector yB0)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
  CVodeBMem cvB_mem;
...
int CVodeInitBS(void* cvode_mem, int which, CVRhsFnBS fBs, realtype tB0,
                N_Vector yB0)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
...
int CVodeSVtolerancesB(void* cvode_mem, int which, realtype reltolB,
                       N_Vector abstolB)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
...
int CVodeQuadSStolerancesB(void* cvode_mem, int which, realtype reltolQB,
                           realtype abstolQB)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
...
int CVodeQuadSVtolerancesB(void* cvode_mem, int which, realtype reltolQB,
                           N_Vector abstolQB)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
...
static CkpntMem CVAckpntNew(CVodeMem cv_mem)
{
  ...
  if (ck_mem->ck_quadr_sensi)
  {
    ...
    if (cv_mem->cv_q < qmax)
    {
      ck_mem->ck_znQS[qmax] = N_VCloneVectorArray(cv_mem->cv_Ns,
                                                  cv_mem->cv_tempvQ);
  ...
  (void)N_VScaleVectorArray(cv_mem->cv_q + 1, cv_mem->cv_cvals, cv_mem->cv_zn,
                            ck_mem->ck_zn);
  ...
  if (ck_mem->ck_quadr_sensi)
  {
    ...
    (void)N_VScaleVectorArray(cv_mem->cv_Ns, cv_mem->cv_cvals, cv_mem->cv_Xvecs,
                              cv_mem->cv_Zvecs);
  ...
  *ck_memPtr = (*ck_memPtr)->ck_next;
  ...
  if (tmp->ck_sensi)
  {
    if (tmp->ck_next != NULL)
    {
      ...
      {
        N_VDestroyVectorArray(tmp->ck_znS[j], tmp->ck_Ns);
      }
      if (tmp->ck_zqm != 0)
      {
  ...
  if (tmp->ck_quadr_sensi)
  {
    if (tmp->ck_next != NULL)
    {
      for (j = 0; j <= tmp->ck_q; j++)
      {
        N_VDestroyVectorArray(tmp->ck_znQS[j], tmp->ck_Ns);
      }
      if (tmp->ck_zqm != 0)
      {
...
static int CVArhs(realtype t, N_Vector yB, N_Vector yBdot, void* cvode_mem)
{
  ...
  {
    retval = (cvB_mem->cv_fs)(t, ca_mem->ca_ytmp, ca_mem->ca_yStmp, yB, yBdot,
                              cvB_mem->cv_user_data);
  }
  else
  {
    retval = (cvB_mem->cv_f)(t, ca_mem->ca_ytmp, yB, yBdot,
                             cvB_mem->cv_user_data);
  }

...
static int cvLsJacBWrapper(realtype t, N_Vector yB, N_Vector fyB, SUNMatrix JB,
                           void* cvode_mem, N_Vector tmp1B, N_Vector tmp2B,
                           N_Vector tmp3B);

static int cvLsJacBSWrapper(realtype t, N_Vector yB, N_Vector fyB, SUNMatrix JB,
                            void* cvode_mem, N_Vector tmp1B, N_Vector tmp2B,
                            N_Vector tmp3B);
...
static int cvLsPrecSolveBWrapper(realtype t, N_Vector yB, N_Vector fyB,
                                 N_Vector rB, N_Vector zB, realtype gammaB,
                                 realtype deltaB, int lrB, void* cvode_mem);
...
static int cvLsJacTimesSetupBWrapper(realtype t, N_Vector yB, N_Vector fyB,
                                     void* cvode_mem);
static int cvLsJacTimesSetupBSWrapper(realtype t, N_Vector yB, N_Vector fyB,
                                      void* cvode_mem);
...
static int cvLsLinSysBSWrapper(realtype t, N_Vector yB, N_Vector fyB,
                               ...
                               N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3);
...
int CVodeSetLinearSolver(void* cvode_mem, SUNLinearSolver LS, SUNMatrix A)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CVLS_MEM_NULL, "CVSLS", "CVodeSetLinearSolver",
                   MSG_LS_CVMEM_NULL);
    return (CVLS_MEM_NULL);
  }
  if (LS == NULL)
  {
    cvProcessError(NULL, CVLS_ILL_INPUT, "CVSLS", "CVodeSetLinearSolver",
                   "LS must be non-NULL");
  ...
  if (cvls_mem == NULL)
  {
    cvProcessError(cv_mem, CVLS_MEM_FAIL, "CVSLS", "CVodeSetLinearSolver",
                   MSG_LS_MEM_FAIL);
  ...
  if (LS->ops->setatimes)
  {
    ...
    {
      cvProcessError(cv_mem, CVLS_SUNLS_FAIL, "CVSLS", "CVodeSetLinearSolver",
...
int CVodeSetEpsLin(void* cvode_mem, realtype eplifac)
{
  ...
  if (eplifac < ZERO)
  {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVSLS", "CVodeSetEpsLin",
                   MSG_LS_BAD_EPLIN);
    return (CVLS_ILL_INPUT);
  }
                           ...
                           CVLsPrecSolveFn psolve)
{
  ...
  {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVSLS", "CVodeSetPreconditioner",
  ...
  if (retval != SUNLS_SUCCESS)
  {
    cvProcessError(cv_mem, CVLS_SUNLS_FAIL, "CVSLS", "CVLsSetPreconditioner",
                   "Error in calling SUNLinSolSetPreconditioner");
    return (CVLS_SUNLS_FAIL);
  }
              ...
              void* cvode_mem, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CVLS_MEM_NULL, "CVSLS", "cvLsDQJac", MSG_LS_CVMEM_NULL);
    return (CVLS_MEM_NULL);
  }
  cv_mem = (CVodeMem)cvode_mem;
  ...
  if (Jac == NULL)
  {
    cvProcessError(cv_mem, CVLS_LMEM_NULL, "CVSLS", "cvLsDQJac",
                   MSG_LS_LMEM_NULL);
                      ...
                      N_Vector vtemp3)
{
  ...
  if (jok)
  {
    ...
    *jcur = SUNFALSE;
    ...
    if (retval)
    {
      cvProcessError(cv_mem, CVLS_SUNMAT_FAIL, "CVSLS", "cvLsLinSys",
  ...
  else
  {
    ...
    *jcur = SUNTRUE;
    ...
    if (retval < 0)
    {
      cvProcessError(cv_mem, CVLS_JACFUNC_UNRECVR, "CVSLS", "cvLsLinSys",
...
int cvLsInitialize(CVodeMem cv_mem)
{
  ...
  {
    cvProcessError(cv_mem, CVLS_LMEM_NULL, "CVSLS", "cvLsInitialize",
                   MSG_LS_LMEM_NULL);
    return (CVLS_LMEM_NULL);
  }
  cvls_mem = (CVLsMem)cv_mem->cv_lmem;
              ...
              N_Vector vtemp3)
{
  ...
  if (cv_mem->cv_lmem == NULL)
  {
    cvProcessError(cv_mem, CVLS_LMEM_NULL, "CVSLS", "cvLsSetup",
                   MSG_LS_LMEM_NULL);
    return (CVLS_LMEM_NULL);
  }
  cvls_mem = (CVLsMem)cv_mem->cv_lmem;
  ...
  if (cvls_mem->A != NULL)
  {
    ...
    if (retval != CVLS_SUCCESS)
    {
      if (cvls_mem->user_linsys)
      {
        if (retval < 0)
        {
          cvProcessError(cv_mem, CVLS_JACFUNC_UNRECVR, "CVSLS", "cvLsSetup",
                         MSG_LS_JACFUNC_FAILED);
          cvls_mem->last_flag = CVLS_JACFUNC_UNRECVR;
          return (-1);
        }
              ...
              N_Vector fnow)
{
  ...
  if (cv_mem->cv_lmem == NULL)
  {
    cvProcessError(cv_mem, CVLS_LMEM_NULL, "CVSLS", "cvLsSolve",
                   MSG_LS_LMEM_NULL);
    return (CVLS_LMEM_NULL);
  }
  cvls_mem = (CVLsMem)cv_mem->cv_lmem;
  ...
  if (cvls_mem->jtsetup)
  {
    ...
    {
      cvProcessError(cv_mem, retval, "CVSLS", "cvLsSolve", MSG_LS_JTSETUP_FAILED);
      return (cvls_mem->last_flag);
    }
  }
  ...
  case SUNLS_PACKAGE_FAIL_UNREC:
    cvProcessError(cv_mem, SUNLS_PACKAGE_FAIL_UNREC, "CVSLS", "cvLsSolve",
                   "Failure in SUNLinSol external package");
    return (-1);
    break;
  case SUNLS_ATIMES_FAIL_UNREC:
    cvProcessError(cv_mem, SUNLS_ATIMES_FAIL_UNREC, "CVSLS", "cvLsSolve",
                   MSG_LS_JTIMES_FAILED);
    return (-1);
    break;
  case SUNLS_PSOLVE_FAIL_UNREC:
    cvProcessError(cv_mem, SUNLS_PSOLVE_FAIL_UNREC, "CVSLS", "cvLsSolve",
                    ...
                    CVLsMem* cvls_mem)
{
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CVLS_MEM_NULL, "CVSLS", fname, MSG_LS_CVMEM_NULL);
    return (CVLS_MEM_NULL);
  }
  *cv_mem = (CVodeMem)cvode_mem;
  if ((*cv_mem)->cv_lmem == NULL)
  {
    cvProcessError(*cv_mem, CVLS_LMEM_NULL, "CVSLS", fname, MSG_LS_LMEM_NULL);
    return (CVLS_LMEM_NULL);
  }
  *cvls_mem = (CVLsMem)(*cv_mem)->cv_lmem;
  return (CVLS_SUCCESS);
                          ...
                          SUNMatrix A)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CVLS_MEM_NULL, "CVSLS", "CVodeSetLinearSolverB",
                   MSG_LS_CVMEM_NULL);
    return (CVLS_MEM_NULL);
  }
  cv_mem = (CVodeMem)cvode_mem;
  ...
  {
    cvProcessError(cv_mem, CVLS_NO_ADJ, "CVSLS", "CVodeSetLinearSolverB",
                   MSG_LS_NO_ADJ);
    return (CVLS_NO_ADJ);
  }
  ...
  {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVSLS", "CVodeSetLinearSolverB",
                   MSG_LS_BAD_WHICH);
    return (CVLS_ILL_INPUT);
  }
  ...
  if (cvlsB_mem == NULL)
  {
    cvProcessError(cv_mem, CVLS_MEM_FAIL, "CVSLS", "CVodeSetLinearSolverB",
                   MSG_LS_MEM_FAIL);
    return (CVLS_MEM_FAIL);
  }
...
int CVodeSetJacFnB(void* cvode_mem, int which, CVLsJacFnB jacB)
{
  ...
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetJacFnB", &cv_mem,
...
int CVodeSetJacFnBS(void* cvode_mem, int which, CVLsJacFnBS jacBS)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetJacFnBS", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
...
int CVodeSetEpsLinB(void* cvode_mem, int which, realtype eplifacB)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetEpsLinB", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
...
int CVodeSetLSNormFactorB(void* cvode_mem, int which, realtype nrmfacB)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetLSNormFactorB", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
                      ...
                      CVLsJacTimesVecFnB jtimesB)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetJacTimesB", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
                       ...
                       CVLsJacTimesVecFnBS jtimesBS)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetJacTimesBS", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
...
int CVodeSetJacTimesRhsFnB(void* cvode_mem, int which, CVRhsFn jtimesRhsFn)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetJacTimesRhsFnB", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
...
int CVodeSetLinSysFnB(void* cvode_mem, int which, CVLsLinSysFnB linsysB)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetLinSysFnB", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
...
int CVodeSetLinSysFnBS(void* cvode_mem, int which, CVLsLinSysFnBS linsysBS)
{
  ...
  /* access relevant memory structures */
  retval = cvLs_AccessLMemB(cvode_mem, which, "CVodeSetLinSysFnBS", &cv_mem,
                            &ca_mem, &cvB_mem, &cvlsB_mem);
  if (retval != CVLS_SUCCESS) { return (retval); }
                           ...
                           N_Vector tmp3B)
{
  ...
  {
    cvProcessError(cv_mem, -1, "CVSLS", "cvLsJacBWrapper", MSG_LS_BAD_TINTERP);
    return (-1);
  }
                            ...
                            N_Vector tmp3B)
{
  ...
  if (retval != CV_SUCCESS)
  {
    cvProcessError(cv_mem, -1, "CVSLS", "cvLsJacBSWrapper", MSG_LS_BAD_TINTERP);
    return (-1);
  }
...
static int cvLsLinSysBWrapper(realtype t, N_Vector yB, N_Vector fyB,
                              SUNMatrix AB, booleantype jokB, booleantype* jcurB,
                              realtype gammaB, void* cvode_mem, N_Vector tmp1B,
                              N_Vector tmp2B, N_Vector tmp3B)
{
  CVodeMem cv_mem;
  CVadjMem ca_mem;
  ...
  if (retval != CV_SUCCESS)
  {
    cvProcessError(cv_mem, -1, "CVSLS", "cvLsLinSysBWrapper", MSG_LS_BAD_TINTERP);
    return (-1);
  }
                     ...
                     CVLsMemB* cvlsB_mem)
{
  ...
  *cv_mem = (CVodeMem)cvode_mem;
  ...
  if ((*cv_mem)->cv_adjMallocDone == SUNFALSE)
  {
    cvProcessError(*cv_mem, CVLS_NO_ADJ, "CVSLS", fname, MSG_LS_NO_ADJ);
    return (CVLS_NO_ADJ);
  }
  *ca_mem = (*cv_mem)->cv_adj_mem;
  ...
  if (which >= (*ca_mem)->ca_nbckpbs)
  {
    cvProcessError(*cv_mem, CVLS_ILL_INPUT, "CVSLS", fname, MSG_LS_BAD_WHICH);
    return (CVLS_ILL_INPUT);
  }
  ...
  *cvB_mem = (*ca_mem)->cvB_mem;
  ...
  {
    cvProcessError(*cv_mem, CVLS_LMEMB_NULL, "CVSLS", fname, MSG_LS_LMEMB_NULL);

...
int CVodeSetNonlinearSolver(void* cvode_mem, SUNNonlinearSolver NLS)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, "CVODES", "CVodeSetNonlinearSolver",
                   MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetNlsRhsFn(void* cvode_mem, CVRhsFn f)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, "CVODES", "CVodeSetNlsRhsFn", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
                                ...
                                N_Vector* zn1, void** user_data)
{
  ...
  if (cvode_mem == NULL)
  {
    cvProcessError(NULL, CV_MEM_NULL, "CVODES", "CVodeGetNonlinearSystemData",
                   MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }

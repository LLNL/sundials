                         ...
                         SUNContext sunctx)
{
  ...
  v = NULL;
  v = SUNCheckCallLastErrNull(N_VNewEmpty_Pthreads(length, num_threads, sunctx));
...
N_Vector N_VClone_Pthreads(N_Vector w)
{
  ...
  v = NULL;
  v = SUNCheckCallLastErrNull(N_VCloneEmpty_Pthreads(w));
...
void N_VScale_Pthreads(realtype c, N_Vector x, N_Vector z)
{
  ...
  if (z == x) {  /* BLAS usage: scale x <- cx */
    SUNCheckCallLastErrNoRet(VScaleBy_Pthreads(c, x));
    return;
  }

  if (c == ONE) {
    SUNCheckCallLastErrNoRet(VCopy_Pthreads(x, z));
    return;
  } else if (c == -ONE) {
    VNeg_Pthreads(x, z);
  } else {
...
SUNErrCode N_VLinearCombination_Pthreads(int nvec, realtype* c, N_Vector* X, N_Vector z)
{
  ...
  if (nvec == 2) {
    SUNCheckCallLastErrNoRet(N_VLinearSum_Pthreads(c[0], X[0], c[1], X[1], z));
    return SUN_SUCCESS;
  }
...
SUNErrCode N_VScaleAddMulti_Pthreads(int nvec, realtype* a, N_Vector x, N_Vector* Y, N_Vector* Z)
{
  ...
  /* should have called N_VLinearSum */
  if (nvec == 1) {
    SUNCheckCallLastErrNoRet(N_VLinearSum_Pthreads(a[0], x, ONE, Y[0], Z[0]));
    return SUN_SUCCESS;
  }

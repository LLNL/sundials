...
#define NVEC_RAJA_HBUFFERp(x) ((sunrealtype*) NVEC_RAJA_PRIVATE(x)->reduce_buffer_host->ptr)
#define NVEC_RAJA_DBUFFERp(x) ((sunrealtype*) NVEC_RAJA_PRIVATE(x)->reduce_buffer_dev->ptr)
...
sunrealtype N_VMin_Raja(N_Vector X)
{
  ...
  RAJA::ReduceMin< SUNDIALS_RAJA_REDUCE, sunrealtype> gpu_result(std::numeric_limits<sunrealtype>::max());
  RAJA::forall< SUNDIALS_RAJA_EXEC_REDUCE >(RAJA::RangeSegment(zeroIdx, N),
    [=] RAJA_DEVICE (sunindextype i) {

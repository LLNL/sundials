                      ...
                      sunrealtype* d_vdata, ::sycl::queue* Q, SUNContext sunctx)
{
  ...
  NVEC_SYCL_CONTENT(v)->length      = length;
  NVEC_SYCL_CONTENT(v)->own_helper  = SUNTRUE;
  NVEC_SYCL_CONTENT(v)->host_data   = SUNMemoryHelper_Wrap(h_vdata,
                                                           SUNMEMTYPE_HOST);
  NVEC_SYCL_CONTENT(v)->device_data = SUNMemoryHelper_Wrap(d_vdata,
                                                           SUNMEMTYPE_DEVICE);
  NVEC_SYCL_CONTENT(v)->stream_exec_policy =
    new ThreadDirectExecPolicy(SYCL_BLOCKDIM(Q));
  NVEC_SYCL_CONTENT(v)->reduce_exec_policy =
                             ...
                             ::sycl::queue* Q, SUNContext sunctx)
{
  ...
  NVEC_SYCL_CONTENT(v)->device_data =
    SUNMemoryHelper_Alias(NVEC_SYCL_CONTENT(v)->host_data);
  NVEC_SYCL_CONTENT(v)->stream_exec_policy =
    new ThreadDirectExecPolicy(SYCL_BLOCKDIM(Q));
  NVEC_SYCL_CONTENT(v)->reduce_exec_policy =
    new BlockReduceExecPolicy(SYCL_BLOCKDIM(Q));
...
void N_VSetHostArrayPointer_Sycl(sunrealtype* h_vdata, N_Vector v)
{
  if (N_VIsManagedMemory_Sycl(v))
  {
    ...
    {
      NVEC_SYCL_CONTENT(v)->host_data = SUNMemoryHelper_Wrap((void*)h_vdata,
                                                             SUNMEMTYPE_UVM);
      NVEC_SYCL_CONTENT(v)->device_data =
        SUNMemoryHelper_Alias(NVEC_SYCL_CONTENT(v)->host_data);
    }
...
void N_VSetDeviceArrayPointer_Sycl(sunrealtype* d_vdata, N_Vector v)
{
  if (N_VIsManagedMemory_Sycl(v))
  {
    ...
    {
      NVEC_SYCL_CONTENT(v)->device_data = SUNMemoryHelper_Wrap((void*)d_vdata,
                                                               SUNMEMTYPE_UVM);
      NVEC_SYCL_CONTENT(v)->host_data =
        SUNMemoryHelper_Alias(NVEC_SYCL_CONTENT(v)->device_data);
    }
  }
  else
  {
...
int N_VSetKernelExecPolicy_Sycl(N_Vector x, SUNSyclExecPolicy* stream_exec_policy,
...
int N_VLinearSumVectorArray_Sycl(int nvec, sunrealtype a, N_Vector* X,

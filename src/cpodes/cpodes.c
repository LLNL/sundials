...
/* 
 * Control constants for lower-level functions used by cpStep 
 * ----------------------------------------------------------
 *
 * cpHin return values:
 *    CP_SUCCESS
 ...
 *    CP_TOO_CLOSE
 *
 * cpStep return values: 
 *    CP_SUCCESS,
 *    CP_LSETUP_FAIL,  CP_LSOLVE_FAIL, 
 *    CP_ODEFUNC_FAIL, CP_RTFUNC_FAIL
 *    CP_CONV_FAILURE, CP_ERR_FAILURE,
 *    CP_FIRST_ODEFUNC_ERR
 * 
 */
          ...
          N_Vector yout, N_Vector ypout, int mode)
{
  ...
  if (nst > 0) {
    ...
    if ( task == CP_ONE_STEP && SUNRabs(tn - tretlast) > troundoff ) {
      tretlast = *tret = tn;
      cpGetSolution(cp_mem, tn, yout, ypout);
      return(CP_SUCCESS);
    }
...
static int cpHinExpl(CPodeMem cp_mem, realtype hlb, realtype hub, int sign, realtype *h0)
{
  ...
  for(count1 = 1; count1 <= H_MAXITERS; count1++) {
    ...
    hnew = (yppnorm*hub*hub > TWO) ? SUNRsqrt(TWO/yppnorm) : SUNRsqrt(hg*hub);
    hrat = hnew/hg;
                       ...
                       realtype alpha0_hat, realtype xi_inv, realtype xistar_inv)
{
  ...
  A2 = ONE + q * A1;
  tq[2] = SUNRabs(alpha0 * (A2 / A1));
  tq[5] = SUNRabs((A2) / (l[q] * xi_inv/xistar_inv));
  if (qwait == 1) {
    C = xistar_inv / l[q];
    A3 = alpha0 + ONE / q;
    A4 = alpha0_hat + xi_inv;
    CPrime = A3 / (ONE - A4 + A3);
    tq[1] = SUNRabs(CPrime / C);
    hsum += tau[q];
    xi_inv = h / hsum;
    A5 = alpha0 - (ONE / (q+1));
    A6 = alpha0_hat - xi_inv;
    CPrimePrime = A2 / (ONE - A6 + A5);
    tq[3] = SUNRabs(CPrimePrime * xi_inv * (q+2) * A5);
  }
  tq[4] = nlscoef * tq[2];
}
                         ...
                         int *nefPtr, realtype *dsmPtr)
{
  ...
  *dsmPtr = dsm; 
  ...
  if ((SUNRabs(h) <= hmin*ONEPSM) || (*nefPtr == maxnef)) return(CP_ERR_FAILURE);
...
static int cpSLdet(CPodeMem cp_mem)
{
  ...
  for (k=1; k<=3; k++) {
    ...
    rav[k] = PT25 * sumrat;
    vrat[k] = SUNRabs(PT25*sumrsq - rav[k]*rav[k]);

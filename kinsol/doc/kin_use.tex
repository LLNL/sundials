...
preconditioner, etc. For example, in the \id{kinkryx} example~\cite{kinsol2.2.0_ex}, 
...
{
  The half-bandwidth pairs \id{mudq},\id{mldq} and \id{mukeep},\id{mlkeep} need
...
}
{
  ...
  $N_l \,(2$ \id{ml} $+$ \id{mu} $+$ \id{smu} $+2)$ \id{realtype} words,
  where \id{smu} = $\min ( N_l - 1 ,$ \id{mu} $+$ \id{ml}$)$.
...
%%==============================================================================
\section{FKINSOL, a FORTRAN-C interface module}\label{ss:fcmix}
%%==============================================================================
...
\end{verbatim}
  in which the arguments are: \id{KEY} = 3, \id{NLOCAL} the local size of vectors for this

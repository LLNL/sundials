...
}
{
  ...
  \item[u\_scale]
    vector containing diagonal elements of scaling matrix for vector \id{u}
    chosen so that the components of \id{u\_scale}$\cdot$\id{u}
  ...
  \item[f\_scale]
    ...
    so that the components of \id{f\_scale}$\cdot F($\id{u}$)$ 
...
{
  The components of vectors \id{u\_scale} and \id{f\_scale} should be positive.
...
{
  The default value of \id{mxnewtstep} is $1000 \| D_u u_0 \|_2$,
...
\index{Jacobian times vector approximation!difference quotient}
...
\index{preconditioning!user-supplied|)}
%%
\index{Jacobian times vector approximation!user-supplied}
...
}
{
  ...
  If all necessary comunication was done in \id{func}, then \id{Gcomm = NULL}
...
\end{verbatim}
  ...
  \index{Jacobian times vector approximation!use in {\fkinsol}}
...
\end{verbatim}
  ...
  the current value of the unknown vector $y$. \id{NEWU} is an input integer 
...
\end{verbatim}
  ...
  called, so that \id{FVAL} will have been updated. \id{U} is the current solution
  iterate. The arrays \id{VTEMP1} and \id{VTEMP2} are available for work space. 
  If scaling is being used, \id{USCALE} and \id{FSCALE} are available for those operatins
...
In addition to the Fortran right-hand side function \id{FKFUN}, the
...
\end{verbatim}
  ...
  words.  This size is local to the current processor.
  \id{NGE} is the number of $G(u)$ evaluations (calls to \id{FKLOCFN}) so far.

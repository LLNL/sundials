...
\item\label{i:kinsol_create} 
  ...
  See \S\ref{sss:kinsolmalloc} for details.
...
\item
  ...
  See \S\ref{ss:optional_inputs} for details.
...
}
{
  ...
  \item[new\_uu]
    ...
    if appropriate, unless \id{new\_uu = FALSE}.
  \item[jac\_data]
    is a pointer to user data --- the same as the \id{jac\_data}      
...
}
{
  ...
  If the user-provided \id{KINSpilsJacTimesVec} function needs the unit roundoff,
...
\id{FKJTIMES} & \id{FKINJtimes}   & \id{KINSpilsJacTimesVecFn} \\\hline
\end{tabular}
\end{center}
...
\end{verbatim}
  ...
  includes the \id{MPI\_Comm\_f2c} function, then \id{COMM} can be any valid

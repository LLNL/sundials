...
\id{KINSpgmrSetPreconditioner} (see \ugref{ss:optional_input}).
The \id{data} pointer passed to \id{KINSpgmrSetPreconditioner} is passed to \id{PrecSetupBD} 
...
In this example, \id{kinkry\_bbd\_p}, we solve the same problem as with \id{kinkryx}
...
The output generated by \id{kinkry\_bbd\_p} is shown below.  Note that 9 Newton
iterations were required, with an average of about 51.6 Krylov iterations
per Newton iteration.

\includeOutput{kinkry\_bbd\_p}{../examples_par/kinkry_bbd_p.out}

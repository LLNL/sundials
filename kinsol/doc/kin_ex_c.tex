...
\index{examples, simulation!serial sample program!problem solved by}.
...
\index{examples, simulation!serial sample program!explanation of|(}
Following the initial comment block, this program has a number
of \id{\#include} lines, which allow access to useful items in {\cvodes}
header files.  The \id{sundialstypes.h} file provides the definitions of the
types \id{realtype} and \id{integertype} (see \S\ref{s:types} for
details).  For now, it suffices to read \id{realtype} as \id{double} and
\id{integertype} as \id{long int}.
The \id{cvodes.h} file provides prototypes for the three {\cvodes}
...
The \id{cvsdense.h} file provides the prototype for the \id{CVDense} 
...
it is included by \id{cvsdense.h}.

This program includes two user-defined accessor macros,
\id{Ith} and \id{IJth}
\index{examples, simulation!serial sample program!user-defined accessor macros}
...
needed in \id{f}.  The second \id{NULL} forces {\cvodes} error messages to
...
\id{y} and \id{abstol}, calls \id{CVodeFree} to free the {\cvodes}
...
See \S\ref{ss:nvec_ser} for details on \id{M\_EnvFree\_Serial}.
...
\index{examples, simulation!serial sample program!output|(}
%%{\small\input{cvs_cvdxo}}
\index{examples, simulation!serial sample program!output|)}
...
\index{examples, simulation!parallel sample program!problem solved by|(}
...
\index{examples, simulation!parallel sample program!explanation of|(}
...
\index{examples, simulation!parallel sample program!output|(}
%%{\small\input{cvs_pvkxo}}
\index{examples, simulation!parallel sample program!output|)}

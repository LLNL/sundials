...
void  F_KINITMPI(integer *nlocal, integer *nglobal, int *ier)
{

  /* Call PVecInitMPI to initialize KINSOL: 
     ...
     *nglobal is the global vector length
     ...
     A pointer to the machEnv is returned and stored as KIN_machEnv. */

 int dumargc; char **dumargv;

  KIN_machEnv =  PVecInitMPI(MPI_COMM_WORLD, *nlocal, *nglobal,
			     &dumargc, &dumargv);

  *ier = (KIN_machEnv == NULL) ? -1 : 0 ;
}
...
void F_KFREEMPI()
{
  ...
  PVecFreeMPI (KIN_machEnv);
}

/***************************************************************************/


void F_PKINMALLOC(integer *neq, integer *ier)
{
  ...
  KIN_kmem = KINMalloc(*neq,  NULL, KIN_machEnv);
...
{ N_Vector uuvec, uscalevec, fscalevec, constraintsvec; integer i;
  ...
  N_VMAKE(uuvec, uu, KIN_machEnv);
  N_VMAKE(uscalevec, uscale, KIN_machEnv);
  N_VMAKE(fscalevec, fscale, KIN_machEnv);
  N_VMAKE(constraintsvec, constraints, KIN_machEnv);

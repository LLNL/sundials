...
#define SUN_ERR_CODE_LIST(ENTRY)                                                                       \
  ...
  ENTRY(SUN_ERR_PROFILER_MAPFULL, "too many SUNProfiler entries, try setting SUNPROFILER_MAX_ENTRIES " \
                                  "in environment to a bigger number")                                 \
  ENTRY(SUN_ERR_PROFILER_MAPGET, "unknown error getting SUNProfiler timer")                            \
  ENTRY(SUN_ERR_PROFILER_MAPINSERT, "unknown error inserting SUNProfiler timer")                       \
  ...
  ENTRY(SUN_ERR_LOGGER_CANNOTOPENFILE, "File provided to SUNLogger could not be opened")               \
  ENTRY(SUN_ERR_MALLOC_FAIL, "malloc returned NULL")                                                   \
  ENTRY(SUN_ERR_NOT_IMPLEMENTED, "operation is not implemented: function pointer is NULL")             \
...
extern "C"
{
  ...
  typedef int (*SUNErrHandlerFn)(int line, const char* func, const char* file, const char* msg, SUNErrCode err_code,
                                 void* err_user_data, SUNContext sunctx);
  struct SUNErrHandler_
  {
    struct SUNErrHandler_* previous; /* next error handler to call (singly linked-list) */
  ...
  int SUNMPIAbortErrHandlerFn(int line, const char* func, const char* file, const char* msg, SUNErrCode err_code,
                              void* err_user_data, SUNContext sunctx);

  int SUNMPIAssertErrHandlerFn(int line, const char* func, const char* file, const char* msg, SUNErrCode err_code,
                               void* err_user_data, SUNContext sunctx);
  ...
  static inline int SUNHandleErr(int line, const char* func, const char* file, SUNErrCode code, SUNContext sunctx)
  {
  ...
  static inline int SUNHandleErrWithMsg(int line, const char* func, const char* file, const char* msg, SUNErrCode code,
                                        SUNContext sunctx)
  {
    sunctx->last_err = code;
    SUNErrHandler eh = sunctx->err_handler;
...
#define SUNCheckCall(call, sunctx)                                                \
  do {                                                                            \
    ...
    if (sun_chk_call_err_code_ < 0) {                                             \
      SUNHandleErr(__LINE__, __func__, __FILE__, sun_chk_call_err_code_, sunctx); \
    }                                                                             \
  } while (0);
...
#define SUNAssert(expr, code, sunctx)                                                                   \
  do {                                                                                                  \
    if (!(expr)) {                                                                                      \
      SUNAssertErrHandlerFn(__LINE__, __func__, __FILE__, "", code, sunctx->err_handler->data, sunctx); \
    }                                                                                                   \
  } while (0);
#endif
...
#define SUNMPIAssert(expr, code, sunctx)                                                                   \
  do {                                                                                                     \
    if (!(expr)) {                                                                                         \
      SUNMPIAssertErrHandlerFn(__LINE__, __func__, __FILE__, "", code, sunctx->err_handler->data, sunctx); \
    }                                                                                                      \
  } while (0);
#else
#define SUNMPIAssert(expr, code, sunctx)

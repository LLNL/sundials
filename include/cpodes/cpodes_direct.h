...
#define CPDIRECT_JACFUNC_RECVR    -6
  ...
  int CPDlsSetJacFn(void *cvode_mem, void *jac, void *jac_data);
  ...
  int CPDlsGetWorkSpace(void *cvode_mem, long int *lenrwLS, long int *leniwLS);
  int CPDlsGetNumJacEvals(void *cvode_mem, long int *njevals);
  int CPDlsGetNumFctEvals(void *cvode_mem, long int *nfevalsLS);
  int CPDlsGetLastFlag(void *cvode_mem, int *flag);

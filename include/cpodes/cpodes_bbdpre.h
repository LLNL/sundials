 ...
 * matrix with banded blocks, for use with CPSPGMR/CPSPBCG/CPSPTFQMR, 
 ...
 *    CPodeMalloc, and the names of the user's gloc and cfn
 *    functions are passed to CPBBDPrecAlloc.
 *
 ...
 * The implementation of this function must have type CPLocalFn.
 *
 ...
 * through the CPodeSetFdata routine.
 *
 * A CPLocalFn should return 0 if successful, a positive value if 
 * a recoverable error occurred, and a negative value if an 
 * unrecoverable error occurred.
 * -----------------------------------------------------------------
 */

typedef int (*CPLocalFn)(long int Nlocal, realtype t, N_Vector y,
 ...
 * Type : CPCommFn
 * -----------------------------------------------------------------
 * The user may supply a function of type CPCommFn which performs
 ...
 * The f_data parameter is the same as that specified by the user
 * through the CPodeSetFdata routine. The CPCommFn cfn is
 * expected to save communicated data in space defined within the
 * structure f_data. Note: A CPCommFn cfn does not have a return value.
 *
 * Each call to the CPCommFn cfn is preceded by a call to the
 * CPRhsFn f with the same (t,y) arguments. Thus cfn can omit any
 * communications done by f if relevant to the evaluation of g.
 * If all necessary communication was done by f, the user can
 * pass NULL for cfn in CPBBDPrecAlloc (see below).
 *
 * A CPCommFn should return 0 if successful, a positive value if 
 * a recoverable error occurred, and a negative value if an 
 * unrecoverable error occurred.
 * -----------------------------------------------------------------
 */

typedef int (*CPCommFn)(long int Nlocal, realtype t, N_Vector y,
                        void *f_data);
 ...
 *     execution of gloc.
 *
 * CPBBDPrecAlloc returns the storage allocated (type *void),
...
int CPBBDPrecReInit(void *bbd_data, long int mudq, long int mldq,
                    realtype dqrely, CPLocalFn gloc, CPCommFn cfn);

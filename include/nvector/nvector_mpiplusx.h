...
{
  SUNAssignSUNCTX(v->sunctx);
  SUNCheckCallLastErrNoRet(N_VSetSubvectorArrayPointer_MPIManyVector(vdata, v, 0));
}
...
{
  SUNAssignSUNCTX(v->sunctx);
  N_Vector result = SUNCheckCallLastErrNoRet(N_VGetSubvector_MPIManyVector(v, 0));
  return result;
}

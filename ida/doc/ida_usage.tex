...
%%--------------
%%
\subsection{Residual function}\label{ss:rhsFn}
\index{residual function}
The user must provide a function of type \ID{IDAResFn} defined as follows:
\usfunction{IDAResFn}
{
  typedef void (*IDAResFn)(&realtype t, N\_Vector y, N\_Vector ydot, \\
                           &void *res\_data);
}
{
...
{
  typedef int (*IDADenseJacFn)(&long int N, DenseMat J, realtype t, \\
...
{
 typedef int (*IDABandJacFn)(&long int N, long int mupper, \\
...
}
{
  ...
  the \id{IDAResFn} user function with the same \id{(t,y)} arguments.  
...
\usfunction{IDABBDLocalFn}
{
  typedef void (*IDABBDLocalFn)(&long int Nlocal, realtype tt,  \\
                                ...
                                &N\_Vector Glocal, void *res\_data);
}
{
   ...
   \id{
     \begin{tabular}[t]{@{}r@{}l@{}}
       bbd\_data = IBBDPrecAlloc(&ida\_mem, local\_N, mudq, mldq, \\
                                 &mukeep, mlkeep, dqrely, Gres, Gcomm);
     \end{tabular}
   }
...
\id{dqrely}, or one of the user-supplied functions \id{Gres} and \id{Gcomm}.
%%
\ucfunction{IBBDPrecReInit}
{
  flag = IBBDPrecReInit(bbd\_data, mudq, mldq, dqrely, Gres, Gcomm);
}
{

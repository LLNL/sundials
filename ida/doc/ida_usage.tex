...
the residual function $G(t,y) \approx F(t,y)$ and which is computed locally,
...
\end{verbatim}
  ...
  \id{NGLOBAL} the system size (and the global size of vectors, that
...
\end{verbatim}
  ...
  approximation.  The argument \id{IER} is an error return flag which can be $0$ 
  for success or non-zero if an error occurred.
...
\end{verbatim}
  ...
  $Jv$.  \id{IER} is an error return flag which can be $0$ to indicate success, $-1$
...
\item {\bf {\idabbdpre} Optional outputs}
  ...
  \id{NPS}, \id{NLI}, \id{NCF}, \id{NJE}, \id{NRE}, and \id{LAST\_FLAG}, stored in
  \id{IOPT(26)} $\cdots$ \id{IOPT(34)}, respectively.
  Optional outputs specific to the {\sptfqmr} solver are \id{LRW}, \id{LIW},
  \id{NPE}, \id{NPS}, \id{NLI}, \id{NCF}, \id{NJE}, \id{NRE}, and \id{LAST\_FLAG},
  stored in \id{IOPT(26)} $\cdots$ \id{IOPT(34)}, respectively.
...
\end{verbatim}
  ...
  words.  This size is local to the current processor.  \id{NGE}

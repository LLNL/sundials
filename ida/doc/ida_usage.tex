...
}
{
  ...
  If \id{IDACalcIC} (see \S\ref{ss:idacalcic}) will be called to compute consistent initial
  conditions, the vectors \id{y0} and \id{yp0} must remain unaltered until after that call.
}
%%
%%
\ucfunction{IDAFree}
...
}
{
  ...
  After a successful return from \id{IDACalcIC} the pointers to the vectors \id{y0} 
  and \id{yp0} (passed as arguments to \id{IDAMalloc}) are released and, if so desired,
  the user can deallocate them.
}
...
Jacobian times vector function & \id{IDASpgmrSetJacTimesVecFn} &
internal DQ \\
...
name through a call to \id{IDASpgmrSetPrecSolveFn}.

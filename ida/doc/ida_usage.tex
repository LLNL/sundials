...
}
{
  ...
  If an error occured, \id{IDACreate} prints an error message to \id{stderr}
  and returns \id{NULL}.
}
{}
%%
...
{
  flag = IDASolve(ida\_mem, icopt, tout1);
}
{
  The function \ID{IDACalcIC} corrects the intial values \id{y0}, \id{yp0}.
}
{
  \begin{args}[ida\_mem]
  ...
  \item[tout1] (\id{realtype})
    is the first value of $t$ at which a soluton will be requested (from \id{IDASolve}).
...
}
{
  ...
  The return value \id{flag} (of type \id{int}) will be one of the following:
  \begin{args}[INTERMEDIATE\_RETURN]
  \item[\Id{SUCCESS}]
    \id{IDASolve} succeeded.
...
}
{
  ...
  Passing a value \id{errfp}$=$\id{NULL} disables all future error message printing.
}
%%
%%
...
{
  The function \ID{IDASetRdata} specifies the user data block \Id{r\_data}
...
}
{
  ...
  \item[suppresslag] (\id{booleantype})
    flag to indicate whether to suppress (\id{TRUE}) or not
    (\id{FALSE}) the algebaric variables in the local error test.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
}
{
  ...
  must be set (through \id{IDASetId}) to specify the algebnraic components.
}
%%
%%
\ucfunction{IDASetId}
...
{
  The vactor \id{id} is required if the algebraic variables are to be 
...
}
{
  ...
  all components will caouse constraint checking to be performed.
}
%%
\index{optional input!solver|)}
%%
...
}
{
  ...
  of $J^{-1}F(t_0, y, {\dot y}) \le $\id{epicon}, where $J$ is the system Jacobian.
}
%%
...
{
  The function \ID{IDASetStepToleranceIC} specifies a positve lower bound
  on the Newton step.
}
{
  \begin{args}[ida\_mem]
...
The user must provide a function of type \ID{ResFn} defined as follows:
\usfunction{ResFn}
{
  typedef void (*ResFn)(realtype t, N\_Vector y, N\_Vector ydot, void *f\_data);
}
{
...
module calls to construct $P$: a function \id{glocal} (of type \id{IDALocalFn}) 
which approximates the right-hand side function $G(t,y,{\dot y}) \approx F(t,y,{\dot y})$ 
and which is computed locally, and a function \id{gcomm} (of type \id{IDACommFn}) which 
performs all inter-process communication necessary to evaluate the approximate 
residual $G$.
...
providing space (presumably within \id{res\_data}) for components of \id{y}
and \id{yp} that are communicated by \id{gcomm} from the other processors, and that are
then used by \id{glocal}, which is not expected to do any communication.
%%
%%
\usfunction{IDALocalFn}
{
  typedef void (*IDALocalFn)(&long int Nlocal, realtype tt,  \\
                             &N\_Vector yy, N\_Vector yp, \\
                             &N\_Vector gval, void *res\_data);
}
{
...
{
  An \id{IDALocalFn} function type does not have a return value.
}
{
...
%%
%%
\usfunction{IDACommFn}
{
  typedef void (*IDACommFn)(&long int Nlocal, realtype tt,  \\
                           &N\_Vector yy, N\_Vector yp, void *f\_data);
}
{
  This function performs all inter-processor communications necessary 
  for the execution of the \id{glocal} function above, using the input 
...
{
  An \id{IDACommFn} function type does not have a return value.
}
{
   ...
   \id{
     \begin{tabular}[t]{@{}r@{}l@{}}
       bbd\_data = IBBDPrecAlloc(&ida\_mem, local\_N, mudq, mldq, \\
                                  &mukeep, mlkeep, dqrely, glocal, gcomm);
     \end{tabular}
   }
...
}
{
  ...
  \item[glocal] (\id{IDALocalFn})
    the {\C} function which computes the approximation 
    $G(t,y,{\dot y}) \approx F(t,y,{\dot y})$. 
...
\id{dqrely}, or one of the user-supplied functions \id{glocal} and \id{gcomm}.
%%
\ucfunction{IBBDPrecReInit}
{
  flag = IBBDPrecReInit(bbd\_data, mudq, mldq, dqrely, glocal, gcomm);
}
{

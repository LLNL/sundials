...
}
{
  ...
  \item[itol] (\id{int}) 
    is one of \Id{IDA\_SS}, \Id{IDA\_SV}, or \ID{IDA\_WF}, where \Id{itol}$=$\id{IDA\_SS}
...
of the functions \Id{IDADense}, \Id{IDABand}, \Id{IDASpgmr}, \Id{IDASpbcg}, \Id{IDASptfqmr},
...
the user can also specify in the call to \id{IDASpgmrSetJacTimesVecFn}, a pointer to a 
...
\id{IDASpbcgSetPreconditioner} to the preconditioner setup and solve functions.  
This allows the user to create an arbitrary structure with relevant problem data 
and access it during the execution of the user-supplied preconditioner functions
without using global data in the program.  
The pointer \id{p\_data} may be identical to \id{res\_data}, if the latter was 
...
\id{IDASptfqmrSetPreconditioner} to the preconditioner setup and solve functions.  
This allows the user to create an arbitrary structure with relevant problem data 
and access it during the execution of the user-supplied preconditioner functions
without using global data in the program.  
The pointer \id{p\_data} may be identical to \id{res\_data}, if the latter was 
...
\label{ss:jtimesFn}
\index{Jacobian approximation function!Jacobian times vector!user-supplied|(}
If an iterative {\spgmr}, {\spbcg} or {\sptfqmr} linear solver is selected (\id{IDASp*} is called in step 
...
}
{
  ...
  \item[tmp3]
    are pointers to memory allocated for variables of type \id{N\_Vector} 
    which can be used by \id{IDASpilsPrecSetupFn} as temporary storage or work space.
  \end{args}
}
{
...
\end{verbatim}
  in which the arguments are: \id{COMM} = {\mpi} communicator, \id{KEY} = 1, \id{NLOCAL} the local size of vectors on this
  ...
  {\warn} If the header file \id{sundials\_config.h} defines \id{SUNDIALS\_MPI\_COMM\_F2C} to be $0$, then the argument

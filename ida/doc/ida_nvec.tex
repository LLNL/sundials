...
within each of the five {\ida} linear solvers ({\idasp} stands for any
of {\idaspgmr}, {\idaspbcg}, or {\idasptfqmr}), the {\idabbdpre}
preconditioner module, and the {\fida} module.

There is one subtlety in the {\idasp} column hidden by the table, explained
...
the implementation files \id{sundials\_spgmr.c} and \id{iterative.c} for the generic
...
\id{idaspgmr.c}, they are called within the implementation file
\id{spgmr.c} and so are required by the {\idaspgmr} solver module.
...
\begin{sideways}{\idasp}     \end{sideways} &

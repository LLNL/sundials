...
\item \id{idakryx}
  solves the same problem as \id{idabanx1}, with the Krylov linear solver
...
\item \id{idakryx1\_p}
  solves the same problem as \id{idakryx1\_bbd\_p} with a user-supplied preconditioner
...
\item \id{idakryx2\_p}
  solves the same problem as \id{idakryx2\_bbd\_p} with a user-supplied preconditioner.
...
\id{sundials/ida/fcmix/examples\_ser} and
\newline \id{sundials/ida/fcmix/examples\_par}, are the following
...
\item \id{fidakryx\_bbd\_p} is a parallel example (\id{BDF}/{\spgmr}/{\idabbdpre})
       equivalent to \id{idakryx1\_bbd\_p}.

...
\item \id{iheatskt}
  solves the same problem as \id{iheatsk}, with three Krylov linear solvers
...
%
\begin{itemize}
\item \id{irobxf} is a serial chemical kinetics example (\id{BDF}/{\dense})
       with rootfinding, equivalent to \id{irobx}.

\item \id{iheatbbdf} is a parallel example (\id{BDF}/{\spgmr}/{\idabbdpre})
       equivalent to \id{iheatbbd}.

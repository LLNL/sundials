...
\cite{SaSc:86}, Bi-CGStab (Bi-Conjugate Gradient Stabilized), or TFQMR
(Transpose Free Quasi-Minimal Residual) linear iterative methods, and
requires almost no matrix storage for solving the Newton equations as
compared to direct methods.  However, the GMRES, Bi-CGStand and TFQMR algorithms
allow for a user-supplied preconditioner matrix, and for most problems
preconditioning is essential for an efficient solution.
...
\subsection*{Changes in vx.y.z}

{\fida}, a {\F}-{\C} interface module, was added (for details see \S\ref{ss:fcmix}).

{\idaspbcg} and {\idasptfqmr} modules have been added to interface with the
Scaled and Preconditioned Bi-CGstab ({\spbcg}) and Scaled and Preconditioned

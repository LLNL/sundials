...
\id{ida.h}, \id{ida\_impl.h} and \id{ida.c}, deals with the evaluation of integration coefficients,
...
methods {\idaspgmr}, {\idaspbcg} and {\idasptfqmr}, the package includes an algorithm for
the approximation by difference quotients of the product between the Jacobian matrix and
a vector of appropriate length. Again, the user has the option of providing
a routine for this operation.
In \index{preconditioning!setup and solve phases} the case of {\idaspgmr}, {\idaspbcg} and
{\idasptfqmr}, the preconditioning must be supplied by the user in two phases: 

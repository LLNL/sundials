...
type \id{realtype} (see \ugref{s:types} in the user guide \cite{ida2.2.0_ug}
...
\id{IDADenseSetJacFn} (see \ugref{ss:optional_input}) specify the {\idadense}
...
\id{IDAFree} to free the {\ida} memory block and \id{NV\_Destroy\_Serial}
...
the number of residual evaluations for Jacobian evaluations (\id{nreD}),
...
Similarly, the function \id{grob} defines the two functions, $g_0$ and $g_1$,
whose roots are to be found.  See \ugref{ss:root_us} for a detailed description
of the \id{grob} function.

The output generated by \id{irobx} is shown below.  It shows the output
...
\id{N\_Vector} type.  The header file \id{smalldense.h} is included for the
...
(\ref{e:iwebsb_PDE}).  It first calls the private function \id{Fweb} to
...
The calls to \id{IDASpgmrSetPrecSetupFn} and \id{IDASpgmrSetPsolveFn}
specify the use of the user-supplied preconditioner with \id{data}
being the pointer to user data passed to \id{PsolveHeat} and \id{PsetupHeat} 
whenever they are called (specified with the call to \id{IDASpgmrSetPrecData}).
%%

...
The \id{iheatpk} main program begins with MPI calls to initialize MPI and to
...
(\id{pp}).  As in \id{iheatsk}, constraints are passed to {\ida} through the
...
In addition, for illustration purposes, \id{iheatsk} also excludes the
...
As with \id{iheatpk}, we use a \id{NPEX} $\times$ \id{NPEY} processor grid,
with an \id{MXSUB} $\times$ \id{MYSUB} submesh on each.  Again, the residual
...
That is the value supplied as \id{mudq} and \id{mldq} in the call to
...
In the \id{iwebbbd} main program, following MPI initializations and
...
allocated, and the {\idabbdpre} preconditioner is initialized, and the

%%==============================================================================
\chapter{FIDA, a {\F}-{\C} interface module}\label{ss:fcmix}
%%==============================================================================
...
the set of five flags - \Id{SUNDIALS\_CASE\_UPPER}, \Id{SUNDIALS\_CASE\_LOWER},
\Id{SUNDIALS\_UNDERSCORE\_NONE}, \Id{SUNDIALS\_UNDERSCORE\_ONE}, and \\
\Id{SUNDIALS\_UNDERSCORE\_TWO} can be explicitly defined in \id{sundial\_config.h}
...
for real optional inputs, and \Id{FIDASETVIN} for real vector optional inputs. 
...
\end{verbatim}
where 
\id{KEY} indicates which optoinal input is set (see Table \ref{t:fida_in}),
\id{IVAL} is the integer value to be used,
\id{RVAL} is the real value to be used,
\id{VVAL} is the real vector to be used, and
\id{IER} is an integer return completion flag which is set to $0$ on success and on 
a negative value if a failure occurred.

When using \id{FIDASETVIN} to specify the variable types (\id{KEY='ID\_VEC'})
the components in the vector \id{VVAL} must be $1.0$ to indicate a differential 
variable, or $0.0$ to indicate an algebraic variable.
Note that this vector is required only if \id{FIDACALCIC} is to be called
...
%
When using \id{FIDASETVIN} to specify optional constraints on the solution vector
(\id{KEY='CONSTR\_VEC'}) the components in the vector \id{VVAL} should be one of
$-2.0$, $-1.0$, $0.0$, $1.0$, or $2.0$. See the description of \id{IDASetConstraints}
for more details.

The optional outputs from the {\ida} solver are accessed not through individual
functions, but rather through a pair of arrays, \Id{IOUT} of dimension at least $21$ 
and  integer type and \Id{ROUT} of dimension at least $6$ and real type. 
These arrays are owned (and allocated) by the user and are passed as arguments to 
\id{FIDAMALLOC}.
Table \ref{t:fida_out} lists the entries in these two arrays and specifies the
optional variable as well as the {\ida} function which is actually called to
...
\Id{SUPPRESS\_ALG}   & Suppress alg. vars. from error test  & FALSE  \\
...
\Id{LS\_OFF\_IC}  & Turn off line search & FALSE  \\
...
\Id{STEP\_TOL\_IC} & Lower bound on Newton step for IC calc. & $(2/3) \cdot$ uround \\
\hline
\multicolumn{3}{c}{}\\
...
{\bf Key} & {\bf Optional input} & Default value \\ 
\hline
\Id{ID\_VEC}   & Variable types & undefined\\
...
calls and to te optional outputs extracted in \id{IOUT}
...
\end{verbatim}
This computes the \id{EWT} array as a function of the input \id{Y} array.
...
\end{verbatim}
  ...
  \id{NLOCAL} = the local size of vectors on this processor, and
...
\item {\bf Problem specification}
  ...
  }
  {
    ...
    \item[IOUT] is an array of length at least 21 for integer optional outputs.
    \item[ROUT] is an array of length at least 6 for real optional outputs.
    \end{args}
  }
  {
...
\end{verbatim}
  ...
  \id{FIDASPGMR}, the user must make the call:
...
\end{verbatim}
  ...
  input arguments other than \id{MAXL}, then the user program should make the call
  \index{FIDASPGMRREINIT@\texttt{FIDASPGMRREINIT}}
\begin{verbatim}
      CALL FIDASPGMRREINIT (IGSTYPE, MAXRS, EPLIFAC, DQINCFAC, IER)
\end{verbatim}
  which reinitializes the {\spgmr} linear solver, but without reallocating its
  memory.  The arguments of \id{FIDASPGMRREINIT} have the same names and meanings
  as those of \id{FIDASPGMR}.  Finally, if the value of \id{MAXL} is being changed,
  then a call to \id{FIDASPGMR} must be made.
...
\end{verbatim}
  ...
  An optional user-supplied routine, \id{FIDAJTIMES}, can be provided for
  Jacobian-vector products.  If it is, then, following the call to
  \id{FIDASPBCG}, the user must make the call:
...
\end{verbatim}
  ...
  linear solver, then following the call to \id{FIDAREINIT} (see below), a call to
  the \id{FIDASPBCGREINIT} routine may or may not be needed.  If there is a change
  in any of the input arguments, then the user program should make the call
  \index{FIDASPBCGREINIT@\texttt{FIDASPBCGREINIT}}
\begin{verbatim}
      CALL FIDASPBCGREINIT (MAXL, EPLIFAC, DQINCFAC, IER)
\end{verbatim}
  which reinitializes the {\spbcg} linear solver, but without reallocating its
  memory.  The arguments of this routine have the same names and meanings as
  those of \id{FIDASPBCG}.
...
\end{verbatim}
  ...
  An optional user-supplied routine, \id{FIDAJTIMES}, can be provided for
  Jacobian-vector products.  If it is, then, following the call to
  \id{FIDASPTFQMR}, the user must make the call:
...
\end{verbatim}
  ...
  linear solver, then following the call to \id{FIDAREINIT} (see below), a call to
  the \id{FIDASPTFQMRREINIT} routine may or may not be needed.  If there is a change
  in any of the input arguments, then the user program should make the call
  \index{FIDASPTFQMRREINIT@\texttt{FIDASPTFQMRREINIT}}
\begin{verbatim}
      CALL FIDASPTFQMRREINIT (MAXL, EPLIFAC, DQINCFAC, IER)
\end{verbatim}
  which reinitializes the {\sptfqmr} linear solver, but without reallocating its
  memory.  The arguments of this routine have the same names and meanings as
  those of \id{FIDASPTFQMR}.
...
\end{verbatim}
  ...
  {\warn} If the user calls \id{FIDASPGMRSETPREC}, \id{FIDASPBCGSETPREC}, or
  \id{FIDASPTFQMRSETPREC}, the subroutine \id{FIDAPSET} must be provided,
  even if it is empty.
...
\end{verbatim}
  ...
  (This subroutine must be supplied even if it contains only the line \id{IER = 0}.)

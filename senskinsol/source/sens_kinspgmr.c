 ...
 *                 Keith E. Grant @ LLNL                          *
 * Version of    : 17 January 2001 (SensKinsol 27 April 2001)     *
 *----------------------------------------------------------------*
...
/* Another readability replacement needed below */
#define fnormtol (kin_mem->kin_fnormtol)

/*********************************************************************
 *                                                                   *
 * Function : KINSpgmrLinSolve                                       *
 *                                                                   *
 * This routine handles the call to the generic SPGMR solver         *
 * SpgmrSolve for the solution of the linear system J w = v.         *
 * Appropriate variables are passed to SpgmrSolve and the counters   *
 * nli, nps, and ncfl are incremented, and the return value is set   *
 * according to the success of SpgmrSolve. The success flag is       *
 * returned if SpgmrSolve converged. Other flags, as given below,    *
 * denote failure of this routine.                                   *
 *                                                                   *
 * This routine differs from KINSpgmrSolve in that it does not have  *
 * any additional coding within it to support the KINSOL nonlinear   *
 * iteration and is not declared static.                             *
 *                                                                   *
 *-------------------------------------------------------------------*
 *                                                                   *
 * Arguments...                                                      *
 *                                                                   *
 * kin_mem     Pointer to the KINSOL memory structure                *
 *                                                                   *
 * ww          The solution vector                                   *
 *                                                                   *
 * vv          The right hand side                                   *
 *                                                                   *
 * res_norm    The resulting residual norm of the linear solution    *
 *                                                                   *
 *                                                                   *
 * Return Values...                                                  *
 *                                                                   *
 * KINSpgmrLinSolve = -8  N_Vector for RHS is NULL                   *
 *                  = -7  N-Vector for solution is NULL              *
 *                  = -6  Pointer to KINSPGMRMEM is NULL             *
...
int KINSpgmrLinSolve(KINMem kin_mem, N_Vector ww, N_Vector vv,
			 real *res_norm)

{

   /* Definitions for return values local to this Krylov Solver      */
   /* Wrapper (KSW) routine                                          */
   enum {
      KSW_UNRECOV_RHSNULL=-8, /* N_Vector for RHS is NULL            */
      KSW_UNRECOV_SOLNNULL,   /* N_Vector for solution is NULL       */
      KSW_UNRECOV_KLSMEMNULL, /* Pointer to KINSPGMRMEM is NULL      */
      KSW_UNRECOV_KINMEMNULL  /* Pointer to KINSOL memory is NULL    */
   };

   /* Define a mapping of SpgmrSolve return flags to the generic     */
   /* Krylov solver flags (see kinspgmr.h)                           */
   const int spgmr_return_map[] = {
      GKS_UNRECOV_OTHER,      /* QRsol found singular R              */
      GKS_UNRECOV_OTHER,      /* Gram-Schmidt routine failed         */
      GKS_UNRECOV_PSOLVE,     /* PSolve failed unrecoverably         */
      GKS_UNRECOV_ATIMES,     /* ATimes returned failure flag        */
      GKS_UNRECOV_KSNULL,     /* Mem argument is NULL                */
      GKS_SUCCESS_LSOLVE,     /* Converged                           */
      GKS_RECOV_NOCONVERGE,   /* Noncoverged, but norm reduced       */
      GKS_RECOV_NOCONVERGE,   /* Failed to converge                  */
      GKS_RECOV_SINGULAR,     /* QRfact found singular matrix        */
      GKS_RECOV_PSOLVE        /* PSolve failed recoverably           */
   };

   KINSpgmrMem kinspgmr_mem;
   int ret, nli_inc, nps_inc;

   const char myself[]="KINSpgmrLinSolve:";


   /* Check pointer to KINSOL memory */
   if ( kin_mem == NULL ) {
      return (KSW_UNRECOV_KINMEMNULL);
   }


   /* Check pointer to KINSpgmr memory */
   kinspgmr_mem = (KINSpgmrMem) lmem;
   if ( kinspgmr_mem == NULL ) {
      return (KSW_UNRECOV_KLSMEMNULL);
   }


   /* Check pointer to the N_Vector for the solution */
   if ( ww == NULL ) {
      return (KSW_UNRECOV_SOLNNULL);
   }


   /* Check pointer to the N_Vector for the right hand side */
   if ( vv == NULL ) {
      return (KSW_UNRECOV_RHSNULL);
   }


   /* Use code derived from KINSpgmrSolve to solve J w = v */

   N_VConst(ZERO, ww);

   ret = SpgmrSolve(spgmr_mem, kin_mem, ww, vv, pretype, gstype,
            fnormtol, maxlinrestarts, kin_mem, uscale, fscale,
            KINSpgmrAtimes, KINSpgmrPSolve,
            res_norm, &nli_inc, &nps_inc);


   /* Map SpgmrSolve return flags to the generic Krylov solver return */
   /* flags defined in kinspgmr.h. Return flags from SpgmrSolve range */
   /* between -5 and and +4. These are mapped by spgmr_return_map to  */
   /* the generic Krylov flags.                                        */

   ret = spgmr_return_map[ret + 5];


   /* Increment counters for number of linear iterations (nli), */
   /* number of preconditioner solves (nps), and number of      */
   /* convergence failures (ncfl).                              */
   ...
   /* Output optional solver diagnostics */
   if(kin_mem->kin_printfl > TWO) {
      fprintf(msgfp,
         "%s Linear (Krylov step) residual norm %12.3g,  fnormtol %12.3g\n",
         myself, *res_norm, fnormtol);
      fprintf(msgfp,"%s Incremental linear solver interations = %d\n",
         myself, nli_inc);
   }

...
weighted root-mean-square norm, denoted $\|\cdot\|_{WRMS}$, for all
...
\begin{equation}\label{errwt}
 W_i = \mbox{rtol} \cdot |y^i| + \mbox{atol}_i \, .
\end{equation}
...
the stepsize.  The basic idea is to pick the order $q$ for which a
...
% Output modes
%-------------
Normally, CVODE takes steps until a user-defined output value $t =
...
CVODE not to integrate past a given stopping point $t =
t_{\mbox{\scriptsize stop}}$.

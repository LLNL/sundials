...
$h_n = t_n - t_{n-1}$ is the stepsize.  The user of CVODE must choose
...
described below, the final computed value $y_{n(m)}$ will have to satisfy a
local error test $\| y_{n(m)} - y_{n(0)} \| \leq \epsilon$.  Letting
$y_n$ denote the exact solution, we want
...
of convergence failures; the default value for this limit is 10, 
...
setup and solve operations, and optionally also for the required matrix-vector
products $Jv$.  If a routine for $Jv$ is not supplied, these
...
$\Delta \equiv y_{n(m)} - y_{n(0)}$ (with $y_{n(m)}$ the final
computed approximation to $y_n$) and takes the form
...
imaginary axis, the stepsizes $h$ for which the method is stable are
...
stability region boundary that is limiting the stepsizes in the
...
that the stepsize is in fact stability-limited, it dictates a

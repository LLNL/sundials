...
asymptotic behavior of the local error in the case of fixed stepsizes.
...
ELTE$(q') = C(q')\|\phi(q'+1)\|$ to select stepsizes.  But the choice
...
new stepsize $h'$.  The latter is based on the $h^{q+1}$ asymptotic
...
stepsize unless it can be doubled.  See \cite{BCP:96} for details.

...
#define TOTAL                   \
  4 * MGRID + 8 * (MGRID - 2) + \
    (MGRID - 4) * (MGRID + 4 * (MGRID - 2)) /* total num of nonzero elements */
...
int main(void)
{
  ...
  if (MGRID >= 4) { retval = IDASetJacFn(mem, jacHeat); }
  else if (MGRID == 3) { retval = IDASetJacFn(mem, jacHeat3); }
  else
  {
            ...
            N_Vector tempv2, N_Vector tempv3)
{
  ...
  /**** last-1 column block ****/
  colptrs[MGRID * MGRID - 2 * MGRID + 1] = TOTAL - 2 * MGRID - 4 * (MGRID - 2) +
                                           2;
  colptrs[MGRID * MGRID - 2 * MGRID + 2] = TOTAL - 2 * MGRID - 4 * (MGRID - 2) +
                                           5;
  /* count by fours in the middle */
  for (i = 0; i < MGRID - 4; i++)
  {
  ...
  for (i = 0; i < MGRID - 4; i++)
  {
    ...
    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 6] = -ONE /
                                                                       (dx * dx);
    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 7] = -ONE /
                                                                       (dx * dx);
    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 8] = beta;
    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 9] = -ONE /
                                                                       (dx * dx);

    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 10] =
      -ONE / (dx * dx);
    data[2 * MGRID + 4 * (MGRID - 2) + repeat + (MGRID - 4) * 5 + 11] = ONE;

    repeat +=
      MGRID +
      4 * (MGRID - 2); /* shift that accounts for accumulated columns and elements */
  }
  ...
  /* alternating pattern in data, separate loop for each pattern  */
  for (i = TOTAL - 2 * (MGRID - 2) - 1; i < TOTAL - 2; i += 2)
  {
    data[i] = -ONE / (dx * dx);
  }
  ...
  for (i = 2; i < MGRID + (MGRID - 2) - 1; i += 2)
  {
    rowvals[i] = i / 2 + MGRID; /* i+1 unnecessary here */
  }
  ...
  rowvals[2 * MGRID + 4 * (MGRID - 2) - 4] =
    MGRID + (MGRID - 2); /* starting from here, add two diag patterns */
  ...
  for (i = 0; i < MGRID - 4; i++)
  {
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat] = MGRID + (MGRID - 2) + 2 +
                                                    MGRID * i;
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 1] = MGRID + (MGRID - 2) +
                                                        2 + MGRID * i + 1;

    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 2] = MGRID + (MGRID - 2) + 2 +
                                                        MGRID * i + 1 - MGRID;
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 3] = MGRID + (MGRID - 2) +
                                                        2 + MGRID * i + 1;
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 4] =
      MGRID + (MGRID - 2) + 2 + MGRID * i + 2; /* *this */
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 5] = MGRID + (MGRID - 2) + 2 +
                                                        MGRID * i + 1 + MGRID;
  ...
  rowvals[TOTAL - 6 * (MGRID - 2) - 3] =
    MGRID * MGRID - 1 - 2 * (MGRID - 1); /* starting with this as base */
  rowvals[TOTAL - 6 * (MGRID - 2) - 2] = MGRID * MGRID - 1 - 2 * (MGRID - 1) -
                                         MGRID;
  ...
  for (i = 0; i < (MGRID - 4); i++)
  {
    rowvals[TOTAL - 6 * (MGRID - 2) + 2 + 4 * i] = MGRID * MGRID - 1 -
                                                   2 * (MGRID - 1) + i;
  }
  for (i = 0; i < (MGRID - 4); i++)
  {
    rowvals[TOTAL - 6 * (MGRID - 2) + 3 + 4 * i] =
      MGRID * MGRID - 1 - 2 * (MGRID - 1) + 1 + i; /*copied above*/
  }
  for (i = 0; i < (MGRID - 4); i++)
  {
    rowvals[TOTAL - 6 * (MGRID - 2) + 4 + 4 * i] = MGRID * MGRID - 1 -
                                                   2 * (MGRID - 1) + 2 + i;
  }
  ...
  for (i = 0; i < (MGRID - 2); i++)
  {
    rowvals[TOTAL - 2 * (MGRID - 2) - 1 + 2 * i] = MGRID * MGRID - 2 * MGRID +
                                                   1 + i;
  }
  for (i = 0; i < (MGRID - 2); i++)
  {
...
#if defined(SUNDIALS_FLOAT128_PRECISION)
  printf(" %5.2Qf %13.5Qe  %d  %3ld  %3ld  %3ld  %4ld  %9.2Qe \n", t, umax,

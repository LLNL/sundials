           ...
           N_Vector tempv1, N_Vector tempv2, N_Vector tempv3)
{
  ...
  // can't use this because i is too high to be added on, rowvals are off
  //for(i=TOTAL-2*(MGRID-2)-1;i<TOTAL-2;i+=2) JacMat -> rowvals[i] = MGRID*MGRID-2*MGRID+1+i;
  //for(i=TOTAL-2*(MGRID-2)  ;i<TOTAL-1;i+=2) JacMat -> rowvals[i] = MGRID*MGRID-MGRID+1+i;

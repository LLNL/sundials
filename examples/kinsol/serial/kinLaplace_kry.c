 ...
 * -----------------------------------------------------------------
 * Based on example by Radu Serban
 * -----------------------------------------------------------------
 ...
 * The nonlinear system is solved by KINSOL using the SPGMR linear
 * solver without a preconditioner.
 * -----------------------------------------------------------------
 */
...
//  maxl = 15;
//  maxlrst = 2;
  flag = KINSpgmr(kmem, 15);
  if (check_flag(&flag, "KINSpgmr", 1)) return(1);

  flag = KINSpilsSetMaxRestarts(kmem, 2);

...
module advdiff_mod
    ...
    double precision, parameter :: dx = xmax / (mx + 1)
    double precision, parameter :: dy = ymax / (my + 1)
    double precision, parameter :: hdcoef = 1.0d0 / (dx * dx)
    double precision, parameter :: hacoef = 0.5d0 / (2.0d0 * dx)
    double precision, parameter :: vdcoef = 1.0d0 / (dy * dy)
    ...
    integer(c_int) function RhsFn(tn, sunvec_u, sunvec_f, user_data) &
      ...
      uvec(1:mxmy) => FN_VGetArrayPointer(sunvec_u)
      fvec(1:mxmy) => FN_VGetArrayPointer(sunvec_f)
      ...
      do i = 1, mx
         ...
         do j = 1, my
            ...
            uij = uvec(ij)
            udn = 0.0d0
            if (j .ne. 1)  udn = uvec(ij - 1)
            uup = 0.0d0
            if (j .ne. my) uup = uvec(ij + 1)
            ult = 0.0d0
            if (i .ne. 1)  ult = uvec(ij - my)
            urt = 0.0d0
            if (i .ne. mx) urt = uvec(ij + my)

            ! Set diffusion and advection terms and load into fvec.
            hdiff = hdcoef * (ult - 2.0d0 * uij + urt)
            hadv = hacoef * (urt - ult)
            vdiff = vdcoef * (uup - 2.0d0 * uij + udn)
            fvec(ij) = hdiff + hadv + vdiff
  ...
  program main
    ...
    do i = 1, mx
       ...
       do j = 1, my
          y = j * dy
          idx = idx0 + j
          u(idx) = x * (xmax - x) * y * (ymax - y) * exp(5.0d0 * x * y)
       end do
    end do

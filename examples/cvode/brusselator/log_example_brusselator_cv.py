...
# get successful step data
_, order_x, order_y = sunlog.get_history(log, "q", "success")
order_x = np.array(order_x)
order_y = np.array(order_y)

order_plt = axes[2].scatter([], [], marker='.', color="blue")

# Pre-set x/y limits
axes[2].set_xlim(sol_x.min(), sol_x.max())
min_y, max_y = 1, np.max(order_y + 1)
axes[2].set_ylim(min_y, max_y)
axes[2].yaxis.set_major_locator(tik.MaxNLocator(integer=True))
...
def update(frame):
    ...
    # Failed: plot all up to current_time
    mask_q = order_x <= current_time
    if np.any(mask_q):
        qdata = np.stack([order_x[mask_q], order_y[mask_q]]).T
        order_plt.set_offsets(qdata)

    # print(f"Frame {frame}: current_time={current_time:.3f}, "
    #       f"solution_pts={frame}, success_pts={mask_p.sum()}, failed_pts={mask_f.sum()}")

    return sol_plts, pass_plt, fail_plt, order_plt

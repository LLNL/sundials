...
int main(int argc, char* argv[])
{
  ...
  {
    retval = SUNLogger_Create(comm, // MPI communicator
                              0,    // output on process 0
                              &logger);
    if (check_retval(&retval, "SUNLogger_Create", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }

    retval = SUNContext_SetLogger(sunctx, logger);
    if (check_retval(&retval, "SUNContext_SetLogger", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }

    retval = SUNLogger_SetErrorFilename(logger, "stderr");
    if (check_retval(&retval, "SUNLogger_SetErrorFilename", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }
  }
  ...
  if (SUNDIALS_LOGGING_LEVEL >= SUN_LOGLEVEL_WARNING)
  {
    ...
    if (check_retval(&retval, "SUNLogger_SetWarningFilename", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }
  }
  ...
  if (SUNDIALS_LOGGING_LEVEL >= SUN_LOGLEVEL_INFO)
  {
    ...
    if (check_retval(&retval, "SUNLogger_SetInfoFilename", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }
  }
  ...
  if (SUNDIALS_LOGGING_LEVEL >= SUN_LOGLEVEL_DEBUG)
  {
    ...
    if (check_retval(&retval, "SUNLogger_SetDebugFilename", 1, my_pe))
    {
      MPI_Abort(comm, 1);
    }
  }

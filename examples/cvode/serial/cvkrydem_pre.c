 ...
 * routines denalloc, denallocpiv, denaddI, denGETRF, denGETRS, 
 * denfreepiv and denfree.
 *
...
/*
 ...
 the identity matrix, and calls denGETRF to do the LU decomposition of
                   ...
                   N_Vector vtemp3)
{
  ...
  for (ig = 0; ig < ngrp; ig++) {
    denaddI(P[ig], mp);
    ier = denGETRF(P[ig], mp, mp, pivot[ig]);
    if (ier != 0) return(1);
  }
                  ...
                  int lr, void *P_data, N_Vector vtemp)
{
  ...
  for (jy = 0; jy < my; jy++) {
    ...
    for (jx = 0; jx < mx; jx++) {
      igx = jigx[jx];
      ig = igx + igy*ngx;
      denGETRS(P[ig], mp, pivot[ig], &(NV_DATA_S(z)[iv]));
      iv += mp;
    }
  }

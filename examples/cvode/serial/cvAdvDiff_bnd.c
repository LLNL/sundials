...
int main(void)
{
  ...
  if(check_retval(SUNLastErr(sunctx), "N_VNew_Serial")) return(1);
  ...
  if(check_retval(SUNLastErr(sunctx), "CVodeCreate")) return(1);
  ...
  if(check_retval(SUNLastErr(sunctx), "SUNBandMatrix")) return(1);

  /* Create banded SUNLinearSolver object for use by CVode */
  LS = SUNLinSol_Band(u, A, sunctx);
  if(check_retval(SUNLastErr(sunctx), "SUNLinSol_Band")) return(1);

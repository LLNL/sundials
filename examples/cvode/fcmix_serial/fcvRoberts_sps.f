...
C     the the SUPERLU sparse direct linear solver, and a user-supplied
C     Jacobian routine.
...
C
      WRITE(6, 10) NEQ
 10   FORMAT('Superlu example problem:'//
...
C     number of threads to use in factorization
      NTHREADS = 1
      CALL FSUNSUPERLUINIT(1, IER)
      IF (IER .NE. 0) THEN
        WRITE(6,28) IER
 28     FORMAT(///' SUNDIALS_ERROR: FSUNSUPERLUINIT returned IER = ',
     1         I5)
        STOP
      ENDIF

...
int main(int argc, char** argv)
{
  ...
  ierr = DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, DM_BOUNDARY_NONE,
                      DMDA_STENCIL_STAR, 8, 8, PETSC_DECIDE, PETSC_DECIDE, 1, 1,
                      NULL, NULL, &da);
  CHKERRQ(ierr);
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = PetscViewerAndFormatCreate(PETSC_VIEWER_STDOUT_WORLD,
...
{
  PetscErrorCode ierr;
  ierr = FormFunction((DM)ptr, t, N_VGetVector_Petsc(x),
                      N_VGetVector_Petsc(xdot), NULL);
  return ierr;
}
  ...
  for (j = ys; j < ys + ym; j++)
  {
    for (i = xs; i < xs + xm; i++)
    {
      ...
      f[j][i] =
        -u * (uxx + uyy) -
        (4.0 - 1.0) *
          ((x[j][i + 1] - x[j][i - 1]) * (x[j][i + 1] - x[j][i - 1]) * .25 * sx +

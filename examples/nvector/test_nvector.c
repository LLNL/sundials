...
int Test_N_VGetArrayPointer(N_Vector W, sunindextype local_length, int myid)
{
  ...
  stop_time  = get_time();
  Wdata++;
  Wdata = NULL; /* Do something with pointer to surpress warning */
...
int Test_N_VWL2Norm(N_Vector X, N_Vector W, sunindextype local_length, int myid)
{
  ...
  failure = (ans < ZERO)
              ? 1
              : SUNRCompare(ans, HALF * HALF * SUNRsqrt((realtype)global_length));
...
int Test_N_VMaxNormLocal(N_Vector X, sunindextype local_length, int myid)
{
  ...
  failure = (ans < ZERO) ? 1
                         : SUNRCompareTol(ans, myidp1, SUNRsqrt(UNIT_ROUNDOFF));
...
int Test_N_VL1NormLocal(N_Vector X, sunindextype local_length, int myid)
{
  ...
  failure = (ans < ZERO)
              ? 1
              : SUNRCompareTol(ans, (realtype)myid, SUNRsqrt(UNIT_ROUNDOFF));
...
int Test_N_VDotProdMultiLocal(N_Vector X, sunindextype local_length, int myid)
{
  ...
  if (ierr == 0) { failure = SUNRCompare(dotprods[0], (realtype)local_length); }
  else { failure = 1; }

...
int main(int argc, char *argv[]) 
{
  ...
  /* Create vectors */
  if (ProbData.nprocs == 1) {
    x = N_VNew_Serial(ProbData.Nloc);
    if (check_flag(x, "N_VNew_Serial", 0)) return 1;
    xhat = N_VNew_Serial(ProbData.Nloc);
    if (check_flag(xhat, "N_VNew_Serial", 0)) return 1;
    b = N_VNew_Serial(ProbData.Nloc);
    if (check_flag(b, "N_VNew_Serial", 0)) return 1;
    ProbData.d = N_VNew_Serial(ProbData.Nloc);
    if (check_flag(ProbData.d, "N_VNew_Serial", 0)) return 1;
    ProbData.s1 = N_VNew_Serial(ProbData.Nloc);
    if (check_flag(ProbData.s1, "N_VNew_Serial", 0)) return 1;
    ProbData.s2 = N_VNew_Serial(ProbData.Nloc);
  ...
  fails += SUNSPBCGSGetWorkspace(LS, &lenrwLS, &leniwLS);
  if (ProbData.myid == 0)
    printf("    SPBCGS linear solver workspace: real = %ld, integer = %ld\n",
           lenrwLS, leniwLS);
  if (fails) {

 ...
 *      Based on idasRoberts_ASAi_dns.c and modified to use SuperLU
 * -----------------------------------------------------------------
...
#include <sunlinsol/sunlinsol_superlu.h> /* access to SuperLU linear solver    */
...
int main(int argc, char *argv[])
{
  ...
  /* Create SuperLU SUNLinearSolver object (one thread) */
  LS = SUNLinSol_SuperLU(yy, A);
  if(check_retval((void *)LS, "SUNLinSol_SuperLU", 0)) return(1);
  ...
  */
  ...
  /* Create SuperLU SUNLinearSolver object (one thread) */
  LSB = SUNLinSol_SuperLU(yB, AB);
  if(check_retval((void *)LSB, "SUNLinSol_SuperLU", 0)) return(1);

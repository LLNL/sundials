...
static int Precond(sunrealtype tt, N_Vector cc, N_Vector cp, N_Vector rr,
...
int main(int argc, char* argv[])
{
  ...
  LS   = SUNLinSol_SPGMR(cc, SUN_PREC_LEFT, maxl,
                   ...
                   sunrealtype cj, void* user_data)
{
  int retval;
  sunindextype ret;
  sunrealtype uround, xx, yy, del_x, del_y;
  sunrealtype **Pxy, *ratesxy, *Pxycol, *cxy, *cpxy, *ewtxy, cctmp;
  sunrealtype inc, fac, sqru, perturb_rates[NUM_SPECIES];
  int is, js, jx, jy;
  void* ida_mem;
  N_Vector ewt;
  sunrealtype hh;
...
static void PrintHeader(int maxl, sunrealtype rtol, sunrealtype atol)
{
  printf("\nidasFoodWeb_kry_omp: Predator-prey DAE OpenMP example problem "
         "using Krylov solver for IDAS \n\n");

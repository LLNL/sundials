...
module ark_analytic_complex_mod
...
end module ark_analytic_complex_mod
! ------------------------------------------------------------------
...
program main
  ...
  type(c_ptr) :: arkode_mem, sunctx, stdout_file
  ...
  arkode_mem = FARKStepCreate(c_funloc(f), c_null_funptr, t0, sunvec_y, sunctx)
  retval = FARKodeSStolerances(arkode_mem, reltol, abstol)

  ! ! Create linear solver
  ! sunls => FSUNLinSol_SPGMR(sunvec_y, SUN_PREC_NONE, 10, sunctx)
  ! sunmat_A => null()
  ...
  do iout = 0, nt-1
    retval = FARKodeEvolve(arkode_mem, tout, sunvec_y, tret, ARK_NORMAL)
    print '(f8.3, " | ", 3(1x, f8.5, " + ", f8.5, "i", 1x))', tret(1), &
      real(yval(1)), aimag(yval(1)), real(yval(2)), aimag(yval(2)), &
      real(yval(3)), aimag(yval(3))
    tout = tout + 1.0d0
  ...
  retval = FARKodePrintAllStats(arkode_mem, stdout_file, SUN_OUTPUTFORMAT_TABLE)

 ...
 * the SUNLinSol_KLU linear solver, and ARKODE fills the Jacobian
 * matrix internally using sparse difference quotients.
 *
...
/* Private helper function to fill the sparse Jacobian pattern */
static int JacPattern(SUNMatrix Jac, UserData udata);
...
int main(void)
{
  ...
  printf(
    "\n1D Brusselator PDE test problem (KLU solver, sparse DQ Jacobian):\n");
  ...
  flag = JacPattern(A, udata);
  if (check_flag(&flag, "JacPattern", 1)) { return 1; }

  LS = SUNLinSol_KLU(y, A, ctx);
  if (check_flag((void*)LS, "SUNLinSol_KLU", 0)) { return 1; }

  /* Attach the matrix and linear solver to ARKODE */
  ...
  flag = ARKodeGetNumLinRhsEvals(arkode_mem, &nfe);
  check_flag(&flag, "ARKodeGetNumLinRhsEvals", 1);
  printf("   Total RHS evals for sparse DQ Jacobian = %li\n", nfe);
...
static int f(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  for (i = 1; i < N - 1; i++)
  {
    ...
    /* u_t = du*u_xx + a - (w+1)*u + v*u^2 */
    dYdata[IDX(i, 0)] = (ul - TWO * u + ur) * uconst + a - (w + ONE) * u +
                        v * u * u;
...
/* Routine to set the sparse Jacobian pattern.  Zero values are inserted into the
   matrix locations that correspond with non-zero entries in the Jacobian. */
static int JacPattern(SUNMatrix Jac, UserData udata)
{
  ...
  if ((SUNSparseMatrix_Rows(Jac) != N * 3) ||
      (SUNSparseMatrix_Columns(Jac) != N * 3))
  {
    printf("Jacobian pattern error: matrix is the wrong size!\n");
    return 1;
  }
  ...
  /* iterate over interior nodes, filling in the Jacobian pattern */
  for (i = 1; i < N - 1; i++)
  {
    ...
    data[nz]      = ZERO; /* fv wrt u */
    rowvals[nz++] = IDX(i, 1);

    data[nz]      = ZERO; /* fw wrt u */
    rowvals[nz++] = IDX(i, 2);
    ...
    data[nz]      = ZERO; /* fu wrt v */
    rowvals[nz++] = IDX(i, 0);

    data[nz]      = ZERO; /* fv wrt v */
    rowvals[nz++] = IDX(i, 1);
    ...
    data[nz]      = ZERO; /* fu wrt w */
    rowvals[nz++] = IDX(i, 0);

    data[nz]      = ZERO; /* fv wrt w */
    rowvals[nz++] = IDX(i, 1);

    data[nz]      = ZERO; /* fw wrt w */
    rowvals[nz++] = IDX(i, 2);

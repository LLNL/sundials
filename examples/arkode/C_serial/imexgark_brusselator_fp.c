...
int main()
{
  ...
  realtype gamma = RCONST(1.0) - SUNRsqrt(2.0)/RCONST(2.0);
  ...
  arkode_mem = IMEXGARKStepCreate(fe, fi, T0, y);
  if (check_flag((void *)arkode_mem, "IMEXGARKStepCreate", 0)) return 1;
  ...
  flag = IMEXGARKStepSetTables(arkode_mem, stages, 2, -1,
                               ce, ci,
                               Aee, Aei,
                               Aie, Aii,
                               be, bi,
  ...
  /* Initialize fixed-point nonlinear solver and attach to IMEXGARKStep */
  NLS = SUNNonlinSol_FixedPoint(y, fp_m);
  if (check_flag((void *)NLS, "SUNNonlinSol_FixedPoint", 0)) return 1;
  flag = IMEXGARKStepSetNonlinearSolver(arkode_mem, NLS);
  if (check_flag(&flag, "IMEXGARKStepSetNonlinearSolver", 1)) return 1;

  /* Set routines */
  flag = IMEXGARKStepSetUserData(arkode_mem, (void *) rdata);     /* Pass rdata to user functions */
  if (check_flag(&flag, "IMEXGARKodeSetUserData", 1)) return 1;
  flag = IMEXGARKStepSStolerances(arkode_mem, reltol, abstol);    /* Specify tolerances */
  if (check_flag(&flag, "IMEXGARKodeSStolerances", 1)) return 1;
  ...
  flag = IMEXGARKStepSetInitStep(arkode_mem, RCONST(0.01));
  if (check_flag(&flag, "IMEXGARKStepSetInitStep", 1)) return 1;
  ...
  /* Main time-stepping loop: calls IMEXGARKStepEvolve to perform the integration, then
  ...
  for (iout=0; iout<Nt; iout++) {
    ...
    flag = IMEXGARKStepEvolve(arkode_mem, tout, y, &t, ARK_NORMAL);      /* call integrator */
    if (check_flag(&flag, "IMEXGARKStepEvolve", 1)) break;
  ...
  flag = IMEXGARKStepGetNumSteps(arkode_mem, &nst);
  check_flag(&flag, "IMEXGARKStepGetNumSteps", 1);
  flag = IMEXGARKStepGetNumStepAttempts(arkode_mem, &nst_a);
  check_flag(&flag, "IMEXGARKStepGetNumStepAttempts", 1);
  flag = IMEXGARKStepGetNumRhsEvals(arkode_mem, &nfe, &nfi);
  check_flag(&flag, "IMEXGARKStepGetNumRhsEvals", 1);
  flag = IMEXGARKStepGetNumErrTestFails(arkode_mem, &netf);
  check_flag(&flag, "IMEXGARKStepGetNumErrTestFails", 1);
  flag = IMEXGARKStepGetNumNonlinSolvIters(arkode_mem, &nni);
  check_flag(&flag, "IMEXGARKStepGetNumNonlinSolvIters", 1);
  flag = IMEXGARKStepGetNumNonlinSolvConvFails(arkode_mem, &ncfn);
  check_flag(&flag, "IMEXGARKStepGetNumNonlinSolvConvFails", 1);
  ...
  IMEXGARKStepFree(&arkode_mem);  /* Free integrator memory */

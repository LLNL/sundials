...
module UserData
  !---------------------------------------------------------------
  ! Description:  -- keep this around (with "type" adjustments)
...
contains
  ...
  !---------------------------------------------------------------
  ! Set up parallel decomposition -- keep this around, since it just sets up the UserData module
  !---------------------------------------------------------------
  ...
  !---------------------------------------------------------------
  ! Free memory allocated within Userdata -- keep around (with potential "type" changes)
  !---------------------------------------------------------------
...
!-----------------------------------------------------------------
! Main driver program -- change this for updated F2003 ARKODE style
!-----------------------------------------------------------------
program driver
  ...
  ! initialize PCG linear solver module -- update this
  ...
  ! Create the solver memory to use DIRK integrator, scalar tolerances -- update this
  ...
  ! set integrator options -- update this
  ...
  ! attach linear solver module to ARKLs interface -- update this
  call FARKLsInit(flag)
  if (flag < 0) then
     write(0,*) "Error in FARKLsInit = ", flag
     call MPI_Finalize(flag)
  end if
  call FARKLsSetPrec(1, flag)     ! Signal user-supplied preconditioner -- update this
  if (flag < 0) then
     write(0,*) "Error in FARKLsSetPrec = ", flag
     call MPI_Finalize(flag)
  end if

  ! specify that the problem is linearly implicit, but that Jacobian does not depend on time -- update this
  ...
  do ioutput=1,Nt
     ...
     call FARKode(tout, t, y, 1, flag)         ! call integrator  -- update this
     if (flag < 0) then
 ...
 ! Clean up and return with successful completion -- update this
...
!-----------------------------------------------------------------
! f routine to compute the ODE RHS function f(t,y). -- update function call structure
!-----------------------------------------------------------------
  ! declarations -- update this
  use UserData
  implicit none
  include "mpif.h"
...
!-----------------------------------------------------------------
! Explicit portion of f routine (empty) -- remove this entirely
!-----------------------------------------------------------------
  ! declarations
  use UserData
  implicit none
...
subroutine farkpset(t, y, fy, jok, jcur, gamma, hcur, ipar, &
                    rpar, ierr)
!-----------------------------------------------------------------
! Preconditioner setup routine (fills inverse of Jacobian diagonal) -- update function call structure
!-----------------------------------------------------------------
  ! declarations
  use UserData
  implicit none
...
subroutine farkpsol(t, y, fy, r, z, gamma, delta, lr, &
                    ipar, rpar, ierr)
!-----------------------------------------------------------------
! Preconditioner solve routine -- update function call structure
!-----------------------------------------------------------------
  ! declarations
  use UserData
  implicit none

...
module diag_non_mod
  ...
  end module diag_non_mod
  
  
  module diag_non_prec_mod
  ...
  contains
    ...
    integer(c_int) function PSetup(t, sunvec_y, sunvec_f, jok, jcurPtr, gamma, &
      ...
      use diag_non_mod, only : Nvar, Npts, Neq, k2, k3, k4, k6
  ...
  contains
    ...
    integer(c_int) function TaskLocalNlsResidual(sunvec_zcor, sunvec_F, arkode_mem) &
      ...
      use diag_non_mod, only : Neq, Reaction
    ...
    integer(c_int) function TaskLocalLSolve(sunvec_delta, arkode_mem) &
      ...
      use diag_non_mod, only : Nvar, Npts, k2, k3, k4, k6
    ...
    function TaskLocalNewton(arkode_mem, sunvec_y) result(sunnls)
      ...
      use diag_non_mod, only : sunctx, Nvar, comm, DFID, monitor
  ...
  program main
    ...
    use diag_non_mod, only : sunctx, logger, comm, myid, Nx, Neq, dx, fused, explicit, printtime, nout
  ...
  subroutine EvolveProblemIMEX(sunvec_y)
    ...
    use diag_non_mod, only : sunctx, comm, myid, Neq, t0, tf, atol, rtol, order, DFID, &
         monitor, global, nout, umask_s, Advection, Reaction
  
    use diag_non_prec_mod, only : sunls_P, sunmat_P, PSetup, PSolve
  ...
  subroutine EvolveProblemExplicit(sunvec_y)
    ...
    use diag_non_mod, only : sunctx, comm, myid, t0, tf, atol, rtol, order, DFID, monitor, &
  ...
  subroutine WriteOutput(t, sunvec_y)
    ...
    use diag_non_mod, only : Nvar, nprocs, myid, Erecv, Nx, Npts, monitor,  nout, &
         umask, vmask, wmask
  ...
  subroutine SetIC(sunvec_y)
    ...
    use diag_non_mod, only : Nvar, myid, Npts, xmax, dx, A, B, k1, k2, k4, k3
  ...
  subroutine ExchangeBCOnly(sunvec_y)
    ...
    use diag_non_mod, only : Nvar, comm, myid, nprocs, Erecv, Wsend
  ...
  subroutine ExchangeAllStart(sunvec_y)
    ...
    use diag_non_mod, only : Nvar, comm, myid, nprocs, reqS, reqR, Wrecv, Wsend, &
         Erecv, Esend, Npts, c
  ...
  subroutine ExchangeAllEnd()
    ...
    use diag_non_mod, only : comm, reqS, reqR, c
  ...
  subroutine FreeProblem()
    ...
    use diag_non_mod, only : sunctx, logger, myid, nout, umask_s, umask, vmask, wmask

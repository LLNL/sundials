...
/* global copy of local diagonal length (for check routines) */
sunindextype local_diag_length;
...
int main(int argc, char *argv[]) 
{
  ...
  if (myid == 0) {
    ...
    printf("  local/global problem sizes = %ld/%ld\n", 
           (long int) Nloc, (long int) (nprocs * Nloc));
  ...
  y = N_VNew_Parallel(comm, Nloc, nprocs*Nloc);
  if (check_flag(y, "N_VNew_Parallel", 0)) return 1;
  ...
  /* SUNMatrix Tests */
  fails += Test_SUNMatGetID(A, SUNMATRIX_DIAGONAL, myid);

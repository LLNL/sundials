 ...
 * using the SensCVSPGMR linear solver) and a block-diagonal matrix with*
 ...
 * Execution: spvkxb -npes N  with N = NPEX*NPEY (see constants below). *
 ************************************************************************/
...
#define Ny           NVARS*MX*MY    /* number of equations          */
...
main(int argc, char *argv[])
{
  ...
  void *cvode_mem;
  int iout, flag, i;
  integer local_N, my_pe, npes, mudq, mldq, mukeep, mlkeep;
  machEnvType machEnv;
  MPI_Comm comm;
...
/* Call SensCVodeMalloc to initialize CVODE: 
  ...
  SensCVSpgmr(cvode_mem, LEFT, MODIFIED_GS, 0, 0.0, PVBBDPrecon, PVBBDPSol, pdata);
...
/* f routine.  Evaluate f(t,y,p).  First call fucomm to do communication of 

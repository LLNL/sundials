 ...
 * preconditioned GMRES linear solver, CVSPGMR, for sensitivity   *
 * analysis.                                                      *
 ******************************************************************/
...
#define CVSPGMR_INIT       "SensCVSpgmrInit-- "
...
/* SensCVSPGMR linit, lsetup, lsolve, and lfree routines */

static int  SensCVSpgmrInit(CVodeMem cv_mem, boole *setupNonNull);

static int  SensCVSpgmrSetup(CVodeMem cv_mem, int convfail, N_Vector ypred,
			     N_Vector fpred, boole *jcurPtr, N_Vector vtemp1,
			     N_Vector vtemp2, N_Vector vtemp3);

static int  SensCVSpgmrSolve(CVodeMem cv_mem, N_Vector b, N_Vector ycur,
			     N_Vector fcur);

static void SensCVSpgmrFree(CVodeMem cv_mem);

/* SensCVSPGMR Atimes and SensPSolve routines called by generic SPGMR solver */

static int SensCVSpgmrAtimesDQ(void *cv_mem, N_Vector v, N_Vector z);

static int SensCVSpgmrPSolve(void *cv_mem, N_Vector r, N_Vector z, int lr);
...
/*************** SensCVSpgmr *****************************************
 ...
 fields specific to the Spgmr linear solver module. SensCVSpgmr sets 
 the cv_linit, cv_lsetup, cv_lsolve, and cv_lfree fields in 
 (*cvode_mem) to be SensCVSpgmrInit, SensCVSpgmrSetup, 
 SensCVSpgmrSolve, and SensCVSpgmrFree, respectively. It allocates 
 memory for a structure of type CVSpgmrMemRec and sets the cv_lmem 
 field in (*cvode_mem) to the address of this structure. SensCVSpgmr 
 sets the following fields in the CVSpgmrMemRec structure:
   g_pretype = pretype                                       
...
void SensCVSpgmr(void *cvode_mem, int pretype, int gstype, int maxl, 
...

{
  ...
  linit  = SensCVSpgmrInit;
  lsetup = SensCVSpgmrSetup;
  lsolve = SensCVSpgmrSolve;
  lfree  = SensCVSpgmrFree;

  /* Get memory for CVSpgmrMemRec */
  lmem = cvspgmr_mem = (CVSpgmrMem) malloc(sizeof(CVSpgmrMemRec));
  if (cvspgmr_mem == NULL) return;  /* SensCVSpgmrInit reports this error */
			    ...
			    N_Vector vtemp2, N_Vector vtemp3)
{
  ...
  int N;
  SensData sdata;
  N_Vector *ypredsub, *fpredsub, *ewtsub;
  N_Vector *vtemp1sub, *vtemp2sub, *vtemp3sub;
  ...
  *jcurPtr = jbad;
  ...
  ypredsub = N_VSUB(ypred);
  fpredsub = N_VSUB(fpred);
  ewtsub = N_VSUB(ewt);
  vtemp1sub = N_VSUB(vtemp1);
  vtemp2sub = N_VSUB(vtemp2);
  vtemp3sub = N_VSUB(vtemp3);

  ier = precond(N, tn, ypredsub[0], fpredsub[0], jok, jcurPtr, gamma, 
		ewtsub[0], h, uround, &nfe, P_data, 
		vtemp1sub[0], vtemp2sub[0], vtemp3sub[0]);
...
static int SensCVSpgmrSolve(CVodeMem cv_mem, N_Vector b, N_Vector ynow,
			    N_Vector fnow)
{
  real bnorm, res_norm;
  CVSpgmrMem cvspgmr_mem;
  ...
  for (i = 0; i <= Ns; i++) {
    ...
    ier = SpgmrSolve(spgmr_mem, cv_mem, x, bsub[i], pretype, gstype, delta, 0,
		     cv_mem, ewtsub[i], ewtsub[i], SensCVSpgmrAtimesDQ, 
		     SensCVSpgmrPSolve, &res_norm, &nli_inc, &nps_inc);
    N_VScale(ONE, x, bsub[i]);
    ...
    if (ier < 0) break;
    if ((ier == SPGMR_RES_REDUCED) && (mnewt > 0)) break;
    if (ier > 1) break;
  }
...
static int SensCVSpgmrAtimesDQ(void *cvode_mem, N_Vector v, N_Vector z)
{
  real rho;
  CVodeMem   cv_mem;
  CVSpgmrMem cvspgmr_mem;
  ...
  RhsFn f;
  void *fdata;
  N_Vector *ewtsub, *ycursub, *fcursub;
  ...
  N_VLinearSum(ONE/rho, v, ONE, ycursub[0], ytemp); 

  /* Set z = f(tn, ytemp) */
  f(N, tn, ytemp, z, fdata); 
  nfe++;

  /* Replace z by v - (gamma*rho)(z - fcur) */
  N_VLinearSum(ONE, z, -ONE, fcursub[0], z);
...
/*************** SensCVSpgmrPSolve ***********************************
 ...
 that SensCVSpgmrPSolve will not be called in the case in which
 preconditioning is not done. This is the only case in which the
 user's psolve routine is allowed to be NULL.

**********************************************************************/

static int SensCVSpgmrPSolve(void *cvode_mem, N_Vector r, N_Vector z, int lr)
{
  CVodeMem   cv_mem;
  CVSpgmrMem cvspgmr_mem;
  int ier;
  ...
  ier = psolve(N, tn, ycursub[0], fcursub[0], ytemp, gamma, 
	       ewtsub[0], delta, &nfe, r, lr, P_data, z);
  return(ier);

...
int EvolveProblemBDF(N_Vector y, UserData* udata, UserOptions* uopt)
{
  ...
  if (check_retval((void*)cvode_mem, "CVodeCreate", 0, udata->myid))
  {
    return 1;
  }
  ...
  if (check_retval(&retval, "CVodeSetMaxNumSteps", 1, udata->myid))
  {
    return 1;
  }
  ...
  if (uopt->nls == "newton")
  {
    ...
    if (check_retval(&retval, "CVodeSetNonlinearSolver", 1, udata->myid))
    {
      return 1;
    }
    ...
    if (check_retval(&retval, "CVodeSetLinearSolver", 1, udata->myid))
    {
      return 1;
    }
    ...
    if (check_retval(&retval, "CVodeSetPreconditioner", 1, udata->myid))
    {
      return 1;
    }
  }
  ...
  {
    fprintf(stderr, "\nERROR: CV-BDF method is not compatible with the nls "
                    "option provided\n");
    return 1;
  }

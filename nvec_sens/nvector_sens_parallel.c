 ...
 *--------------------------------------------------------------*
 *                                                              *
 * This is the implementation file for the sensitivity analysis *
 * version of the MPI NVECTOR package. It contains the          *
 * implementation of the N_Vector kernels listed in nvector.h.  *
...
N_Vector N_VNew(integer N, machEnvType machEnv)
{
  ...
  v->data = (real *) malloc((1+Ns)*N_local*sizeof(real)); 
  if (v->data == NULL) { 
    free(v); 
    return(NULL); 
  } 

  v->length = N_local*(1+Ns); 
  v->global_length = N_global*(1+Ns); 
  v->machEnv = machEnv; 
  ...
  for (i = 0; i <= Ns; i++){ 
    ...
    (ptr[i])->data = v->data + (i*N_local); 
    (ptr[i])->length = N_local; 
    (ptr[i])->global_length = N_global; 
    (ptr[i])->machEnv = machEnv; 
    (ptr[i])->subptr = NULL; 
  } 
  v->subptr = ptr;

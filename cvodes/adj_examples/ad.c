...
int main(int argc, char *argv[])
{
  ...
  udata = N_VDATA(u);
  sum = 0.0;
  for(i=0; i<NEQ; i++) sum += udata[i];
  fprintf(ff,"%14.6e %14.6e %14.6e \n",t,h,sum);

  for(i=0;i<OPT_SIZE;i++) iopt[i] = 0;
  for(i=0;i<OPT_SIZE;i++) ropt[i] = 0.0;
  iopt[ISTOP] = 1;
  ropt[TSTOP] = TOUT;
  ...
  do {
    flag = CVode(cvode_mem, TOUT, u, &t, ONE_STEP);
    udata = N_VDATA(u);
    sum = 0.0;
    for(i=0; i<NEQ; i++) sum += udata[i];  
    h = ropt[HU];
...
static void SetIC(N_Vector u, UserData data)
{
  ...
  for (j=1; j <= MY; j++) {
    ...
    for (i=1; i <= MX; i++) {
      x = i*dx;
      IJth(udata,i,j) = 1.1*x*(XMAX - x)*y*(YMAX - y)*exp(5.0*x*y);
    }
  }  

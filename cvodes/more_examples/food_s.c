...
static void PrintIntro(WebData wdata);
...
static void v_sum_prods(real u[], real p[], real q[], real v[], real w[], int n);
...
static void jtimes(integer N, N_Vector v, N_Vector Jv, RhsFn f, 
...
main()
{
  ...
  /* Call CVodeMalloc then CVSpgmr to set up problem */
  ...
  flag = CVSpgmr(cvode_mem, LEFT, MODIFIED_GS, MAXL, DELT, Precond, PSolve, 
                 wdata, NULL, NULL);
  ...
  if(sensi) {
    ...
    Nsens = 1;
    pbar[0] = wdata->param[0];
    plist[0] = 1;

    cS = N_VNew_S(Nsens,NEQ,NULL);
    N_VConst(ZERO,cS[0]);
    ...
    flag = CVodeSensMalloc(cvode_mem,Nsens,sensi_meth,wdata->param,pbar,plist,
                           ALLSENS,NULL,err_con,rhomax,cS,NULL,NULL);
...
/* This routine evaluates the Jacobian times vector product
   Right now it is just a copy of the default CVSPGMR DQ routine
   to be used as a test of user defined jtimes routine */

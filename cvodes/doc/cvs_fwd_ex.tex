...
\begin{equation}\label{e:cvfnx_ODE}
  u^\prime_i= q_1 \frac{u_{i+1}-2u_{i}+u_{i-1}}{(\Delta x)^{2}}
...
\begin{equation}\label{e:cvfdx_ode}
  \begin{split}
    y^\prime_1 &= -p_1 y_1 + p_2 y_2 y_3   \\
    y^\prime_2 &=  p_1 y_1 - p_2 y_2 y_3 - p_3 y_2^2 \\
    y^\prime_3 &=  p_3 y_2^2 \, ,
  \end{split}
\end{equation}
...
the model parameters \id{p}, the array \id{plist} of integer flags specifying
...
nonzero model parameters) and the array of flags \id{plist}. The choice \id{plist}$_i=i+1$
...
\id{sensi\_meth} (read from the command line arguments) as \id{CV\_SIMULTANEOUS}, 
\id{CV\_STAGGERED}, or \id{CV\_STAGGERED1}.
...
\id{f\_data}, which must first be cast to the \id{UserData} type. The user-supplied
...
obtain an ODE system $u^\prime = f(t,u)$ representing (\ref{e:pvfkx_PDE}).  

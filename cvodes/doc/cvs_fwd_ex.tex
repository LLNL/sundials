...
directory, the following three serial examples for forward sensitivity analysis:
...
The {\cvodes} distribution contains, in the \id{sundials/cvodes/examples\_par}
directory, the following two parallel examples for forward sensitivity analysis:
...
The \id{ifS = ONESENS} parameter indicates that the user-provided sensitivity right hand side
routine \id{fS} computes the right hand side one sensitivity at a time. 
The next two \id{NULL} parameters in the call to \id{CVodeSensMalloc}
...
The last argument specifies that the structure \id{data} containing user-specified data
should be passed to each call of \id{fS}.

...
\item
  ...
  The final value of \id{tout} (or of \id{t}), denoted \id{tlast}, is then
  the endpoint $t_1$;
...
\item \label{i:back_end}
  ...
  routine \id{CVode}, to integrate the backward problem from \id{tlast}
  to \id{t0}, returning the solution of the backward problem at \id{t0} into
...
starts with the \id{tlast} value from the last \id{CVodeF} call. Finally, the 
argument \id{fB}, the {\C} function to compute the right hand side of the 
backward problem, must be of type \id{RhsFnB} and
has the form \id{fB(NB, t, y, yB, yBdot, f\_dataB)} 
(see \S\ref{ss:user_fct_adj} for details). 
All other arguments are equivalent to those of \id{CVodeMalloc}.
...
\Id{CVBM\_NO\_MEM} if \id{cvadj\_mem} was \id{NULL}, or \id{CVBM\_MEM\_FAIL}

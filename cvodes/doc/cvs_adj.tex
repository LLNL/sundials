...
\item
  \textcolor{gray}{\bf Initialize the vector specification for the forward problem}
...
\item
  ...
  function \id{CVode}, either in \Id{NORMAL} mode to the time 
  ...
  The final value of of \id{tret}, denoted \id{tfinal}, is then
...
\item
  {\bf Initialize the vector specification for the backward problem}

  Call the appropriate implementation-dependent {\nvector} specification 
  function (typically the same as for the forward problem).
...
\item
  ...
  Call \Id{CVodeCreateB}, a wrapper around \id{CVodeCreate}, to
...
\item
  ...
  Call \Id{CVodeMallocB}, a wrapper around \id{CVodeMalloc}, to 
...
\item \label{i:lin_solverB}
  ...
  the initialization functions \id{CVDenseB}, \id{CVBandB}, \id{CVDiagB},
...
\item
  ...
  (see \S\ref{sss:cvodeb}). This function can be called either in \id{NORMAL}
  or \id{ONE\_STEP} mode. Typically, \id{CVodeB} will be called in \id{NORMAL}
  mode with an end time equal to the iitial time of the forward problem.
...
\item
  ...
  functions. These include calls to \id{NV\_Free} for the vectors \id{y} and \id{yB},
  calls to the appropriate implementation-dependent {\nvector} free function for
  the forward and backward vector specification blocks, and calls to \id{CVodeFree}
  to free the {\cvodes} memory block for the forward problem and to
  \id{CVadjFree} (see \S\ref{sss:cvadjmalloc}) to free the memory allocated for the 
...
After the setup phase for the forward problem and before the call
...
}
{
  ...
  \item[Nd] (\id{long int}) 
    is the number of integration steps between two consecutive check points.
  \end{args}
}
{
...
{
  The function \ID{CVadjFree} frees the pointer allocated by
...
}
{
  ...
  \item[\Id{CVADJ\_NO\_ADJMEM}]
    The \id{cvadj\_mem} argument was \id{NULL}.
  \item[\Id{CVADJ\_NO\_MEM}]
...
}
{
  ...
  At this time, \id{CVodeF} stores check point information into memory
  ...
  interpolation is already available from the last check point forward. In particular,
  ...
  with \id{itask}$=$\id{NORMAL\_TSTOP} or \id{itask}$=$\id{ONE\_STEP\_TSTOP}, after
  a return with \id{flag}$=$\id{TSTOP\_RETURN}, the integration should not be 
...
{
  If successful, \id{CVodeCreateB} stores internally a pointer to the newly created 
...
{
  flag = CVodeMallocB(cvadj\_mem, fB, tB0, yB0, itolB, reltolB, abstolB, nvSpecB);
}
{
...
}
{
  ...
  \item[tB0] (\id{realtype})
    specifies the end point where final conditions are provided for the 
    backward problem.
  ...
  \item[nvSpecB] (\id{NV\_Spec})
    is a pointer to the vector specification structure for the backward problem. 
  \end{args}
}
{
...
%%
%%
Note that memeory allocated by \id{CVodeMallocB} is deallocated by the function 
\id{CVadjFree}.
%%
...
\end{verbatim}
Their return flag \id{flag} (of type \id{int}) can have any of the return values 
of their counterparts, but can also be \Id{CVADJ\_NO\_ADJMEM} if the \id{cvadj\_mem} 
...
It is essentially a wrapper around the {\cvodes} main integration function 
\id{CVode} and, in the case in which check points were needed, it evolves 
the solution of the backward problem through a sequence of forward-backward 
integrations between consecutive check points. 
...
}
{
  ...
  The return value \id{flag} (of type \id{int}) will be one of the following.
  For more details see \S\ref{sss:cvode}.
  \begin{args}[CVADJ\_REIFWD\_FAIL]
  \item[\Id{SUCCESS}]
    \id{CVodeB} succeeded.
  \item[\Id{CVODE\_NO\_MALLOC}]
    The {\cvodes} memory for the backward problem was not allocated through 
    a call to \id{CVodeMallocB}.
  \item[\Id{ILL\_INPUT}]
...
%%
The adjoint module in {\cvodes} provides wrappers around most of the optional 
...
\begin{verbatim}
  flag = CVodeResetIterTypeB(cvadj_mem, iterB);
...
{
  The function \ID{CVSpgmrSetGSTypeB} specifies the 
...
%%
%%
\ucfunction{CVSpgmrResetPrecTypeB}
{
  flag = CVSpgmrResetPrecTypeB(cvadj\_mem, pretypeB);
}
{
  The function \ID{CVSpgmrResetPrecTypeB} resets the type
...
%%
The use of the adjoint module in {\cvodes} has access to any of the optional output 
...
}
{
  ...
  memeory block for the backward problem.
}
{
  The argument \id{cvadj\_mem}, of type \id{void *} is the pointer to the
...
{
flag = CVodeQuadMallocB(cvadj\_mem, fQB, nvSpecQB);
}
{
  The function \ID{CVodeQuadMallocB} provides required problem,
...
}
{
  ...
  \item[nvSpecQB] (\id{NV\_Spec})
    is a pointer to the backward quadrature vector specification structure.
  \end{args}
}
{
  The return flag \id{flag} (of type \id{int}) will be one of the following:
...
{
  flag = CVodeQuadReInitB(cvadj\_mem, fQB);
}
{
  The function \id{CVodeQuadReInitB} provides required problem specifications 
  and reinitializes the backward quadrature integration.
}
{
  \begin{args}[cvadj\_mem]
  \item[cvadj\_mem] (\id{void *})
...
of \id{CVodeB}, {\cvodes} provides a wrapper around the function \ID{CVodeGetQuad}
...
}
{
  ...
  successful return from \id{CVodeB}.
}
{
  \begin{args}[cvadj\_mem]
  \item[cvadj\_mem] (\id{void *})
    pointer to the adjoint memory previously allocated by \id{CVadjMalloc}.
...
}
{
  ...
  \item[CVG\_NO\_MEM] 
    ...
    previous call to \id{CVodeCreateB}.
  \item[CVG\_NO\_QUAD] 
    Quadrature integration was not initialized throough a 
    previous call to \id{CVodeQuadMallocB}..
  \item[BAD\_DKY] 
    \id{yQB} is \id{NULL}.
  \item[\Id{CVADJ\_NO\_ADJMEM}]
...
from their default values through calls to one of the following functions which wrap around 
...
}
{
  ...
  the addresses in the linked list of checkpoints and the time points at
  which checkpoints were defined.
}
{
  The argument \id{cvadj\_mem}, of type \id{void *} is the pointer to the
  adjoint memory block returned by \id{CVadjMalloc}.
...
{
  For a typical output of \id{CVadjGetCheckPointsList}, see \cite{cvodes1.1_ex}.
}
%%
%%
%%-------------------------------------------------------------------------------------
...
The user must provide a function of type \ID{RhsFnB} defined as follows:
\usfunction{RhsFnB}
{
  typedef void (*RhsFnB)(&realtype t, N\_Vector y, N\_Vector yB, \\
...
\usfunction{QuadRhsFnB}
{
  typedef void (*QuadRhsFnB)(&realtype t, N\_Vector y, N\_Vector yB, \\
...
}
{
  ...
  \item[tmp3B]
    ...
    for vectors of length \id{nB} which can be used by           
...
{
  A user-supplied Jacobian times vector function must load the vector \id{JvB}
...
{
  typedef int (*CVSpgmrPrecSetupFnB)(&realtype t, N\_Vector y, N\_Vecotr yB, \\ 
...
}
{
  ...
  \item[jcurPtrB]
    ...
    to \id{FALSE} if Jacobian data was not recomputed, but saved data was reused.
  \item[gammaB]
    is the scalar $\gamma$ appearing in the Newton matrix.
  \item[P\_dataB]
    is a pointer to user data, the same as the \id{P\_dataB}      
...
%%
%%-------------------------------------------------------------------
\section{Using the CVODES preconditioner modules for the backward problem}
%%-------------------------------------------------------------------
%%

\subsection{Using the serial banded preconditioner CVBANDPRE}
...
{
  If successful, \id{CVBandPrecAlloc} stores internally a pointer to the newly created 
  {\cvbandpre} memory block.
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[CVADJ\_NO\_ADJMEM]
  \item[\Id{SUCCESS}]
    The call to \id{CVBandPrecAllocB} was successful.
  \item[\Id{CVADJ\_BP\_NULL}]
...
%%-------------------------------------------------------------------
\subsection{Using the parallel band-block-diagonal preconditioner CVBBDPRE}
...
and the parallel vector module {\nvecp}.

In order to use the {\cvbbdpre} module in the solution of the backward problem,
the user must define two additional functions, described at the end of this section. 
...
{
  If successful, \id{CVBBDPrecAlloc} stores internally a pointer to the newly created 
  {\cvbbdpre} memory block.
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[CVADJ\_NO\_ADJMEM]
  \item[\Id{SUCCESS}]
    The call to \id{CVBBDPrecAllocB} was successful.
  \item[\Id{CVADJ\_BBD\_NULL}]
...
module calls to construct the preconditioner: a function \id{glocB} 
(of type \id{CVLocalFnB}) which approximates the right-hand side of the backward
problem and which is computed locally, and a function \id{cfnB} 
(of type \id{CVCommFnB}) which performs all inter-process communication necessary 
...
}
{
  ...
  \id{glocalB} as a function of \id{t}, \id{y}, and \id{yB}.  
}
{
  \begin{args}[NlocalB]
  \item[NlocalB] 
...
}
{
  ...
  calculate \id{glocalB} has already been done, and this data is accessible within
  \id{f\_dataB}.
}
%%
%%
...
}
{
  ...
  evaluates the right-hand side of the backward problem
  with the same \id{t}, \id{y}, and \id{yB} arguments.
}
%%
%%
\index{CVBBDPRE@{\cvbbdpre} preconditioner!usage with adjoint module|)}

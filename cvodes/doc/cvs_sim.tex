...
\item Set the vector \id{y0} of initial values.  Use macros
  ...
  Otherwise, make the call \id{y0 = }\Id{N\_VNew}\id{(N, machEnv);} and load 
...
\end{verbatim}
where \id{N} is the number of ODEs in the system, \id{f}
is the C function to compute $f$ in the ODE, \id{t0} is the initial
...
The function \ID{CVReInit} reinitializes the main {\cvodes} solver for
the solution of a problem, where a prior call to \Id{CVodeMalloc} has
been made with the same problem size \id{N}. \id{CVReInit} performs the 
...
The use of \id{CVReInit} requires that the maximum method order,    
...
If \id{iter = }\Id{NEWTON}, then following the call to \id{CVReInit}, a call  
...
The call to the \id{CVReInit} function has the form
\begin{verbatim}
...
memory returned by \id{CVodeMalloc}.
All the remaining arguments to \id{CVReInit} have names and         
meanings identical to those of \id{CVodeMalloc}.  Note that the     
problem size \id{N} is not passed as an argument to \id{CVReInit},       
as that is assumed to be unchanged since the \id{CVodeMalloc} call. 

The return value \id{ier} of \id{CVReInit} is equal to: 
\begin{itemize}
...
to \Id{CVReInit} to solve another problem of the same size if there is a change
...
\item {\em Dense linear solver reinitialization} 
  ...
  After solving one problem using {\cvdense}, call \id{CVReInit} and then
...
\item {\em Banded linear solver reinitialization}
  ...
  After solving one problem using {\cvband}, call \id{CVReInit} and then 
...
\item {\em {\spgmr} linear solver reinitialization}
  ...
  After solving one problem using {\cvspgmr}, call \id{CVReInit} and then
...
\end{verbatim}
  ...
  This function takes as input the problem size \id{N}, the independent variable  
...
\end{verbatim}
  ...
  Typically, a user-supplied Jacobian function \id{djac} would be
  expected to access the arguments \id{N}, \id{t}, \id{y}, \id{J}, 
  \id{f\_data}, and \id{jac\_data}, at most.  The remaining arguments 
  ...
  difference quotient approximation to $J$, when the user specifies that
  option.
  \index{Jacobian approximation routine!dense!user-supplied|)}
%
%--------------
...
\end{verbatim}
  ...
  Typically, a user-supplied Jacobian function \id{bjac} would be
  expected to access the arguments \id{N}, \id{mupper}, \id{mlower},
  \id{t}, \id{y}, \id{J}, \id{f\_data}, and \id{jac\_data}, at most.
  ...
  that computes a difference quotient approximation to $J$, when the
...
\end{verbatim} 
  ...
  Typically, a user-supplied Jacobian-times-vector function \id{jtimes} would be
  expected to access the arguments \id{N}, \id{v},
  \id{t}, \id{y}, \id{Jv}, \id{f\_data}, and \id{jac\_data}, at most.
  The remaining arguments would not typically be accessed, but appear in
  the call list because they are needed by the function \id{CVSpgmrDQJtimes}
  that computes a difference quotient approximation to $Jv$, when the
  user specifies that option.
...
\end{verbatim}
  ...
  \begin{itemize}
  \item 
    \id{N} is the length of all vector arguments;
  \item 
...
\begin{itemize}
  ...
  \item \id{cvode\_mem = CVodeMalloc(N, f, \ldots );}
...
\item  \id{y = N\_VNew(N, machEnv)};

\item  \id{cvode\_mem = CVodeMaloc(N, f, \ldots )};

...
}
{
  ...
  \item[f] (\Id{RhsFn})
    ...
    \id{f(t, y, ydot, f\_data)} (for full details see \S\ref{ss:user_fct_sim}).
  \item[t0] (\id{realtype})
...
\item\label{i:quad_optional_inputs}
  ...
  of the quadrature equation right-hand sides. See \S\ref{ss:quad_optional_input}
  for details.
...
\item
  ...
  side function and integration tolerances, and to allocate internal
  memory related to quadrature integration. See \S\ref{ss:quad_malloc}
  for details.
...
{
flag = CVodeQuadMalloc(cvode\_mem, fQ, itolQ, reltolQ, abstolQ, nvSpecQ);
}
{
  The function \ID{CVodeQuadMalloc} provides required problem and solution specifications, 
  allocates internal memory, and initializes quadraure integration.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  \item[\Id{QCVM\_MEM\_FAIL}] 
    A memory allocation request has failed.
  \item[\Id{QCVM\_ILL\_INPUT}] 
    An input argument to \id{CVodeQuadMalloc} has an illegal value.
  \end{args}
}
{
...
{
  flag = CVodeQuadReInit(cvode\_mem, fQ, itolQ, reltolQ, abstolQ);
}
{
...
}
{
  ...
  \item[fQ] (\Id{RhsFn})
    ...
    equations.
  \item[itolQ] (\id{int}) 
    is either \Id{SS} or \Id{SV} (see \id{CVodeQuadMalloc} above).
  \item[reltolQ] (\id{realtype *})
    \index{tolerances}
    is a pointer to the relative error tolerance.
  \item[abstolQ] (\id{void *})
...
}
{
  ...
  \item[\Id{QCVREI\_ILL\_INPUT}] 
    An input argument to \id{CVodeQuadReInit} has an illegal value.
  \end{args}
}
{
...
}
{
  ...
  \item \id{dkyQ} (\id{N\_Vector})
    the vector continaing the derivative. This vector must be allocated by the caller. 
  \end{args}
}
{
  The return value \id{flag} of \id{CVodeGetQuad} is one of:
...
{\cvodes} provides the following two optional input functions:
%%
%%
...
{
  The function \ID{CVodeQuadErrCon} specifies error control strategy for
...
{
  By default, \id{errconQ} is set to \id{FALSE}. If \id{errconQ}$=$\id{FALSE}, 
  then the tolerances passed to \id{CVodeQuadMalloc} (and \id{CVodeQuadReInit})
  are ignored.
}
%%
%%
...
{
  The function \ID{CVodeGetQuadStats} returns the {\cvode} integrator statistics
  as a group.
}
{
  \begin{args}[cvode\_mem]

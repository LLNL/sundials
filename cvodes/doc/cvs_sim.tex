...
}
{
  ...
  \item[itol] (\id{int}) 
    is one of \ID{CV\_SS}, \ID{CV\_SV}, or \ID{CV\_WF}, where \ID{itol}$=$\id{SS} indicates scalar
...
and specify its name through a call to \id{CVSpgmrSetPreconditioner}.
...
\id{CVSpgmrSetPreconditioner} to the preconditioner setup and solve functions.  
...
the user can also specify int the call to \id{CVSpgmrSetJacFn}, a pointer to a 
...
}
{
  ...
  the actual size of the real workspace given in \id{realtype} words is:
  \begin{itemize}
  \item Base value: \id{lenrw} $=86 +$ (\id{maxord+5})$N$
  \item With \id{itol = CV\_SV}: \id{lenrw} $=$ \id{lenrw} $+ N$ 
  \item With rootfinding for $N_g$ functions (see \S\ref{s:using_rootfinding}): 
    \id{lenrw} $=$ \id{lenrw} $+ 3N_g$ 
  \end{itemize}

  The size of the integer workspace (without distinction between \id{int} 
  and \id{long int}) is:
  \begin{itemize}
  \item Base value: \id{leniw} $=52 +$ (\id{maxord+5})$N$
  \item With \id{itol = CV\_SV}: \id{leniw} $=$ \id{leniw} $+ N$ 
  \item With rootfinding for $N_g$ functions: \id{leniw} $=$ \id{leniw} $+ N_g$ 
  \end{itemize}

  For the default value of \id{maxord}, the base values are:
  \begin{itemize}
  \item For the Adams method: \id{lenrw} $=96 + 17N$ and \id{leniw} $=52 + 17N$ 
  \item For the BDF method: \id{lenrw} $=96 + 10N$ and \id{leniw} $=52 + 10N$ 
  \end{itemize}

  Note that additional memory is allocated if quadratures and/or forward sensitivity
  integration is enabled. See \S\ref{ss:quad_malloc} and \S\ref{ss:sensi_malloc} for 
  more details.
}
%%
...
{
  The function \ID{CVodeSetQuadErrCon} specifies whether or not quadrature variables

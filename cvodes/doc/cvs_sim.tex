...
This chapter is concerned with the use of {\cvodes} for the integration
of IVPs.  The following sections treat the header files, the layout of
the user's main program, description of the {\cvodes} user-callable
...
may also be helpful.  Those codes are intended to serve as templates and are included 
...
\begin{itemize}
\item {\em build\_tree}\id{/lib/libcvodes.a},
...
{
  The {\cvdiag} solver is the simplest of all the current {\cvodes} linear solvers. 
...
{
  By default, {\cvodes} estimates the initial stepsize as the solution $h$ 
...
\end{Steps}
%%
Quadrature-related optional inputs (step \ref{i:quad_optional_inputs} above),
...
}
{
  ...
  \item[itolQ] (\id{int}) 
    ...
    while \id{itolQ}$=$\id{CV\_SV} indicates scalar relative error tolerance and 
...
While integrating the IVP, {\cvodes} has the capability of finding the

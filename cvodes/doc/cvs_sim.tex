...
Jacobian. The following {\cvodes} modules can only be used with {\nvecs}:
...
\item {\em build\_tree}\id{/lib/libsundials\_nvec*.}{\em lib} (up to two files)
\end{itemize}
...
A high-level overview of the combined user program and {\cvodes} package is
...
but before any calls to \id{CVode}, the user's program must call
\Id{CVDense}, \Id{CVBand}, \Id{CVDiag}, or \Id{CVSpgmr},
...
In each case, except in the diagonal approximation case ({\cvdiag}), the linear
...
}
{
  ...
  \index{preconditioning!advice on|(}With the {\spgmr} method, preconditioning 
...
}
{
  ...
  \item[\Id{CV\_NO\_MALLOC}]
    The {\cvodes} memory block was not allocated by a call to \id{CVodeMalloc}.
  \item[\Id{CV\_ILL\_INPUT}]
...
Data for Jacobian function & \id{CVDenseSetJacData} & {\tt NULL} \\
\hline
\multicolumn{3}{|c|}{\bf CVBAND linear solver} \\
\hline
Band Jacobian function & \id{CVBandSetJacFn} & internal DQ \\
Data for Jacobian function & \id{CVBandSetJacData} & {\tt NULL} \\
\hline
\multicolumn{3}{|c|}{\bf CVSPGMR linear solver} \\
\hline
Preconditioner solve function & \id{CVSpgmrSetPrecSolveFn} & {\tt NULL} \\
Preconditioner setup function & \id{CVSpgmrSetPrecSetupFn} & {\tt NULL} \\
Data for preconditioner functions & \id{CVSpgmrSetPrecData} & {\tt NULL} \\
Jacobian-vector product function & \id{CVSpgmrSetJacTimesVecFn} & {\tt NULL} \\
Data for Jacobian-vector product function &\id{CVSpgmrSetJacData} & {\tt NULL} \\ 
...
\id{CVodeSetErrFile} is to be called, it should be called first, in order to
take affect for any later error message.
...
}
{
  ...
  needed only if the tolerances are being changed from their values in the
  prior call to \id{CVode}.
}
%%
...
create an arbitrary data structure with relevant problem data and to access it
...
}
{
  ...
  \item[djac] (\id{CVDenseJacFn})
    user-supplied dense Jacobian approximation function.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
The call to \id{CVSpgmr} is used to communicate both the type of preconditioning 
(\id{pretype}) and the maximum dimension of the Krylov subspace to be used (\id{maxl}). 
The \id{pretype} parameter can be \id{PREC\_NONE}, \id{PREC\_LEFT}, \id{PREC\_RIGHT}, or \id{PREC\_BOTH}.
If no preconditioning is desired, the specify \id{PREC\_NONE}.
...
%%
The preprocessing and evaluation of any Jacobian-related data needed
...
fully described in \S\ref{ss:user_fct_sim}.
...
This allows the user to create an arbitrary data structure with relevant problem data 
and to access it during the execution of the user-supplied preconditioner functions
...
\index{Jacobian approximation function!Jacobian-vector product!difference quotient}
that comes with the {\cvspgmr} solver.
A user-defined Jacobian-vector product function must be of type \id{CVSpgmrJtimesFn} and 
can be specified through a call to \id{CVSpgmrSetJacTimesVecFn} 
(see \S\ref{ss:user_fct_sim} for specification details).
%%
As with the preconditioner-related data structure \id{P\_data}, 
the user can specify, through a call to \id{CVSpgmrSetJacData}, a pointer to a 
user-defined data structure (\id{jac\_data}) which
the {\cvspgmr} solver passes to the Jacobian-vector product function \id{jtimes} each
time it is called.  
The pointer \id{jac\_data} may be identical to \id{P\_data} or \id{f\_data}.
%%
%%
...
}
{
  ...
  If \id{NULL} is given, this default function is used.
...
This function should only be called after a successful return from \id{CVode} as it provides 
...
{
  The function \ID{CVodeGetDky} computes the \id{k}-th derivative of the function \id{y}
  at time \id{t}, i.e. $d^{(k)}y/dt^{(k)} (t)$, where $t_n - h_u \le$
...
}
{
  ...
  successful return from \id{CVode}. Use \id{CVodeGetLastOrder} 
  and \id{CVodeGetLastStep} from the next section to access 
  $q_u$ and $h_u$.
}
...
{
  The user does not need to allocate space for \id{eweight} and should not modify
...
{
  The user does not need to allocate space for \id{ele}.
}
%%
%%
...
}
{
  ...
  then the value of \id{flag} corresponds to the column index (numbered from one) of
...
}
{
  ...
  the value of \id{flag} corresponds to the column index (numbered from one) of
  a diagonal element with value zero that was encountered during the LU factorization of the 
...
}
{
  ...
  the value of \id{flag} is equal to \id{CVDIAG\_INV\_FAIL}, indicating that a
...
{
  If the {\cvspgmr} setup function failed (\id{CVode} returned \id{CV\_LSETUP\_FAIL}),
  then \id{flag} contains the return value of the preconditioner setup function \id{psetup}.

  If the {\cvspgmr} solve function failed (\id{CVode} returned \id{CV\_LSETUP\_FAIL}),
  ...
  indicating a singular matrix was found during the QR factorization, \id{SPGMR\_PSOLVE\_FAIL\_REC}
  ...
  indicating a failure of the Jacobian-vector product function, \id{SPGMR\_PSOLVE\_FAIL\_UNREC}
...
of type \ID{CVSpgmrJacTimesVecFn} defined as follows:
...
\item\label{i:quad_optional_inputs}
  ...
  shoule be used in the step size control mechanism, and
...
\item
  ...
  Typically, the quadrature variables should be initialized 
  to $0$.
...
\end{Steps}
%%
Quadrature-related optional inputs (step \ref{i:quad_optional_inputs} above) can be set,
and \id{CVodeQuadMalloc} can be called, anywhere between steps 
...
}
{
  ...
  \item[\Id{CV\_MEM\_NULL}] 
    The {\cvodes} memory block was not initialized by a prior call to \id{CVodeCreate}.
  \item[\Id{CV\_MEM\_FAIL}] 
    A memory allocation request failed.
  \end{args}
}
...
{
  The function \ID{CVodeQuadErrCon} specifies the error control strategy to be used for
  quadrature variables.
}
{
  \begin{args}[cvode\_mem]
...
{
  By default, \id{errconQ} is set to \id{FALSE}. If \id{errconQ = TRUE}, 
  then quadrature integration tolerances must be specified via a call
...
{
  The user does not need to allocate space for \id{eQweight} and should not modify
  any of its components. 

  Note that if quadratures were not included in the
  error control mechanism (through a call to \id{CVodeSetQuadErrCon} with
  \id{errconQ = TRUE}), \id{CVodeGetQuadErrWeights} returns
  \id{eQweights = NULL}.
}
%%
%%
...
%%
%%
There is only one optional input function associated with rootfinding. 
%%
...
{
  The function \ID{CVodeSetGdata} specifies the user-defined data block \ID{g\_data}
...
\item \label{i:bandpre_init}
  ...
  to allocate memory for and to initialize a data structure (pointed to by \id{bp\_data}) to be 
...
\item \label{i:bandpre_attach}
  ...
  The function \Id{CVBPSpgmr} is a wrapper for the {\cvspgmr} specification
  ...
  The last argument to \id{CVBPSpgmr} is a pointer to the {\cvbandpre} data
  structure returned by \id{CVBandPrecAlloc}.
...
\item \label{i:bbdpre_init}
  ...
  to allocate memory for and to initialize a data structure (pointed to by \id{bbd\_data}) to be 
...
\item \label{i:bbdpre_attach}
  ...
  \end{itemize}
  The arguments \id{pretype} and \id{maxl} are described below.
  The last argument passed to \id{CVBBDSpgmr} is a pointer to the {\cvbbdpre} memory block

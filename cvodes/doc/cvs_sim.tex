...
{
  The function \ID{CVodeQuadTolerances} specifies the integration tolerances for
  quadrature variables.
}
{
  \begin{args}[cvode\_mem]
...
}
{
  ...
  \begin{args}[CVS\_ILL\_INPUT]
  \item[\Id{SUCCESS}] 
    The optional values have been successfuly set.
  \item[\Id{CVS\_NO\_MEM}]
    The \id{cvode\_mem} pointer is \id{NULL}.
  \item[\Id{CVS\_ILL\_INPUT}] 
    An input argument to \id{CVodeSetTolerances} has an illegal value.
  \end{args}
}
{
  By default, \id{errconQ} is set to \id{FALSE}. If \id{errconQ}$=$\id{TRUE}, 

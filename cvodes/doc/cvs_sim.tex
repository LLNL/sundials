...
This usage is essentially equivalent to using {\cvode}~\cite{cvode2.1_ug}.
...
The listings of the sample programs in the companion document \cite{cvodes1.1_ex} may also be helpful. 
...
preconditioner, etc. In one of the examples in \cite{cvodes1.1_ex}, preconditioning
...
\item\label{i:lin_solver} 
  ...
  {\s} \id{flag = }\Id{CVDense}\id{(...);}

  {\s} \id{flag = }\Id{CVBand}\id{(...);}

  \id{flag = }\Id{CVDiag}\id{(...);}

  \id{flag = }\Id{CVSpgmr}\id{(...);}
...
\item
  ...
  \id{flag = }\Id{CVode}\id{(cvode\_mem, tout, yout, \&tret, itask);}
...
{
  The function \ID{CVodeCreate} instantiates a {\cvodes} solver object and specifies 
  the solution method.
}
{
  \begin{args}[iter]
  ...
  (\id{ADAMS}, \id{FUNCTIONAL}) for non-stiff problems and
...
}
{
  ...
  \item[\Id{CVODE\_NO\_MALLOC}]
    The {\cvodes} memory was not allocated through a call to \id{CVodeMalloc}.
  \item[\Id{ILL\_INPUT}]
...
Table~\ref{t:optional_input} lists all optional input functions in {\cvodes} 
(relevant to solving IVPs) 
...
{
  By default, {\cvodes} estimates the initial step as solution of 
...
{
  The function \ID{CVSpgmrSetPrecSolveFn} specifies the preconditioner
  solve function.
}
{
  \begin{args}[cvode\_mem]
...
{
  flag = CVSpgmrResetPrecType(cvode\_mem, pretype);
}
{
...
}
{
  ...
  \item[leniw] (\id{long int})
    number of integer values in the {\cvodes} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
the sequence of calls in the user main program in this situation. Steps that are
unchanged from the user main program presented in \S\ref{ss:skeleton_sim} are 
grayed-out.
...
\item
  ...
  {\s} Set \id{N}, the problem size $N$ (excluding quadrature variables) 
  and \id{Nq}, the number of quadrature variables.

  {\p} Set \id{Nlocal}, the local vector length (excluding quadrature variables)
  and \id{Nqlocal}, the local number of quadrature variables.
  
\item
  {\bf Initialize vector specification}\index{nvSpec@{\tt nvSpec}}

  {\s} \id{nvSpec = }\Id{NV\_SpecInit\_Serial}\id{(N);} and \\
  \id{nvSpecQ = }\Id{NV\_SpecInit\_Serial}\id{(Nq);}

  {\p} \id{nvSpec = }\Id{NV\_SpecInit\_Parallel}\id{(comm, Nlocal, N, \&argc, \&argv);} and \\
  \id{nvSpecQ = }\Id{NV\_SpecInit\_Parallel}\id{(comm, Nqlocal, Nq, \&argc, \&argv);}
...
\item\label{i:quad_optional_inputs}
  ...
  Call \id{CVodeSetQuadErrCon} to indicate whether quadrature variables
  do or do not participate in the step size control mechanism and
  \id{CVodeSetQuadFdata} to specify user data required for the evaluation
  of the quadrature equation right-hand sides. 
  If quadrature variables are included in the error test, then call
  \id{CVodeSetQuadTolerances} to specify the integration tolerances for
...
\item
  ...
  {\s} \Id{NV\_SpecFree\_Serial}\id{(nvSpec);} 
  and \id{NV\_SpecFree\_Serial}\id{(nvSpecQ);} 

  {\p} \Id{NV\_SpecFree\_Parallel}\id{(nvSpec);} 
  and \id{NV\_SpecFree\_Parallel}\id{(nvSpecQ);} 
...
Note that quadrature-related optional inputs (step \ref{i:quad_optional_inputs} above)
as well as the call to \id{CVodeQuadMalloc} can be made anywhere between steps 
...
{
flag = CVodeQuadMalloc(cvode\_mem, fQ, nvSpecQ);
}
{
  The function \ID{CVodeQuadMalloc} provides required problem,
...
}
{
  ...
  \item[nvSpecQ] (\id{NV\_Spec})
    is a pointer to the quadrature vector specification structure.
  \end{args}
}
{
  The return flag \id{flag} (of type \id{int}) will be one of the following:
  \begin{args}[QCVM\_MEM\_FAIL]
  \item[\Id{SUCCESS}]
    The call to \id{CVodeQuadMalloc} was successful.
  \item[\Id{QCVM\_NO\_MEM}] 
    The {\cvodes} memory block was not initialized through a previous call to \id{CVodeCreate}.
  \item[\Id{QCVM\_MEM\_FAIL}] 
    A memory allocation request has failed.
  \end{args}
}
{
  ...
  that these variables are always $0.0$ at the initial time and {\cvodes} performs this
...
{
  flag = CVodeQuadReInit(cvode\_mem, fQ);
}
{
...
{
  The return flag \id{flag} (of type \id{int}) will be one of the following:
  \begin{args}[QCVREI\_NO\_QUAD]
  \item[\Id{SUCCESS}]
    The call to \id{CVodeReInit} was successful.
  \item[\Id{QCVREI\_NO\_MEM}] 
    The {\cvodes} memory block was not initialized through a 
    previous call to \id{CVodeCreate}.
  \item[\Id{QCVREI\_NO\_QUAD}] 
    Memory space for the quadrature integration was not allocated through a 
    previous call to \id{CVodeQuadMalloc}.
  \end{args}
}
{
...
{
  The function \id{CVodeGetQuad} returns the quadrature solution vector after
...
}
{
  ...
  vector after successful return from \id{CVode}.
}
{
  \begin{args}[cvode\_mem]
...
{
  The function \ID{CVodeSetQuadFdata} specifies the user data block \Id{fQ\_data}
...
}
{
  ...
  Note that \id{fQ\_data} can be the same as the user data
...
{
  The function \ID{CVodeQuadErrCon} specifies the error control strategy for
  quadrature variables.
}
{
  \begin{args}[cvode\_mem]
...
{
  By default, \id{errconQ} is set to \id{FALSE}. If \id{errconQ}$=$\id{TRUE}, 
  then quadrature integration tolerances must be specified through a call
  to \id{CVodeSetQuadTolerances} defined below.
}
%%
%%
\ucfunction{CVodeSetQuadTolerances}
{
  flag = CVodeSetQuadTolerances(cvode\_mem, itolQ, reltolQ, abstolQ);
}
{
  The function \ID{CVodeSetQuadTolerances} specifies the integration tolerances for
  quadrature variables.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvodes} memory block.
  \item[itolQ] (\id{int}) 
    is either \Id{SS} or \Id{SV}, where \Id{itolQ}$=$\id{SS} indicates scalar relative error 
    tolerance and scalar absolute error tolerance, while \id{itolQ}$=$\id{SV} indicates scalar
    relative error tolerance and vector absolute error tolerance. 
    The latter choice is important when the absolute error tolerance needs to
...
}
{
  ...
  \item[\Id{CVS\_NO\_MEM}]
    The \id{cvode\_mem} pointer is \id{NULL}.
  \item[\Id{CVS\_ILL\_INPUT}] 
    An input argument to \id{CVodeSetQuadTolerances} has an illegal value.
  \end{args}
}
{
  By default, \id{errconQ} is set to \id{FALSE}. If \id{errconQ}$=$\id{TRUE}, 
...
}
{
  ...
  number of calls to the user's quadrature right-hand side evaluation function.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
{
  The user need not allocate space for \id{eQweight} and should not modify
  any of its components. Note that if quadratures were not included in the
  error control mechanism (through a call to \id{CVodeSetQuadErrCon} with
  \id{errconQ}$=$\id{TRUE}), \id{CVodeGetQuadErrWeights} returns
  \id{eQweights}$=$\id{NULL}.
}
%%
%%
...
must be of type \ID{QuadRhsFn} defined as follows:
\usfunction{QuadRhsFn}
{
  typedef void (*QuadRhsFn)(realtype t, N\_Vector y, N\_Vector yQdot, void *fQ\_data);
}
{
...
{
  Allocation of memory for \id{yQdot} is handled within {\cvodes}.
...
%%
%%===================================================================================
While integrating the IVP, {\cvodes} has the capability of finding the roots of a set 
...
{
  Allocation of memory for \id{gout} is handled within {\cvodes}.
}
...
{
  The banded approximate Jacobian will have non-zeros only in location $(i,j)$
...
To use the {\cvbbdpre} module, the user must supply two functions which the
...
}
{
  ...
  exceeds the value \id{local\_N}$-1$, it is replaced with 0 or \id{local\_N}$-1$.

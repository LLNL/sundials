...
\begin{equation}\label{e:cvadx_ADJ}
  \begin{split}
    &\lambda^\prime = - (f_y)^T  \lambda - (g_y)^T \\
    &\lambda(t_1) = 0 \, .
  \end{split}
\end{equation}
...
initializes the forward integration by specifying the initial conditions and integration 
...
\begin{equation}\label{e:pvanx:quad}
  \begin{split}
    &\xi^\prime_1 = \int_{x_0}^{x_1} dx \mu \frac{\partial^2 u}{\partial x^2} \, , \quad
    \xi_1(t_1) = 0 \, , \\
    &\xi^\prime_2 = \int_{x_0}^{x_1} dx \mu \frac{\partial u}{\partial x} \, , \quad
    \xi_2(t_1) = 0 \, ,
  \end{split}
\end{equation}
yielding

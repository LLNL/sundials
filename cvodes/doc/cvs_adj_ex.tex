...
the header file \id{cvdense.h} for the {\cvdense} linear solver module, and
...
The \id{main} function begins with type declarations and continues with
...
initializes \id{y} with the initial conditions of the forward problem, allocates and 
initializes \id{q} for the quadrature used in computing the value $G$ and finally
sets the scalar relative tolerance \id{reltol} and vector absolute tolerance \id{abstol}
...
mathematical formulation (\ref{e:cvadx_ODE}). The fucntion \id{fQ} implements
...
Using \id{MX} $=40$ on 4 procesors, the gradient of $g(t_f)$ with respect to 
...
Communication among processors for the evaluation of the ODE right-hand sides involves
...
%%
\begin{figure}
  {\centerline{\epsfig{file=pvakx2D.eps,width=0.75\textwidth}}}

...
problem at time \id{T0} in the vector \id{yB}.

The main program ends by printing the value of $G$ and its gradient and by
freeing previously allocated memory through calls to \id{CVodeFree} (for the
{\cvodes} memory for the forward problem), \id{CVadjFree} (for the memory
allocated for the combined problem), \id{N\_VFree} (for the various vectors), 
and \id{M\_EnvFree\_Serial} (for the two machine environment variables
\id{machEnvF} and \id{machEnvB}).

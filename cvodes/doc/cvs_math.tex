...
\begin{equation}\label{e:nonlinear}
  G(y_n) \equiv y^n - h_n \beta_{n,0} f(t_n,y^n) - a_n = 0 \, ,
\end{equation}
...
{s^\prime_i}  = \frac{\partial f}{\partial y} s_i + \frac{\partial f}{\partial p_i} \, ,
...
\item {\em Staggered Direct}
  ...
  after the (BDF or Adams) discretization is used to eliminate $s^\prime_i$. 
...
\item {\em Staggered corrector}
  ...
  \begin{multline}\label{e:stgr_iterations}
    ...
    -a_{i,n} \right] \, ,
  \end{multline}
  where $a_{i,n} = \sum_{j>0}(\alpha_{n,j}s_{i}^{n-j}+h_n\beta_{n,j}{s^\prime_i}^{n-j})$.
...
\lambda^* \left( s^\prime - f_y s - f_p \right)dt \, ,
\end{equation}
...
Applying integration by parts to the term $\lambda^* s^\prime$ and selecting
$\lambda$ such that
\begin{equation}\label{e:adj_eqns}
\begin{split}
&\lambda^\prime = -\left( \dfdy \right)^* \lambda - 
...
therefore, $\mu(t_1) = -\lambda^\prime(t_1)$. 
...
which means that there may be points in time where only $y$ and $y^\prime$
...
intervals, the number and size of the vectors $y$ and $y^\prime$ that would 
...
the number $N_d$ of data pairs ($y$, $y^\prime$) that can be kept in memory for 
...
($y$, $y^\prime$) are generated and stored in memory for interpolation.
(see Fig.~\ref{f:ckpnt}).
%
\begin{figure}
...
($y$, $y^\prime$) are available from the last checkpoint to the end of the interval of integration.

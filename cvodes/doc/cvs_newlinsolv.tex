...
routine should be: \id{CV*\_SUCCESS} = 0 if the routine was successful,
\id{CV*\_MEM\_FAIL} = -4 if a memory allocation failed, \id{CV*\_ILL\_INPUT} = -3
if some input was illegal or the {\nvector} implementation is not compatible, or
\id{CV*\_MEM\_NULL} = -1 if the pointer to the main {\cvodes} memory block is \id{NULL}.
...
these routines.  Note that the argument list of each routine includes a

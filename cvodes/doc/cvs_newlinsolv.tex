...
\id{linit}, \id{lsetup}, \id{lsolve}, \id{lsolveS}, and \id{lfree}.
...
\item \id{lsolveS}: solve the linear system;
\item \id{lfree}: free the linear solver memory.
\end{itemize}
The \id{lsolveS} routine is intended only for use during forward sensitivity
...
\item \id{ypred}: the predicted \id{y} vector for the current {\cvodes} internal   
  step.                                                   
...
%------------------------------------------------------------------------------
\paragraph{Sensitivity solve routine.}
The type definition of \id{lsolve} is
\begin{verbatim}
int (*cv_lsolveS)(CVodeMem cv_mem, N_Vector b, N_Vector ycur, 
                  N_Vector fcur, integertype iS);  
\end{verbatim}
The routine \id{lsolveS} must solve the linear system $M x = b$
corresponding to the \id{iS}-th sensitivity system.
This routine is typically identical to \id{lsolve} (except for the
additional argument \id{iS}). Of the four linear solvers modules provided with 
{\cvodes}, only the {\cvspgmr} module has an \id{lsolveS} routine that takes
into account \id{iS} to select the correct scaling vectors for left or right
preconditioning.

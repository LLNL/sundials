...
}
{
  ...
  \item[dkyS] (\id{N\_Vector *})
    the vectors containing the derivatives. The space for dkyS must be allocated by 
    the user. 
  \end{args}
}
{
...
}
{
  ...
  \item[is] (\id{int}) specifies the sensitivity derivative vector that is to be returned
    ($0\le$\id{is}<$N_s$).
  \item[dkyS] (\id{N\_Vector})
    the vector containing the derivative. The space for dkyS must be allocated by 
    the user. 
  \end{args}
}
{
...
}
{
  ...
  number of nonlinear solver iterations for sensitivity variables at one step.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  user's right-hand side evaluation function due to finite difference approximation
...
{
  This counter is incremented only if either the \id{ism} was \id{CV\_STAGGERED} or 

...
be of interest, such as the machine roundoff \id{uround} and error
weights for states and sensitivities. The value \id{uround} used 
internally by {\cvodes} can be obtained by calling the routine
\Id{UnitRoundoff}() defined in \id{sundialsmath.c}. Error weight vectors
...
{\tt CVodeMalloc}, \id{weight} is of type \id{N\_Vector} and \id{weightS}
is of type \id{N\_Vector *}. Note that the user need not allocate space 
for either \id{weight} or \id{weightS} in \Id{CvodeGetEwt} and 
\Id{CVodeGetEwtS}, respectivley.
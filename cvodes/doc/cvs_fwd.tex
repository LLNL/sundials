...
\item \label{i:fwd_start}
  ...
  passed to \id{CVodeSensMalloc} below.
...
}
{
  ...
  \item[p] (\id{realtype *})
    ...
    The user data block \id{f\_data} must include a \id{realtype} pointer
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvodes} memory block.
  \item[itolS]  (\id{int}) 
    is either \Id{CV\_SS} or \Id{CV\_SV}, where \Id{itolS}$=$\id{CV\_SS} indicates scalar relative error 
  ...
  \item[abstolS] (\id{void *})
    is a pointer to the absolute error tolerance. If \id{itolS}$=$\id{CV\_SS}, \id{abstolS}
    must be a pointer to a \id{realtype} variable. If \id{itolS}$=$\id{CV\_SV}, \id{abstolS}
    must be an array of \id{Ns} variables of ype \id{N\_Vector}. In the latter case, \id{abstolS}
    should be created and set in the same manner as the vectors of initial values for the 
...
{
  The function \ID{CVodeGetSensNonlinSolvStats} returns the sensitivity-related
  nonlinear solver statistics as a group, in the \id{CV\_STAGGERED1} case.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
{
  Note that a sensitivity right-hand side function of type \id{CVSensRhsFn} is not

...
\item \label{i:fwd_start}
  ...
  \begin{itemize}
    ...
    \item Parameter list
      ...
      ${\text{plist}}_i = j$, for some $i=1,\ldots,N_s$. Note that the parameters
      are numbered from $1$ for the purpose of \id{plist}.

      If \id{plist} is not specified, {\cvodes} will compute sensitivities with respect
      to the first \id{Ns} parameters; i.e., ${\text{plist}}_i = i$, $i=1,\ldots,N_s$.

    \item Parameter scaling factors
      ...
      the right-hand sides of the sensitivity systems, the results will be more
      accurate if order of magnitude information is provided.
...
}
{
  ...
  \item[is] (\id{int}) specifies the sensitivity derivative vector that is to be returned
    ($0\le$\id{is}$< N_s$).
  \item[dkyS] (\id{N\_Vector})
...
{
  {\warn} This type of sensitivity right-hand side function is not compatible
...
}
{
  ...
  \item[pbar] (\id{realtype *})
    ...
    (See \S\ref{s:forward_usage}).
  \item[plist] (\id{int *}) 
    an array of \id{Ns} positive flags to specify which parameters to use in
    estimating the sensitivity equations. If non-\id{NULL}, \id{plist} must
    have all components $\ge 1$.
    (See \S\ref{s:forward_usage}).
  \end{args}
}
{
...
{
  {\warn} While \id{plist} and \id{pbar} have default values (see \S\ref{s:forward_usage}),
  an error will occur if neither \id{p} nor a user-provided sensitivity right-hand side
  function were not specified. 
}
%%
%%
...
}
{
  ...
  \item[itolS]  (\id{int}) 
    ...
    \id{itolS}$=$\id{CV\_EE}, the arguments \id{reltolS} and \id{abstolS} are ignored and
  ...
  \item[abstolS] (\id{void *})
    ...
    If \id{itolS}$=$\id{CV\_SS}, \id{abstolS} must be a pointer to an array of \id{realtype} 
    variables. 
    If \id{itolS}$=$\id{CV\_SV}, \id{abstolS} must be an array of \id{Ns} variables of type 
    \id{N\_Vector}. In the latter case, \id{abstolS} should be created and set in the same 
    manner as the vectors of initial values for the sensitivity variables (see \S\ref{s:forward_usage}).
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of:
...
{
  A \id{CVSensRhsFn} function type does not have a return value.                        
}
{
...
{
  A \id{CVSensRhs1Fn} function type does not have a return value.                        
}
{
  Allocation of memory for \id{ySdot} is handled within {\cvodes}.
}
...
reported for \id{cvdx} and the second run of the \id{cvfdx} example 

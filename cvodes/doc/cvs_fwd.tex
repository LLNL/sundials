...
\item
  ...
  \end{itemize}
  Otherwise, make the call \id{yS0 = }\Id{N\_VNew\_S}\id{(Ns,N,machEnv);} 
...
The routine \ID{CVSensReInit}, useful during the solution of a sequence of problems of 
...
The call to the \id{CVSensReInit} function has the form
\begin{verbatim}
...
\id{CVSensReInit}, as that is assumed to be unchanged since the \id{CVodeSensMalloc} call. 

The return value \id{ier} of \id{CVSensReInit} is equal to: 
\begin{itemize}
\item \Id{SUCCESS}=0 if there were no errors; 
...
Note that \id{CVSensReInit} is not a completely genuine reinitialization
routine as it may perform some memory allocation. This can happen
if \id{atolS = NULL} is passed to \id{CVSensReInit}, in which case {\cvodes}
...
arrays of length \id{Ns} are allocated by \id{CVSensReInit}.
...
\end{verbatim}
  ...
  \id{Ns}, \id{N}, \id{t}, \id{y}, \id{yS}, \id{p}, \id{plist}, and \id{f\_data}.
  ...
  \id{CVSensRhsDQ}.
  \begin{itemize}
  \item \id{f} is a pointer to the user-supplied routine of type \id{RhsFn} 
    passed to \id{CVodeMalloc};
  \item \id{Ns} is the number of sensitivity parameters;
  \item \id{N} is the problem dimension;
  ...
  \item \id{p} is the vector of problem parameters;
  \item \id{pbar} is the vector containing scaling factors for \id{p};
  \item \id{plist} is a vector of flags specifying the sensitivity parameters 
    among the problem parameters;
  \item \id{f\_data} is a pointer to the user-data space passed to \id{CVodeMalloc};
  \item \id{ewt} contains error weights for the state variables \id{y};
  \item \id{ewtS} is an array of vectors \id{ewtS[i]} containing
    error weights for sensitivity variables \id{yS[i]};
  \item \id{reltol} and \id{reltolS} are the relative error tolerances for state 
    and sensitivity variables, respectively;
  \item \id{uround} is the machine unit roundoff;
  \item \id{rhomax} is the finite difference threshold parameter used by 
    \id{CVSensRhsDQ};
  \item \id{nfePtr} is a pointer to the number of function evaluation
    calls counter. If any calls to \id{f} are made, the user has the 
    option of accounting for them in the final {\cvodes} statistics;
  \item \id{ytemp} and \id{ftemp} are pointers to memory allocated
...
\end{verbatim}
  ...
  to access the arguments \id{iS}, \id{Ns}, \id{N}, \id{t}, \id{y}, \id{yS}, \id{p}, 
  \id{plist}, and \id{f\_data}. The remaining arguments would not typically be accessed,

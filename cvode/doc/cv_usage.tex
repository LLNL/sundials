...
}
{
  ...
  and \id{long int}) is:
  \begin{itemize}
  \item Base value: \id{leniw} $=40 +$ (\id{maxord+5})$N$
  ...
  \begin{itemize}
  \item For the Adams method: \id{lenrw} $=89 + 17N$ and \id{leniw} $=40 + 17N$ 
  \item For the BDF method: \id{lenrw} $=89 + 10N$ and \id{leniw} $=40 + 10N$ 
  \end{itemize}
}
%%
%%
...
\item Interface to the main {\cvode} module
  ...
  \item \id{FCVEWTSET}    
    interfaces to \id{CVodeSetEwtFn}.
  \end{itemize}  
\item Interface to the linear solver modules
 ...
 \item \id{FCVSPGMRSETPREC}
   interfaces to \id{CVSpgmrSetPreconditioner}.
 \end{itemize}
...
\end{verbatim}
  in which the arguments are: \id{KEY} = 1, \id{NLOCAL} the local size of vectors on this
...
\end{verbatim}
  ...
  \index{FCVSPGMRSETPREC@\texttt{FCVSPGMRSETPREC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  {\warn} If the user calls \id{FCVSPGMRSETPREC}, the routine \id{FCVPSET} must
...
\end{verbatim}
  ...
  $-4$: \Id{CV\_TOO\_MUCH\_WORK}, $-5$: \Id{CV\_TOO\_MUCH\_ACC},
  $-6$: \Id{CV\_ERR\_FAILURE}, $-7$: \Id{CV\_CONV\_FAILURE},
  $-8$: \Id{CV\_LINIT\_FAIL}, $-9$: \Id{CV\_LSETUP\_FAIL}, and
  $-10$: \Id{CV\_LSOLVE\_FAIL} from \id{CVode} (see \S\ref{sss:cvode}).
...
\item {\bf Memory deallocation}
  ...
  calling \id{FCVFREE}, the user must make the call

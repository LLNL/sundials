...
companion document \cite{cvode2.1_ex} may also be helpful.  Those
...
\begin{itemize}
\item {\em build\_tree}\id{/lib/libcvode.a},
\item {\em build\_tree}\id{/lib/libfcvode.a},
\item {\em build\_tree}\id{/lib/libshared.a}, and
\item {\em build\_tree}\id{/lib/lib*nvec*.a} (up to four files)
\end{itemize}
...
{\cvodes} problem (IVP with sensitivity analysis), \index{CVODES}
...
must reference {\em build\_tree}\id{/lib/libcvodes.a}, and not 
{\em build\_tree}\id{/lib/libcvode.a}.
...
}
{
  ...
  through the macro \id{DBL\_EPSILON} defined in \id{float.h}.
}
\index{Jacobian approximation function!dense!user-supplied|)}
...
{\C} type \id{long int}. These integer variables are: the array
...
\end{verbatim}
  ...
  it may need to use the error weights provided as the \id{EWT}
  argument, and it may also need to access quantities that are not in
  the call list.  These include the current stepsize and the unit
  roundoff, which can be obtained from the \id{ROPT} array of optional
  outputs (see Table \ref{t:fcvode_io}).  In this case, we suggest
  putting \id{ROPT} in a \id{COMMON} block in order to access it here.
...
\end{verbatim}
  ...
  The argument \id{DELTA} is an input tolerance for use by
  \id{FCVPSOL} if it uses an iterative method in its solution.  In
  that case, the residual vector $\Delta = r - Pz$ of the system should
  be made less than \id{DELTA} in weighted L2 norm, i.e.
  $\sqrt{\sum(\Delta_i * \id{EWT}[i])^2} < $ \id{DELTA}.
...
\end{verbatim}
  ...
  If the user's \id{FCVPSET} uses difference quotient approximations,
  it may need to use the error weights provided as the \id{EWT}
  argument, and it may also need to access quantities that are not in
  the call list.  These include the current stepsize and the unit
  roundoff, which can be obtained from the \id{ROPT} array of optional
...
if the \id{CVODE} memory was \id{NULL}, and $-2$ if a memory allocation failed.

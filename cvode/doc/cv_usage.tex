...
etc. In one of the examples in \cite{cvode2.1_ex}, preconditioning is done
...
\item
  ...
  particular {\nvector} implementation.  If a data structure \id{ydata}
...
\item
  ...
  by calling a function defined by the {\nvector} implementation.  If
  \id{y} was created from \id{ydata}, make the call:

  {\s} \id{NV\_Dispose\_Serial(y);}

  {\p} \id{NV\_Dispose\_Parallel(y);}

  If \id{y} was created by a call to \id{N\_VNew\_Serial} or
  \id{N\_VNew\_Parallel}, then make the call:
...
}
{
  ...
  \item[itol] (\id{int}) 
    is either \ID{SS} or \ID{SV}, where \ID{itol}$=$\id{SS} indicates scalar
    relative error tolerance and scalar absolute error tolerance, while
    \id{itol}$=$\id{SV} indicates scalar relative error tolerance and vector
...
}
{
  ...
  \item[pretype] (\id{int})
    ...
    preconditioning type. Must be one of: \Id{NONE}, \Id{LEFT}, \Id{RIGHT},
    or \Id{BOTH}.
  \item[maxl] (\id{int})
...
}
{
  ...
  \item[lenrwSG] (\id{long int})
    the global number of \id{realtype} values in the {\cvspgmr} workspace.
  \item[leniwSG] (\id{long int})
    the global number of integer values in the {\cvspgmr} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
}
{
  ...
  the actual size of the real workspace is
  $N*($ \id{maxl} $+ 5) +$ \id{maxl} $*($ \id{maxl} $ + 4) + 1$ \id{realtype}
  words.  (In a parallel setting, this value is global --- totalled over
  all processors.)
}
%%
\index{CVSPGMR@{\cvspgmr} linear solver!memory requirements|)} 
...
appropriate \id{Set} or \id{Reset} calls, as described in
...
\usfunction{RhsFn}
{
  typedef void (*RhsFn)(&realtype t, N\_Vector y, N\_Vector ydot, \\
                        &void *f\_data);
}
{
...
\usfunction{RootFn}
{
  typedef void (*RootFn)(&realtype t, N\_Vector y, realtype *gout, \\
                         &void *g\_data);
}
{
...
}
{
  ...
  \item[pretype] (\id{int})
    \index{pretype@\texttt{pretype}}
    preconditioning type. Must be one of \Id{LEFT} or \Id{RIGHT}.
  \item[maxl] (\id{int})
    \index{maxl@\texttt{maxl}}
    maximum dimension of the Krylov subspace to be used. Pass $0$ to use the 
    default value \id{CVSPGMR\_MAXL}$=5$.
...
{
  In terms of the problem size $N$, and the extended upper half-bandwidth
  \id{smu} = $\min(N-1 ,$ \id{mu} $+$ \id{ml}$)$, the actual size of the
  real workspace is $N\,(2$ \id{ml} $+$ \id{mu} $+$ \id{smu} $+2)$
  \id{realtype} words, and the actual size of the integer workspace is $N$
  integer words.
}
%%
\index{memory requirements!CVBANDPRE@{\cvbandpre} preconditioner|)}
...
\item \label{i:bbdpre_init}
  ...
  \id{CVBBDPrecAlloc} are the two mandatory user-supplied functions described above.
...
{
  The function \ID{CVBBDPrecGetWorkSpace} returns the
...
}
{
  ...
  \item[lenrwBBDP] (\id{long int})
    the local number of \id{realtype} values in the {\cvbbdpre} workspace.
  \item[leniwBBDP] (\id{long int})
    the local number of integer values in the {\cvbbdpre} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
{
  In terms of the local vector dimension $N_l$, and the local extended upper
  half-bandwidth \id{smu} = $\min ( N_l - 1 ,$ \id{mukeep} $+$ \id{mlkeep}$)$,
  the actual size of the real workspace is
  $N_l \,(2$ \id{mlkeep} $+$ \id{mukeep} $+$ \id{smu} $+2)$
  \id{realtype} words, and the actual size of the integer workspace is
  $N_l$ integer words.  (These values are local to the current processor.)
}
%%
\index{memory requirements!CVBBDPRE@{\cvbbdpre} preconditioner|)}
%%
...
\item
  ...
  \item \id{FNVFREES} (defined by {\nvecs})
    interface to \id{NV\_DestroyEmpty\_Serial}.
  \item \id{FNVFREEP}  (defined by {\nvecp})
    interfaces to \id{NV\_DestroyEmpty\_Parallel}.
  \end{itemize}
...
Those mapping definitions depend in turn on a set of parameters --- 
\Id{SUNDIALS\_CASE\_UPPER}, \Id{SUNDIALS\_CASE\_LOWER},
\Id{SUNDIALS\_UNDERSCORE\_NONE}, \Id{SUNDIALS\_UNDERSCORE\_TWO},
which can be set when configuring {\sundials} (see Chapter \ref{s:install}).
Depending on these parameters, the names into which the dummy names are mapped
...
%
12 & O & \id{FCVODE}                   & \id{CVodeGetWorkSpace} [real \\
%
13 & O & \id{FCVODE}                   & \id{CVodeGetWorkSpace} [integer] \\
%
...
\hline
16 & O & \id{FCVODE}                   & \id{CVDenseGetWorkSpace} [real] \\ 
17 & O & \id{FCVODE}                   & \id{CVDenseGetWorkSpace} [integer]\\ 
18 & O & \id{FCVODE}                   & \id{CVDenseGetNumJacEvals} \\ 
\hline
\multicolumn{4}{|c|}{{\cvband} linear solver}\\
\hline
16 & O & \id{FCVODE}                   & \id{CVBandGetWorkSpace} [real] \\ 
17 & O & \id{FCVODE}                   & \id{CVBandGetWorkSpace} [integer] \\ 
18 & O & \id{FCVODE}                   & \id{CVBandGetNumJacEvals} \\ 
\hline
\multicolumn{4}{|c|}{{\cvdiag} linear solver}\\
\hline
16 & O & \id{FCVODE}                   & \id{CVDiagGetWorkSpace} [real] \\ 
17 & O & \id{FCVODE}                   & \id{CVDiagGetWorkSpace} [integer] \\ 
\hline
\multicolumn{4}{|c|}{{\cvspgmr} linear solver}\\
\hline
16 & O & \id{FCVODE}                   & \id{CVSpgmrGetWorkSpace} [real] \\ 
17 & O & \id{FCVODE}                   & \id{CVSpgmrGetWorkSpace} [integer] \\ 
...
\item {\bf Problem specification}
  ...
  {
    The optional inputs associated with \id{FCVMALLOC} are 
    \id{MAXORD}, \id{MXSTEP}, \id{MXHNIL}, \id{SLDET},
    \id{MAXERRTESTFAILS}, \id{MAXNONLINITERS}, \id{MAXCONVFAILS},
    \id{H0}, \id{HMAX}, \id{HMIN}, \id{TSTOP}, and
    \newline \id{NONLINCONVCOEF}, stored in \id{IOPT(1)}, \id{IOPT(2)},
    \id{IOPT(3)}, \id{IOPT(14)}, \id{IOPT(22)},
    \newline  \id{IOPT(23)}, \id{IOPT(24)},
    \id{ROPT(1)}, \id{ROPT(2)}, \id{ROPT(3)}, \id{ROPT(8)}, \id{ROPT(9)},
    respectively.  If any of these optional inputs are used, set the others
    to zero to indicate default values.

    The optional outputs are \id{NST}, \id{NFE}, \id{NSETUPS}, \id{NNI}, 
    \id{NCFN}, \id{NETF}, \id{QU}, \id{QCUR}, \id{LENRW}, \id{LENIW},
    \id{NOR}, \id{HU}, \id{HCUR}, \id{TCUR}, \id{TOLSF}, \id{UROUND},
    stored in \id{IOPT(4)}, ..., \id{IOPT(13)}, \id{IOPT(15)},
    \id{ROPT(4)}, ..., \id{ROPT(7)}, \id{ROPT(10)}, respectively.
  }
...
\end{verbatim}
  ...
  Optional outputs specific to the {\dense} case are \id{NJE}, \id{LRW}, and
  \id{LIW}, stored in \id{IOPT(16)}, \id{IOPT(17)}, and \id{IOPT(18)}, respectively.
...
\end{verbatim}
  ...
  Optional outputs specific to the {\band} case are \id{NJE}, \id{LRW}, and
  \id{LIW}, stored in \id{IOPT(16)}, \id{IOPT(17)}, and \id{IOPT(18)}, respectively.
...
\end{verbatim}
  ...
  \id{NCFL}, \id{LRW}, and \id{LIW}, stored in \id{IOPT(16)} $\cdots$
  \id{IOPT(21)}, respectively (see Table~\ref{t:fcvode_io}).
...
\end{verbatim}
  ...
  from \id{CVode} (see \S\ref{sss:cvode})   The current values of the optional
  outputs are available in \id{IOPT} and \id{ROPT}.
...
program.  Additionally, based on the \Id{SUNDIALS\_UNDERSCORE\_NONE} and 
...
\end{verbatim}
  ...
  \id{IER} is a return completion flag.  A value of $0$ indicates success, a
  value of $-1$ indicates that a memory failure occurred, and a value of $-2$
  indicates an illegal input.

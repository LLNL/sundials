...
}
{
  ...
  \item[pretype] (\id{int})
    ...
    specifies the type of preconditioning and may be
...
{
  The function \ID{CVodeRootInit} specifies that roots of a set of
...
}
{
  ...
  \item[rootsfound] (\id{int *})
    ...
    indices of the user functions $g_i$ found to have a root. 
...
{
  If one of the half-bandwidths \id{mudq} or \id{mldq} is negative or it
  exceeds the value \id{local\_N}$-1$, it is replaced with 0 \id{local\_N}$-1$.
}
%%
\index{CVBBDPRE@{\cvbbdpre} preconditioner!user-callable functions|)}
...
$0$ if \id{EWT} was successfully set and non-zero if an error occurred.
...
\end{verbatim}
  ...
  it may use the error weights provided as the \id{EWT} argument.
  Other possibly required quantities (such as the current stepsize, the unit 
  roundoff, etc.) can be obtained from the \id{ropt} array of optional outputs.
...
\end{verbatim}
  ...
  $-1$ if a memory allocation failure occurred, or $-2$ to indicate an illegal input.
...
\end{verbatim}
  ...
  \id{IER} is a completion flag and will be set to a positive value upon successful return
  or to a negative value if an error occurred. These values correspond to the
...
Additionally, based on the \Id{SUNDIALS\_UNDERSCORE\_NONE} and 
...
\end{verbatim}
  ...
  \id{IER} is a return completion flag.  A value of $0$ indicates success, a value of
  $-1$ indicates that a memory failure occurred, and a value of $-2$ indicates an 
  illegal input.

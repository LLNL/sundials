...
}
{
  ...
  The actual lengths \id{lenrw} and \id{leniw} are increased from these base values
  by small amounts that depend on the particular version of the {\nvector} module
  used.  The same is true for all other work array lengths described below.

  For the default value of \id{maxord}, and with no rootfinding, the base
  values are:
  \begin{itemize}
...
}
{
  ...
  denoted roughly as \id{eweight[i]*ele[i]}.
}
%%
%%

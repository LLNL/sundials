...
\item \Id{cvode\_spgmr.h}, 
  ...
  context of {\cvode}. This in turn includes a header file (\id{sundials\_iterative.h})
...
}
{
  ...
  If an error occurred, \id{CVodeMalloc} also send an eror message to the
...
to the error handler function.\index{error message}
...
%%
Optionally, the {\cvspgmr} solver passes the pointer \id{p\_data}
received through \id{CVSpgmrSetPreconditioner} to the preconditioner
...
%%
Optionally, the {\cvspbcg} solver passes the pointer \id{p\_data}
received through \id{CVSpbcgSetPreconditioner} to the preconditioner
\id{psetup} and \id{psolve} functions.  This allows the user to create
an arbitrary structure with relevant problem data and access it during
the execution of the user-supplied preconditioner functions without using
global data in the program.  The pointer \id{p\_data} may be identical
...
If used, the \id{psetup} function should be specified in the call to
\id{CVSptfqmrSetPreconditioner}.
\index{CVSPTFQMR@{\cvsptfqmr} linear solver!preconditioner setup function}
%%
Optionally, the {\cvsptfqmr} solver passes the pointer \id{p\_data}
(the same as that specified through \id{CVSptfqmrSetPreconditioner}) to the preconditioner
\id{psetup} and \id{psolve} functions.  This allows the user to create
an arbitrary structure with relevant problem data and access it during
the execution of the user-supplied preconditioner functions without using
global data in the program.  The pointer \id{p\_data} may be identical
...
}
{
  ...
  functions and the pointer to user data.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
{
  The return value of a \id{CVRhsFn} function type is currently not used
  (return a value of 0).
}
{
...
{
  The return value of a \id{CVDenseJacFn} function type is currently not used
  (return a value of 0).
}
{
...
{
  The return value of a \id{CVBandJacFn} function type is currently not used
  (return a value of 0).
}
{
...
{
  The return value of a \id{CVRootFn} function type is currently not used
  (return a value of 0).
}
{
...
{
  The return value of a \id{CVLocalFn} function type is currently not used
  (return a value of 0).
}
{
...
{
  The return value of a \id{CVCommFn} function type is currently not used
  (return a value of 0).
}
{

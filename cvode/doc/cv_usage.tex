...
acording to the {\C} type \id{long int}, while \id{ROPT} must be declared
...
\end{verbatim}
  ...
  \d{MUDQ} and \id{MLDQ} are the upper and lower half-bandwidths to be used in 
...
\begin{verbatim}
       CALL FCVBBDINIT(IPRETYPE, IGSTYPE, MAXL, DELT, IER)
\end{verbatim}
  Its arguments are the same as those of \id{FCVSPGMR}
  (see step \ref{i:fcvode_lin_solv_spec} in \S\ref{ss:fcvode_usage}).
...
\end{verbatim}
  ...
  Finally, if the value of \id{MU}, \id{ML}, or \id{MAXL} is being changed, then a call to 
  \id{FCVBBDINIT} must be made; in this case the {\spgmr} memory is reallocated.

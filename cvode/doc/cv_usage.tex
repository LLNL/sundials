...
\end{verbatim}
  ...
  \id{ITASK} is a task indicator and should be set to $0$ for normal mode 
  (overshoot \id{TOUT} and interpolate) or to $1$ for one-step mode 
  (return after each internal step taken).
  \id{IER} is a completion flag and will be set to $0$ upon successful return, or
  to a negative value in case of a failure.
  These correspond to the returns \Id{CVODE\_NO\_MEM}, \Id{ILL\_INPUT},
  \Id{TOO\_MUCH\_WORK},\Id{TOO\_MUCH\_ACC}, \Id{ERR\_FAILURE},
  \Id{CONV\_FAILURE}, \Id{SETUP\_FAILURE}, and \Id{SOLVE\_FAILURE}

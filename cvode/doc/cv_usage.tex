...
\item Interface to the linear solver modules
 ...
 \end{itemize}
\item Interface to the {\cvbbdpre} preconditioner module  
  \begin{itemize}
    \item \id{FCVBBDIN}
      interfaces to \id{CVBBDPrecAlloc} and {\spgmr} optional input routines.
    \item \id{FCVREINBBD}
...
CVPSOL   & CVPSol     & CVSpgmrPrecSolveFn \\
CVPRECO  & CVPreco    & CVSpgmrPrecSetupFn \\
CVJTIMES & CVJtimes   & CVSpgmrJacTimesVecFn \\\hline
CVLOCFN  & CVgloc     & CVLocalFn \\
CVCOMMF  & CVcfn      & CVCommFn \\ \hline
\end{tabular}
\end{center}
...
\subsection{Usage of the {\fcvode} Interface Module}
...
\item  {\bf {\nvector} module initialization}
  ...
  \begin{itemize}
  \item {\s}
    To initialize the serial {\nvector} module, the user must make
    the following call:
\begin{verbatim}
...
\subsection{Usage of the {\fcvode} interface to {\cvbbdpre}}
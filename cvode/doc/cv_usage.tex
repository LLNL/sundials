%%===================================================================================
\chapter{Using {\cvode} for IVP Solution}\label{s:simulation}
%%===================================================================================
...
{
  The function \ID{CVodeDky} computes the \id{k}-th derivative of the \id{y} function at      
...
}
{
  ...
  \item[leniw] (\id{long int *})
    number of \id{integertype} values in the {\cvode} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
\item
  Interface to the {\nvector} modules
  \begin{itemize}
  \item \id{FNVSPECINITS} (defined by {\nvecs}) 
    interfaces to \id{NV\_SpecInit\_Serial}.
  \item \id{FNVSPECINITP} (defined by {\nvecp}) 
    interfaces to \id{NV\_SpecInit\_Parallel}.
  \item \id{FNVSPECFREES} (defined by {\nvecs})
    interface to \id{NV\_SpecFree\_Serial}.
  \item \id{FNVSPECFREEP}  (defined by {\nvecp})
...
\id{CVFUN}    & \id{CVf}        & \id{RhsFn} \\
\id{CVDJAC}   & \id{CVDenseJac} & \id{CVDenseJacFn} \\
\id{CVBJAC}   & \id{CVBandJac}  & \id{CVBandJacFn} \\
\id{CVPSOL}   & \id{CVPSol}     & \id{CVSpgmrPrecSolveFn} \\
\id{CVPRECO}  & \id{CVPreco}    & \id{CVSpgmrPrecSetupFn} \\
\id{CVJTIMES} & \id{CVJtimes}   & \id{CVSpgmrJacTimesVecFn} \\\hline
\end{tabular}
\end{center}
...
pair of arrays, \Id{IOPT} of type \id{int} and \Id{ROPT} of type \id{realtype}.
...
\item  {\bf {\nvector} module initialization}
  ...
  \item {\s}
    ...
    \index{FNVSPECINITS@\texttt{FNVSPECINITS}}
\begin{verbatim}
...
\end{verbatim}
  ...
  \item {\p}
    ...
    \index{FNVSPECINITP\@texttt{FNVSPECINITP}}
\begin{verbatim}
...
\end{verbatim}
    ...
    If the \id{CVDJAC} routine is provided, then, 
...
\end{verbatim}
    The argument \id{FLAG} specifies using the {\cvode} internal difference-quotient 
    Jacobian approximation (\id{FLAG}$=0$) or the user-supplied Jacobian approximation
    (otherwise). The argument \id{IER} is an error return flag which can be $0$ 
...
\end{verbatim}
    ...
    If the \id{CVBJAC} routine is provided, then, following the call to \id{FCVBAND},
    the user must make the call:
...
\end{verbatim}
    ...
    \index{CVJTIMES@\texttt{CVJTIMIES}}
\begin{verbatim}
...
\end{verbatim}
    ...
    If the \id{CVJTIMES} routine is provided, then, 
    following the call to \id{FCVDENSE}, the user must make the call:
...
\end{verbatim}
    The argument \id{FLAG} specifies using the {\cvode} internal difference-quotient 
    Jacobian-vector approximation (\id{FLAG}$=0$) or the user-supplied Jacobian-vector
    approximation (otherwise). 
    The argument \id{IER} is an error return flag which can be $0$ 
    for success or non-zero if an error occured.

    If preconditioning is to be done (\id{IPRETYPE}$\neq 0$), then, following the call to \id{FCVSPGMR}, 
    the user must call
...
\end{verbatim}
    ...
    \index{CVPSOL@\texttt{CVPSOL}}
\begin{verbatim}
      SUBROUTINE CVPSOL(T, Y, FY, VT, GAMMA, EWT, DELTA, R, LR, Z, IER)
...
\end{verbatim}
    ...
    \index{FCVSPGMRSETPRECO@\texttt{FCVSPGMRSETPRECO}}
\begin{verbatim}
      CALL FCVSPGMRSETPRECO(FLAG, IER)
\end{verbatim}
    with \id{FLAG}$\neq 0$.
...
\end{verbatim}
    ...
    program should call the routine \id{FCVREINSPGMR} which
    reinitializes the {\spgmr} linear solver, but without reallocating its memory.
    The arguments of \id{FCVREINSPGMR} routine have the same names and meanings
...
\end{verbatim}
  ...
  \index{FNVSPECFREES@\texttt{FNVSPECFREES}}
\begin{verbatim}
      CALL FNVSPECFREES
\end{verbatim}
  or
  \index{FNVSPECFREEP@\texttt{FNVSPECFREEP}}
\begin{verbatim}
      CALL FNVSPECFREEP  
\end{verbatim}
  respectivley.
...
\begin{itemize}
\item \id{FCVBBDIN}
  interfaces to \id{CVBBDPrecAlloc}, \id{CVBBDSpgmr},
  and {\spgmr} optional input routines.
\item \id{FCVREINBBD}
  interfaces to \id{CVBBDPrecReInit} and {\spgmr} optional input routines.
\item \id{FCVBBDOPT}
...
In addition to the Fortran right-hand side function \id{CVFUN}, the
...
\id{CVLOCFN}  & \id{CVgloc}     & \id{CVLocalFn} \\
\id{CVCOMMF}  & \id{CVcfn}      & \id{CVCommFn} \\
\id{CVJTIMES} & \id{CVJtimes}   & \id{CVSpgmrJacTimesVecFn} \\ \hline
\end{tabular}
\end{center}
...
\end{verbatim}
  ...
  \id{MU}, \id{ML} are the upper and lower half-bandwidths of the band matrix that 
  ...
  \id{IPRETYPE} is the preconditioner type: 1 for left only, 2 for right only, 
  3 for both sides.
  \id{IGSTYPE} is the Gram-Schmidt process type: 0 for modified G-S, 1 for classical G-S.
  \id{MAXL} is the maximum Krylov subspace dimension; 0 indicates default.
  \id{DELT} is the linear convergence tolerance factor; 0.0 indicates default.
  ...
  Optionally, to specify that {\spgmr} should use the supplied \id{CVJTIMES}, 
  make the call
  \index{FCVSPGMRSETJAC@\texttt{FCVSPGMRSETJAC}}
\begin{verbatim}
       CALL FCVSPGMRSETJAC(FLAG, IER)
...
\end{verbatim}
  ...
  \id{LENIPW} is the length of integer preconditioner work space, in \id{integertype} words.
  This size is local to the current processor.
  \id{NGE} is the number of $g(t,y)$ evaluations (calls to \id{CVLOCFN}) so far.
...
\item {\bf Problem reinitialization}
  ...
  by calling \id{FCVREINIT}, following which, a call to FCVBBDIN may or may not 
  be needed.
  ...
  \index{FCVREINBBD@\texttt{FCVREINBBD}}
\begin{verbatim}
       CALL FCVREINBBD(NLOCAL, MUDQ, MLDQ, DQRELY, IPRETYPE, 
      &                IGSTYPE, DELT, IER)
\end{verbatim}
  This reinitializes the SPGMR linear solver, but without reallocating its memory.  
  The arguments of the FCVREINBBD routine have the same names and meanings as FCVBBDIN.
...
\item {\bf Memory deallocation}
  ...
  To free the internal memory created by the call to \id{FCVBBDIN}, before calling
  \id{FCVFREE} and \id{FNVSPECFREEP}, the user must call
  \index{FCVBBDF@\texttt{FCVBBDF}}
\begin{verbatim}
      CALL FCVBBDF
\end{verbatim}
\index{FCVODE@{\fcvode} interface module!interface to the {\cvbbdpre} module|)}

...
\hline
Preconditioner functions and data & \id{CVSpbcgSetPreconditioner} & NULL, NULL, NULL \\
Jacobian-times-vector function and data & \id{CVSpbcgSetJacTimesVecFn} & internal DQ, NULL \\
...
\hline
Preconditioner functions and data & \id{CVSptfqmrSetPreconditioner} & NULL, NULL, NULL \\
Jacobian-times-vector function and data & \id{CVSptfqmrSetJacTimesVecFn} & internal DQ, NULL \\
...
}
{
  ...
  {\warn} It is illegal to call \id{CVodeSetTolerances} before a call to \id{CVodeMalloc}.
}
%%
%%
...
the user can also specify, in the call to \id{CVSpgmrSetJacFn}, a pointer to a 
...
\index{preconditioning!user-supplied|(}
If any type of preconditioning is to be done within the {\spbcg} method,
then the user must supply a preconditioner solve function \id{psolve}
and specify its name through a call to \id{CVSpbcgSetPreconditioner}.
...
A user-defined Jacobian-vector function must be of type \id{CVSpbcgJtimesFn} and 
can be specified through a call to \id{CVSpbcgSetJacTimesVecFn} 
(see \S\ref{ss:user_fct_sim} for specification details).
%%
As with the preconditioner user data structure \id{p\_data}, 
the user can also specify, in the call to \id{CVSpbcgSetJacFn}, a pointer to a 
user-defined data structure, \id{jac\_data}, which
the {\cvspbcg} solver passes to the Jacobian-times-vector function \id{jtimes}
each time it is called.  
The pointer \id{jac\_data} may be identical to \id{p\_data} and/or \id{f\_data}.
%%
%%
...
{
  The function \ID{CVSpbcgSetMaxl} specifies maximum of the Krylov subspace
...
\index{preconditioning!user-supplied|(}
If any type of preconditioning is to be done within the {\sptfqmr} method,
then the user must supply a preconditioner solve function \id{psolve}
and specify its name through a call to \id{CVSptfqmrSetPreconditioner}.
...
A user-defined Jacobian-vector function must be of type \id{CVSptfqmrJtimesFn} and 
can be specified through a call to \id{CVSptfqmrSetJacTimesVecFn} 
(see \S\ref{ss:user_fct_sim} for specification details).
%%
As with the preconditioner user data structure \id{p\_data}, 
the user can also specify, in the call to \id{CVSptfqmrSetJacFn}, a pointer to a 
user-defined data structure, \id{jac\_data}, which
the {\cvsptfqmr} solver passes to the Jacobian-times-vector function \id{jtimes}
each time it is called.  
The pointer \id{jac\_data} may be identical to \id{p\_data} and/or \id{f\_data}.
%%
%%
...
{
  The function \ID{CVSptfqmrSetMaxl} specifies maximum of the Krylov subspace
...
\begin{table}
\centering
\caption{Optional outputs from {\cvode}, {\cvdense}, {\cvband}, {\cvdiag},
...
}
{
  ...
  \begin{itemize}
  \item Base value: \id{lenrw} $= 89 +$ (\id{maxord+5})$* N$
  ...
  and \id{long int} words) is given by the following:
  \begin{itemize}
  \item Base value: \id{leniw} $=40$
  \item With rootfinding for $N_g$ functions: \id{leniw} $=$ \id{leniw}$+ N_g$ 
  \end{itemize}

  For the default value of \id{maxord}, the base values are:
  \begin{itemize}
  \item For the Adams method: \id{lenrw} $=89 + 17N$ and \id{leniw} $=40$ 
  \item For the BDF method: \id{lenrw} $=89 + 10N$ and \id{leniw} $=40$ 
  \end{itemize}
}
%%
%%
...
}
{
  ...
  In terms of the problem size $N$ and maximum subspace size \id{maxl}, 
  the actual size of the real workspace is $10*N$ \id{realtype} words.
...
{
  The value \id{nfevalsSB} is incremented only if the default 
...
}
{
  ...
  In terms of the problem size $N$ and maximum subspace size \id{maxl}, 
  the actual size of the real workspace is $13*N$ \id{realtype} words.
  (In a parallel setting, this value is global --- summed over all processors.)
}
%%
%%
...
{
  The value \id{nfevalsSQ} is incremented only if the default 
...
\item \label{i:bandpre_attach}
  ...
  \begin{itemize}
    \item Attaches the {\cvsp}* linear solver to the main {\cvode} solver memory;
...
\item
  ...
  or solve function through calls to {\cvsp}* optional input functions.
...
\item \label{i:bbdpre_init}
  ...
  arguments of \id{CVBBDPrecAlloc} are the two user-supplied functions described
  above.

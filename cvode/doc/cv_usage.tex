...
}
{
  ...
  If preconditioning is done, user-supplied functionss define left and right 
...
Jacobian times vector function & \id{CVSpgmrSetJacTimesVecFn} & NULL \\
...
}
{
  ...
  \item[itol] (\id{int}) 
    is either \Id{SS} or \Id{SV}, where \Id{itol}$=$\id{SS} indicates scalar relative error 
    tolerance and scalar absolute error tolerance, while \id{itol}$=$\id{SV} 
...
\item Interface to the main {\cvode} module
  ...
  \item \id{FCVODE}
    interfaces to \id{CVode}, \id{CVodeGet*} functions, and to the optional output functions 
...
All real variables must be declared as \id{REAL} or \id{DOUBLE PRECISION},
depending on whether {\cvode} was built in single or double precision 
(see \S\ref{s:install}). Moreover, some of the {\F} integer variables
must be declared as \id{INTEGER*4} or \id{INTEGER*8} according to the 
{\C} type \id{long int}. This integer variables are: the array
of integer optional inputs and outputs (\id{IOPT}), problem dimensions (\id{NEQ},
\id{NLOCAL}, \id{NGLOBAL}), and Jacobian bandwidths (\id{MU}, \id{ML},
...
\hline
{\bf Index} & {\bf I/O} & {\fcvode} {\bf routine} & {\cvode} {\bf function} \\ 
\hline

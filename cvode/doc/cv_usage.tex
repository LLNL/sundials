...
errors.  So \id{reltol} = 1.0E-4 means error is controlled to .01\%.  We do
not recommending using \id{reltol} larger than 1.0E-3, and \id{reltol} should
not be so small that is comparable to the unit roundoff of the machine
...
meaningless.  For example, if \id{y[i]} starts at about 1, but in time
...
then \id{abstol} should be a vector.  See the example \id{cvdx} in the CVODE
package, and the discussion of it in the CVODE Examples document.  In
...
The final (global) errors may be some sort of accumulation of those
...
positive or non-negative, though at times very small, but in the
numerical solution small negative (hence unphysical) values can then
occur.  In most cases, these values are harmless, and simply need to
be controlled, not eliminated. The following pieces of advice are
relevant.
...
avoiding a long explanation of them, if nothing else), then eliminate
them, but only in the context of the output medium.  The internal
values carried by the solver are unaffected.

(3) The user's right-hand side routine \id{f} should never change a
negative value to zero or small positive number, as a "solution" to
this problem.  This can cause instability.  If the \id{f} routine
cannot tolerate a zero or negative value (e.g. because there is a
square root or log of it), then the offending value should be changed
to zero or a tiny positive number in a temporary variable (not in the
input \id{y} vector) for the purposes of computing $f(t,y)$.
...
the user can also specify, in the call to \id{CVSpgmrSetJacTimesVecFn}, a
...
%%
As with the preconditioner user data structure \id{p\_data}, 
the user can also specify, in the call to \id{CVSpbcgSetJacTimesVecFn}, a
pointer to a user-defined data structure, \id{jac\_data}, which
...
%%
As with the preconditioner user data structure \id{p\_data}, 
the user can also specify, in the call to \id{CVSptfqmrSetJacTimesVecFn}, a
pointer to a user-defined data structure, \id{jac\_data}, which
...
\item \label{i:bandpre_init}
  ...
  (of type \id{void *}) to be passed to the appropriate {\cvsp} linear solver.
...
where \id{nlinsetups} is an optional {\cvode} output (see

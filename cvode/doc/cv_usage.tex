...
\item
  ...
  To set the vector \id{y0} of initial values, use macros defined by a
...
{
  In terms of the local vector dimension $N_{l}$, the actual size of the integer
  workspace is $N_l$ integer words.
}
%%
%%
...
{
  In terms of the local vector dimension $N_l$, the actual size of the real
  workspace is $N_l \,(2$ \id{mlkeep} $+$ \id{mukeep} $+$ \id{smu} $+2)$
  \id{realtype} words, where \id{smu} = $\min ( N_l - 1 ,$ \id{mukeep}
  $+$ \id{mlkeep}$)$.
}
%%
\index{memory requirements!CVBBDPRE@{\cvbbdpre} preconditioner|)}

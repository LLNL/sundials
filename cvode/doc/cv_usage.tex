...
preconditioner, etc. In one of the examples in \cite{cvode2.1_ex}, preconditioning
...
{
  The function \ID{CVodeCreate} instantiates a {\cvode} solver object and specifies 
  the solution method.
}
{
  \begin{args}[iter]
...
}
{
  ...
  If an error occured, \id{CVodeCreate} prints an error message to \id{stderr}
  and returns \id{NULL}.
}
{}
%%
...
{
  The function \ID{CVodeMalloc} provides required problem and solution specifications, 
...
}
{
  ...
  \item[f] (\Id{RhsFn})
    ...
    \id{f(t, y, ydot, f\_data)} (for full details see \S\ref{ss:rhsFn}).
  \item[t0] (\id{realtype})
...
}
{
  ...
  \item[\Id{CVM\_NO\_MEM}] 
    The {\cvode} memory block was not initialized through a previous call to \id{CVodeCreate}.
...
}
{
  ...
  \item[\ID{ROOT\_RETURN}]
    \id{CVode} succeeded and found one or more roots. 
    See \S\ref{s:using_rootfinding} for more information.
  \item[\ID{CVODE\_NO\_MEM}]
    The \id{cvode\_mem} argument was \id{NULL}.
  \item[\ID{CVODE\_NO\_MALLOC}]
    The {\cvode} memory was not allocated through a call to \id{CVodeMalloc}.
  \item[\ID{ILL\_INPUT}]
    ...
    where a component of the error weight vectors becomes negative during internal 
    ...
    \id{CVodeCreate}) failed to set one of the linear solver-related fields 
...
We note that, on error return, all these functions also print an error message to \id{stderr} 
...
{
  The function \ID{CVodeGetDky} computes the \id{k}-th derivative of the \id{y} function at      
...
Rootfinding information & \id{CvodeGetRootInfo} \\
No. of calls to the rootfinding function & \id{CVodeGetNumGEvals} \\
\hline
...
}
{
  ...
  \item[lenrw] (\id{long int *})
    number of \id{realtype} values in the {\cvode} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
{
  The function \ID{CVodeGetIntegratorStats} returns the {\cvode} integrator statistics
  as a group.
}
{
  \begin{args}[cvode\_mem]
...
While integrating the IVP, {\cvode} has the capability of finding the roots of a set 
of user-defined functions. This section describes the user-callable functions used
to initialize and define the root finding problem, as well as the required additional 
...
{
  The function \ID{CVodeRootInit} specifies thaqt roots of a set of
...
{
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[CVRT\_MEM\_FAIL]
  \item[SUCCESS]
    The call to \id{CVodeRootInit} was successfull.
  \item[CVRT\_NO\_MEM]
    The \id{cvode\_mem} argument was \id{NULL}.
  \item[CVRT\_MEM\_FAIL]
    A memory allocation failed.
  \end{args}
}
{
  If a new IVP is to be solved with a call to \id{CVodeReInit}, where the new
  IVP has no rootfinding problem but the prior one did, then call \id{CVodeRootInit}
  with \id{nrtfn}$=0$.
}
%%
%%
...
}
{
  ...
  \item[rootsfound] (\id{int **})
    pointer to an array of \id{int} of length \id{nrtfn}, showing the
    indeces of the user functions $g_i$ found to have a root. 
    For $i=0,\ldots,$\id{nrtfn}$-1$, \id{rootsfound}[$i$]$=1$ if $g_i$
...
{
  The function \ID{CVodeGetNumGEvals} returns the 
...
}
{
  ...
  \item[ngevals] (\id{long int *})
    number of calls to the user's \id{g} function.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
the user must supply a {\C} function of type \id{RootFn}, defined as follows:
%%
\usfunction{RootFn}
{
  typedef void (*RootFn)(realtype t, N\_Vector y, realtype *gout, void *g\_data);
}
{
...
{
  A \id{RootFn} function type does not have a return value.                        
}
{
...
\item
  ...
  \item \id{FNVINITS} (defined by {\nvecs}) 
    interfaces to \id{NV\_SpecInit\_Serial}.
  \item \id{FNVINITP} (defined by {\nvecp}) 
    interfaces to \id{NV\_SpecInit\_Parallel}.
  \item \id{FNVFREES} (defined by {\nvecs})
    interface to \id{NV\_SpecFree\_Serial}.
  \item \id{FNVFREEP}  (defined by {\nvecp})
    interfaces to \id{NV\_SpecFree\_Parallel}.
  \end{itemize}
\item Interface to the main {\cvode} module
  \begin{itemize}
  \item \id{FCVMALLOC}
    interfaces to \id{CVodeCreate}, \id{CVodeSet*} routines, and \id{CVodeMalloc}.
  \item \id{FCVREINIT}  
    interfaces to \id{CVodeReInit} and \id{CVodeSet*} routines.
  \item \id{FCVODE}
    interfaces to \id{CVode}, \id{CVodeGet*} routines, and to the optional output routines 
    for the selected linear solver module.
  \item \id{FCVDKY}     
    interfaces to the interpolated output routine \id{CVodeGetDky}.
  \item \id{FCVFREE}    
...
\item Interface to the linear solver modules
  ...
  \item \id{FCVBANDSETJAC}
    interfaces to \id{CVBandSetJac}.
  \item \id{FCVSPGMR}
    interfaces to \id{CVSpgmr} and {\spgmr} optional input routines.
...
\Id{SUNDIALS\_UNDERSCORE\_NONE} and \Id{SUNDIALS\_UNDERSCORE\_TWO} 
...
Note that \id{IOPT} must be declared as \id{INTEGER*4} or \id{INTEGER*8}
according to the {\C} type \id{long int}, while \id{ROPT} must be declared
as \id{REAL} or \id{DOUBLE PRECISION} depending on whether {\cvode} was
...
There is only one optional output routine in {\fcvode}. If one of the
user-provided Jacobian information routines uses difference quotient approximations,
it may need to access the array of error weights. This array can be obtained
by making the call:
\begin{verbatim}
      CALL FCVGETEWT (EWT, IER)
\end{verbatim}
where \id{EWT} is an array containing the error weights (which need not be 
allocated by the user) and \id{IER} is an error return flag which can be 
$0$ if \id{EWT} was successfully set and non-zero if an error occured.
...
%
22 & I & \id{FCVMALLOC}/\id{FCVREINIT} & \id{CVodeSetMaxErrTestFails} \\
%
23 & I & \id{FCVMALLOC}/\id{FCVREINIT} & \id{CVodeSetMaxNonlinIters} \\
%
24 & I & \id{FCVMALLOC}/\id{FCVREINIT} & \id{CVodeSetMaxConvFails} \\
%
25 & O & \id{FCVODE}                   & \id{CVodeGetNumGEvals} \\ 
%
\hline
...
%
8 & I & \id{FCVMALLOC}/\id{FCVREINIT} & \id{CVodeSetStopTime} \\
%
9 & I & \id{FCVMALLOC}/\id{FCVREINIT} & \id{CVodeSetNonlinConvCoef} \\
%
10 & O & \id{FCVMALLOC}                & unit roundoff \\
\hline
%
\end{tabular}
\end{table}                                                                  
...
\end{verbatim}
  ...
  \index{FNVINITP\@texttt{FNVINITP}}
\begin{verbatim}
...
\end{verbatim}
  ...
  to the {\diag} case are \id{LRW} and \id{LIW}, stored in \id{IOPT(16)} 
  and \id{IOPT(17)}, respectively.
...
\end{verbatim}
  ...
  If the user's \id{FCVDJAC} uses difference quotient approximations,
  it may need to access quantities not in the call list. 
  These include the current stepsize, the error weights, the unit roundoff, etc.
  To obtain these, use the \id{ropt} array of optional outputs.
  The array of error weights can be obtained by calling the optional output
  routine \id{FCVGETEWT} described before.
...
\end{verbatim}
  The arguments are: \id{MU}, the upper bandwidth; \id{ML}, 
...
\end{verbatim}
  ...
  \id{FY} contains $f(t,y)$. The vectors \id{WK1}, \id{WK2}, and \id{WK3}
  of length \id{NEQ} are provided as work space for use in \id{FCVBJAC}.
...
\end{verbatim}
  ...
  If the user's \id{FCVBJAC} uses difference quotient approximations,
  it may need to access quantities not in the call list. 
  These include the current stepsize, the error weights, the unit roundoff, etc.
  To obtain these, use the \id{ropt} array of optional outputs.
  The array of error weights can be obtained by calling the optional output
...
\end{verbatim}
  ...
  and store the product in \id{FJV}.  On return, set \id{IER=0} if \id{FCVJTIMES}
...
\end{verbatim}
  ...
  If the user's \id{FCVJTIMES} uses difference quotient approximations,
  it may need to access quantities not in the call list. 
  These include the current stepsize, the error weights, the unit roundoff, etc.
  To obtain these, use the \id{ropt} array of optional outputs.
  The array of error weights can be obtained by calling the optional output
  ...
  to \id{FCVSPGMR}, the user must call
...
\end{verbatim}
  ...
  or preprocessed, then, following the call to \id{FCVSPGMRSETPSOL}, the user must call
  \index{FCVSPGMRSETPSET@\texttt{FCVSPGMRSETPSET}}
\begin{verbatim}
...
\end{verbatim}
  ...
  The input argument \id{JOK} allows for Jacobian data to be saved and reused:  If 
  ...
  linear solver, then following the call to \id{FCVREINIT} (see below), a call to the 
...
\end{verbatim}
  ...
  \id{IER} is a completion flag and will be set to a positive value upon successfull return
  or to a negative value if an error occured. These values corrspond to the
  \id{CVode} returns (see \S\ref{sss:cvode}) as follows:
  $0$: \Id{SUCCESS}, $1$: \Id{TSTOP\_RETURN}, $2$: \Id{ROOT\_RETURN},
  $-1$: \Id{CVODE\_NO\_MEM}, $-2$: \Id{CVODE\_NO\_MALLOC}, $-3$: \Id{ILL\_INPUT},
  $-4$: \Id{TOO\_MUCH\_WORK}, $-5$: \Id{TOO\_MUCH\_ACC}, $-6$: \Id{ERR\_FAILURE},
  $-7$: \Id{CONV\_FAILURE}, $-8$: \Id{SETUP\_FAILURE}, and $-9$: \Id{SOLVE\_FAILURE}
  from \id{CVode} (see \S\ref{sss:cvode}) 
...
\item {\bf Memory deallocation}
  ...
  To free the internal memory created by the call to \id{FCVMALLOC} call
...
%%
The user-callable functions constituting the {\fcvroot}, with the corresponding
...
After calling \id{FCVMALLOC} but prior to calling \id{FCVODE}, the user must
allocate and initialize memory for the \id{FCVROOT} module by making the
following call:
\begin{verbatim}
...
\id{IER} is a return completion flag. Values are $0$ for success, $-1$ if the
\id{cvode} memory was \id{NULL} and $-2$ if a memory allocation failed.

The total number of calls made to the root function \id{FCVROOTFN}, \id{NGE},
can be obtained from \id{IOPT(25)}.
%%
If the {\fcvode}/{\cvode} memory block is reinitialized to solve a different
problem via a call to \id{FCVREINIT}, then the counter variable \id{NGE} is 
reset to zero.
...
\begin{verbatim}
      CALL FCVROOTFREE
\end{verbatim}
See \S\ref{s:using_rootfinding} for additional information.
\index{FCVODE@{\fcvode} interface module!rootfinding|)}
%%
%%------------------------------------------------------------------------------
...
\item \id{FCVBPSPGMR}
  interfaces to \id{CVBPSpgmr} and {\spgmr} optional input routines.
\item \id{FCVBPREINIT}
...
As with the rest of the {\fcvode} routines, the names of the user-supplied routines 
...
\end{verbatim}
  ...
  \id{MU} and \id{ML} are the upper and lower half-bandwidths of the band matrix that 
  is retained as an approximation of the Jacobian.
  \id{IER} is a return completion flag.  A value of $0$ inidicates success, while 
...
\item {\bf {\cvbbdpre} Optional outputs}
  ...
  Optional outputs specific to the {\spgmr} solver are \id{NPE}, \id{NLI}, \id{NPS}, 
  \id{NCFL}, \id{LRW}, and \id{LIW}, stored in \id{IOPT(16)} $\cdots$ \id{IOPT(21)}, 
  respectively.
...
\end{verbatim}
  ...
  \id{LENRPW} is the length of real preconditioner work space, in \id{realtype} words.
  \id{LENIPW} is the length of integer preconditioner work space, in integer words.
  \id{NFE} is the number of $f(t,y)$ evaluations (calls to \id{FCVFUN}) for
  finite difference banded Jacobian approximation.
  
\item {\bf Memory deallocation}

  To free the internal memory created by the call to \id{FCVBPINIT}, before calling
  \id{FCVFREE} and \id{FNVFREEP}, the user must call
...
\item \id{FCVBBDSPGMR}
  interfaces to \id{CVBBDSpgmr} and {\spgmr} optional input routines.
...
Additonally, based on the \Id{SUNDIALS\_UNDERSCORE\_NONE} and 
\Id{SUNDIALS\_UNDERSCORE\_TWO} parameters, the names of the user-supplied routines 
are mapped to actual values through a series of definitions in the header file 
\id{fcvbbd.h}.
...
\end{verbatim}
  ...
  \id{DQRELY} is the relative increment factor in $y$ for difference quotients (optional). 
  A value of $0.0$ indicates the default, $\sqrt{\text{unit roundoff}}$.
  \id{IER} is a return completion flag.  A value of $0$ inidicates success, a value of
...
\item {\bf Problem reinitialization}
  ...
  If there is a change in input arguments other than \id{MU}, \id{ML}, or \id{MAXL}, 
...
\end{verbatim}
  ...
  The arguments of the \id{FCVBBDREINIT} routine have the same names and meanings as 
  those of \id{FCVBBDINIT}.
  Finally, if the value of \id{MU} or \id{ML}, then a call to \id{FCVBBDINIT} must 
  be made. If \id{MAXL} is being changed, the a call to \id{FCVBBDSPGMR} must
  be made; in this case the {\spgmr} memory is reallocated.
  
\item {\bf Memory deallocation}

  To free the internal memory created by the call to \id{FCVBBDINIT}, before calling
  \id{FCVFREE} and \id{FNVFREEP}, the user must call

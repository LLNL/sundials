...
\subsection{A nonstiff example: \id{cvnonx\_p}}\label{ss:cvnonx_p}
...
The \id{cvnonx\_p.c} file begins with \id{\#include} lines, which include lines for
...
%%
\includeOutput{cvnonx\_p}{../examples_par/cvnonx_p.out}
%%

%-------------------------------------------------------------------------------

\subsection{A user preconditioner example: \id{cvkry\_p}}\label{ss:cvkry_p}
...
The organization of the \id{cvkry\_p} program deserves some comments. The
...
\id{uext} only.  Most other features of \id{cvkry\_p.c} are the same as in
\id{cvkryx.c}.

The following is a sample output from \id{cvkry\_p}, for four processors
(in a $2 \times 2$ array) with a $5 \times 5$ subgrid on each.
The output will vary slightly if the number of processors is changed.

%%
\includeOutput{cvkry\_p}{../examples_par/cvkry_p.out}
%%

%-------------------------------------------------------------------------------

\subsection{A CVBBDPRE preconditioner example: \id{cvkry\_bbd\_p}}\label{ss:cvkry_bbd_p}

In this example, \id{cvkry\_bbd\_p}, we solve the same problem in \id{cvkry\_p} above,
...
As in \id{cvkry\_p.c}, the \id{f} routine in \id{cvkry\_bbd\_p.c} simply calls a communication
...
In \id{cvkry\_bbd\_p.c}, the problem is solved twice --- first with preconditioning
...
Sample output from \id{cvkry\_bbd\_p} follows, again using $5 \times 5$ subgrids 
...
\id{nli/nni}, is very close to that of \id{cvkry\_p} when preconditioning is on
the left, but slightly poorer when it is on the right.

%%
\includeOutput{cvkry\_bbd\_p}{../examples_par/cvkry_bbd_p.out}
%%

...
The source file for this problem, \id{pvnx.c}, is listed in Appendix
...
The \id{pvnx.c} file begins with \id{\#include} lines, which include lines for
...
The \id{f} function is an implementation of Eq.(\ref{pvnxode}), but preceded
...
The \id{pvnx.c} file includes a routine \id{check\_flag} that checks the
return values from calls in \id{main}.  This routine was written to be used
by any parallel {\sundials} application.

The output below is for \id{pvnx} with \id{MX} = 10 and four processors. 
...
%%
\VerbatimInput[frame=single,framesep=0.1in,label={\tt pvnx} sample output,
  fontsize=\small]{../examples_par/pvnx.out}
%%
...
problem based on the system PDEs given above for the \id{cvkx} example.  
As before, we discretize the PDE system with central differencing, to
obtain an ODE system $\dot{u} = f(t,u)$ representing (\ref{e:cvkx:pde}).  
...
that in \id{pvnx}, and to provide a template for a stiff ODE system
...
\id{uext} only.  Most other features of \id{pvkx.c} are the same as in
\id{cvkx.c}.

The following is a sample output from \id{pvkx}, for four processors
(in a $2 \times 2$ array) with a $5 \times 5$ subgrid on each.
The output will vary slightly if the number of processors is changed.

%%
\VerbatimInput[frame=single,framesep=0.1in,label={\tt pvkx} sample output,
  fontsize=\small]{../examples_par/pvkx.out}
%%
...
This means that the Jacobian elements are computed with a differnce quotient
...
As in \id{pvkx.c}, the \id{f} routine in \id{pvkxb.c} simply calls a communication
routine, \id{fucomm}, and then a strictly computational routine, \id{flocal}.  
However, the call to \id{CVBBDPrecAlloc} specifies the pair of routines to
be called as \id{ucomm} and \id{flocal}, where \id{ucomm} is an {\em empty} routine.
This is because each call by the solver to \id{ucomm} is preceded by a call
to \id{f} with the same \id{(t,u)} arguments, and therefore the communication
needed for \id{flocal} in the solver's calls to it have already been done.

In \id{pvkxb.c}, the problem is solved twice --- first with preconditioning
...
as well as the preconditioner type (\id{CVSpgmrResetPrecType}).

Sample output from \id{pvkxb} follows, again using $5 \times 5$ subgrids 
...
\id{nli/nni}, is very close to that of \id{pvkx} when preconditioning is on
the left, but slightly poorer when it is on the right.

%%
\VerbatimInput[frame=single,framesep=0.1in,label={\tt pvkxb} sample output,
  fontsize=\small]{../examples_par/pvkxb.out}
%%

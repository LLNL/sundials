...
kinetics-advection-diffusion partial differential equations,
corresponding to a simplified ozone transport model.  The problem
includes nonlinear diurnal kinetics with 2 species, horizontal
advection and diffusion, and nonuniform vertical diffusion.  The
spatial domain is a rectangle, and the equations are discretized
spatially with standard central finite differences on a $10 \times 10$
mesh, giving an ODE system of size $200$.
...
tolerances.   It calls  \id{CVSpgmr} to specify the {\cvspgmr} linear solver
with left preconditioning, and the default value (indicated by a zero argument)
...
\S\ref{sss:lin_solv_init} and \S\ref{ss:optional_input} in the user
guide for details on these calls.
...
All of these optional outputs are described in \S\ref{ss:optional_output}.
...
%%
\VerbatimInput[frame=single,framesep=0.1in,label={\tt cvkx} sample output,
  fontsize=\small]{../examples_ser/cvkx.out}
%%

...
\id{check\_flag} function was written to be used for any {\sundials}
application.
...
The calls to \id{CVDense} and \id{CVDenseSetJacFn} specify the {\cvdense}
linear solver with an analytic Jacobian supplied by the user-supplied function
\id{Jac}.  See \S\ref{sss:lin_solv_init} and \S\ref{ss:optional_input}
...
a dense Jacobian.  It calls various \id{CVodeGet***} and \id{CVDenseGet***}
...
The example program \id{cvbx.c} solves the semi-discretized form
...
The \id{cvbx.c} program  (listed in \A\ref{s:cvbx_c}) includes files \id{cvband.h} and
...
coordinates $(i,j)$. The user-defined macro \id{IJth} isolates the
...
The \id{NV\_DATA\_S} macro returns the component array for a given
\id{N\_Vector}, and this array is passed to \id{IJth} in order to do
the actual \id{N\_Vector} access.

The type \id{UserData} is a pointer to a structure containing
problem data that is needed in the \id{f} and \id{Jac} functions.
This structure is allocated and initialized at the beginning
of \id{main}. The pointer to this structure, called \id{data},
is passed to both \id{CVodeSetFData} and \id{CVBandSetJacData},
and as a result it will be passed back to the \id{f} and \id{Jac}
functions each time they are called.  (If appropriate, two different
data structures could be defined and passed to \id{f} and \id{Jac}.)
The use of the \id{data} pointer eliminates the need for global
program data.
...
these values are passed.  The call to \id{CVBand} specifies the {\cvband}
linear solver, and specifies that both half-bandwidths of the Jacobian
are equal to \id{MY}.  The call to \id{CVBandSetJacFn} specifies
...
These optional outputs are described in \S\ref{ss:optional_output}.
Note that \id{PrintFinalStats} is suitable for general use in connection
...
See \S\ref{ss:bjacFn} for a detailed description of the banded \id{Jac}
function.

The output generated by \id{cvbx} is shown below.

%%
\VerbatimInput[frame=single,framesep=0.1in,label={\tt cvbx} sample output,
  fontsize=\small]{../examples_ser/cvbx.out}
%%
...
include \id{iterativ.h}, \id{cvspgmr.h}, and \id{sundialsmath.h}.
...
of \id{sundialsmath.h} is done to access the \id{SQR} macro for the
square of a \id{realtype} number.

The \id{main} function calls \id{CVodeCreate} specifying \id{CV\_BDF} and
\id{CV\_NEWTON}, and \id{CVodeMalloc} with scalar tolerances.  
%%
It calls \id{CVSpgmr} specifying the {\cvspgmr} linear solver with left 
preconditioning and default value (indicated by a zero argument) for 
\id{maxl}. The Gram-Schmidt orthogonalization is set to \id{MODIFIED\_GS}
through the function \id{CVSpgmrSetGSType} and
user-supplied preconditioner setup and solve functions, \id{Precond} and
\id{PSolve}, are specified through calls to \id{CVSpgmrSetPrecSetupFN}
and \id{CVSpgmrSetPrecSolveFn}, respectively.
The \id{data} pointer passed to \id{CVSpgmrSetPrecData} is passed to
\id{Precond} and \id{PSolve} whenever these are called. 
%%
...
function and backsolved in the \id{PSolve} function.  
...
functions work with the types \id{DenseMat} and \id{BandMat}, respectively.
The ``small''dense and band functions work with \id{realtype **} as the
...
\id{denaddI}, \id{denfree}, and \id{denfreepiv} are used.
The macro \id{IJth} defined near the top of the file is used to access 

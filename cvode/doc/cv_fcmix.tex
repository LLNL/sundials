%%==============================================================================
\chapter{FCVODE, an Interface Module for {\F} Applications}\label{s:fcmix}
%%==============================================================================
...
\item Interface to the linear solver modules
  ...
  \item \id{FCVSPBCGSETJAC}
    interfaces to \id{CVSpbcgSetJacTimesVecFn}.
  \item \id{FCVSPBCGSETPREC}
    interfaces to \id{CVSpbcgSetPreconditioner}.
  \item \id{FCVSPTFQMR}
  ...
  \item \id{FCVSPTFQMRSETJAC}
    interfaces to \id{CVSptfqmrSetJacTimesVecFn}.
  \item \id{FCVSPTFQMRSETPREC}
    interfaces to \id{CVSptfqmrSetPreconditioner}.
 \end{itemize}
...
\end{verbatim}
  in which the arguments are: \id{KEY} = 1, \id{NLOCAL} = the local size of
...
\begin{verbatim}
      SUBROUTINE FCVEWT (Y, EWT, IER, IPAR, RPAR)
...
\begin{verbatim}
      SUBROUTINE FCVDJAC (NEQ, T, Y, FY, DJAC, EWT, H, 
     &                    IPAR, RPAR, WK1, WK2, WK3)
      DIMENSION Y(*), FY(*), EWT(*), DJAC(NEQ,*), 
     &          IPAR(*), RPAR(*), WK1(*), WK2(*), WK3(*)
\end{verbatim}
...
\begin{verbatim}
     ...
     &                   EWT, H, IPAR, RPAR, WK1, WK2, WK3)
      DIMENSION Y(*), FY(*), EWT(*), BJAC(MDIM,*), 
     &                IPAR(*), RPAR(*), WK1(*), WK2(*), WK3(*)
\end{verbatim}
...
\end{verbatim}
  ...
  \id{FCVSPGMR}, the user must make the call:
...
\end{verbatim}
  ...
  If the optional function \id{FCVJTIMES} is provided, then, following the call to
  \id{FCVSPBCG}, the user must make the call:
  \index{FCVSPBCGSETJAC@\texttt{FCVSPBCGSETJAC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  If preconditioning is to be done (\id{IPRETYPE} $\neq 0$), then, following the
  call to \id{FCVSPBCG}, the user must call
  \index{FCVSPBCGSETPREC@\texttt{FCVSPBCGSETPREC}}
\begin{verbatim}
      CALL FCVSPBCGSETPREC(FLAG, IER)
\end{verbatim}
  with \id{FLAG} $\neq 0$, and the user program must include preconditioner routines
  \id{FCVPSOL} and \id{FCVPSET} (see below).
...
\end{verbatim}
  ...
  If the optional function \id{FCVJTIMES} is provided, then, following the call to
  \id{FCVSPTFQMR}, the user must make the call:
  \index{FCVSPTFQMRSETJAC@\texttt{FCVSPTFQMRSETJAC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  If preconditioning is to be done (\id{IPRETYPE} $\neq 0$), then, following the
  call to \id{FCVSPTFQMR}, the user must call
  \index{FCVSPTFQMRSETPREC@\texttt{FCVSPTFQMRSETPREC}}
\begin{verbatim}
...
\begin{verbatim}
      SUBROUTINE FCVJTIMES (V, FJV, T, Y, FY, EWT, H, IPAR, RPAR, WORK, IER)
...
\end{verbatim}
  ...
  {\warn} If the user calls \id{FCVSPGMRSETPREC}, \id{FCVSPBCGSETPREC}, or 
  \id{FCVSPTFQMRSETPREC}, the routine \id{FCVPSET} must
  be provided, even if it's empty.

  {\bf Note} 

  \begin{description}
  \item[] If the user's \id{FCVPSET} uses difference quotient approximations, 
    it may need to use the error weight array \id{EWT} and current stepsize \id{H}
    in the calculation of suitable increments and it may also need the unit
    roundoff. If \id{FCVPSOL} uses an iterative method in its solution, the residual 
    vector $\rho = r - Pz$ of the system should be made less than \id{DELTA} in weighted
    $\ell_2$ norm, i.e. $\sqrt{\sum(\rho_i * \id{EWT}[i])^2} < $ \id{DELTA}.
  \item[] If needed, the unit roundoff can be obtained as the optional output \id{ROUT(6)}
    (available after the call to \id{FCVMALLOC}) and can be passed to \id{FCVPSET}
    using either the \id{RPAR} user data array or a common block.
  \item[] If needed, the error weight array \id{EWT} can be obtained by calling
    \id{FCVGETERRWEIGHTS} using one of the work arrays as temporary storage for
    \id{EWT}.
  \end{description}
...
\end{verbatim}
  ...
  $-10$: \Id{CV\_LSOLVE\_FAIL} from \id{CVode}.
...
\end{verbatim}
  ...
  Optionally, to specify that {\spbcg} should use the supplied \id{FCVJTIMES}, 
  make the call
  \index{FCVSPBCGSETJAC@\texttt{FCVSPBCGSETJAC}}
\begin{verbatim}
       CALL FCVSPBCGSETJAC(FLAG, IER)
...
\end{verbatim}
  ...
  Optionally, to specify that {\sptfqmr} should use the supplied \id{FCVJTIMES}, 
  make the call
  \index{FCVSPTFQMRSETJAC@\texttt{FCVSPTFQMRSETJAC}}
\begin{verbatim}
       CALL FCVSPTFQMRSETJAC(FLAG, IER)
...
\end{verbatim}
  ...
  \id{FCVFUN}) for difference-quotient banded Jacobian approximation.
...
\end{verbatim}
  The arguments returned are as follows.
  \id{LENRPW} is the length of real preconditioner work space, in \id{realtype}
  words. \id{LENIPW} is the length of integer preconditioner work space, in integer
  words.  These sizes are local to the current processor.

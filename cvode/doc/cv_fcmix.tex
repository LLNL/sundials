...
\end{verbatim}
where 
\id{KEY} indicates which optoinal input is set (see Table \ref{t:fcvode_in}),
\id{IVAL} is the integer value to be used,
\id{RVAL} is the real value to be used, and
...
\end{verbatim}
  Its arguments are the same as thost of \id{FCVSPGMR}.
...
\end{verbatim}
  ...
  linear solver, then following the call to \id{FCVREINIT} (see below), a call
  to the \id{FCVSPBCG} routine may or may not be needed.  
  If there is a change in input arguments other than \id{MAXL}, then the user 
  program should call
\begin{verbatim}
...
\end{verbatim}
  ...
  The arguments of \id{FCVSPBCGREINIT} routine have the same names and meanings
  as those of \id{FCVSPBCG} routine.  Finally, if the value of \id{MAXL} is
  being changed, then a call to \id{FCVSPBCG} must be made.
...
\end{verbatim}
  ...
  linear solver, then following the call to \id{FCVREINIT} (see below), a call
  to the \id{FCVSPTFQMR} routine may or may not be needed.  
  If there is a change in input arguments other than \id{MAXL}, then the user 
  program should call
\begin{verbatim}
...
\end{verbatim}
  which reinitializes the {\spbcg} linear solver, but without reallocating its memory.
  The arguments of \id{FCVSPTFQMRREINIT} routine have the same names and meanings
  as those of \id{FCVSPTFQMR} routine.  Finally, if the value of \id{MAXL} is
  being changed, then a call to \id{FCVSPTFQMR} must be made.

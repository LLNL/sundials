...
\item Interface to the main {\cvode} module
  \begin{itemize}
  \item \id{FCVMALLOC}
    interfaces to \id{CVodeCreate} and \id{CVodeMalloc}.
  \item \id{FCVREINIT}  
...
etc.). This is particularly important when using
...
the {\cvbandpre} preconditioner module (see \S\ref{sss:cvbandpre}) and
...
the {\cvbbdpre} preconditioner module (see \S\ref{sss:cvbbdpre}) and
any of the Krylov iterative linear solvers.
...
\end{verbatim}
  ...
  words.  These size are local to the current processor.
...
\end{verbatim}
  ...
  changed, then a call to \id{FCVBBDSPGMR}, \id{FCVBBDSPBCG}, or
  \id{FCVBBDSPTFQMR} must be made; in this case
...
\end{verbatim}
  ...
  Jacobian-vector products, as described above in step \ref{i:fcv_lin_solv_spec}
  in \S\ref{ss:fcv_usage}.

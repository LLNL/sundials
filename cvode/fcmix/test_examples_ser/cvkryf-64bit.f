...
C
      ...
      INTEGER*8 IOPT(40)
      INTEGER*8 NEQ, MESHX, MESHZ, NST, NFE, NPSET, NPE, NPS, NNI
      INTEGER*8 NLI, NCFN, NCFL
...
C Routine to set problem constants and initial values
      IMPLICIT NONE
      INTEGER*8 MESHX, MESHZ
      INTEGER*8 MX, MZ, MM, JZ, JX
      DOUBLE PRECISION Y0
...
C Routine for right-hand side function f
      ...
      INTEGER*8 JX, JZ, MX, MZ, MM, IBLOK0, IBLOK, IDN, IUP
...
C Add identity matrix and do LU decompositions on blocks, in place.
      DO 40 IBLOK = 1,MM
        ...
        CALL DGEFA (P(1,1,IBLOK), TEMPINT1, TEMPINT2, IPP(1,IBLOK), IER)
        IF (IER .NE. 0) RETURN
 40     CONTINUE
C
      RETURN

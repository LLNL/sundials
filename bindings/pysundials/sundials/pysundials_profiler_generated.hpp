...
m.def("SUNProfiler_GetTimerResolution",
    ...
    {
        auto SUNProfiler_GetTimerResolution_adapt_modifiable_immutable_to_return = [](SUNProfiler p, double resolution) -> std::tuple<SUNErrCode, double>
        {
            double * resolution_adapt_modifiable = & resolution;

            SUNErrCode r = SUNProfiler_GetTimerResolution(p, resolution_adapt_modifiable);
            return std::make_tuple(r, resolution);
        };

        return SUNProfiler_GetTimerResolution_adapt_modifiable_immutable_to_return(p, resolution);
...
m.def("SUNProfiler_GetElapsedTime",
    ...
    {
        auto SUNProfiler_GetElapsedTime_adapt_modifiable_immutable_to_return = [](SUNProfiler p, const char * name, double time) -> std::tuple<SUNErrCode, double>
        {
        ...
        return SUNProfiler_GetElapsedTime_adapt_modifiable_immutable_to_return(p, name, time);

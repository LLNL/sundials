...
i.e. with components numbered from 1 instead of from 0.  The :literal:`Ith`
...
0. The :literal:`IJth` macro is used to access elements of a dense matrix of
...
Its arguments include the name of the function :math:`f` defining the
...
Specifically, these are: the cumulative number of steps (``nst``), 
the number of :math:`f` evaluations (``nfe``) (excluding those for
difference-quotient Jacobian evaluations),
the number of matrix factorizations (``nsetups``),
the number of :math:`f` evaluations for Jacobian evaluations (``nfeLS``
:math:`= 0` here),
...
values at the 12 preset values of :math:`t_{out}`.  It also shows the two root
...
linear solver data structures, :literal:`cvDiurnal_kry.c` uses underling small

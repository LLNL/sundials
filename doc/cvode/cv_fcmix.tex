...
\item Interface to the main {\cvode} module
  \begin{itemize}
  \item \id{FCVMALLOC}
    interfaces to \id{CVodeCreate}, \id{CVodeSetFdata}, and \id{CVodeMalloc}.
  \item \id{FCVREINIT}  
...
(see Chapter \ref{c:install}). Either way, the names into which the dummy names
...
of integer optional outputs (\id{IOUT}), problem dimensions (\id{NEQ},
\id{NLOCAL}, \id{NGLOBAL}), Jacobian half-bandwidths (\id{MU}, \id{ML},
etc.), as well as the array of user integer data, \id{IPAR}.
...
%%====================
  ...
  of the ODE system.  {\cvode} presently includes six choices for the treatment
...
\end{verbatim}
  ...
  call \id{FCVBAND} as described above.
...
\end{verbatim}
  ...
  preconditioner, make the following call:
  \index{FCVBPSPBCG@\texttt{FCVBPSPBCG}}
\begin{verbatim}
       CALL FCVBPSPBCG(IPRETYPE, MAXL, DELT, IER)
\end{verbatim}
  Its arguments are the same as those of \id{FCVSPBCG}
  (see step \ref{i:fcvode_lin_solv_spec} in \S\ref{ss:fcvode_usage}).
  

  To specify the {\spbcg} linear system solver and use the {\cvbandpre}
  preconditioner, make the following call:
  \index{FCVBPSPTFQMR@\texttt{FCVBPSPTFQMR}}
\begin{verbatim}
       CALL FCVBPSPTFQMR(IPRETYPE, MAXL, DELT, IER)
\end{verbatim}
  Its arguments are the same as those of \id{FCVSPTFQMR}
  (see step \ref{i:fcvode_lin_solv_spec} in \S\ref{ss:fcvode_usage}).
...
\item \id{FCVBBDSPBCG}
  interfaces to \id{CVBBDSpbcg} and {\spbcg} optional input functions.
\item \id{FCVBBDSPTFQMR}
  interfaces to \id{CVBBDSptfqmr} and {\sptfqmr} optional input functions.
...
\end{verbatim}
  ...
  preconditioner, make the following call:
  \index{FCVBBDSPBCG@\texttt{FCVBBDSPBCG}}
\begin{verbatim}
...
\end{verbatim}
  ...
  preconditioner, make the following call:
  \index{FCVBBDSPTFQMR@\texttt{FCVBBDSPTFQMR}}
\begin{verbatim}
...
\end{verbatim}
  ...
  linear solver inputs, then a call to \id{FCVBBDSPGMR}, \id{FCVBBDSPBCG}, or
  \id{FCVBBDSPTFQMR} must be made; in this case the linear solver memory is
  reallocated.

...
\item
  ...
  \item \id{FNVINITS\_OPENMP} (defined by {\nvecopenmp}) 
    interfaces to \id{N\_VNewEmpty\_OpenMP}.
  \item \id{FNVINITS\_PTHREADS} (defined by {\nvecpthreads}) 
...
\end{verbatim}
  ...
  following call:
  \index{FNVINITS\_OPENMP@\texttt{FNVINITS\_OPENMP}}
\begin{verbatim}
      CALL FNVINITS_OPENMP(KEY, NEQ, NUMTHREADS, IER)
\end{verbatim}
  where 
  \id{KEY} is the solver id (\id{KEY} = 1 for {\cvode}),
  ...
  following call:
  \index{FNVINITS\_PTHREADS@\texttt{FNVINITS\_PTHREADS}}
\begin{verbatim}
      CALL FNVINITS_PTHREADS(KEY, NEQ, NUMTHREADS, IER)
\end{verbatim}
  where 
  \id{KEY} is the solver id (\id{KEY} = 1 for {\cvode}),
  \id{NEQ} is the size of vectors,
  ...
  {\p} To initialize the MPI-parallel {\nvector} module {\nvecp}, the user must make the
  following call:

...
A Fortran 2003 module (\id{fcvode\_mod}) as well as a Fortran 77 style interface
(\fcvode) are provided to support the use of {\cvode}, for the solution of ODE
systems $dy/dt = f(t,y)$, in a mixed Fortran/{\CC} setting. While {\cvode} is
...
The \ID{fcvode\_mod} Fortran module defines interfaces to most {\cvode} {\CC}
...
Chapter \ref{s:simulation}. The Fortran 2003 {\cvode} interface module can be
accessed by the \id{use} statement, i.e. \id{use fcvode\_mod}, and linking to the
library \id{libsundials\_fcvode\_mod}.{\em lib} in addition to

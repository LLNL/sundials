...
In the instructions below, steps marked {\s} apply to the {\nvector}
module {\nvecs}, steps marked {\omp} apply to {\nvecopenmp}, steps
marked {\pt} apply to {\nvecpthreads}, while steps marked {\p} apply
to {\nvecp},
...
\end{verbatim}
  ...
  following call:
  \index{FNVINITOMP@\texttt{FNVINITOMP}}
\begin{verbatim}
      CALL FNVINITOMP(KEY, NEQ, NUMTHREADS, IER)
\end{verbatim}
  where 
  \id{KEY} is the solver id (\id{KEY} = 1 for {\cvode}),
  \id{NEQ} is the size of vectors,
  \id{NUMTHREADS} is the number of threads, and
  \id{IER} is a return completion flag which is $0$ on success and $-1$ 
  if a failure occurred.
  
  {\pt} To initialize the {\nvecpthreads} {\nvector} module, the user must make the
  following call:
  \index{FNVINITPTS@\texttt{FNVINITPTS}}
\begin{verbatim}
...
\end{verbatim}
  ...
  {\p} To initialize the distributed memory 
...
\end{verbatim}
  ...
  \id{KEY} = 1 for {\cvode}, \id{NLOCAL} = the local size of
...
%%====================
  ...
  of the ODE system.  The user of 
  {\fcvode} must call a routine with a specific name to make the desired
...
\end{verbatim}
  ...
  approximation of the system Jacobian $J = df_i(y)/dy$.  If supplied, 
...
\end{verbatim}
 ...
 >0 if a recoverable error occurred, or <0 if an unrecoverable error ocurred.)
...
\hline
\multicolumn{3}{|c|}{{\cvsparse} linear solvers}\\
\hline

...
\item
  ...
  \item \id{FSUNDIAGONALMATINIT} (defined by {\sunmatdiag}) 
    interfaces to \id{SUNDiagonalMatrix}.
...
\item
  ...
  \item \id{FSUNDIAGLINSOLINIT} (defined by {\sunlinsoldiag}) 
    interfaces to \id{SUNDiagonalLinearSolver}.
...
\item Interface to the linear solver interfaces
  \begin{itemize}
  \item \id{FCVDLSINIT}    
    interfaces to \id{CVDlsSetLinearSolver}.
  \item \id{FCVDENSESETJAC}
    interfaces to \id{CVDlsSetJacFn}.
  \item \id{FCVBANDSETJAC}
    interfaces to \id{CVDlsSetJacFn}.
  \item \id{FCVDIAGSETJAC}
    interfaces to \id{CVDlsSetJacFn}.
  \item \id{FCVSPARSESETJAC}
    interfaces to \id{CVDlsSetJacFn}.
  \item \id{FCVDLSSETMSBJ}
    interfaces to \id{CVDlsSetMSBJ}.
  \item \id{FCVDLSSETDGMAX}
    interfaces to \id{CVDlsSetDGMax}.
  \item \id{FCVSPILSINIT}
    interfaces to \id{CVSpilsSetLinearSolver}.
  \item \id{FCVSPILSSETEPSLIN}
    interfaces to \id{CVSpilsSetEpsLin}.
  \item \id{FCVSPILSSETJAC}
    interfaces to \id{CVSpilsSetJacTimes}.
...
\id{FCVDJAC}    & \id{FCVDenseJac}       & \id{CVDlsJacFn} \\
\id{FCVBJAC}    & \id{FCVBandJac}        & \id{CVDlsJacFn} \\
\id{FCVDIAGJAC} & \id{FCVDiagJac}        & \id{CVDlsJacFn} \\
\id{FCVSPJAC}   & \id{FCVSparseJac}      & \id{CVDlsJacFn} \\
...
\id{FCVJTSETUP} & \id{FCVJTSetup}        & \id{CVSpilsJacTimesSetupFn} \\
\end{tabular}
\end{center}
...
%%====================
  ...
  of the ODE system.  If using a Newton iteration with direct {\sunlinsol} linear solver
...
%%====================
  ...
  the {\cvdls} or {\cvspils} linear solver interface.

  %%-------------------------------------------------

  {\bf {\cvdls} direct linear solver interface}
  \index{CVDLS@{\cvdls} linear solver interface!use in {\fcvode}}
  
  To attach a direct {\sunlinsol} object and corresponding
  {\sunmatrix} object to the {\cvdls} interface, then following calls
  to initialize the {\sunlinsol} and {\sunmatrix} objects in steps
  \ref{i:fcv_matrix_init} and \ref{i:fcv_linsol_init} above, the user
  must make the call: 
  \index{FCVDLSINIT@\texttt{FCVDLSINIT}}
\begin{verbatim}
      CALL FCVDLSINIT(IER)
\end{verbatim}
  ...
  Optional outputs specific to the {\cvdls} case are listed in
  Table~\ref{t:fcvode_out}.

  {\bf {\cvdls} with dense Jacobian matrix}
  \index{Jacobian approximation function!dense!use in {\fcvode}}
  As an option when using the {\cvdls} interface with
...
\end{verbatim}
  ...
  If the \id{FCVDJAC} routine is provided, then, following the call to \id{FCVDLSINIT}, 
...
\end{verbatim}
  ...
  {\bf {\cvdls} with band Jacobian matrix}
...
\end{verbatim}
  ...
  If the \id{FCVBJAC} routine is provided, then, following the call to \id{FCVDLSINIT}, 
  the user must make the call:
...
\end{verbatim}
  ...
  {\bf {\cvdls} with sparse Jacobian matrix}
  \index{Jacobian approximation function!sparse!use in {\fcvode}}
  As an option when using the {\cvdls} interface with
  {\sunlinsolklu} or {\sunlinsolslumt} linear solvers, the user must
  supply the \id{FCVSPJAC} routine that computes a
...
\end{verbatim}
  It must load the \id{N} by \id{N} compressed sparse column [or
  compressed sparse row] matrix with storage for \id{NNZ} nonzeros,
  stored in the arrays \id{JDATA}, \id{JINDEXVALS} and
  \id{JINDEXPTRS}, with the Jacobian matrix at the current $(t, y)$ in
  CSC [or CSR] form (see \id{sunmatrix\_sparse.h} for more
  information).  The arguments are \id{T}, the current time; \id{Y}, an
  array containing state variables; \id{FY}, an array containing state
  derivatives; \id{N}, the number of matrix rows/columns in the
  Jacobian; \id{NNZ}, allocated length of nonzero storage; \id{JDATA},
  nonzero values in the Jacobian (of length \id{NNZ});
  \id{JINDEXVALS}, row [or column] indices for each nonzero in
  Jacobian (of length \id{NNZ}); \id{JINDEXPTRS}, pointers to each
  Jacobian column [or row] in the two preceding arrays (of length
  \id{N}+1); \id{H}, the current step size; \id{IPAR}, an array
  containing integer user data that was passed to \id{FCVMALLOC};
  \id{RPAR}, an array containing real user data that was passed to
  \id{FCVMALLOC}; \id{WK*}, work arrays containing temporary workspace 
  ...
  To indicate that the \id{FCVSPJAC} routine has been provided, then
  following the call to \id{FCVDLSINIT}, the following call must be made    
  \index{FCVSPARSESETJAC@\texttt{FCVSPARSESETJAC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  {\bf {\cvdls} with diagonal Jacobian matrix}
  \index{Jacobian approximation function!diagonal!use in {\fcvode}}
  As an option when using the {\cvdls} interface with the
  {\sunlinsoldiag} linear solver, the user may supply a routine that
  computes a diagonal approximation of the system Jacobian  $J =
  \partial f / \partial y$. If supplied, it must have the following
  form:  
  \index{FCVDIAGJAC@\texttt{FCVDIAGJAC}}
\begin{verbatim}
      SUBROUTINE FCVDIAGJAC (T, Y, FY, DJAC, H, IPAR, RPAR,
     &                       WK1, WK2, WK3, IER)
      DIMENSION Y(*), FY(*), DJAC(*), IPAR(*), RPAR(*),
     &          WK1(*), WK2(*), WK3(*)
\end{verbatim}
  Typically this routine will use only \id{T}, \id{Y}, and \id{DJAC}. 
  It must compute the diagonal of the Jacobian and store it in the
  array in \id{DJAC}.  The input arguments \id{T}, \id{Y}, and \id{FY}
  contain the current values of $t$, $y$, and $f(t,y)$, respectively.
  The arrays \id{IPAR} (of integers) and \id{RPAR} (of reals) contain user data
  and are the same as those passed to \id{FCVMALLOC}.
  The vectors \id{WK1}, \id{WK2}, and \id{WK3} are provided as work
  space for use in \id{FCVDIAGJAC}.   \id{IER} is an error return flag
  which should be set to $0$ if successful, a positive value if a
  recoverable error occurred (in which case {\cvode} will attempt to
  correct), or a negative value if \id{FCVDIAGJAC} failed
  unrecoverably (in which case the integration is halted). 

  The arrays \id{Y}, \id{DJAC}, \id{WK1}, \id{WK2} and \id{WK3} are
  commensurate with the size of the input \id{Y0} to \id{FCVMALLOC}.
  
  If the user's \id{FCVDIAGJAC} uses difference quotient approximations, it
  may need to use the error weight array \id{EWT} and current stepsize \id{H}
  in the calculation of suitable increments.  The array \id{EWT} can be
  obtained by calling \id{FCVGETERRWEIGHTS} using one of the work arrays as
  temporary storage for \id{EWT}.  It may also need the unit roundoff,
  ...
  If the \id{FCVDIAGJAC} routine is provided, then, following the call
  to \id{FCVDLSINIT}, the user must make the call:
  \index{FCVDIAGSETJAC@\texttt{FCVDIAGSETJAC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  {\bf {\cvspils} iterative linear solver interface}
  \index{CVSPILS@{\cvspils} linear solver interface!use in {\fcvode}}
  
  To attach an iterative {\sunlinsol} object to the {\cvdls}
  interface, then following the call to initialize the {\sunlinsol}
  object in step \ref{i:fcv_linsol_init} above, the user must make the
  call:
  \index{FCVSPILSINIT@\texttt{FCVSPILSINIT}}
\begin{verbatim}
...
\end{verbatim}
  ...
  Optional outputs specific to the {\cvspils} case are listed in
  Table~\ref{t:fcvode_out}.


  {\bf Functions used by {\cvspils}}

  Optional user-supplied routines \id{FCVJTIMES} and \id{FCVJTSETUP}
  (see below), can be provided for Jacobian-vector products.  If it
  is, then, following the call to \id{FCVSPILSINIT}, the user must
  make the call:
...
\end{verbatim}
 ...
 {\bf User-supplied routines for {\cvspils}}

  With treatment of the linear systems by any of the Krylov iterative
  solvers, there are four optional user-supplied routines ---
  \id{FCVJTIMES}, \id{FCVJTSETUP}, \id{FCVPSOL}, and \id{FCVPSET}.
  ...
  As an option when using the {\cvspils} linear solver interface, 
...
\end{verbatim}
  Typically this routine will use only \id{T}, \id{Y}, \id{V}, and
  ...
  The arrays \id{IPAR} (of integers) and \id{RPAR} (of reals) contain user data
  and are the same as those passed to \id{FCVMALLOC}.
  The vector \id{WORK}, of length commensurate with the input \id{Y0}
  to \id{FCVMALLOC}, is provided as work space for use in \id{FCVJTIMES}.
  ...
  of this data:
  \index{FCVJTSETUP@\texttt{FCVJTSETUP}}
\begin{verbatim}
      SUBROUTINE FCVJTSETUP (T, Y, FY, H, IPAR, RPAR, IER)
      DIMENSION Y(*), FY(*), IPAR(*), RPAR(*)
\end{verbatim}
  Typically this routine will use only \id{T} and \id{Y}.  It should
  compute any necessary data for subsequent calls to \id{FCVJTIMES}.  On return, set
  \id{IER = 0} if \id{FCVJTIMES} was successful, and nonzero otherwise.
  The arrays \id{IPAR} (of integers) and \id{RPAR} (of reals) contain user data
  and are the same as those passed to \id{FCVMALLOC}.

  {\warn}If the user calls \id{FCVSPILSSETJAC}, the routine \id{FCVJTSETUP} must
...
\end{verbatim}
  ...
  \item[(b) ] If needed in \id{FCVJTIMES}, \id{FCVJTSETUP},
    \id{FCVPSOL}, or \id{FCVPSET}, the error weight array \id{EWT} can
    be obtained by calling \id{FCVGETERRWEIGHTS} using a
    user-allocated array as temporary storage for \id{EWT}. 
  \item[(c) ] If needed in \id{FCVJTIMES}, \id{FCVJTSETUP},
    \id{FCVPSOL}, or \id{FCVPSET}, the unit roundoff can be obtained
    as the optional output \id{ROUT(6)} (available after the call to
    \id{FCVMALLOC}) and can be passed using either the \id{RPAR} user
    data array, a common block or a module.
  \end{itemize}
...
\end{verbatim}
  ...
  module and then attach it to the {\cvdls} or {\cvspils} interface,
  as shown above.  If linear solver parameters are being modified,
  then these calls may be made without re-attaching to the {\cvdls} or
  {\cvspils} interface. 
...
%%====================
  ...
  \id{FCVDLSINIT}/\id{FCVSPILSINIT}, \id{FNVINIT*} and
...
\hline
\multicolumn{3}{|c|}{{\cvdls} linear solver interface}\\
\hline
...
\hline
\multicolumn{3}{|c|}{{\cvspils} linear solver interface}\\
\hline
...
the {\cvspils} interface and any of the Krylov iterative linear solvers.
...
\item \textcolor{gray}{\bf {\sunmatrix} module initialization}
...
\item {\bf Linear solver interface specification}
  ...
  First, initialize the {\cvspils} iterative linear solver interface
  by calling one of \id{FCVSPILSINIT}.
...
\end{verbatim}
  ...
  Optionally, to specify that {\cvspils} should use the supplied
  \id{FCVJTIMES} and \id{FCVJTSETUP}, make the call 
  \index{FCVSPILSSETJAC@\texttt{FCVSPILSSETJAC}}
\begin{verbatim}
       CALL FCVSPILSSETJAC(FLAG, IER)
\end{verbatim}
...
\end{verbatim}
  ...
  linear solver inputs, then a call to one of \id{FSUNPCGINIT},
  \id{FSUNSPBCGSINIT}, \id{FSUNSPFGMRINIT}, \id{FSUNSPGMRINIT} or
  \id{FSUNSPTFQMRINIT}, followed by a call to \id{FCVSPILSINIT} must
...
\end{verbatim}
  ...
  \index{FCVJTIMES@\texttt{FCVJTIMES}}
  \index{FCVJTSETUP@\texttt{FCVJTSETUP}}
  Optionally, the user can supply routines \id{FCVJTIMES} and
  \id{FCVJTSETUP} for the evaluation of Jacobian-vector products, as

...
\item Interface to the linear solver modules
  ...
  \item \id{FCVSPARSESETJAC}
    interfaces to \id{CVSlsSetSparseJacFn}.
  \item \id{FCVSPGMR}
...
\end{verbatim}
  where \id{NTHREADS} is the number of threads requested and the other 
  arguments have the same meanings as for \id{FCVKLU}, except
...
\end{verbatim}
  ...
  $(t, y)$ in CSC [or CSR] form (see \id{sundials\_sparse.h} for more information).
 ...
 $>0$ if a recoverable error occurred, or $<0$ if an unrecoverable error occurred.)
  ...
  following either the call to \id{FCVKLU} or \id{FCVSUPERLUMT}, the

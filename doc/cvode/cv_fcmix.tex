%%==============================================================================
\chapter{Using CVODE for FORTRAN Applications}\label{s:cvfort}
%%==============================================================================

A {\F} 2003 module (\id{fcvode\_mod}) as well as a {\F} 77 style interface
(\fcvode) are provided to support the use of {\cvode}, for the solution of ODE
systems $dy/dt = f(t,y)$, in a mixed {\F}/{\CC} setting. While {\cvode} is
...
\section{CVODE FORTRAN 2003 Interface Module}\label{s:cvf2003}

The \ID{fcvode\_mod} {\F} module defines interfaces to most {\cvode} {\CC}
...
example, the {\cvode} function \id{CVodeCreate} is interfaced as
\id{FCVodeCreate}. Thus, the steps to use {\cvode} and the function calls in
{\F} 2003 are identical (ignoring language differences) to those in {\CC}. The
{\CC} functions with {\F} 2003 interfaces indicate this in their description in
Chapter \ref{s:simulation}.

The {\F} 2003 {\cvode} interface module can be accessed by the \id{use}
statement, i.e. \id{use fcvode\_mod}, and linking to the library
\id{libsundials\_fcvode\_mod}.{\em lib} in addition to
\id{libsundials\_cvode}.{\em lib}.
...
% FCVODE.
%%==============================================================================
\section{FCVODE, an Interface Module for FORTRAN Applications}\label{s:fcmix}

The {\fcvode} interface module is a package of {\CC} functions which support
the use of the {\cvode}. This package provides the necessary interface to
{\cvode} for all supplied serial and parallel {\nvector} implementations.
...
%%==============================================================================
\subsection{FCVODE routines}\label{sss:fcvroutines}
%%==============================================================================
...
\item Interface to the linear solver interfaces
  \begin{itemize}
  \item \id{FCVLSINIT}    
    interfaces to \id{CVodeSetLinearSolver}.
  \item \id{FCVLSSETEPSLIN}
    interfaces to \id{CVodeSetEpsLin}.
  \item \id{FCVLSSETJAC}
    interfaces to \id{CVodeSetJacTimes}.
  \item \id{FCVLSSETPREC}
    interfaces to \id{CVodeSetPreconditioner}.
  \item \id{FCVDENSESETJAC}
    interfaces to \id{CVodeSetJacFn}.
...
\item Interface to the nonlinear solver interface
  \begin{itemize}
  \item \id{FCVNLSINIT}
    interfaces to \id{CVSetNonlinearSolver}.
 \end{itemize}
...
%%====================
  ...
  create a {\sunnonlinsol} object by calling the appropriate {\F} interface
...
%%====================
  ...
  must make the call: 
  \index{FCVLSINIT@\texttt{FCVLSINIT}}
\begin{verbatim}
      CALL FCVLSINIT (IER)
\end{verbatim}
  \id{IER} is an error return flag set on $0$ on success, $-1$ if a memory 
  failure occurred, or $-2$ for an illegal input.

  The previous routines \Id{FCVDLSINIT} and \Id{FCVSPILSINIT} are now
...
\end{verbatim}
  ...
  If the \id{FCVDJAC} routine is provided, then, following the call to \id{FCVLSINIT}, 
...
\end{verbatim}
  ...
  If the \id{FCVBJAC} routine is provided, then, following the call to \id{FCVLSINIT}, 
  the user must make the call:
...
\end{verbatim}
  ...
  \noindent {\bf {\cvls} with sparse Jacobian matrix}
...
\end{verbatim}
  ...
  following the call to \id{FCVLSINIT}, the following call must be made    
...
\end{verbatim}
  ...
  \noindent {\bf {\cvls} with Jacobian-vector product}
...
\end{verbatim}
  ...
  \id{IER = 0} if \id{FCVJTSETUP} was successful, and nonzero otherwise.
  The arrays \id{IPAR} (of integers) and \id{RPAR} (of reals) contain user data
  and are the same as those passed to \id{FCVMALLOC}.

  To indicate that the \id{FCVJTIMES} and \id{FCVJTSETUP} routines
  have been provided, then following the call to \id{FCVLSINIT}, the
  following call must be made
  \index{FCVLSSETJAC@\texttt{FCVLSSETJAC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  The previous routine \Id{FCVSPILSETJAC} is now a wrapper for this
  ...
  {\warn}If the user calls \id{FCVLSSETJAC}, the routine
  \id{FCVJTSETUP} must be provided, even if it is not needed, and it
  must return \id{IER=0}.

  {\bf Notes} 
  \begin{itemize}
  \item[(a) ] If the user's \id{FCVJTIMES} routine uses difference
    ...
    suitable increments.  
  \item[(b) ] If needed in \id{FCVJTIMES} or \id{FCVJTSETUP}, the
    error weight array \id{EWT} can be obtained by calling
    \id{FCVGETERRWEIGHTS} using a user-allocated array as temporary
    storage for \id{EWT}.
  \item[(c) ] If needed in \id{FCVJTIMES} or \id{FCVJTSETUP}, the unit
    roundoff can be obtained as the optional output \id{ROUT(6)}
    (available after the call to \id{FCVMALLOC}) and can be passed
    using either the \id{RPAR} user data array, a common block or a
    module.
  \end{itemize}
  
 

  \noindent {\bf {\cvls} with preconditioning}
  \index{Preconditioner setup routine!use in {\fcvode}}
  \index{Preconditioner solve routine!use in {\fcvode}}
...
\end{verbatim}
  ...
  To indicate that the \id{FCVPSET} and \id{FCVPSOL} routines are
  supplied, then the user must call 
  \index{FCVLSSETPREC@\texttt{FCVLSSETPREC}}
\begin{verbatim}
...
\end{verbatim}
  ...
  preconditioner routines \id{FCVPSOL} and \id{FCVPSET}.

  The previous routine \Id{FCVSPILSETPREC} is now a wrapper for this
  routine, and may still be used for backward-compatibility.  However,
  this will be deprecated in future releases, so we recommend that
  users transition to the new routine name soon.
  
  {\warn}If the user calls \id{FCVLSSETPREC}, the routine \id{FCVPSET} must
  be provided, even if it is not needed, and it must return \id{IER=0}.

  {\bf Notes} 
  \begin{itemize}
  \item[(a) ] If the user's \id{FCVPSET} routine uses difference
    quotient approximations, it may need to use the error weight array
    \id{EWT}, the current stepsize \id{H}, and/or the unit roundoff,
    in the calculation of suitable increments.  Also, If \id{FCVPSOL}
    uses an iterative method in its solution, the residual vector
    $\rho = r - Pz$ of the system should be made less than \id{DELTA}
    in weighted $\ell_2$ norm, i.e. $\sqrt{\sum(\rho_i *
      \id{EWT}[i])^2} < $ \id{DELTA}. 
  \item[(b) ] If needed in \id{FCVPSOL} or \id{FCVPSET}, the error
    weight array \id{EWT} can be obtained by calling
    \id{FCVGETERRWEIGHTS} using a user-allocated array as temporary
    storage for \id{EWT}.
  \item[(c) ] If needed in \id{FCVPSOL} or \id{FCVPSET}, the unit
    roundoff can be obtained as the optional output \id{ROUT(6)}
    (available after the call to \id{FCVMALLOC}) and can be passed
    using either the \id{RPAR} user data array, a common block or a
    module.
  ...
  \index{CVDIAG@{\cvdiag} linear solver interface!use in {\fcvode}}
...
\end{verbatim}
  ...
  iterations) by calling appropriate {\F} interface routines (see Chapter
  \ref{c:sunnonlinsol}). 
...
%%====================
  ...
  \id{FCVLSINIT}, \id{FNVINIT*} and \id{FSUN***MATINIT}, make the call
...
%%==============================================================================
\subsection{FCVODE optional input and output}\label{fcv_opt_inout}
%%==============================================================================
...
three routines: \Id{FCVSETIIN} for integer optional inputs, \Id{FCVSETRIN}
for real optional inputs, and \Id{FCVSETVIN} for real vector (array) optional
...
\id{RVAL} is the real input value to be used,
\id{VVAL} is the real input array to be used, and
...
When using \id{FCVSETVIN} to specify optional constraints on the
solution vector (\id{KEY = 'CONSTR\_VEC'}) the components in the
array \id{VVAL} should be one of $-2.0$, $-1.0$, $0.0$, $1.0$, or $2.0$.
See the description of \id{CVodeSetConstraints} (\S\ref{sss:optin_main})
for details.
...
\hline
\multicolumn{3}{c}{}\\
\multicolumn{3}{c}{Real vector optional inputs (\id{FCVSETVIN})}\\
\hline
{\bf Key} & {\bf Optional Input} & Default value \\
\hline
...
\item {\bf Problem reinitialization}
  ...
  subsequent problems by calling \id{FCVREINIT}, following which a
  call to \id{FCVBBDINIT} may or may not be needed.  If the input
  arguments are the same, no \id{FCVBBDINIT} call is needed. If there
  is a change in input arguments other than \id{MU} or \id{ML}, then
...
\end{verbatim}
  ...
  \id{FSUNSPTFQMRINIT}, followed by a call to \id{FCVLSINIT} must

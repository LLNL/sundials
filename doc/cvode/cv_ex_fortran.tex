...
The source program \id{fcvDiurnal\_kry.f} is listed in Appendix \ref{s:fcvDiurnal_f}.
...
operations on the blocks.  The remainder of \id{fcvDiurnal\_kry.f} consists of
...
This example, \id{fcvDiurnal\_kry\_bbd\_p}, uses a simple diagonal ODE system to illustrate
...
The source file, \id{fcvDiurnal\_kry\_bbd\_p.f}, is listed in Appendix \ref{s:fcvDiurnal_bbd_p_f}.
...
The following is a sample output from \id{fcvDiurnal\_kry\_bbd\_p}, with \id{NPES} = 4.
As expected, the performance is identical for left vs right preconditioning.

%%
\includeOutput{fcvDiurnal\_kry\_bbd\_p}{../../examples/cvode/fcmix_parallel/fcvDiurnal_kry_bbd_p.out}
%%

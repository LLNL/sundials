...
implementations having type \id{SUNLINSOL\_DIRECT} (see Chapter
\ref{s:sunlinsol}).  These linear solvers utilize direct methods for
solution of linear systems stored using one of the {\sundials} generic
{\sunmatrix} implementations (dense, banded, diagonal or sparse; see
...
{\sunlinsol} implementations having type \id{SUNLINSOL\_ITERATIVE}
...
approximation of dense, banded or diagonal Jacobians through
difference quotients, but the user also has the option of supplying
the Jacobian (or an approximation to it) directly.  This user-supplied
...
Each {\cvode} linear solver interface consists of four routines,
...
the central {\cvode} module to each of the four associated functions is

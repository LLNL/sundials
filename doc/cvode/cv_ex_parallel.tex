...
The source file for this problem, \id{cvAdvDiff\_non\_p.c}, is listed in Appendix
\ref{s:cvAdvDiff_p_c}.  It uses the Adams (non-stiff) integration formula and
...
problem based on the system PDEs given above for the \id{cvDiurnal\_kry} example.  
...
The code listing for this example is given in Appendix \ref{s:cvDiurnal_p_c}.
The purpose of this code is to provide a more realistic example than
that in \id{cvAdvDiff\_non\_p}, and to provide a template for a stiff ODE system
...
\id{uext} only.  Most other features of \id{cvDiurnal\_kry\_p.c} are the same as in
\id{cvDiurnal\_kry.c}.
...
In this example, \id{cvDiurnal\_kry\_bbd\_p}, we solve the same problem in \id{cvDiurnal\_kry\_p}
...
Appendix \ref{s:cvDiurnal_bbd_p_c}. 

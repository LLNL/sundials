...
\item\label{i:lin_solver} 
  ...
  \id{ier = }\Id{CVDense}\id{(...);}

  \id{ier = }\Id{CVBand}\id{(...);}

  \id{ier = }\Id{CVLapackDense}\id{(...);}

  \id{ier = }\Id{CVLapackBand}\id{(...);}

  \id{ier = }\Id{CVKLU}\id{(...);}

  \id{ier = }\Id{CVSuperLUMT}\id{(...);}
...
}
{
  ...
  For the sake of uniformity, the argument \id{N} is of type \id{sunindextype},
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvode} memory block.
  \item[ngevalsBBDP] (\id{sunindextype})

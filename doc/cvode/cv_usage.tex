...
For the {\cvklu} solver, we provide a reinitialization.
...
appropriate \id{CV***} calls, as described in \S\ref{sss:lin_solv_init}
...
}
{
  ...
  To obtain these, the user will need to add the \id{cv\_mem} 
  structure to their \id{user\_data} and use the \id{CVodeGet*} 
...
}
{
  ...
  step size, the error weights, etc. 
  To obtain these, the user will need to add the \id{cv\_mem} 
  structure to their \id{user\_data} and use the \id{CVodeGet*} 
  functions described in
  \S\ref{sss:optout_main}. The unit roundoff can be

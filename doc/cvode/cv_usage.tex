...
{\cvsls} and {\cvbandpre}. Also, the preconditioner module {\cvbbdpre}
...
usage specifications are given for the vector implementations provided
...
In each case except the diagonal approximation case {\cvdiag} and the
externally-provided direct solvers (Lapack, KLU and SuperLU\_MT),
...
{
  flag = CVSuperLUMT(cvode\_mem, num\_threads, M, N, NNZ);
}
{
...
Sparse matrix ordering algorithm & \id{CVKLUSetOrdering} & \id{COLAMD} \\
Sparse matrix ordering algorithm & \id{CVSuperLUMTSetOrdering} & \id{COLAMD} \\
\hline
...
{\cvklu} and {\cvsuperlumt} solvers need a function to compute a
...
}
{
  ...
  \id{last\_flag} on \Id{CVSLS\_JACFUNC\_RECVR}), or a negative value
  if it failed unrecoverably (in which case the integration is halted,
  \id{CVode} returns \Id{CV\_LSETUP\_FAIL} and {\cvklu} or
  {\cvsuperlumt} sets \id{last\_flag} on \Id{CVSLS\_JACFUNC\_UNRECVR}).
}
{
  ...
  these, use the \id{CVodeGet*} functions described in

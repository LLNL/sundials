...
compatible with the dense, band or sparse {\sunmatrix} types, or with
the corresponding dense, band or sparse {\sunlinsol} modules.  Please
...
vector implementations.  Also, the preconditioner module {\cvbbdpre}
...
of the form \id{nvector/nvector\_***.h}.  See Chapter \ref{s:nvector}
...
%%
\begin{itemize}
\item \Id{cvode/cvode\_direct.h}, 
  which is used with the {\cvdls} direct linear solver interface;

\item \Id{cvode/cvode\_spils.h}, 
  which is used with the {\cvspils} iterative linear solver interface;
...
\item \Id{sunlinsol/sunlinsol\_diagonal.h},
  which is used with the banded linear solver module,
  {\sunlinsoldiag}; 
...
includes the header file which defines the {\sunmatdiag} matrix
module, \id{sunmatrix/sunmatrix\_diagonal.h}, as well as various
functions and macros acting on such matrices.
...
\item
  ...
  Note: The variables \id{N} and \id{Nlocal} should be of type \id{long int}.
...
\item
  ...
  \id{ ydata = N\_VGetArrayPointer\_***(y0)}.
  ...
  \id{N\_VGetArrayPointer\_***(...)} are not available for these vector wrappers.
...
NOTE: The dense, band and sparse matrix objects are usable only in a
...
\item
  ...
  See the documentation for each {\sunlinsol} module for details.
...
\item\label{i:lin_solver_interface} 
  ...
  \id{ier = }\Id{CVDlsSetLinearSolver}\id{(...);}

  \id{ier = }\Id{CVSpilsSetLinearSolver}\id{(...);}

\item
  {\bf Set linear solver interface optional inputs}

  Call \id{CVDlsSet*} or \id{CVSpilsSet*} functions to change optional
...
To specify a linear solver to {\cvode}, after the call to \id{CVodeCreate}
but before any calls to \id{CVode}, the user's program must create the
appropriate {\sunlinsol} object and call either of the functions
\Id{CVDlsSetLinearSolver} or \Id{CVSpilsSetLinearSolver}, as documented below. 
The first argument passed to these functions is the {\cvode}
memory pointer returned by \id{CVodeCreate}; the second argument
...
\index{CVODE@{\cvode} linear solver interfaces!CVDLS@{\cvdls}}
\index{CVDLS@{\cvdls} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{CVDlsSetLinearSolver}
{
  flag = CVDlsSetLinearSolver(cvode\_mem, LS, J);
}
{
  The function \ID{CVDlsSetLinearSolver} attaches a direct
  {\sunlinsol} object \id{LS} and corresponding template
  Jacobian {\sunmatrix} object \id{J} to {\cvode}, initializing the
  {\cvdls} direct linear solver interface.

  The user's main program must include the \id{cvode\_direct.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
  ...
  \item[LS] (\id{SUNLinearSolver})
    {\sunlinsol} object to use for solving Newton systems. 
  \item[J] (\id{SUNMatrix})
    {\sunmatrix} object for Jacobian template (must have type
    compatible with the linear solver object).
  \end{args}
}
{
  ...
  \item[\Id{CVDLS\_ILL\_INPUT}]
    The {\cvdls} solver is not compatible with the \id{LS} or
...
%%
%%
\index{CVODE@{\cvode} linear solver interfaces!CVSPILS@{\cvspils}}
\index{CVSPILS@{\cvspils} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{CVSpilsSetLinearSolver}
{
  flag = CVSpilsSetLinearSolver(cvode\_mem, LS);
}
{
  The function \ID{CVSpilsSetLinearSolver} attaches an iterative
  {\sunlinsol} object \id{LS} to {\cvode}, initializing the
  {\cvspils} scaled, preconditioned, iterative linear solver
  interface.

  The user's main program must include the \id{cvode\_spils.h} header
  file.
}
{
  \begin{args}[cvode\_mem]
...
}
{
  ...
  \item[\Id{CVSPILS\_ILL\_INPUT}]
    The {\cvspils} solver is not compatible with the \id{LS} object,
...
\index{CVDLS@{\cvdls} linear solver interface!optional input|(}
The \index{CVDLS@{\cvdls} linear solver interface!Jacobian approximation used by}
{\cvdls} solver interface needs a function to compute an approximation to
the Jacobian matrix $J(t,y)$.  This function must be of type \id{CVDlsJacFn}. 
The user can supply his/her own Jacobian function, or if using a
...
To specify a user-supplied Jacobian function \id{jac}, {\cvdls} provides 
the function \id{CVDlsSetJacFn}.
The {\cvdls} interface passes the pointer \id{user\_data} 
...
\ucfunction{CVDlsSetJacFn}
{
  flag = CVDlsSetJacFn(cvode\_mem, jac);
}
{
  The function \ID{CVDlsSetJacFn} specifies the Jacobian
...
}
{
  ...
  If \id{NULL} is passed to \id{jac}, this default function is used.

  The function type \id{CVDlsJacFn} is described in \S\ref{ss:jacFn}.
}

As described in Section \ref{ss:ivp_sol}, the
\index{CVDLS@{\cvdls} linear solver interface!Jacobian approximation update} 
...
the decision to reevaluate $J$ (and in turn, update the factorization
...
\index{Jacobian approximation update!MSBJ}
the function \id{CVDlsSetMSBJ}.   
%%
\index{Jacobian approximation update!MSBJ}
\ucfunction{CVDlsSetMSBJ}
{
  flag = CVDlsSetMSBJ(cvode\_mem, msbj);
}
{
  The function \ID{CVDlsSetMSBJ} specifies the maximum number of time
...
{
  If a negative value for \id{msbj} is supplied, the default value of
  50 will be used.
}
%%
Similarly, to update the threshold on $|\gamma/\bar{\gamma} - 1|$,
{\cvdls} provides
\index{Jacobian approximation update!DGMax}
the function \id{CVDlsSetDGMax}.
%%
\index{Jacobian approximation update!DGMax}
\ucfunction{CVDlsSetDGMax}
{
  flag = CVDlsSetDGMax(cvode\_mem, dgmax);
}
{
  The function \ID{CVDlsSetDGMax} specifies the threshold on the
...
{
  If a negative value for \id{dgmax} is supplied, the default value of
  0.2 will be used.
}
\index{optional input!direct linear solver interface|)}
\index{CVDLS@{\cvdls} linear solver interface!optional input|)}
...
\index{CVSPILS@{\cvspils} linear solver interface!optional input|(}
\index{preconditioning!user-supplied|(}
If any preconditioning is to be done within the {\cvspils} linear
solver interface, then the user must supply a preconditioner solve
function \id{psolve} and specify its name in a call to
\id{CVSpilsSetPreconditioner}. 
\index{CVSPILS@{\cvspils} linear solver interface!preconditioner solve function}
%%
...
\id{CVSpilsSetPreconditioner}.
\index{CVSPILS@{\cvspils} linear solver interface!preconditioner setup function}
...
\index{CVSPILS@{\cvspils} linear solver interface!Jacobian approximation used by}
Ther {\cvspils} solver interface requires a function to compute an
approximation to the product between the Jacobian matrix $J(t,y)$ and
a vector $v$. The user can supply his/her own Jacobian-times-vector
...
can be specified through a call to \id{CVSpilsSetJacTimes}.  (see
\S\ref{ss:jtimesFn} for specification details) 
%%
As with the preconditioner user-supplied functions, the evaluation and
...
function \id{jtsetup}.  (\S\ref{ss:jtsetupFn} for specification
details).  Also as with preconditioning, a pointer to the user-defined
data structure, \id{user\_data}, specified through
\id{CVodeSetUserData} (or a \id{NULL} pointer otherwise) is passed to
...
\ucfunction{CVSpilsSetJacTimes}
{
  flag = CVSpilsSetJacTimes(cvode\_mem, jtsetup, jtimes);
}
{
  The function \ID{CVSpilsSetJacTimes} specifies the Jacobian-vector 
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvode} memory block.
  \item[jtsetup] (\id{CVSpilsJacTimesSetupFn})
...
}
{
  ...
  quotient function.  If \id{NULL} is passed to \id{jtimes}, this
  default function is used. 

  The function type \id{CVSpilsJacTimesSetupFn} is described in \S\ref{ss:jtsetupFn}.
...
\index{CVDLS@{\cvdls} linear solver interface!optional output|(}
...
%%
%%
\index{CVDLS@{\cvdls} linear solver interface!memory requirements}
\index{memory requirements!CVDLS@{\cvdls} linear solver interface}
...
}
{
  ...
  internal difference quotient functions (dense, band, or diagonal) is
  used.
}
%%
%%
...
}
{
  ...
  (\id{CVode} returned \id{CV\_LSETUP\_FAIL}), then the value of
...
\index{CVSPILS@{\cvspils} linear solver interface!optional output|(} 
...
%%
%%
\index{CVSPILS@{\cvspils} linear solver interface!memory requirements} 
\index{memory requirements!CVSPILS@{\cvspils} linear solver interface}
...
%%
%%
\ucfunction{CVSpilsGetNumJTSetupEvals}
{
  flag = CVSpilsGetNumJTSetupEvals(cvode\_mem, \&njtsetup);
}
{
  The function \ID{CVSpilsGetNumJTSetupEvals} returns the
...
}
{
  ...
  \id{CV\_LSETUP\_FAIL}), \id{lsflag} will be \id{SUNLS\_PSET\_FAIL\_UNREC},
  \id{SUNLS\_ASET\_FAIL\_UNREC}, or \id{SUNLS\_PACKAGE\_FAIL\_UNREC}.

  If the {\cvspils} solve function failed (\id{CVode} returned
...
\Id{CVDlsSetLinearSolver} is called in Step
\ref{i:lin_solver_interface} of \S\ref{ss:skeleton_sim}), the user may
provide a function of type \Id{CVDlsJacFn} defined by: 
%%
\usfunction{CVDlsJacFn}
{
...
}
{
  ...
  \item[tmp3]
    ...
    \id{CVDlsJacFn} as temporary storage or work space.    
  \end{args}
}
{
  A \id{CVDlsJacFn} should return 0 if successful, a positive value if a recoverable
  ...
  returns \Id{CV\_LSETUP\_FAIL} and {\cvdls} sets \id{last\_flag} to 
...
}
{
  ...
  If the user's \id{CVJacFn} function uses difference quotient
  ...
  element of the dense matrix \id{Jac} (\id{i}, \id{j }$= 0\ldots
  N-1$). This macro is meant for small problems for which efficiency
  ...
  (\id{j }$= 0\ldots N-1$), and the elements of the \id{j}-th column
  ...
  of access is not a major concern.  Thus, in terms of the indices $m$
  and $n$ ranging from $1$ to $N$ with $(m,n)$ within the band defined
  ...
  {\bf diagonal}:\\
  A user-supplied diagonal Jacobian function must load the \id{N} by
  \id{N} diagonal matrix \id{Jac} with an approximation to the
  Jacobian matrix $J(t,y)$ at the point (\id{t}, \id{y}).  The
  accessor macro \Id{SM\_DATA\_DIAG} allows the user to access the
  underlying {\nvector} that holds the diagonal of \id{Jac}.  The user
...
(\id{CVSpilsSetLinearSolver} is called in step
\ref{i:lin_solver_interface} of \S\ref{ss:skeleton_sim}), the user may
provide a function of type \Id{CVSpilsJacTimesVecFn} in the following form,
...
user-supplied {\CC} function of type \Id{CVSpilsJacTimesSetupFn}, defined as follows:

\usfunction{CVSpilsJacTimesSetupFn}
{
...
{  
  The value to be returned by the Jacobian-vector setup function
  ...
  unrecoverable error (in which case the integration is halted). } 
{
  ...
  If the user's \id{CVSpilsJacTimesSetupFn} function uses difference quotient
  approximations, it may need to access quantities not in the argument
  list. These include the current step size, the error weights, etc.
  To obtain these, the user will need to add a pointer to \id{cv\_mem} 
  to \id{user\_data} and then use the \id{CVodeGet*} functions described in
...
\item
  ...
  or solve function through calls to the \id{CVSpilsSetPreconditioner}
...
}
{
  ...
  corresponding function \id{CVSpilsGetWorkSpace}.
}
%%
%%
...
returned by the corresponding function \id{CVSpilsGetNumRhsEvals}, and
...
one or more of the corresponding \id{CVSpilsSet***} functions, must

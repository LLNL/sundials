...
\item  \Id{cvode/cvode.h},
  ...
  includes the header file for {\cvls}, \Id{cvode/cvode\_ls.h}.
\end{itemize}
%%
...
linear solver modules available for use with {\cvode} are:
%%
\begin{itemize}
...
\item
  {\bf Specify rootfinding problem} (\textit{optional})
  \index{Rootfinding}

  Call \id{CVodeRootInit} to initialize a rootfinding problem
...
\item
  ...
  Call \Id{CVodeFree}\id{(\&cvode\_mem)} to free the memory allocated by {\cvode}.
...
\id{CVodeCreate} but before any calls to \id{CVode}, the user's
...
it to {\cvode} via a call to \Id{CVodeSetLinearSolver}.  The first
...
\index{CVODE@{\cvode} linear solver interfaces!selecting one|)}

\index{CVODE@{\cvode} linear solver interfaces!CVLS@{\cvls}}
...
}
{
  ...
  object \id{J} (if applicable) to {\cvode}, initializing the
...
%%
%%
\index{CVODE@{\cvode} linear solver interfaces!CVDIAG@{\cvdiag}}
...
When changing the nonlinear solver in {\cvode}, \id{CVodeSetNonlinearSolver} must
be called after \id{CVodeInit}. If any calls to \id{CVode} have been made, then
{\cvode} will need to be reinitialized by calling \id{CVodeReInit} to ensure
...
{\cvode} memory pointer returned by \id{CVodeCreate} and the second argument is
...
%%==============================================================================
\subsection{Projection initialization function}\label{ss:cvprojinit}
\index{Projection}

When solving an IVP with a constraint equation, {\cvode} has the capability to
project the solution onto the constraint manifold after each time step. To
activate the projection capability with a user-defined projection function, call
the following set function:
%%
%%
\ucfunctionf{CVodeSetProjFn}
{
  flag = CVodeSetProjFn(cvode\_mem, proj);
}
{
  The function \ID{CVodeSetProjFn} enables or disables projection with a
  user-defined projection function.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block returned by \id{CVodeCreate}.
  \item[proj] (\id{CVProjFn})
    is the {\CC} function which defines the projection. See \S\ref{ss:projFn}
    for details.
  \end{args}
}
{
  ...
  \begin{args}[CV\_ILL\_INPUT]
  \item[CV\_SUCCESS]
    The call to \id{CVodeSetProjFn} was successful.
  \item[CV\_MEM\_NULL]
    The \id{cvode\_mem} argument was \id{NULL}.
  \item[CV\_MEM\_FAIL]
    A memory allocation failed.
  \item[CV\_ILL\_INPUT]
    The projection function is \id{NULL} or the method type is not \id{CV\_BDF}.
  \end{args}
}
{
  At this time projection is only supported with BDF methods.

  If a new IVP is to be solved with a call to \id{CVodeReInit}, where the new
  IVP does not have a constraint equation but the prior one did, then call
  \id{CVodeSetProjFrequency} with an input of \id{0} to disable projection.
}
...
Flag to activate specialized fused kernels & \id{CVodeSetUseIntegratorFusedKernels} & {\tt SUNFALSE} \\
\hline
...
%%
%%
\ucfunctionf{CVodeSetMonitorFn}
{
  flag = CVodeSetMonitorFn(cvode\_mem, monitorfn);
}
{
  The function \ID{CVodeSetMonitorFn} specifies a user function,
  \id{monitorfn}, to be called at some interval of successfully
  completed {\cvode} time steps.
}
{
  \begin{args}[cvode\_data]
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvode} memory block.
  \item[monitorfn] (\id{CVMonitorFn})
    user-supplied monitor function (\id{NULL} by default); a \id{NULL}
...
{
  The frequency with which the monitor function is called can be
  set with the function \id{CVodeSetMonitorFrequency}.

  {\warn}Modifying the solution in this function will result in
  undefined behavior. This function is only intended to be used
  for monitoring the integrator.
...
%%
%%
\ucfunctionf{CVodeSetMonitorFrequency}
{
  flag = CVodeSetMonitorFrequency(cvode\_mem, nst);
}
{
  The function \ID{CVodeSetMonitorFrequency} specifies the
  interval, measured in successfully completed {\cvode} time-steps,
  at which the monitor function should be called.
}
{
  \begin{args}[cvode\_data]
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvode} memory block.
  \item[nst] (\id{long int})
    number of successful steps inbetween calls to the monitor function (0 by default);
    a 0 input will turn off monitoring
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
%%
\ucfunction{CVodeSetUseIntegratorFusedKernels}
{
  flag = CVodeSetUseIntegratorFusedKernels(cvode\_mem, onoff);
}
{
  The function \ID{CVodeSetUseIntegratorFusedKernels} informs {\cvode} that it should
  use specialized fused kernels internally, if available. The specialized
  kernels may offer performance improvements for small problem sizes. Users
  should beware that these kernels can cause changes in the behavior of the
  integrator. \textit{By default, these kernels are not used}.
  Must be called after \ID{CVodeInit}.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
{
  \textbf{{\sundials} must be compiled appropriately for specialized kernels to be available}.
  The CMake option \id{SUNDIALS\_BUILD\_PACKAGE\_FUSED\_KERNELS} must be set to
  \id{ON} when {\sundials} is compiled. See the entry for this option in
  section~\ref{ss:configuration_options_nix} for more requirements.
  Currently, the fused kernels are only supported when using {\cvode}
  with the {\nveccuda} (see \ref{ss:nvec_cuda}) or {\nvechip}
  (see \ref{ss:nvec_hip}) modules.
}
\index{optional input!solver|)}
...
{\cvode} provides user-callable routines to modify this behavior.  Recall
...
\id{psolve}, that are supplied to {\cvode} using the function
...
}
{
  ...
  ({\cvode} v5.0.0) the value of \id{nrmfac} was computed using the vector
...
%%==============================================================================
\subsubsection{Projection optional input functions}
\label{sss:optin_proj}
\index{optional input!projection|(}
%%==============================================================================
%%
The following functions can be called to set optional inputs to control the
projection when solving an IVP with constraints.
%%
%%
\ucfunctionf{CVodeSetProjErrEst}
{
  flag = CVodeSetProjErrEst(cvode\_mem, onoff);
}
{
  The function \ID{CVodeSetProjErrEst} enables or disables projection of
  the error estimate by the projection function.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block.
  \item[onoff] (\id{booleantype})
    is a flag indicating if error projection should be enabled (\id{SUNTRUE},
    default) or disabled (\id{SUNFALSE}).
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[CV\_PROJ\_MEM\_NULL]
  \item[\Id{CV\_SUCCESS}]
    The optional value has been successfully set.
  \item[\Id{CV\_MEM\_NULL}]
    The \id{cvode\_mem} pointer is \id{NULL}.
  \item[\Id{CV\_PROJ\_MEM\_NULL}]
    The projection memory is \id{NULL} i.e., the projection functionality has
    not been enabled.
  \end{args}
}
{}
%%
%%
\ucfunctionf{CVodeSetProjFrequency}
{
  flag = CVodeSetProjFrequency(cvode\_mem, freq);
}
{
  The function \ID{CVodeSetProjFrequency} specifies the frequency with which the
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block.
  \item[freq] (\id{long int})
    is the frequency with which to perform the projection. The default is 1
    (project every step), a value of 0 will disable projection, and a
    value $< 0$ will restore the default.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
%%
%%
\ucfunctionf{CVodeSetMaxNumProjFails}
{
  flag = CVodeSetMaxNumProjFails(cvode\_mem, max\_fails);
}
{
  The function \ID{CVodeSetMaxNumProjFails} specifies the maximum number of
  projection failures in a step attempt before an unrecoverable error is
  returned.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block.
  \item[max\_fails] (\id{int})
    is the maximum number of projection failures. The default is 10 and an
    input value $< 1$ will restore the default.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
%%
%%
\ucfunctionf{CVodeSetEpsProj}
{
  flag = CVodeSetEpsProj(cvode\_mem, eps);
}
{
  The function \ID{CVodeSetEpsProj} specifies the tolerance for the nonlinear
  constrained least squares problem solved by the projection function.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block.
  \item[eps] (\id{realtype})
    is the tolerance (default 0.1) for the the nonlinear constrained least
    squares problem solved by the projection function. A value $\leq 0$ will
    restore the default.
  \end{args}
}
{
...
%%
%%
\ucfunctionf{CVodeSetProjFailEta}
{
  flag = CVodeSetProjFailEta(cvode\_mem, eta);
}
{
  The function \ID{CVodeSetProjFailEta} specifies the time step reduction factor
  to apply on a projection function failure.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    is a pointer to the {\cvode} memory block.
  \item[eps] (\id{realtype})
    is the time step reduction factor to apply on a projection function failure
    (default 0.25). A value $\leq 0$ or $> 1$ will restore the default.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
Get all linear solver statistics in one function call & \id{CVodeGetLinSolvStats} \\
...
\index{optional output!rootfinding|(}
%%
...
%%==============================================================================
\subsubsection{Projection optional output functions}
\label{sss:optout_proj}
\index{optional output!projection|(}
%%==============================================================================
%%
The following optional output functions are available for retrieving information
and statistics related the projection when solving and IVP with constraints.
%%
%%
\ucfunctionf{CVodeGetNumProjEvals}
{
  flag = CVodeGetNumProjEvals(cvode\_mem, nproj);
}
{
  The function \ID{CVodeGetNumProjEvals} returns the current total number of
...
%%
%%
\ucfunctionf{CVodeGetNumProjFails}
{
  flag = CVodeGetNumProjFails(cvode\_mem, npfails);
}
{
  The function \ID{CVodeGetNumProjFails} returns the current total number of
  projection evaluation failures.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvode} memory block.
  \item[npfails] (\id{long int *})
...
%%
%%
\ucfunctionf{CVodeGetLinSolvStats}
{
  flag = CVodeGetLinSolvStats(cvode\_mem,
                              \&njevals, \&nfevalsLS,
                              \&nliters, \&nlcfails,
                              \&npevals, \&npsolves,
                              \&njtsetups, \&njtimes);
...
{
  The function \ID{CVodeGetLinSolvStats} returns {\cvode} linear solver
  statsitics.
}
{
  \begin{args}[cvode\_mem]
    \item[\id{cvode\_mem}] - (\id{void *}) the \id{CVodeMem} structure
    \item[\id{njevals}] - (\id{long int*}) the current number of calls to the
      Jacobian function
    \item[\id{nfevalsLS}] - (\id{long int*}) the current number of calls made to
      the user-supplied right-hand side function by the linear solver
    \item[\id{nliters}] - (\id{long int*}) the current number of linear iterations
    \item[\id{nlcfails}] - (\id{long int*}) the current number of linear
      convergence failures
    \item[\id{npevals}] - (\id{long int*}) the current number of calls to \id{psetup}
    \item[\id{npsolves}] - (\id{long int*}) the current number of calls to \id{psolve}
    \item[\id{njtsetup}] - (\id{long int*}) the current number of calls to \id{jtsetup}
    \item[\id{njtimes}] - (\id{long int*}) the current number of calls to \id{jtimes}
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
%%==============================================================================
\index{monitor}
A user may provide a function of type \Id{CVMonitorFn} to monitor the
integrator progress throughout a simulation. For example, a user may
want to check integrator statistics as a simulation progresses.
\usfunction{CVMonitorFn}
{
  typedef void (*CVMonitorFn)(void* cvode\_mem, void* user\_data);
}
{
  This function is used to monitor the {\cvode} integrator
  throughout a simulation.
}
{
  \begin{args}[user\_data]
  \item[cvode\_mem]
    the {\cvode} memory block
  \item[user\_data]
...
{
  A \id{CVMonitorFn} should return 0 if successful, or a negative value
  if unsuccessful.
}
{
  {\warn} This function should only be utilized for monitoring the integrator.
}
...
%%==============================================================================
\subsection{Projection function}\label{ss:projFn}
%%
When solving an IVP with a constraint equation and providing a user-defined
projection operation the projection function must have type \Id{CVProjFn},
defined as follows:
%%
\usfunction{CVProjFn}
{
  typedef int (*CVProjFn)(realtype t, N\_Vector ycur, N\_Vector corr,\\
                          realtype epsProj, N\_Vector err, void *user\_data);
}
{
  This function computes the projection of the solution and, if enabled, the
  error on to the constraint manifold.
}
{
  \begin{args}[user\_data]
  \item[t]
  ...
  \item[ycur]
    is the current value of the dependent variable vector, $y(t)$.
  \item[corr]
    is the correction, $c$, to the dependent variable vector so that $y(t) + c$
    satisfies the constraint equation.
  \item[epsProj]
    is the tolerance to use in the nonlinear solver stopping test when solving
    the nonlinear constrainted least squares problem.
  \item[err]
    is on input the current error estimate, if error projection is enabled (the
    default) then this should be overwritten with the projected error on output.
    If error projection is disabled then \id{err} is \id{NULL}.
  \item[user\_data]
    is a pointer to user data, the same as the \Id{user\_data} parameter passed
    to \id{CVodeSetUserData}.
  \end{args}
}
{
  A \id{CVProjFn} should return 0 if successful, a negative value if an
  unrecoverable error occurred (the integraiton is halted), or a positive value
  if a recoverable error occurred (the integrator will, in most cases, try to
  correct and reattempt the step).
}
{
  The tolerance passed to the projection function (\id{epsProj}) is the
  ...
  accessed by calling \id{CVodeGetErrWeights}, and the unit roundoff is
  available as \id{UNIT\_ROUNDOFF} defined in \id{sundials\_types.h}.
}
...
}
{
  ...
  call to the user's \id{CVLsJacFn} function is preceded by a call to the
  \id{CVRhsFn} user function with the same \id{(t,y)} arguments. Thus, the
  Jacobian function can use any auxiliary data that is computed and saved during
  the evaluation of the ODE right-hand side. In the case of a user-supplied or
  external nonlinear solver, this is also true if the nonlinear system function
  is evaluated prior to calling the linear solver setup function (see
  \S\ref{ss:sunnonlinsol_sunsuppliedfn} for more information).

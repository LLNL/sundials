...
The header files corresponding to the linear solver modules
available for use with {\cvode} are:
%%
\begin{itemize}
...
vector, $Jv$.  With most of these methods, preconditioning can be done
...
When using matrix-based linear solver modules, the
{\cvls} solver interface needs a function to compute an approximation to
the Jacobian matrix $J(t,y)$.  This function must be of type \id{CVLsJacFn}.
...
To specify a user-supplied Jacobian function \id{jac}, {\cvls} provides
the function \id{CVodeSetJacFn}.
The {\cvls} interface passes the pointer \id{user\_data}
...
{
  The function \ID{CVSetJacTimes} specifies the Jacobian-vector
...
{
  typedef (*CVLsJacFn)(&realtype t, N\_Vector y, N\_Vector fy, \\

...
\item\label{i:nonlin_solver_interface}
  ...
  If using a non-default nonlinear solver, then initialize the {\cvnls}
  nonlinear solver interface by attaching the nonlinear solver object by calling
  \id{ier = }\Id{CVodeSetNonlinearSolver}\id{(cvode\_mem, NLS);}
...
either of the functions \Id{CVDlsSetLinearSolver} or
\Id{CVSpilsSetLinearSolver}, as documented below.  The first argument
passed to these functions is the {\cvode} memory pointer returned by
\id{CVodeCreate}; the second argument passed to these functions is the
...
%%==================================================================================
%%
\subsection{Nonlinear solver interface function}\label{ss:nonlin_solv_init}
...
and attach the {\sunnonlinsol} object by calling \Id{CVodeSetNonlinearSolver},
as documented below.

When changing the nonlinear solve in {\cvode}, \id{CVodeSetNonlinearSolver} must
be called after \id{CVodeInit}. If any calls to \id{CVode} have been made, then
{\cvode} will need to be reinitialized by calling \id{CVodeReInit} to ensure the
nonlinear solver is initialized correctly before any subsequent calls to
\id{CVode}.

The first argument passed to \id{CVodeSetNonlinearSolver} is the {\cvode} memory
pointer returned by \id{CVodeCreate} and the second argument is the
{\sunnonlinsol} object to use for solving the nonlinear system
(\ref{e:nonlinear}) or (\ref{e:nonlinear_fixedpoint}). A call to this function
...
}
{
  ...
  \item[\Id{CV\_SUCCESS}]
    The optional value has been successfully set.
  \item[\Id{CV\_MEM\_NULL}]
    The \id{cvode\_mem} pointer or the {\sunnonlinsol} module is \id{NULL}.
  \end{args}
}
{
...
\noindent\id{CVSpilsSetPreconditioner}.
...
\index{CVSPILS@{\cvspils} linear solver interface!convergence test}
Finally, as described in Section \ref{ss:ivp_sol}, the {\cvspils}
...
\Id{CVDlsSetLinearSolver} is called in the steps described in
\S\ref{ss:skeleton_sim}), the user may
provide a function of type \Id{CVDlsJacFn} defined as follows:
%%
...
}
{
  ...
  \item[tmp3]
    ...
    \id{CVDlsJacFn} function as temporary storage or work space.
  \end{args}
}
{
...
}
{
  ...
  If the user's \id{CVDlsJacFn} function uses difference quotient
...
(i.e., \id{CVSpilsSetLinearSolver} is called in the steps described in
\S\ref{ss:skeleton_sim}), the user may
...
user-supplied function of type \Id{CVSpilsJacTimesSetupFn}, defined as follows:
...
be preprocessed or evaluated, then this needs to be done in a
user-supplied function of type \Id{CVSpilsPrecSetupFn}, defined as follows:
...
\id{nfevals} returned by \id{CVodeGetNumRhsEvals}.

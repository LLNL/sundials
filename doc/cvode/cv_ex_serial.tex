...
The \id{cvode\_direct.h} file provides the prototype for the
\id{CVDlsAttachLinearSolver} function.  The \id{sunlinsol\_dense.h}
...
\id{SUNDenseLinearSolver} (see \ugref{ss:sunlinsol_dense}) creates the 
...
solver interface with the  call to \id{CVDlsSetLinearSolver} (see
\ugref{sss:lin_solv_init}), and the subsequent call to
\id{CVDlsSetJacFn} (see \ugref{ss:optional_input}) specifies the
...
linear solver.  It calls various \id{CVodeGet***} and \id{CVDlsGet***}
...
\id{sunmatrix\_band.h}, \id{sunlinsol\_band.h} and
\id{cvode\_direct.h} in order to use the {\sunlinsolband} linear
solver. The \id{sunmatrix\_band.h} file contains the definition of the
banded {\sunmatrix} type, and the \id{SM\_COLUMN\_B} and
\id{SM\_COLUMN\_ELEMENT\_B} macros for accessing banded matrix 
elements (see \ugref{ss:sunmat_band}).  The \id{sunlinsol\_band.h}
file contains the definition of the banded {\sunlinsol} type.  We note
that have explicitly included \id{sunmatrix\_band.h}, but this is not
necessary because it is included by \id{sunlinsol\_band.h}.  The
\id{cvode\_direct.h} file contains the prototype for the
\id{CVDlsSetLinearSolver} routine. The file \id{nvector\_serial.h} is
...
\id{CVDlsSetLinearSolver} (see \ugref{sss:lin_solv_init}) specifies
the {\sunlinsolband} linear solver to the {\cvdls} interface.
The call to \id{CVDlsSetJacFn} (see \ugref{ss:optional_input}) specifies
...
argument to \id{SUNSPGMR}. The inclusion of \id{sundials\_math.h} is
...
It calls \id{SUNSPGMR} to create the {\spgmr} linear solver with left
preconditioning, and the default value (indicated by a zero argument)
for \id{maxl}.  It then calls \id{CVSpilsSetLinearSolver} (see
\ugref{sss:lin_solv_init}) to attach this linear solver to the
{\cvspils} interface.  The call to \id{CVSpilsSetJacTimes} specifies a
...
\id{CVSpilsSetPreconditioner} function.
...
\id{leniw}, and those used by {\cvspils}, denoted \id{lenrwLS} and \id{leniwLS}.
...
and \id{SUNFALSE} otherwise, to inform {\cvspils} of the status of Jacobian data.

...
The \id{sundials\_dense.h} file provides the definition of the dense matrix type
\id{DlsMat (type=1)} and a macro for accessing matrix elements.
...
\id{CVDlsSetDenseJacFn} (see \ugref{ss:optional_input}) specify the {\cvdense}
...
elements of a dense matrix of type \id{DlsMat}.  Here the problem
...
\id{DlsMat} elements.  Note that in this example, \id{Jac}
...
The \id{cvAdvDiff\_bnd.c} program includes files \id{cvode\_band.h} and
...
file contains the definition for band matrix type \id{DlsMat (type=2)} and the
...
are equal to \id{MY}.  
The call to \id{CVDlsSetBandJacFn} (see \ugref{ss:optional_input}) specifies
...
The source file, \id{cvDiurnal\_kry.c}, is listed in Appendix \ref{s:cvDiurnal_c}
in this document.
...
zero argument) for \id{maxl}.
The call to \id{CVSpilsSetJacTimesVecFn} specifies a user-supplied function
for Jacobian-vector products.
The Gram-Schmidt orthogonalization is set to \id{MODIFIED\_GS}
through the function \id{CVSpilsSetGSType}.  Next, user-supplied preconditioner
setup and solve functions, \id{Precond} and \id{PSolve}, are specified.
See \ugref{ss:optional_input} for details on the \id{CVSpilsSetPreconditioner}
function.
...
\id{leniw}, and those used by {\cvspgmr}, denoted \id{lenrwLS} and \id{leniwLS}.
...
matrices.  The large dense functions work with the type \id{DlsMat (type=1)},
...
type \id{DlsMat} for the $N \times N$ dense Jacobian and Newton
matrices, and calls the large matrix functions.  But to avoid the
extra layer of function calls, \id{cvDiurnal\_kry.c} uses the small dense
functions for all operations on the $2 \times 2$ preconditioner blocks.  
Thus it includes \id{sundials\_smalldense.h} (by way of \id{sundials\_dense.h}),
and calls the small dense matrix functions \id{newDenseMat}, \id{newIntArray},
\id{denseCopy}, \id{denseScale}, \id{denseAddI}, \id{denseGETRF}, and \id{denseGETRS}.

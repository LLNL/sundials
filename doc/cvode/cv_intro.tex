...
\item
  ...
  a banded implementation (\S\ref{ss:sunmat_band}),
  a diagonal implementation (\S\ref{ss:sunmat_diag}) and
...
\item
  ...
  a dense implementation (\S\ref{ss:sunlinsol_dense}),
  a banded implementation (\S\ref{ss:sunlinsol_band}),
  a diagonal implementation (\S\ref{ss:sunlinsol_diag}),
  a GMRES implementation (\S\ref{ss:sunlinsol_spgmr}),
  a FGMRES implementation (\S\ref{ss:sunlinsol_spfgmr}),
  a Bi-CGStab implementation (\S\ref{ss:sunlinsol_spbcgs}),
  a TFQMR implementation (\S\ref{ss:sunlinsol_sptfqmr}),
  and a PCG implementation (\S\ref{ss:sunlinsol_pcg}).  The chapter
  also contains details on the {\sunlinsol} implementations provided
  with {\sundials} that interface with external linear solver
  libraries:
  a dense LAPACK interface (\S\ref{ss:sunlinsol_lapdense}),
  a banded LAPACK interface (\S\ref{ss:sunlinsol_lapband}),
  a sparse {\klu} interface (\S\ref{ss:sunlinsol_klu}),
  and a sparse {\superlumt} interface (\S\ref{ss:sunlinsol_klu}).
%% \item

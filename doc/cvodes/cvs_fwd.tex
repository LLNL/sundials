...
\item
  ...
  Call \id{flag = }\Id{CVodeSensInit}\id{(\ldots);} to activate forward 
...
}
{
  ...
  \item[ism] (\id{int})
    ...
    be \id{CV\_SIMULTANEOUS} or \id{CV\_STAGGERED}:
    \begin{itemize}
...
}
{
  ...
  If an error occured, \id{CVodeSensInit} also sends an error message to the
  error handler function.

  {\warn}It is illegal to use \id{ism} cannot be \id{CV\_STAGGERED1}. This option
...
}
{
  ...
  If an error occured, \id{CVodeSensInit1} also sends an error message to the
...
and must follow a call to \Id{CVodeSensInit}/\Id{CVodeSensInit1} (and maybe a call to \id{CVodeReInit}). 
The number \id{Ns} of sensitivities is assumed to be unchanged since the call to the initialization
function.
%%
...
}
{
  ...
  \id{CVodeSensInit}/\id{CVodeSensInit1}.

  If an error occured, \id{CVodeSensReInit} also send an error to the
  error handler function.
...
}
{
  ...
  computaions by a previous call to \id{CVodeSensInit} or \id{CVodeSensInit1}.
}
{
...
}
{
  ...
  only by calling again \id{CVodeSensInit} or \id{CVodeSensInit1}.
}
%%
%%
...
}
{
  ...
  \item[abstolS] (\id{realtype*})
    is a pointer to an array containing the scalar absolute error tolerances,
...
%%
%%
\ucfunction{CVodeEEtolerances}
{
  flag = CVodeEEtolerances(cvode\_mem);
}
{
  When this function is used, {\cvodes} will estimate tolerances for sensitivity variables based on the 
...
If forward sensitivity computations have been initialized by a call to \id{CVodeSensInit} or
\id{CVodeSensInit1}, or reinitialized by a call to \id{CVSensReInit}, then {\cvodes} computes 
both a solution and sensitivities at time \id{t}. However, \id{CVode} will still return only 
the solution $y$ in \id{y}. 
...
DQ approximation method & \id{CVodeSetSensDQMethod} & 0.0 \\
...
{
  {\warn}This function must be preceeded by a call to \id{CVodeSensInit} or \id{CVodeSensInit1}.
}
%%
%%
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvodes} memory block.
  \item[DQtype] (\id{int})
    specifies the difference quotient type and can be one of \ID{CV\_CENTERED}
    or \ID{CV\_FORWARD}.
...
and is {\em required} if \id{ism}$=$\Id{CV\_STAGGERED1} in the call to \id{CVoeSensInit1}.
...
\item
  ...
  Call \id{CVodeGetQuadSens*} functions to obtain optional output related to
...
\end{Steps}
%%
\id{CVodeQuadSensInit} (step \ref{i:quad_sens_init} above) can be called and quadrature-related 
optional inputs (step \ref{i:quad_sens_optional_input} above) can be set, anywhere 
between steps \ref{i:quad_sens_sens_def} and \ref{i:quad_sens_cvode_solve}.
...
}
{
  ...
  \item[cvode\_mem] (\id{void *})
    pointer to the {\cvodes} memory block returned by \id{CVodeCreate}.
  \item[rhsQS] (\Id{CVodeQuadSensRhsFn})
...
}
{
 ...
 {\warn} Before calling \id{CVodeQuadSensInit}, the user must enable the sensitivites
  ...
  If an error occured, \id{CVodeQuadSensInit} also sends an error message to the
  error handler function.
}
%%
\index{memory requirements!CVODES@{\cvodes} solver}
...
{
  If an error occured, \id{CVodeQuadSensReInit} also sends an error message to the
  error handler function.
}
%%
%%
...
function \id{CVode} is exactly the same as in \S\ref{sss:cvode}. However, in this
case the return value \id{flag} can also be one of the following:
\begin{args}[CV\_FIRST\_QRHS\_FAIL]
\item[\Id{CV\_QSRHS\_FAIL}]
  The sensitivity quadrature right-hand side function failed in an unrecoverable manner.
\item[\Id{CV\_FIRST\_QSRHS\_ERR}]
  The sensitivity quadrature right-hand side function failed at the first call.
\item[\Id{CV\_REP\_QSRHS\_ERR}]
  Convergence tests occurred too many times due to repeated recoverable errors in
  the quadrature right-hand side function. The \id{CV\_REP\_RES\_ERR} will also
...
If sensitivity-dependent quadratures have been initialized by a call to \id{CVodeQuadSensInit},
or reinitialized by a call to \id{CVodeQuadSensReInit}, then {\cvodes} computes both a solution,
sensitivites and quadratures depending on sensitivities at time \id{t}. However, 
\id{CVode} will still return only the solutions $y$ and $y'$. Sensitivity-dependent 
quadratures can be obtained using one of the following functions:
%%
%%
\ucfunction{CVodeGetQuadSens}
{
  flag = CVodeGetQuadSens(cvode\_mem, \&t, yQS);
}
{
...
{
  flag = CVodeQuadGetSens1(cvode\_mem, \&t, is, yQS);
}
{
...
}
{
  ...
  mechanism. If so, the user must call \id{CVodeQuadSensSStolerances} or
  \id{CVodeQuadSensSVtolerances} to specify the 
...
{
  flag = CVodeQuadSensEEtolerances(cvode\_mem, reltolQS, abstolQS);
}
{
  The function \ID{CVodeQuadSensEEtolerances} specifies that tolerances for
...
}
{
  ...
  call to \id{CVodeSetQuadSensErrCon} with \id{errconQS = TRUE}), 
  \id{CVodeGetQuadSensErrWeights} does not set the \id{eQSweight} vector.
}
%%
%%
...
must be of type \Id{CVodeQuadSensRhsFn} defined as follows:
\usfunction{CVodeQuadSensRhsFn}
{
  typedef int (*CVodeQuadSensRhsFn)(&Ns, realtype t, N\_Vector yy, N\_Vector yp,\\
...
{
  A \id{CVodeQuadSensRhsFn} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\cvodes} will attempt to correct), or a negative 
  value if it failed unrecoverably (in which case the integration is halted and
...
}
{
  ...
  implementations provided with {\cvodes} do not perform any consistency checks with respect 
  to their \id{N\_Vector} arguments (see \S\ref{ss:nvec_ser} and \S\ref{ss:nvec_par}).

  There are two situations in which recovery is not possible even if \id{CVodeQuadSensRhsFn}
  function returns a recoverable error flag. This include the situation when this
  occurrs at the very first call to the \id{CVodeQuadSensRhsFn} (in which case {\cvodes} returns
  \Id{CV\_FIRST\_QRHS\_ERR}) or if a recoverable error is reported when \id{CVodeQuadSensRhsFn}
  is called after an error test failure, while the linear multistep method order is

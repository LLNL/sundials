...
\item
  ...
  \id{Ns} ``empty'' vectors by making the appropriate call

  \id{yS0 = N\_VCloneVectorArrayEmpty\_***(Ns, y0);}

  and then attach the \id{realtype} array \id{yS0[i]} containing the initial
  values of the $i$-th sensitivity vector using the appropriate function

  \id{N\_VSetArrayPointer\_***(yS0\_i, yS0[i]);}

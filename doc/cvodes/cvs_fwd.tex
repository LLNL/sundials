...
}
{
  ...
  \begin{args}[CV\_ILL\_INPUT]
  \item[\Id{CV\_SUCCESS}]
    The call to \id{CVodeEEtolerances} was successful.
  \item[\Id{CV\_MEM\_NULL}] 
    The {\cvodes} memory block was not initialized through a previous call to
    \id{CVodeCreate}.
  \item[\Id{CV\_NO\_SENS}] 
...
}
{
  ...
  \item[iS]
    ...
    side must be computed $(i \leq$ \id{iS} $ < $ \id{Ns}).
  \item[yS]
...
{
  The function \ID{CVodeQuadSensEEtolerances} specifies that the tolerances for
...
This user function must be of type \Id{CVodeQuadSensRhsFn} defined as follows:
...
}
{
  ...
  \id{CVodeQuadSensRhsFn} function returns a recoverable error flag.
  One is when this occurs at the very first call to the \id{CVodeQuadSensRhsFn}
  (in which case {\cvodes} returns \Id{CV\_FIRST\_QSRHSFUNC\_ERR}).
  The other is when a recoverable error is reported by \id{CVodeQuadSensRhsFn}

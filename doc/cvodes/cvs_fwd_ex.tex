...
The source file for this problem, \id{cvsAdvDiff\_FSA\_non.c}, is listed in Appendix
...
\id{cvsAdvDiff\_FSA\_non.c} contains definitions of private functions.  The last
...
\end{figure}
%%
The output generated by \id{cvsAdvDiff\_FSA\_non} when computing sensitivities with the \id{CV\_SIMULTANEOUS}
method and full error control (\id{cvsAdvDiff\_FSA\_non -sensi sim t}) is:
...
The source code for this example is listed in Appendix \ref{s:cvsRoberts_FSA_dns_c}.
...
function \id{f}. In the \id{cvsRoberts\_FSA\_dns} example this is done by defining \id{f\_data} to be 
...
\id{CVodeMalloc}, computes the righ-hand side of the ODE (\ref{e:cvsRoberts_FSA_dns_ode}), while
...
a typical user code), parse and verify the command line arguments to \id{cvsRoberts\_FSA\_dns}, respectively.
...
{\cvodes} interface functions called by \id{cvsRoberts\_FSA\_dns}.
...
command line arguments. \id{cvsRoberts\_FSA\_dns} can be run with or without sensitivity computations
...
%%
The next block of code is identical to that in \id{cvsdenx.c} (see \cite{cvode_ex})
...
Results generated by \id{cvsRoberts\_FSA\_dns} are shown in Fig.~\ref{f:cvsRoberts_FSA_dns}. 
%%
\begin{figure}
...
The source code for this example is listed in \A\ref{s:cvsDiurnal_FSA_kry_p_c}.
...
\id{cvsDiurnal\_kry\_p} described in \cite{cvode_ex}. The only difference is in the
...
the user data structure \id{data}. The program \id{cvsDiurnal\_FSA\_kry\_p} defines no additional
...
Sample results generated by \id{cvsDiurnal\_FSA\_kry\_p} are shown in Fig.~\ref{f:cvsDiurnal_FSA_kry_p}. 

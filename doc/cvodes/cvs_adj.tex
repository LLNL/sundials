...
\item
  \textcolor{gray}{\bf Initial {\cvodes} for the forward problem}
...
\item \label{i:lin_solverB}
  ...
  with the {\cvdls} linear solver module and the backward problem with 
  {\cvspils} linear solver module.

\item
  {\bf Set linear solver interface optional inputs for the backward problem}

  Call \id{CVDlsSet*B} or \id{CVSpilsSet*B} functions to change optional
...
\item\label{i:lin_solver_interfaceB}
  ...
  problem (e.g., the default Newton iteration), then initialize the {\cvdls} or
  {\cvspils} linear solver interface by attaching the linear solver object (and
  matrix object, if applicable) with one of the following calls (for details see
  \S\ref{sss:lin_solv_init}):

  \id{ier = }\Id{CVDlsSetLinearSolverB}\id{(...);}

  \id{ier = }\Id{CVSpilsSetLinearSolverB}\id{(...);}
...
\item
  ...
  (see \S\ref{ss:nonlin_solv_init} for details).
...
\index{CVODES@{\cvodes} linear solver interfaces!CVDLS@{\cvdls}}
\ucfunction{CVDlsSetLinearSolverB}
{
  flag = CVDlsSetLinearSolverB(cvode\_mem, which, LS, A);
}
{
  The function \ID{CVDlsSetLinearSolverB} attaches a direct
  {\sunlinsol} object \id{LS} and corresponding template
  Jacobian {\sunmatrix} object \id{A} to {\cvodes}, initializing the
  {\cvdls} direct linear solver interface for solution of the backward 
  problem.

  The user's main program must include the \id{cvodes\_direct.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  \item[\Id{CVDLS\_ILL\_INPUT}]
    The {\cvdls} solver is not compatible with the current {\nvector} module.
...
{
  The {\cvdls} linear solver is not compatible with all
...
%%
\index{CVODES@{\cvodes} linear solver interfaces!CVSpils{\cvspils}}
\ucfunction{CVSpilsSetLinearSolverB}
{
  flag = CVSpilsSetLinearSolverB(ida\_mem, which, LS);
}
{
  The function \ID{CVSpilsSetLinearSolver} attaches an iterative
  {\sunlinsol} object \id{LS} to {\cvodes}, initializing the
  {\cvspils} scaled, preconditioned, iterative linear solver
  interface to use for the backward problem.

  The user's main program must include the \id{cvs\_spils.h} header
  file.
}
{
  \begin{args}[cvode\_mem]
...
}
{
  ...
  \item[\Id{CVSPILS\_ILL\_INPUT}]
    The {\cvspils} solver is not compatible with the current {\nvector} module.
...
\begin{verbatim}
  flag = CVodeSetNonlinearSolverB(cvode_mem, which, NLSB);
...
a call to \id{CVDlsSetLinearSolverB}.  The Jacobian evaluation function can
be attached through a call to either \id{CVDlsSetJacFnB} or \ida{CVDlsSetJacFnBS},
...
\ucfunction{CVDlsSetJacFnB}
{
  flag = CVDlsSetJacFnB(ida\_mem, which, jacB);
}
{
  The function \ID{CVDlsSetJacFnB} specifies the Jacobian
...
}
{
  ...
  \item[\Id{CVDLS\_LMEM\_NULL}]
    ...
    \id{CVDlsSetLinearSolverB}.
  \item[\Id{CVDLS\_ILL\_INPUT}]
    The parameter \id{which} represented an invalid identifier.
  \end{args}
}
{
  The function type \id{CVDlsJacFnB} is described in \S\ref{ss:djacFn_b}.
}
%%
\index{Jacobian approximation function!direct!user-supplied (backward)}
\ucfunction{CVDlsSetJacFnBS}
{
  flag = CVDlsSetJacFnBS(cvode\_mem, which, jacBS);
}
{
  The function \ID{CVDlsSetJacFnBS} specifies the Jacobian
...
{
  The function type \id{CVDlsJacFnBS} is described in \S\ref{ss:djacFn_b}.
}
\index{CVDLS@{\cvdls} linear solver interface!optional input|)}
\index{optional input!direct linear solver interface|)}
...
\ucfunction{CVSpilsSetJacTimesB}
{
  flag = CVSpilsSetJacTimesB(cvode\_mem, which, jsetupB, jtvB);
}
{
  The function \ID{CVSpilsSetJacTimesB} specifies the Jacobian-vector 
...
}
{
  ...
  \item[which] (\id{int})
    the identifier of the backward problem.
  \item[jtsetupB] (\id{CVSpilsJacTimesSetupFnB})
...
{
  The function types \id{CVSpilsJacTimesVecFnB} and 
  \id{CVSpilsJacTimesSetupFnB} are described in \S\ref{ss:jtimesv_b}.
}
%%
\ucfunction{CVSpilsSetJacTimesBS}
{
  flag = CVSpilsSetJacTimesBS(cvode\_mem, which, jtvBS);
}
{
  The function \ID{CVSpilsSetJacTimesBS} specifies the Jacobian-vector 
...
}
{
  ...
  \item[which] (\id{int})
    the identifier of the backward problem.
  \item[jtsetupBS] (\id{CVSpilsJacTimesSetupFnBS})
    user-defined function to set up the Jacobian-vector product.
    Pass \id{NULL} if no setup is necessary.
...
{
  The function types \id{CVSpilsJacTimesVecFnBS} and 
  \id{CVSpilsJacTimesSetupFnBS} are described in \S\ref{ss:jtimesv_b}.
}
%%
...
\Id{CVDlsSetLinearSolverB} is called in the step described in 
\S\ref{ss:skeleton_adj}), the user may
provide a function of type \ID{CVDlsJacFnB} or \ID{CVDlsJacFnBS} 
(see \S\ref{ss:optional_input_b}), defined as follows:
%%
\index{Jacobian approximation function!user-supplied (backward)}
\usfunction{CVDlsJacFnB}
{
  typedef int (*CVDlsJacFnB)(&realtype t, N\_Vector y, \\
...
}
{
  ...
  \item[tmp3B]
    ...
    can be used by the \id{CVDlsJacFnB} function as temporary storage or work space.    
  \end{args}
}
{
  A \id{CVDlsJacFnB} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\cvodes} will attempt to correct, while {\cvdls} sets
...
}
{
  ...
  {\warn}Before calling the user's \id{CVDlsJacFnB}, {\cvodes} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurs in the interpolation, {\cvodes} triggers an unrecoverable
  failure in the Jacobian function which will halt the integration
  (\id{CVodeB} returns \id{CV\_LSETUP\_FAIL} and {\cvdls} sets \id{last\_flag} to 
  \Id{CVDLS\_JACFUNC\_UNRECVR}).
}
%%
\index{Jacobian approximation function!user-supplied (backward)}
\usfunction{CVDlsJacFnBS}
{
  typedef int (*CVDlsJacFnBS)(&realtype t, N\_Vector y, \\
...
}
{
  ...
  \item[tmp3B]
    are pointers to memory allocated  for variables of type \id{N\_Vector} which 
    can be used by \id{CVDlsJacFnBS} as temporary storage or work space.    
  \end{args}
}
{
  A \id{CVDlsJacFnBS} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\cvodes} will attempt to correct, while {\cvdls} sets
  \id{last\_flag} to \Id{CVDLS\_JACFUNC\_RECVR}), or a negative 
  value if it failed unrecoverably (in which case the integration is halted, \id{CVodeB}
  returns \Id{CV\_LSETUP\_FAIL} and {\cvdls} sets \id{last\_flag} to 
...
\index{Jacobian approximation function!Jacobian-vector product!user-supplied (backward)}
If the {\cvspils} solver interface is selected for the backward problem
(i.e., \id{CVSpilsSetLinearSolverB} is called in the steps described in
\S\ref{ss:skeleton_adj}), the user may
provide a function of type \Id{CVSpilsJacTimesVecFnB} or 
\Id{CVSpilsJacTimesVecFnBS} in the following form,
...
{  
  The return value of a function of type \id{CVSpilsJtimesVecFnB} should be
...
{  
  The return value of a function of type \id{CVSpilsJtimesVecFnBS} should be
  $0$ if successful or nonzero if an error was encountered, in which case
  the integration is halted.
}
{
...
user-supplied function of type \Id{CVSpilsJacTimesSetupFnB}, defined as follows:
%%
\usfunction{CVSpilsJacTimesSetupFnB}
{
  typedef int (*CVSpilsJacTimesSetupFnB)(&realtype t, \\
...
} 
{
  ...
  If the user's \id{CVSpilsJacTimesVecFnB} function uses difference quotient
...
{}
%%
\usfunction{CVSpilsJacTimesSetupFnBS}
{
  typedef int (*CVSpilsJacTimesSetupFnBS)(&realtype t, \\
...
\index{preconditioning!user-supplied}
\index{CVSPILS@{\cvspils} linear solver!preconditioner solve function}
...
Here $P$ should approximate (at least crudely) the matrix 
...
\index{preconditioning!user-supplied}
\index{CVSPILS@{\cvspils} linear solver!preconditioner setup function}

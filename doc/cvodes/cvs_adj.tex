...
}
{
  ...
  \item[which] (\id{int})
    represents the identifier of the backward problem returned by \id{CVodeCreateB}.
...
}
{
  ...
  \item[\Id{CVDLS\_LMEM\_NULL}]
    The linear solver has not been initialized with a call to \id{CVDlsSetLinearSolverB}.
  \item[\Id{CVDLS\_ILL\_INPUT}]
    The parameter \id{which} represented an invalid identifier.
  \end{args}
}

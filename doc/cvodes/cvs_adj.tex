...
{
  flag = CVodeGetB(ida\_mem, which, \&tret, yB);
}
{
...
}
{
  ...
  \item[ida\_mem] (\id{void *})
    pointer to the {\idas} memory returned by \id{CVodeCreate}.
  \item[which] (\id{int})
...
{
  typedef int (*CVDlsDenseJacFnB)(&int NeqB, realtype t, N\_Vector y, \\
...
{
  \begin{tabular}[t]{@{}r@{}l@{}}
    flag = CVBBDPrecInitB(&cvode\_mem, int which, NlocalB, mudqB, mldqB,\\
...
{
  typedef int (*CVBBDLocalFnB)(&int NlocalB, realtype t, N\_Vector y,\\
...
{
  typedef int (*CVBBDCommFnB)(&int NlocalB, realtype t, N\_Vector y,\\

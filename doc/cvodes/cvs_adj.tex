...
\item
  ...
  \Id{CVadjMalloc} also specifies the type of interpolation used 
...
\item \label{i:lin_solverB}
  ...
  \id{CVSpgmrB}, \id{CVSpbcgB}, or \id{CVSptfqmr} (see \S\ref{sss:lin_solv_b}). 
...
}
{
  ...
  user-callable functions. In case of failure (\id{cvode\_mem} is \id{NULL}, an input
  has an illegal value, or a memory request fails), \id{CVadjMalloc} prints an error
...
%%
%%
If the user wishes to change the interpolation method after the call to \id{CVadjMalloc},
this can be done through a call to the function \id{CVadsSetInterpType}.
The form of the call to this function is
%%
\ucfunction{CVadjSetInterpType}
{
  flag = CVadjSetInterpType(cvadj\_mem, interpType);
}
{
  The function \ID{CVadjSetInterpType} resets the interpolation method to \id{interpType}.
}
{
  \begin{args}[interpType]
...
}
{
  ...
  \item[\Id{CV\_SUCCESS}]
    The call to \id{CVadjSetInterpType} was successful.
...
{
  The function \id{CVadjSetInterpType} performs memory deallocation and allocation.
}
...
}
{
  ...
  in \id{CV\_NORMAL\_TSTOP} or \id{CV\_ONE\_STEP\_TSTOP} modes, 
...
}
{
  ...
  The memory allocated by \id{CVodeMallocB} is deallocated by the function 
  \id{CVadjFree}.
...
If the \id{cvadj\_mem} argument was \id{NULL}, \id{flag} will be
\id{CVDENSE\_ADJMEM\_NULL}, \id{CVDIAG\_ADJMEM\_NULL}, 
\id{CVBAND\_ADJMEM\_NULL}, or \id{CVSPILS\_ADJMEM\_NULL}.
...
}
{
  ...
  \begin{args}[CVDENSE\_ADJMEM\_NULL]
...
{
  The return value \id{flag} (of type \id{int}) is one of:
  \begin{args}[CVBAND\_ADJMEM\_NULL]
...
{
  The return value \id{flag} (of type \id{int}) is one of:
  \begin{args}[CVSPILS\_ADJMEM\_NULL]
...
}
{
  ...
  \item[cvadj\_mem] (\id{void *})
    pointer to the adjoint memory block.
  \item[jtimesB] (\id{CVSpilsJacTimesVecFnB})
...
}
{
  ...
  to \id{CVSpbcgB} or \id{CVSptfqmrB}.
  The call to \id{CVSpilsSetMaxlB} is needed only if \id{maxl} is being changed from its 
  previous value.
...
the linear solver modules. The first argument of these \id{CVodeGet*} and \id{CV*Get*}
...
\id{CVadjGetCVodeBmem} (see \S\ref{ss:optional_output_B}).
...
For debugging purposes, {\cvodes} provides a function \ID{CVadjGetCheckPointsInfo} which
...
%%
%%
\ucfunction{CVadjGetCheckPointsInfo}
{
  flag = CVadjCheckPointsList(cvadj\_mem, ckpnt);
}
{
  The function \ID{CVadjGetCheckPointsList} returns a structure array
...
{
  The return value \id{flag} of \id{CVadjGetCheckPointsInfo} is one of:
  \begin{args}[CV\_ADJMEM\_NULL]
  \item[\Id{CV\_SUCCESS}] 
    \id{CVadjGetCheckPointsInfo} was successful.
  \item[\Id{CV\_ADJMEM\_NULL}]
    The \id{cvadj\_mem} argument was \id{NULL}.
  \end{args}
}
{
  For an example of using \id{CVadjGetCheckPointsInfo}, see the \id{cvadjdenx} 
  example.
}
%%
%%
The type \Id{CVadjCheckPointRec} is defined in the header file \id{cvodea.h}:
\begin{verbatim}
     typedef struct {
       void *my_addr;
       void *next_addr;
...
%%
\ucfunction{CVadjGetDataPointHermite}
{
  int = CVadjGetDataPointHermite(cvadj\_mem, which, \&t, y, yd);
}
{
  The function \ID{CVadjGetDataPointHermite} returns the
...
}
{
  ...
  \begin{args}[CV\_ADJMEM\_NULL]
  \item[\Id{CV\_SUCCESS}] 
    \id{CVadjGetDataPointHermite} was successful.
  \item[\Id{CV\_ILL\_INPUT}]
...
%%
\ucfunction{CVadjGetDataPointPolynomial}
{
  int = CVadjGetDataPointPolynomial(cvadj\_mem, which, \&t, order, y);
}
{
  The function \ID{CVadjGetDataPointPolynomial} returns the
  time and two vectors associated with the \id{which} interpolation data point.
}
{
  \begin{args}[cvadj\_mem]
...
}
{
  ...
  \begin{args}[CV\_ADJMEM\_NULL]
  \item[\Id{CV\_SUCCESS}] 
    \id{CVadjGetDataPointPolynomial} was successful.
  \item[\Id{CV\_ILL\_INPUT}]
...
%%
The names of constants associated with {\cvodea}-specific return values can
be obtained by calling the following function:
%%
\ucfunction{CVadjGetReturnFlagName}
{
  name = CVadjGetReturnFlagName(flag);
}
{
  The function \ID{CVadjGetReturnFlagName} returns the
  name of the {\cvodea} constant corresponding to \id{flag}.
}
{
  The only argument, of type \id{int} is a return flag from a {\cvodea} function.
}
{
...
{
  A \id{CVDenseJacFnB} should return 0 if successful, a positive value if a recoverable
...
}
{
  ...
  {\warn}Before calling the user's \id{CVDenseJacFnB}, {\cvodea} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\cvodea} triggers an unrecoverable
...
{
  A \id{CVBandJacFnB} should return 0 if successful, a positive value if a recoverable
...
}
{
  ...
  {\warn}Before calling the user's \id{CVBandJacFnB}, {\cvodea} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\cvodea} triggers an unrecoverable
  failure in the Jacobian function which will halt the integration
...
is selected (\id{CVSp*B} is called in step \ref{i:lin_solverB} of \S\ref{ss:skeleton_adj}), 
...
}
{
  ...
  \item[jac\_dataB]
    is a pointer to user data - the same as the \id{jac\_dataB}      
    parameter passed to \id{CVSp*SetJacTimesVecFnB}.   
  \item[tmpB]
...
{  
  ...
  \item[P\_dataB]
    ...
    parameter passed to the function \id{CVSp*SetPreconditionerB}.
  \item[tmpB]
    is a pointer to memory allocated for a vector which can be used for work space.
  \end{args}
}
...
}
{
  ...
  \item[P\_dataB]
    is a pointer to user data - the same as the \id{P\_dataB}      
    parameter passed to \id{CVSpilsSetPreconditionerB}.
  \item[tmp1B]
  \item[tmp2B]
  \item[tmp3B]
...
}
{
  ...
  The return value \id{flag} (of type \id{int}) is one of:
  \begin{args}[CVBANDPRE\_ADJMEM\_NULL]
...
preconditioner module, make the following call:
%%
\ucfunction{CVBPSpbcgB}
{
  flag = CVBPSpbcgB(cvadj\_mem, pretypeB, maxlB);
}
{
  The function \ID{CVBPSpbcgB} links the {\cvbandpre} data to the
...
preconditioner module, make the following call:
%%
\ucfunction{CVBPSptfqmrB}
{
  flag = CVBPSptfqmrB(cvadj\_mem, pretypeB, maxlB);
}
{
  The function \ID{CVBPSptfqmrB} links the {\cvbandpre} data to the
...
%%
\ucfunction{CVBandPrecFreeB}
{
  CVBandPrecFreeB(cvadj\_mem);
}
{
  The function \ID{CVBandPrecFreeB} frees the memory allocated by
  \id{CVBandPrecAllocB}.
}
{
  The only argument passed to \id{CVBandPrecFreeB} is the pointer to the {\cvodea} 
  memory block (of type \id{void *}).
}
{
  The function \id{CVBandPrecFreeB} has no return value.
}
{
  The call to \id{CVBandPrecFreeB} must preceed that to \id{CVadjFree}.
}
%%
%%
\index{CVBANDPRE@{\cvbandpre} preconditioner!user-callable functions|)}
...
}
{
  ...
  The return value \id{flag} (of type \id{int}) is one of:
  \begin{args}[CVBBDPRE\_ADJMEM\_NULL]
...
preconditioner module, make the following call:
%%
\ucfunction{CVBBDSpbcgB}
{
  flag = CVBBDSpbcgB(cvadj\_mem, pretypeB, maxlB);
}
{
  The function \ID{CVBBDSpbcgB} links the {\cvbbdpre} data to the
  {\cvspbcg} linear solver and attaches the latter to the {\cvodes}
  memory block for the backward problem.
}
{
...
preconditioner module, make the following call:
%%
\ucfunction{CVBBDSptfqmrB}
{
  flag = CVBBDSptfqmrB(cvadj\_mem, pretypeB, maxlB);
}
{
  The function \ID{CVBBDSptfqmrB} links the {\cvbbdpre} data to the
  {\cvsptfqmr} linear solver and attaches the latter to the {\cvodes}
  memory block for the backward problem.
}
{
...
backward integration, make the following call:
%%
\ucfunction{CVBBDPrecFreeB}
{
  CVBBDPrecFreeB(cvadj\_mem);
}
{
  The function \ID{CVBBDPrecFreeB} frees the memory allocated by
  \id{CVBBDPrecAllocB}.
}
{
  The only argument passed to \id{CVBBDPrecFreeB} is the pointer to the {\cvodea} 
  memory block (of type \id{void *}).
}
{
  The function \id{CVBBDPrecFreeB} has no return value.
}
{
  The call to \id{CVBBDPrecFreeB} must preceed that to \id{CVadjFree}.
}
%%
\index{CVBBDPRE@{\cvbbdpre} preconditioner!user-callable functions|)}
%%
...
{
  A \id{CVLocalFnB} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\cvodes} will attempt to correct), or a negative 
  value if it failed unrecoverably (in which case the integration is halted and
...
}
{
  ...
  {\warn}Before calling the user's \id{CVLocalFnB}, {\cvodea} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\cvodea} triggers an unrecoverable

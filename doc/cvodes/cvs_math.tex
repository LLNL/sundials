...
Typical preconditioners used with {\cvodes} are based on
...
original ODE (\ref{e:ivp_p}) on the parameters $p$ is through its initial conditions only.
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where $y_p$ is the first-order sensitivity matrix, solution of $N_p$ 
...
that of 2 forward and 2 backward integrations of a system of ODEs of size $N$.

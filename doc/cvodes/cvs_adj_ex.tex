...
The next three sections describe in detail a serial example (\id{cvsRoberts\_ASAi\_dns}) and
two parallel examples (\id{cvsAdvDiff\_ASAp\_non\_p} and \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p}). For details on the other
...
The source code for this example is listed in \A\ref{s:cvsRoberts_ASAi_dns_c}.
...
\id{cvsRoberts\_ASAi\_dns} uses its own function to compute the error weights for WRMS norm
...
(\ref{e:cvsRoberts_ASAi_dns_ZETA}), while \id{fB}, \id{JacB}, and \id{fQB} are mere translations 
of the backward problem (\ref{e:cvsRoberts_ASAi_dns_ADJ}) and (\ref{e:cvsRoberts_ASAi_dns_XI}).
...
\begin{equation}\label{e:cvsAdvDiff_ASAp_non_p:adj_pde}
  ...
  \end{split}
\end{equation}
Both the forward problem (\ref{e:cvsAdvDiff_ASAp_non_p:orig_pde}) and the backward problem 
(\ref{e:cvsAdvDiff_ASAp_non_p:adj_pde}) are discretized on a uniform spatial grid of size
$M_x + 2$ with central differencing and with boundary values eliminated,
leaving ODE systems of size $N = M_x$ each. 
As always, we deal with the time quadratures in (\ref{e:cvsAdvDiff_ASAp_non_p:dgdp}) by introducing
...
\end{equation*}
The space integrals in (\ref{e:cvsAdvDiff_ASAp_non_p:delg}) and (\ref{e:cvsAdvDiff_ASAp_non_p:quad}) are
...
The source code for this example is listed in \A\ref{s:cvsAdvDiff_ASAp_non_p_c}. Both the forward
...
\id{cvsRoberts\_ASAi\_dns}, the example \id{cvsAdvDiff\_ASAp\_non\_p} illustrates computation of the additional
...
\id{cvsAdvDiff\_ASAp\_non\_p} uses {\mpi} calls to parallelize the calculations of the right-hand side
...
(\ref{e:cvsAdvDiff_ASAp_non_p:orig_pde}) and (\ref{e:cvsAdvDiff_ASAp_non_p:adj_pde}) and receive
...
(\ref{e:cvsAdvDiff_ASAp_non_p:quad}), and is therefore inactive during the forward phases.
...
of the forward phase. However, the approach taken in \id{cvsAdvDiff\_ASAp\_non\_p} has the 
...
Fig.~\ref{f:cvsAdvDiff_ASAp_non_p}. The gradient is plotted superimposed over the initial conditions.
%%
\begin{figure}
...
\end{figure}
%%
Sample output generated by \id{cvsAdvDiff\_ASAp\_non\_p}, for \id{MX} $=20$, is shown below.
...
the \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p} code provided with {\cvodes}. This example models an atmospheric 
...
\end{equation}
%%
The PDE (\ref{e:cvsAtmDisp_ASAi_kry_bbd_p_PDE}) is semi-discretized in space with central finite differences,
...
The adjoint PDE (\ref{e:cvsAtmDisp_ASAi_kry_bbd_p_ADJ}) is discretized to a system of ODEs in a similar fashion.
The space integrals in (\ref{e:cvsAtmDisp_ASAi_kry_bbd_p_G}) and (\ref{e:cvsAtmDisp_ASAi_kry_bbd_p_dGdp}) are simply approximated
by their Riemann sums, while the time integrals are resolved by appending pure quadrature
equations to the systems of ODEs.

The code for this example is listed in \A\ref{s:cvsAtmDisp_ASAi_kry_bbd_p_c}. It uses BDF with the {\cvspgmr}
...
For the sake of clarity, the \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p} example does not use the most 
...
Note that if \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p} is given any command line argument, it will generate
a series of MATLAB files which can be used to visualize the solution.
Results for a 2-D simulation and adjoint sensitivity analysis with \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p}
on a $80 \times 80$ grid and $2 \times 4 = 8$ processes are shown in Fig.~\ref{f:cvsAtmDisp_ASAi_kry_bbd_p2D}.
Results in 3-D \footnote{The name of executable for the 3-D version is \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p3D}.}, 
on a $80 \times 80 \times 40$ grid and $2 \times 4 \times 2= 16$ processes
are shown in Figs.~\ref{f:cvsAtmDisp_ASAi_kry_bbd_p3D_a} and \ref{f:cvsAtmDisp_ASAi_kry_bbd_p3D_b}.
%%
\begin{figure}
...
\begin{figure}
  ...
  They correspond to values of $0.25$ (green) and $0.4$ (blue).}\label{f:cvsAtmDisp_ASAi_kry_bbd_p3D_b}  
\end{figure}
%%
A sample output generated by \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p} for a 2D calculation is shown below.

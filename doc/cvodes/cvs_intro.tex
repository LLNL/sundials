...
\subsection*{Changes in v3.0.0-dev.2}

{\cvodes} has been updated to use the {\sunnonlinsol} API. By default {\cvodes}
uses the {\sunnonlinsolnewton} module implementation of a Newton iteration. With
the introduction of {\sunnonlinsol} modules, the input parameter \id{iter}
to \id{CVodeCreate} has been removed as well as the constants \id{CV\_NEWTON}
and \id{CV\_FUNCTIONAL}. User-supplied or non-default nonlinear solvers may be
attached to {\cvodes} via the \id{CVodeSetNonlinearSolver} function. As a result
of this new nonlinear solver object creation and attachment approach, the
function \id{CVodeSetIterType} has been removed. {\cvodes} functions for
...
No changes were made to {\cvodes} in this release.

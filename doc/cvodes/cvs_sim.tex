...
ODE problems and ODEs with sensitivity analysis, should use {\cvodes}.
...
\id{cvodes\_direct.h} which defines common functions and which in turn includes a header file
...
}
{
  ...
  The user's main function must include the \id{cvodes\_lapack.h} header file.
}
{
...
{
  typedef (*CVDlsDenseJacFn)(&long int N, realtype t, \\
...
{
  If an error occured, \id{CVodeQuadInit} also sends an error message to the
  error handler function.
}
%%
\index{memory requirements!CVODES@{\cvodes} solver}
...
{
  The argument is the address of the pointer to the {\cvodes} memory block (of type \id{void *}).
}
{
...
}
{
  ...
  mechanism. If so, the user must call \id{CVodeQuadSStolerances} or
  \id{CVodeQuadSVtolerances} to specify the 

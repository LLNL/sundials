...
internal and Lapack) include the file \id{cvodes\_direct.h}, which
...
include the file \id{cvodes\_sparse.h}, which defines common functions.
...
with {\cvodes}: steps marked {\p} correspond to {\nvecp}, steps marked
...
\item\label{i:lin_solver} 
  ...
  {\s, \omp, \pt} \id{ier = }\Id{CVLapackDense}\id{(...);}

  {\s, \omp, \pt} \id{ier = }\Id{CVLapackBand}\id{(...);}

  \id{ier = }\Id{CVDiag}\id{(...);}

  {\s, \omp, \pt} \id{ier = }\Id{CVKLU}\id{(...);}

  {\s, \omp, \pt} \id{ier = }\Id{CVSuperLUMT}\id{(...);}
...
\Id{CVDiag}, \Id{CVKLU}, \Id{CVSuperLUMT}, \Id{CVSpgmr}, \Id{CVSpbcg},
...
}
{
  ...
  The user's main program must include the \id{cvodes\_lapack.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
%%
%%
\index{CVODES@{\cvodes} linear solvers!CVKLU@{\cvklu}}
...
}
{
  ...
  The user's main program must include the \id{cvodes\_klu.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
\index{CVODES@{\cvodes} linear solvers!CVSUPERLUMT@{\cvsuperlumt}}
...
}
{
  ...
  The user's main program must include the \id{cvodes\_superlumt.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{cvodes\_spgmr.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{cvodes\_spbcgs.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{cvodes\_sptfqmr.h} header file.
}
{
  \begin{args}[cvode\_mem]
  \item[cvode\_mem] (\id{void *})
...
\begin{table}
\centering
\caption{Optional inputs for {\cvodes}, {\cvdls}, and {\cvspils}}
...
\begin{table}
\centering
\caption{Optional outputs from {\cvodes}, {\cvdls}, {\cvdiag}, and {\cvspils}}
...
}
{
  ...
  To obtain these, the user will need to add the \id{cvode\_mem} 
...
}
{
  ...
  {\cvodes} returns \Id{CV\_LSETUP\_FAIL} and {\cvklu} or

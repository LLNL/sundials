...
%%
\begin{itemize}
\item \Id{cvodes/cvodes\_direct.h},
...
\item \Id{cvodes/cvodes\_spils.h},
...
\item\label{i:lin_solver}
  ...
  options, as discussed in Chapter {\ref{s:sunlinsol}}.
...
iteration), there are three {\cvodes} linear solver interfaces currently
...
passed to these functions is the {\cvodes} memory pointer returned by
...
to one of these functions initializes the appropriate {\cvodes} linear
solver interface, linking this to the main {\cvodes} integrator, and
...
}
{
  ...
  Jacobian {\sunmatrix} object \id{J} to {\cvodes}, initializing the
...
}
{
  ...
  The user's main program must include the \id{cvodes\_spils.h} header
  file.
}
{
  \begin{args}[cvode\_mem]
...
{
  The {\cvdiag} solver is the simplest of all of the current {\cvodes}

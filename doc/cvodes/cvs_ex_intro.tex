...
\begin{center}
  \begin{tabular}{|p{\colone}|l|l|} \hline
    ...
    {}  & \id{cvsRoberts\_dns\_uw} \id{cvsRoberts\_klu}     & \id{cvsDiurnal\_kry\_p}      \\
    {}  & \id{cvsRoberts\_sps}     \id{cvsAdvDiff\_bnd}     & \id{cvsDiurnal\_kry\_bbd\_p} \\
    {}  & \id{cvsAdvDiff\_bndL}    \id{cvsDiurnal\_kry}     & {}                           \\
    {}  & \id{cvsDiurnal\_kry\_bp} \id{cvsDirectDemo\_ls}   & {}                           \\
    {}  & \id{cvsKrylovDemo\_ls}   \id{cvsKrylovDemo\_prec} & {}                           \\
    \hline
    
    FSA & \id{cvsRoberts\_FSA\_dns} \id{cvsRoberts\_FSA\_klu} & \id{cvsAdvDiff\_FSA\_non\_p} \\
    {}  & \id{cvsRoberts\_FSA\_sps}                           & \id{cvsDiurnal\_FSA\_kry\_p} \\
    {}  & \id{cvsAdvDiff\_FSA\_non} \id{cvsDiurnal\_FSA\_kry} & {}                           \\
    \hline
    
    ASA & \id{cvsRoberts\_ASAi\_dns} \id{cvsRoberts\_ASAi\_klu} & \id{cvsAdvDiff\_ASAp\_non\_p}      \\
    {}  & \id{cvsRoberts\_ASAi\_sps} \id{cvsAdvDiff\_ASAi\_bnd} & \id{cvsAtmDisp\_ASAi\_kry\_bbd\_p} \\
    {}  & \id{cvsFoodWeb\_ASAi\_kry} \id{cvsFoodWeb\_ASAp\_kry} & {}             \\
...
\item [{[ls]}] identifies the linear solver module used (for examples using
  functional iteration for the nonlinear system solver, \id{non} specifies
...
\item \id{cvsRoberts\_dnsL}
  is the same as \id{cvsRoberts\_dns} but uses the Lapack implementation of {\cvdense}.
...
\item \id{cvsRoberts\_klu}
  is the same as \id{cvsRoberts\_dns} but uses the KLU sparse direct linear solver.
\item \id{cvsRoberts\_sps}
  is the same as \id{cvsRoberts\_dns} but uses the SuperLUMT sparse direct linear solver
  (with one thread).
...
\item \id{cvsAdvDiff\_bndL}
  is the same as \id{cvsAdvDiff\_bnd} but uses the Lapack implementation of {\cvband}.
...
\item \id{cvsRoberts\_FSA\_klu}
  is the same as \id{cvsRoberts\_FSA\_dns} but uses the KLU sparse direct linear solver.
\item \id{cvsRoberts\_FSA\_sps}
  is the same as \id{cvsRoberts\_FSA\_dns} but uses the SuperLUMT sparse direct linear solver.
...
\item \id{cvsRoberts\_ASAi\_klu}
  is the same as \id{cvsRoberts\_ASAi\_dns} but uses the KLU sparse direct linear solver.
\item \id{cvsRoberts\_ASAi\_sps}
  is the same as \id{cvsRoberts\_ASAi\_dns} but uses the SuperLUMT sparse direct linear solver.

...
\item \id{cvsRoberts\_FSA\_dns}
  solves a 3-species chemical kinetics problem (from \id{cvsRoberts\_dns}).
  \newline
...
\item \id{cvsFoodWeb\_ASAp\_kry}
  solves the same problem as \id{cvsFoodWeb\_ASAp\_kry}, but computes gradients of the
...
\item \id{cvsDiurnal\_kry\_p}
  is the parallel implementation of \id{cvsDiurnal\_kry}.
\item \id{cvsDiurnal\_kry\_bbd\_p}
  solves the same problem as \id{cvsDiurnal\_kry\_p}, with the BDF/GMRES method 
...
\item \id{cvsAdvDiff\_FSA\_non\_p}
  is the parallel version of \id{cvsAdvDiff\_FSA\_non}.
\item \id{cvsDiurnal\_FSA\_kry\_p}
  is the parallel version of \id{cvsDiurnal\_FSA\_kry}.
%% ASA examples

...
:eq:`CVODES_Newton`, its errors must also be controlled. To this end,
...
:eq:`CVODES_errwt`. The linear iteration error should be at least as small as
...
relating the nonlinear solver error to the CVODES error estimate. If the linear
...
however, the CVODES linear solver interface must be initialized with a template

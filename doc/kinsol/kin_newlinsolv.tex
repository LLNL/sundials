...
}
{
  ...
  calls to \id{lsolve}, in the solution of systems $J x = b$, where         
...
{
  The \id{lsolve} function must solve the linear system $J x = b$, where $J$ is
...
to \id{KINSol}.  Specifically, \id{sFdotJp} must be computed in an
iterative linear solver if \id{strategy} is either \id{KIN\_NONE} or
\id{KIN\_LINESEARCH}, and the Stopping Criterion Forcing Term is Choice 1
(the default). (See \id{KINSetEtaForm}.)  And \id{sFdotJp} must be computed
if either  (a) \id{strategy} is \id{KIN\_LINESEARCH}, or (b) \id{lsolve}
is an iterative linear solver and the Stopping Criterion Forcing Term
is Choice 1 (the default).  The values of \id{strategy} and \id{etachoice}
are available from the \id{kin\_mem} fields \id{kin\_global\_strategy}
and \id{kin\_etaflag}, respectively.

...
{\sunmatrix}, and {\sunlinsol} objects. The constructors \id{N\_VNewEmpty()},
\id{SUNMatNewEmpty()}, and \id{SUNLinSolNewEmpty()} allocate ``empty''
{\nvector}, {\sunmatrix}, and {\sunlinsol} objects respectively with the
object's content pointer and the function pointers in the operations structure
are initialized to \id{NULL}. Additionally, the functions \id{N\_VCopyOps(w, v)}
...
{\sunlinsol} APIs by ensuring only required operations need to be set and all

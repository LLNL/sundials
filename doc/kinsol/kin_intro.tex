...
{\sunmatrix} objects. The constructors \id{N\_VNewEmpty()} and
\id{SUNMatNewEmpty()} allocate ``empty'' {\nvector} and {\sunmatrix} objects
respectively with the object's content pointer and the function pointers in the
operations structure are initialized to \id{NULL}. Additionally, the functions
\id{N\_VCopyOps(w, v)} and \id{SUNMatCopyOps(A, B)} have been added to copy the
operation function pointers between vector and matrix objects respectively. When
used in the constructor and clone routines for custom {\nvector} and
{\sunmatrix} objects these functions will ease the introduction of any new
optional operations to the {\nvector} or {\sunmatrix} API by ensuring only
required operations need to be set and all operations are copied when cloning
objects.

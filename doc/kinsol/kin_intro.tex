...
\begin{itemize}
\item Fixed problem with index types which would occur with some compilers
  (e.g. armclang) that did not define \id{\_\_STDC\_VERSION\_\_}.
\item Deprecate current behavior of the \id{SUNDIALS\_INDEX\_TYPE} CMake
  option. Add \id{SUNDIALS\_INDEX\_SIZE} CMake option.
\end{itemize}
...
\id{CMAKE\_<language>\_COMPILER} can compile MPI programs before
...
module. This included changing \id{MPI\_MPICC} to
\id{MPI\_C\_COMPILER}, \id{MPI\_MPICXX} to \id{MPI\_CXX\_COMPILER},
combining \id{MPI\_MPIF77} and \id{MPI\_MPIF90} to
\id{MPI\_Fortran\_COMPILER}, and changing \id{MPI\_RUN\_COMMAND} to
\id{MPIEXEC\_EXECUTABLE}. When a Fortran name-mangling scheme is needed (e.g.,
\id{LAPACK\_ENABLE} is \id{ON}) the build system will infer the scheme
...
options \id{SUNDIALS\_F77\_FUNC\_CASE} and
\id{SUNDIALS\_F77\_FUNC\_UNDERSCORES} can be used to manually set the
name-mangling scheme and bypass trying to infer the
scheme. Additionally, parts of the main CMakeLists.txt file were moved
to new files in the \id{src} and \id{example} directories to make the

...
systems. See Chapter \ref{ss:sunlinsol_onemkldense} for more details. This
module is experimental and is subject to change from version to version.
...
iteration (see \id{KINSetReturnNewest}).

The installed SUNDIALSConfig.cmake file now supports the \id{COMPONENTS} option
to \id{find\_package}. The exported targets no longer have \id{IMPORTED\_GLOBAL}
set.

A bug was fixed in \id{SUNMatCopyOps} where the matrix-vector product setup
...
A bug was fixed in the Picard iteration where the value of
\id{KINSetMaxSetupCalls} would be ignored.

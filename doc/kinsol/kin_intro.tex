...
Two functions were added to aid in creating custom {\nvector} objects. The empty
constructor \id{N\_VNewEmpty()} allocates a new {\nvector} object and
initializes the content pointer and function pointers in the operations
structure to \id{NULL}. The function \id{N\_VCopyOps(w, v)} copies the function
pointers from the vector \id{w} into the operations structure of the vector
\id{v}. When used in custom {\nvector} constructors and clone routines these

...
A large number of errors have been fixed.  Among these are the following:
...
Two minor errors were fixed -- involving setting \id{etachoice} in the
Matlab/{\kinsol} interface, and a missing error case in \id{KINPrintInfo}.
In each linear solver interface function, the \id{**Free} function now
includes a line setting to NULL the main memory pointer to the linear
solver memory.

...
\newlength{\tcolone}
\settowidth{\tcolone}{\id{KIN\_LINSOLV\_NO\_RECOVERY}}
\newlength{\tcoltwo}
...
\id{KIN\_NONE}            & 0 & Use Newton iteration. \\
\id{KIN\_LINESEARCH}      & 1 & Use Newton iteration with linesearch globalization. \\
\id{KIN\_PICARD}          & 2 & Use Picard iteration. \\
\id{KIN\_FP}              & 3 & Use fixed point iteration. \\
\id{KIN\_ORTH\_MGS}       & 0 & Use Modified Gram-Schmidt for Anderson acceleration. \\ 
\id{KIN\_ORTH\_ICWY}      & 1 & Use Inverse Compact WY Modified Gram-Schmidt for Anderson
                                acceleration. \\
\id{KIN\_ORTH\_CGS2}      & 2 & Use Classical Gram-Schmidt with Reorthogonalization (CGS-2)
                                for Anderson Acceleration. \\
\id{KIN\_ORTH\_DCGS2}     & 3 & Use CGS-2 with Delayed Reorthogonalization for Anderson
                                acceleration.
...
\id{KINLS\_SUCCESS}       &  0 & Successful function return. \\
\id{KINLS\_MEM\_NULL}     & -1 & The \id{kin\_mem} argument was \id{NULL}.\\
\id{KINLS\_LMEM\_NULL}    & -2 & The {\kinls} linear solver has not been initialized.\\
\id{KINLS\_ILL\_INPUT}    & -3 & The {\kinls} solver is not compatible with the current {\nvector} module, or an input value was illegal.\\
\id{KINLS\_MEM\_FAIL}     & -4 & A memory allocation request failed.\\
\id{KINLS\_PMEM\_NULL}    & -5 & The preconditioner module has not been initialized. \\
\id{KINLS\_JACFUNC\_ERR}  & -6 & The Jacobian function failed \\
\id{KINLS\_SUNMAT\_FAIL}  & -7 & An error occurred with the current {\sunmatrix} module. \\
\id{KINLS\_SUNLS\_FAIL}   & -8 & An error occurred with the current {\sunlinsol} module. \\

...
\id{KINDIRECT\_SUCCESS}    &  0 & Successful function return. \\
\id{KINDIRECT\_MEM\_NULL}  & -1 & The \id{kin\_mem} argument was \id{NULL}.\\
\id{KINDIRECT\_LMEM\_NULL} & -2 & The {\kindense} linear solver has not been initialized.\\
\id{KINDIRECT\_ILL\_INPUT} & -3 & The {\kindense} solver is not compatible with the current {\nvector} module.\\
\id{KINDIRECT\_MEM\_FAIL}  & -4 & A memory allocation request failed.\\
\id{KINDIRECT\_JACFUNC\_UNRECVR} & -5 & The Jacobian function failed in an unrecoverable manner. \\
\id{KINDIRECT\_JACFUNC\_RECVR}   & -6 & The Jacobian function had a recoverable error. \\

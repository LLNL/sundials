...
include the header file \id{kinsol\_sparse.h}, which defines common functions.
...
For the steps that are not, refer to Chapter \ref{s:nvector} for the
...
\item
  ...
  \id{ ydata = N\_VGetArrayPointer\_***(u)}.  
...
\item\label{i:lin_solver} 
  ...
  \id{ier = }\Id{KINDense}\id{(...);}

  \id{ier = }\Id{KINBand}\id{(...);}

  \id{ier = }\Id{KINLapackDense}\id{(...);}

  \id{ier = }\Id{KINLapackBand}\id{(...);}

  \id{ier = }\Id{KINKLU}\id{(...);}

  \id{ier = }\Id{KINSuperLUMT}\id{(...);}
...
NOTE: If using solver strategy KIN\_FP, no linear solver is needed.
...
\ucfunction{KINKLU}
{
  flag = KINKLU(kin\_mem, NP, NNZ, sparsetype);
}
{
...
}
{
  ...
  \item[kin\_mem] (\id{void *})
    pointer to the {\kinsol} memory block.
  \item[msbsetsub] (\id{sunindextype})
...
%%==============================================================================
\subsubsection{Dense/band direct linear solver optional input functions}\label{sss:optin_dls}
%%==============================================================================
...
In this case, for the {\kinklu} solver, we provide the following reinitialization
...
}
{
  ...
  and \id{sunindextype}) is $22 + 5 N$ (increased by $N$ if constraint checking is enabled).
}
%%
%%
...
{
  typedef int (*KINDlsDenseJacFn)(&sunindextype N, N\_Vector u, N\_Vector fu,\\
...
{
 typedef int (*KINDlsBandJacFn)(&sunindextype N, sunindextype mupper,\\
                                &sunindextype mlower, N\_Vector u, N\_Vector fu,\\
                                &DlsMat J, void *user\_data,\\
                                &N\_Vector tmp1, N\_Vector tmp2);
}
{
...
}
{
  ...
  If the user's \id{KINDlsBandJacFn} function uses difference quotient
  approximations, it may need to access quantities not in the call list.
  These might include the scale vectors and the unit roundoff.
  To obtain the scale vectors, the user will need to add to \id{user\_data}
  pointers to \id{u\_scale} and/or \id{f\_scale} as needed.
...
}
{
  ...
  may need to access quantities not in the argument list.
  These might include the scale vectors and the unit roundoff.
  To obtain the scale vectors, the user will need to add to \id{user\_data}
  pointers to \id{u\_scale} and/or \id{f\_scale} as needed.
  The unit roundoff can be accessed as \id{UNIT\_ROUNDOFF} defined in
...
user-supplied {\CC} function of type \ID{KINSpilsPrecSetupFn}, defined as follows:

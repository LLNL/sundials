...
solver modules available for use with {\kinsol} are:
%%
\begin{itemize}
\item \Id{kinsol/kinsol\_direct.h}, 
  which is used with the {\kindls} direct linear solver interface to
  ...
  \item \Id{sunlinsol/sunlinsol\_lapackdense.h},
    which is used with the LAPACK package dense linear solver interface module,
    {\sunlinsollapdense}; 
    
  \item \Id{sunlinsol/sunlinsol\_lapackband.h},
    which is used with the LAPACK package banded linear solver interface module,
    {\sunlinsollapband}; 
...
\item \Id{kinsol/kinsol\_spils.h}, 
  which is used with the {\kinspils} iterative linear solver interface to
...
\item\label{i:matrix} 
  ...
  If a direct linear solver is to be used within a Newton or Picard iteration,
...
\item\label{i:lin_solver_interface} 
  ...
  If a Newton or Picard iteration is chosen, initialize the {\kindls} or
  {\kinspils} linear solver interface by attaching the linear solver
  object (and matrix object, if applicable)
  with one of the following calls (for details see \S\ref{sss:lin_solv_init}):

  \id{ier = }\Id{KINDlsSetLinearSolver}\id{(...);}

  \id{ier = }\Id{KINSpilsSetLinearSolver}\id{(...);}

\item
  {\bf Set linear solver interface optional inputs}

  Call \id{KINDlsSet*} or \id{KINSpilsSet*} functions to change optional
...
linear systems of the form $J\delta = -F$. There are two {\kinsol} linear
solvers currently available for this task: {\kindls} and {\kinspils}.
...
in \S\ref{ss:optional_input} and \S\ref{ss:user_fct_sol}.
A preconditioner matrix $P$ must approximate the Jacobian $J$, at least
crudely.  

With any of the Krylov solvers, only right preconditioning is supported
through the {\kinspils} interfaces.
...
Jacobian matrix $J$.  For the specification
of a preconditioner, see the iterative linear solver sections 
in \S\ref{ss:optional_input} and \S\ref{ss:user_fct_sol}.
A preconditioner matrix $P$ must approximate the Jacobian $J$, at least
crudely.  
...
either of the functions \Id{KINDlsSetLinearSolver} or
\Id{KINSpilsSetLinearSolver}, as documented below.  The first argument
passed to these functions is the {\kinsol} memory pointer returned by
\id{KINCreate}; the second argument passed to these functions is the
desired {\sunlinsol} object to use for solving Newton or Picard systems.  A call
to one of these functions initializes the appropriate {\kinsol} linear
solver interface, linking this to the main {\kinsol} solver, and
...
\index{KINSOL@{\kinsol} linear solver interfaces!selecting one|)}

\index{KINSOL@{\kinsol} linear solver interfaces!KINDLS@{\kindls}}
\index{KINDLS@{\kindls} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{KINDlsSetLinearSolver}
{
  flag = KINDlsSetLinearSolver(kin\_mem, LS, J);
}
{
  The function \ID{KINDlsSetLinearSolver} attaches a direct
  {\sunlinsol} object \id{LS} and corresponding template
  Jacobian {\sunmatrix} object \id{J} to {\kinsol}, initializing the
  {\kindls} direct linear solver interface.

  The user's main program must include the \id{kinsol\_direct.h} header file.
}
{
  \begin{args}[kin\_mem]
  \item[kin\_mem] (\id{void *})
...
}
{
  ...
  \item[\Id{KINDLS\_SUCCESS}] 
    The {\kindls} initialization was successful.
  \item[\Id{KINDLS\_MEM\_NULL}]
    The \id{kin\_mem} pointer is \id{NULL}.
  \item[\Id{KINDLS\_ILL\_INPUT}]
    The {\kindls} solver is not compatible with the current {\nvector} module.
  \item[\Id{KINDLS\_MEM\_FAIL}]
    A memory allocation request failed.
  \end{args}
}
{
  The {\kindls} linear solver is not compatible with all
  implementations of the {\sunlinsol} and {\nvector}
  modules.  Specifically, {\kindls} requires use of a \emph{direct}
...
%%
%%
\index{KIN@{\kinsol} linear solver interfaces!KINSPILS@{\kinspils}}
\index{KINSPILS@{\kinspils} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{KINSpilsSetLinearSolver}
{
  flag = KINSpilsSetLinearSolver(kin\_mem, LS);
}
{
  The function \ID{KINSpilsSetLinearSolver} attaches an iterative
  {\sunlinsol} object \id{LS} to {\kinsol}, initializing the
  {\kinspils} scaled, preconditioned, iterative linear solver
  interface.

  The user's main program must include the \id{kin\_spils.h} header
  file.
}
{
  \begin{args}[kin\_mem]
...
}
{
  ...
  \item[\Id{KINSPILS\_SUCCESS}] 
    The {\kinspils} initialization was successful.
  \item[\Id{KINSPILS\_MEM\_NULL}]
    The \id{kin\_mem} pointer is \id{NULL}.
  \item[\Id{KINSPILS\_ILL\_INPUT}]
    The {\kinspils} solver is not compatible with the \id{LS} object
    or is incompatible with the current {\nvector} module.
  \item[\Id{KINSPILS\_MEM\_FAIL}]
    A memory allocation request failed.
  \item[\Id{KINSPILS\_SUNLS\_FAIL}]
    A call to the \id{LS} object failed.
  \end{args}
}
{
  The {\kinspils} linear solver interface is not compatible with all
  implementations of the {\sunlinsol} and {\nvector}
  modules.  Specifically, {\kinspils} requires use of an
...
\begin{table}
\centering
\caption{Optional inputs for {\kinsol}, {\kindls}, and {\kinspils}}
...
\hline
Jacobian function & \id{KINDlsSetJacFn} &  DQ \\
\hline
...
}
{
  ...
  modules (meaning {\kindense}, {\kinband}, {\kinklu}, and {\kinsuperlumt}).
}
%%
%%
...
}
{
  ...
  product [see Eq.(\ref{e:sigmaDQ_iterative})].
}
{
  \begin{args}[relfunc]
  \item[kin\_mem] (\id{void *})
...
%%==============================================================================
\subsubsection{Direct linear solver interface optional input functions}\label{sss:optin_dls}
%%==============================================================================
\index{optional input!direct linear solver interface|(}
\index{KINDLS@{\kindls} linear solver interface!optional input|(}
The \index{KINDLS@{\kindls} linear solver interface!Jacobian approximation used by}
{\kindls} solver interface needs a function to compute an approximation to
the Jacobian matrix $J(u)$.  This function must be of type \id{KINDlsJacFn}. 
...
that comes with the {\kindls} solver.
To specify a user-supplied Jacobian function \id{jac}, {\kindls} provides 
the function \id{KINDlsSetJacFn}.
The {\kindls} interface passes the pointer \id{user\_data} 
...
\ucfunction{KINDlsSetJacFn}
{
  flag = KINDlsSetJacFn(ida\_mem, jac);
}
{
  The function \ID{KINDlsSetJacFn} specifies the Jacobian
...
}
{
  ...
  \item[\Id{KINDLS\_LMEM\_NULL}]
    The {\kindls} linear solver interface has not been initialized.
  \end{args}
}
{
  By default, {\kindls} uses an internal difference quotient function
  ...
  The function type \id{KINDlsJacFn} is described in \S\ref{ss:djacFn}.
}
%%
\index{optional input!direct linear solver interface|)}
\index{KINDLS@{\kindls} linear solver interface!optional input|)}
...
\index{KINSPILS@{\kinspils} linear solver!optional input|(}
\index{preconditioning!user-supplied|(}
If any preconditioning is to be done with one of the {\kinspils} linear
solver interfaces, then the user must supply a preconditioner solve function, \id{psolve},
and specify its name in a call to to the routine \id{KINSpilsSetPreconditioner}.
\index{KINSPILS@{\kinspils} linear solver interface!preconditioner solve function}
%%
...
\index{KINSPILS@{\kinspils} linear solver interface!preconditioner setup function}
%%
...
\index{KINSPILS@{\kinspils} linear solver!Jacobian approximation used by}
...
that comes with the {\kinspils} solver interface.  A user-defined Jacobian-vector
function must be of type \id{KINSpilsJacTimesVecFn} and 
can be specified through a call to \id{KINSpilsSetJacTimes} 
...
}
{
  ...
  \item[\Id{KINSPILS\_SUNLS\_FAIL}]
    ...
    {\sunlinsol} object used by the {\kinspils} interface.
  \end{args}
}
{
...
\ucfunction{KINSpilsSetJacTimes}
{
  flag = KINSpilsSetJacTimes(kin\_mem, jsetup, jtimes);
}
{
  The function \ID{KINSpilsSetJacTimes} specifies the Jacobian-vector 
  setup and product.
}
{
  \begin{args}[kin\_mem]
  \item[kin\_mem] (\id{void *})
    pointer to the {\kinsol} memory block.
  \item[jtsetup] (\id{KINSpilsJacTimesSetupFn})
...
}
{
  ...
  \item[\Id{KINSPILS\_SUNLS\_FAIL}]
    ...
    routines in the {\sunlinsol} object used by the {\kinspils}
    interface.
  \end{args}
}
{
...
%%
%%
\index{KINSPILS@{\kinspils} linear solver interface!optional input|)}
\index{optional input!iterative linear solver|)}
...
\hline
\multicolumn{2}{|c|}{\bf KINDLS linear solver interface} \\
\hline
...
\hline
\multicolumn{2}{|c|}{\bf KINSPILS linear solver interface} \\
\hline
...
\index{KINDLS@{\kindls} linear solver interface!optional output|(}
...
%%
%%
\index{KINDLS@{\kindls} linear solver!memory requirements} 
\index{memory requirements!KINDLS@{\kindls} linear solver}
...
}
{
  ...
  {\kindls} real and integer workspace sizes.
}
{
  \begin{args}[kin\_mem]
  \item[kin\_mem] (\id{void *})
...
}
{
  ...
  matrix allocated by the user outside of {\kindls} is not included in
  this report.
...
{
  The function \ID{KINDlsGetNumJacEvals} returns the cummulative
  number of calls to the Jacobian approximation function.
}
{
  \begin{args}[kin\_mem]
  \item[kin\_mem] (\id{void *})
...
}
{
  ...
  (\id{KINSolve} returned \id{KIN\_LSETUP\_FAIL}), then the value of
...
%%==============================================================================
\subsubsection{Iterative linear solver interface optional output functions}\label{sss:optout_spils}
%%==============================================================================
\index{optional output!iterative linear solver interface|(}
\index{KINSPILS@{\kinspils} linear solver interface!optional output|(} 
...
setup and product routines, number of calls to the system function routine for
difference quotient Jacobian-vector product approximation, and last
return value from a linear solver function.
%%
%%
\index{KINSPILS@{\kinspils} linear solver interface!memory requirements} 
\index{memory requirements!KINSPILS@{\kinspils} linear solver interface}
...
%%
%%
\ucfunction{KINSpilsGetNumJTSetupEvals}
{
  flag = KINSpilsGetNumJTSetupEvals(kin\_mem, \&njtsetup);
}
{
  The function \ID{KINSpilsGetNumJTSetupEvals} returns the
...
}
{
  ...
  \id{SUNLS\_PSET\_FAIL\_UNREC}, \id{SUNLS\_ASET\_FAIL\_UNREC},   
  \id{SUNLS\_PACKAGE\_FAIL\_UNREC}, 

  If the {\kinspils} solve function failed (\id{KINSol} returned
  \id{KIN\_LSOLVE\_FAIL}), \id{lsflag} contains the error return flag from
  the {\sunlinsol} object, which will be one of the following: \\
...
%%
%%
\ucfunction{KINSpilsGetReturnFlagName}
{
  name = KINSpilsGetReturnFlagName(lsflag);
}
{
  The function \ID{KINSpilsGetReturnFlagName} returns the
  name of the {\kinspils} constant corresponding to \id{lsflag}.
}
{
  The only argument, of type \id{long int}, is a return flag from an {\kinspils} function.
}
{
...
(\Id{KINDlsSetLinearSolver} is called in teh step describd in 
\S\ref{ss:skeleton_sol}), the user may provide a function of type 
\ID{KINDlsJacFn} defined as follows
%%
\usfunction{KINDlsJacFn}
{
  typedef int (*KINDlsJacFn)(&N\_Vector u, N\_Vector fu,\\
...
}
{
  ...
  If the user's \id{KINDlsJacFn} function uses difference quotient
  ...
  dense matrix \id{Jac} with an approximation to the Jacobian matrix $J(u)$
 ...
 {\bf banded}:\\
  ...
  banded matrix \id{Jac} with an approximation to the Jacobian matrix $J(u)$
  at the point (\id{u}).
...
\index{Jacobian approximation function!Jacobian times vector!user-supplied|(}
%%
If the {\kinspils} solver interface is selected
(i.e., \id{KINSpilsSetLinearSolver} is called in the steps described in
\S\ref{ss:skeleton_sol}), the user may
provide a function of type \Id{KINSpilsJacTimesVecFn} in the following form,
...
{ If a user-defined routine is not given, then an internal \id{jtimes}
  ...
  If the user's \id{KINSpilsJacTimesVecFn} function uses difference quotient
  approximations, it may need to access quantities not in the call list.
...
user-supplied function of type \ID{KINSpilsPrecSetupFn}, defined as follows:
...
}
{
  ...
  If the user's \id{KINSpilsPrecSetupFn} function uses difference quotient
  approximations, it may need to access quantities not in the call list.
  These might include the scale vectors and the unit roundoff.
  To obtain the scale vectors, the user will need to add to \id{user\_data}
  pointers to \id{u\_scale} and/or \id{f\_scale} as needed.
...
{
  typedef void (*KINBBDLocalFn)(&sunindextype Nlocal, N\_Vector u, \\
...
{
  A \id{KINBBDLocalFn} function type does not have a return value.
}
{
...
{
  typedef void (*KINBBDCommFn)(sunindextype Nlocal, N\_Vector u, void *user\_data);
}
{
...
{
  A \id{KINBBDCommFn} function type does not have a return value.
}
{
...
\item
  \textcolor{gray}{\bf Initialize {\kinsol} solver}
...
\item
  ...
  or solve function through calls to \id{KINSpilsSetPreconditioner} 

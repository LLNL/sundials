...
\item
  ...
  {\p} {\h} Set \id{Nlocal}, the local vector length (the sub-vector length for this process).
...
\item
  ...
  {\omp} \id{u = N\_VMake\_OpenMP(N, num\_threads, ydata);}

  {\pt} \id{u = N\_VMake\_Pthreads(N, num\_threads, ydata);}

  {\p} \id{u = N\_VMake\_Parallel(comm, Nlocal, N, udata);}

  {\h} \id{u = N\_VMake\_ParHyp(comm, Nlocal, N, udata);}

  Otherwise, make the call:

  {\s} \id{u = N\_VNew\_Serial(N);}

  {\omp} \id{u = N\_VNew\_OpenMP(N, num\_threads);}

  {\pt} \id{u = N\_VNew\_Pthreads(N, num\_threads);}

  {\p} \id{u = N\_VNew\_Parallel(comm, Nlocal, N);}

  {\h} \id{u = N\_VNew\_ParHyp(comm, Nlocal, N);}

  and load initial values into the structure defined by:

  {\s} \id{NV\_DATA\_S(u)}

  {\omp} \id{NV\_DATA\_OMP(u)}

  {\pt} \id{NV\_DATA\_PT(u)}
...
\item
  ...
  {\omp} \id{N\_VDestroy\_OpenMP(u);}

  {\pt} \id{N\_VDestroy\_Pthreads(u);}

  {\p} \id{N\_VDestroy\_Parallel(u);}

  {\h} \id{N\_VDestroy\_ParHyp(u);}

...
programs in the companion document \cite{kinsol_ex} may also be helpful.
...
the heeader file \id{kinsol\_direct.h} which defines common functions and which in turn 
includes a header file (\id{sundials\_direct.h}) which defines the matrix type for these 
direct linear solvers (\id{DlsMat}), as well as various functions and macros acting on such matrices.
...
For example, in the \id{kinFoodWeb\_kry} example~\cite{kinsol_ex}, preconditioning is 
done with a block-diagonal matrix. For this, even though the {\kinspgmr} linear
solver is used, the header \id{sundials\_dense.h} is included for access to the
underlying generic dense linear solver.
...
\item\label{i:lin_solver} 
  ...
  {\s} \id{flag = }\Id{KINLapackDense}\id{(...);}

  {\s} \id{flag = }\Id{KINLapackBand}\id{(...);}
...
}
{
  ...
  \item[func] (\Id{KINSysFn})
    ...
    This function has the form \id{func(u, fval, user\_data)} 
...
{\kinspbcg}, and {\kinsptfqmr}, with the first two providing
a choice of either internal or Lapack implementations.

The first two are direct solvers and their names indicate the type of approximation
used for the Jacobian $J = \partial{F}/\partial{u}$; 
{\kindense} and {\kinband} work with dense and banded approximations to $J$, respectively.
The {\sundials} suite includes both internal implementations of these two linear solvers
and interfaces to Lapack implementations.
Together, these linear solvers are refered to as {\kindls} (from direct linear solvers).

The last three {\kinsol} linear solvers, {\kinspgmr}, {\kinspbcg}, and {\kinsptfqmr},
...
Together, they are referred to as {\kinspils} (from scaled preconditioned 
...
of the functions \Id{KINDense} or \Id{KINLapackDense}, 
\Id{KINBand} or \Id{KINLapackBand}, \Id{KINSpgmr},
...
In each case, with the exception of the Lapack-based direct solvers,
...
}
{
  ...
  \begin{args}[KINDIRECT\_ILL\_INPUT]
  \item[\Id{KINDIRECT\_SUCCESS}] 
    The {\kindense} initialization was successful.
  \item[\Id{KINDIRECT\_MEM\_NULL}]
    The \id{kin\_mem} pointer is \id{NULL}.
  \item[\Id{KINDIRECT\_ILL\_INPUT}]
    The {\kindense} solver is not compatible with the current {\nvector} module.
  \item[\Id{KINDIRECT\_MEM\_FAIL}]
...
}
{
  ...
  The user's main function must include the \id{kinsol\_lapack.h} header file.
}
{
  The input arguments are identical to those of \id{KINDense}.
}
{
...
{
  The input arguments are identical to those of \id{KINBand}.
}
{
  The values of the return flag \id{flag} (of type \id{int}) are identical
...
{
  The default value for \id{user\_data} is \id{NULL}.
}
%%
%%
...
using global data in the program.  The pointer \id{use\_data} may be
...
{
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[KINDIRECT\_LMEM\_NULL]
  \item[\Id{KINDIRECT\_SUCCESS}] 
    The optional value has been successfully set.
  \item[\Id{KINDIRECT\_MEM\_NULL}]
    The \id{kin\_mem} pointer is \id{NULL}.
  \item[\Id{KINDIRECT\_LMEM\_NULL}]
...
{
  flag = KINDlsSetBandJacFn(cvode\_mem, bjac);
}
{
...
\newlength{\colAA}
\settowidth{\colAA}{No. of fct. calls for finite diff. Jacobian-vector evals.}
\newlength{\colBB}
...
No. of r.h.s. calls for finite diff. Jacobian evals. & \id{KINDlsGetNumFuncEvals} \\ 
...
{
  The function \ID{KINDlsGetNumFuncEvals} returns the
...
}
{
  ...
  \item[user\_data]
    is a pointer to user data, same as the pointer \Id{user\_data}      
...
(i.e. \Id{KINDense} or \id{KINLapackDense} is called in Step \ref{i:lin_solver} 
...
({\em i.e.}, \Id{KINBand} or \id{KINLapackBand} is called in Step \ref{i:lin_solver}
of \S\ref{s:skeleton_sol}), the user may provide a function of type 
\ID{KINDlsBandJacFn} defined as follows:
...
}
{
  ...
  of type \Id{DlsMat} with the elements of the Jacobian $J$ at \id{u}.
...
%%
The user-callable function that initialize (steps \ref{i:bbdpre_init},
...
}
{
  ...
  \item[KINSPILS\_SUCCESS]
    The call to \id{KINBBDPrecInit} was successfull.
  \item[\id{KINSPILS\_MEM\_NULL}] 

...
\item
  ...
  {\s} Set \id{N}, the problem size $N$ (of type \id{int}).

  {\p} Set \id{Nlocal} (of type \id{int}), the local vector length (the
  sub-vector length for this process). Set \id{N} (of type \id{int}), the global
  vector length (the problem size $N$, and the sum of all the values of \id{Nlocal}).
  Set the active set of processes.
...
\Id{KINSpgmr}, \Id{KINSpbcg}, or \Id{KINSptfqmr}, as documented below.
...
}
{
  ...
  \item[noInitSetup] (\id{booleantype})
    ...
    function is made (pass \id{FALSE}) or not made (pass \id{TRUE}).
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of:
...
{
flag = KINSetMAA(kin\_mem, maa, setaamem);
}
{
...
}
{
  ...
  \item[maa] (\id{long int})
    subspace size for various methods.
  \item[setaamem] (\id{booleantype})
    needs to be TRUE or 1 when using Anderson Acceleration, FALSE or 0 for the
    other methods
  \end{args}
}
{
  ...
  \item[\Id{KIN\_MEM\_NULL}]
    The \id{kin\_mem} pointer is \id{NULL}.
  \item[\Id{KIN\_ILL\_INPUT}]
    The argument \id{maa} was negative or zero.
  \end{args}
}
{
  This function sets the subspace size, which needs to be $> -1$ for Anderson Acceleration.
  It enables additional memory allocation necessary for Anderson Acceleration.
}
%%
%%
%%
\ucfunction{KINSetAAStopCrit}
{
flag = KINSetAAStopCrit(kin\_mem, setstop);
}
{
  The function \ID{KINSetAAStopCrit} allows the user to define a 
  stopping criterion through the user-provided function that evaluates 
  the nonlinear system function $F(u)$ or $G(u)$.
}
{
  \begin{args}[kin\_mem]
  \item[kin\_mem] (\id{void *})
    pointer to the {\kinsol} memory block.
  \item[setstop] (\id{booleantype})
    parameter indicates that user will supply stopping criterion.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of:
...
{
  This function will allow the user to set a different stopping criterion for
  Picard with or without Anderson Acceleration.
  The vector to be tested needs to be provided in the function defined by
  \ID{KINSetSysFunc}. 
}
%%
%%
...
}
{
  ...
  \item[u]
    is the current value of the variable vector, $u$. For fixed point and Picard
    iterations, it is possible to use a 
    different stopping criterion than the default test, by returning the 
    value that should be tested in $u$ and calling \ID{KINSetAAStopCrit}. 
    The stopping criterion is then
    $\| u \|_{D_F,\infty} < {\sc FTOL}$, 
    where $D_F$ should be a suitable scaling matrix or the identity matrix.
  \item[fval]

...
When using ERKStep as the inner (fast) integrator with MRIStep, the
utility function :c:func:`ERKStepCreateMRIStepInnerStepper` should be used to
wrap an ERKStep memory block as an :c:type:`MRIStepInnerStepper`.

.. c:function:: int ERKStepCreateMRIStepInnerStepper(void *inner_arkode_mem, MRIStepInnerStepper *stepper)

   Wraps an ERKStep memory block as an :c:type:`MRIStepInnerStepper` for use
   with MRIStep.
   ...
   **Example usage:**
      .. code-block:: C
         ...
         inner_arkode_mem = ERKStepCreate(ff, t0, y0, sunctx);

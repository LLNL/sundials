...
section :numref:`Installation` for further details).



.. _Usage.ARKStep.DataTypes:
...
configuration stage (see the section :numref:`Installation.CMake.Options`).
...
the correct precision (for details see :numref:`Installation`).
...
- ``arkode/arkode_arkstep.h``, the main header file for the ARKStep
  time-stepping module, which defines several types and various
...
parallelism.  See section :numref:`NVectors` for details for the
...
the nonlinear solver module (see section :numref:`SUNNonlinSol` for

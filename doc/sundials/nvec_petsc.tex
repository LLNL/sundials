...
\item \ID{N\_VNewEmpty\_petsc}
  ...
  the wrapped {\petsc} vector set to (\id{NULL}). It is used by 
...
\item \ID{N\_VMake\_petsc}
  ...
  wrapper around user-provided {\petsc} vector. It does {\em not} 
  allocate memory for the \id{pvec} itself.
...
\item \ID{N\_VGetVector\_petsc}
  ...
  This function returns pointer to underlying {\petsc} vector.
...
\item
  ...
  it is recommeded to extract {\petsc} vector via       

...
hardware abstraction layer. In this implementation, {\raja}
allows for {\sundials} vector kernels to run on GPU devices. It is intended for users 
who are already familiar with {\raja} and GPU programming. Building this vector 
module requires C++11 compliant compiler and CUDA software development toolkit. 
Besides {\cuda} backend, {\raja} has other backends such as serial, OpenMP 
and OpenAC. These backends are not used in this {\sundials} release.
Class \id{Vector} in namespace \id{sunrajavec} manages vector data layout. 
\begin{verbatim} 
...
progress in of {\raja} development, we expect that \id{sunrajavec::Vector}
...
\id{sunrajavec::Vector} class without requiring changes to user API.
...
%%--------------------------------------------
%%
The {\nvecraja} module defines implementations of all vector operations listed 

...
}
{
  ...
  \id{SUNNONLINERASOLVER\_ROOTFIND} modules or $G(y)$ in $G(y)=y$ for
  \id{SUNNONLINEARSOLVER\_STATIONARY} modules.
}
{
  \begin{args}[SysFn]
  \item[NLS] (\id{SUNNonlinearSolver})
...
{
  The \textit{optional} function \ID{SUNNonlinSolSetLSolveFn} is used
...
The {\sundials} packages provide a {\sunnonlinsol} module with
...
}
{
  ...
  for \id{SUNNONLINEARSOLVER\_STATIONARY} type modules. Memory
...
}
{
  ...
  \item[jcur]
    is the Jacobian current (\id{SUNTRUE}) or stale (\id{SUNFALSE}).
  \item[mem]
    is the {\sundials} package memory structure.
  \end{args}
}
...
}
{
  ...
  \item[\id{*\_MEM\_NULL}]
    the {\sunnonlinsol} or {\sundials} package memory was \id{NULL}
  \end{args}
  In the above return codes \id{*} is a {\sundials} package specific
  prefix (\id{CV} for {\cvode} or {\cvodes}, \id{IDA} for {\ida} or
  {\idas}, and \id{ARK} for {\arkode}).
...
namely \id{SUNNonlinSolInitialize}, which initializes a
...
\item Define and implement additional user-callable routines acting on
  a newly created \id{SUNNonlinearSolver} object that are not defined
...
\item Provide functions as needed for the particular {\sunnonlinsol}
  ...
  structure of the newly defined \id{SUNNonlinearSolver} object 

...
solver statistics. The routines to get number of iterations in the most recent
solve (\id{SUNNonlinSolGetNumIters}) and number of convergence failures
...
{
  The \textit{optional} function \ID{SUNNonlinSolGetNumIters} returns the number
  of nonlinear solver iterations in the most recent solve. This is typically
  called by the {\sundials} integrator to store the nonlinear solver statistics,
  but may also be called by the user.
}
{
  \begin{args}[numiters]
  \item[NLS] (\id{SUNNonlinearSolver})
...
}
{
  ...
  the number of nonlinear solver convergence failures in the most recent solve.

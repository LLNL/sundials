...
}
{
  ...
  following:
  \begin{args}[SUNLINEARSOLVER\_MATRIX\_ITERATIVE]
  \item[\Id{SUNLINEARSOLVER\_DIRECT}]
    \id{0}, the {\sunlinsol} module requires a matrix, and computes an
    `exact' solution to the linear system defined by that matrix.
  \item[\Id{SUNLINEARSOLVER\_ITERATIVE}]
    \id{1}, the {\sunlinsol} module does not require a matrix (though one may be
    ...
    system defined by the package-supplied \id{ATimes} routine
    (see \newline
    \Id{SUNLinSolSetATimes} below), even if that linear system differs from
  ...
  \item[\Id{SUNLINEARSOLVER\_MATRIX\_ITERATIVE}]
    \id{2}, the {\sunlinsol} module requires a matrix, and computes an inexact
    ...
    \id{ATimes} routine. As the solver computes the solution only inexactly (or

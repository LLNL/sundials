...
\item \id{NewIndexArray}: allocation of an array of \id{long int} integers for use
...
\item \ID{newIndexArray}
  \par \id{newIndexArray(n)} allocates an array of \id{n} integers, all \id{long int}.
...
\item \ID{destroyArray}
  \par \id{destroyArray(p)} frees the array \id{p} allocated by \id{newIndexArray},
...
\item \id{DestroyMat}: free memory for a \id{DlsMat} matrix;
\item \id{PrintMat}: print a \id{DlsMat} matrix to standard output.
\item \id{NewIndexArray}: allocation of an array of \id{int} integers for use

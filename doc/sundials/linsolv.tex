...
\begin{description}
  ...
  \item[NP]  - number of index pointers (e.g. number of column pointers for 
    CSC matrix). For CSC matrix $NP=N$ and for CSR matrix $NP=M$. This 
    value is set automatically based on user setting of \verb|sparsetype|. 
  \item[data]  - pointer to a contiguous block of \id{realtype}
    variables (of length \id{NNZ}), containing the values of the
    nonzero entries in the matrix
  \item[sparsetype]  - type of the sparse matrix (CSC or CSR)
  \item[indexvals] - pointer to a contiguous block of \id{int} variables
    (of length \id{NNZ}), containing the row indices of CSC (or 
    column indices of CSR) matrix of each nonzero entry held in \id{data}
  \item[indexptrs]  - pointer to a contiguous block of \id{int}
    variables (of length \id{NP+1}). When matrix is of CSC type each 
    ...
    \id{data} and \id{rowvals} arrays. For CSR matrices, each entry provides 
    the index of the first row entry into the \id{data} and \id{indexvals} 
    arrays.
\end{description}
\noindent Following pointers are added to \id{SlsMat} type for user convenience, 
to provide more intuitive interface to sparse matrix data structure. They are set 
automatically by \id{NewSparseMat} function, based on sparse matrix storage type. 
\begin{description}
  \item[rowvals] - pointer to \verb|indexvals| when CSC type is selected,
    otherwise set to \verb|NULL|.
  \item[colptrs] - pointer to \verb|indexptrs| when CSC type is selected,
    otherwise set to \verb|NULL|.
  \item[colvals] - pointer to \verb|indexvals| when CSR type is selected,
    otherwise set to \verb|NULL|.
  \item[rowptrs] - pointer to \verb|indexptrs| when CSR type is selected,
    otherwise set to \verb|NULL|.
\end{description}
...
\item \ID{NewSparseMat}
  \par \id{NewSparseMat(M, N, NNZ, sparsetype)} allocates storage for an \id{M} by \id{N}
...
\item \ID{CopySparseMat}
  ...
  storage to hold all the nonzero entries of \id{A}, the data and row 

...
\item \id{NewIndexArray}: allocation of an array of integers for use
...
\item \ID{newIndexArray}
  \par \id{newIndexArray(n)} allocates an array of \id{n} integers, all \id{indextype}.
...
\item \ID{newIndexArray}
  \par \id{newIndexArray(n)} allocates an array of \id{n} integers, all \id{sunindextype}. 
  It returns a pointer to the first element in the array if successful. 
  It returns \id{NULL} if the memory request could not be satisfied.

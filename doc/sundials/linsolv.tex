...
of type \id{SlsMat} need not be square): 
\begin{description}
  \item[M]  - number of rows
  \item[N]  - number of columns
...
% \caption[Diagram of the storage for a compressed-sparse-column matrix of type \id{SlsMat}]
%   {Diagram of the storage for a compressed-sparse-column matrix of type \id{SlsMat}. Here \id{A} is an
%   $M \times N$ band matrix of type {\id SlsMat} with a maximum of {\id NNZ} 
%   nonzero entries.  The row indices and column pointer indices are
%   numbered from $0$ to $M-1$ and $0$ to $N-1$, respectively, 
%   and the ($i,j$)-th element of \id{A} is denoted \id{A(i,j)}. Here,
%   although {\id NNZ} values are allocated, only {\id nnz} are actually
%   filled in; the greyed-out portions of {\id data} and {\id rowvals}
%   indicate extra allocated space.}\label{f:cscmat}
% \end{figure}
%%
%%
...
appropriately to link with {\klu} (see Section \ref{c:install} for details).
...
{\superlumt} (see Section \ref{c:install} for details).

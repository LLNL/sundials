...
Note in both cases that the final value in \id{colptrs} is $8$.  The work 
...
\caption[Diagram of the storage for a compressed-sparse-column matrix
  ...
  denoted as \id{A(i,j)}.  The \id{colptrs} array contains $N+1$
...
factorization and teh second is a numeric 
...
in shared memory paralell environments.  It should be noted that threads are

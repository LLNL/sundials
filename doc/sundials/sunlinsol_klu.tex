...
\item \id{SUNLinSolInitialize\_KLU} -- this sets the
  \id{first\_factorize} flag to 1, forcing bot symbolic and numerical
...
4 for {\arkode}); \id{ier} is an error return flag equal 0 for success
and -1 for failure (declared so as to match C type \id{int}).  This
...
Additionally, the \id{SUNKLUReInit} and \ID{SUNKLUSetOrdering}

...
\sunmodfun{N\_VSetKernelExecPolicy\_Cuda}
{
  ...
  the vector is setup to use the \id{SUNCudaStreamingExecPolicy} and 
  \id{SUNCudaReduceExecPolicy}. Any custom execution policy must ensure that
  ...
  below for more information about the \id{SUNCudaExecPolicy} class.
...
the abstract class and implements the methods. The {\sundials} provided \id{sundials::CudaStreamingExecPolicy}
...
A user can also utilize the provided \id{sundials::CudaStreamingExecPolicy}
(aka globally \id{SUNCudaStreamingExecPolicy}) and \id{sundials::CudaReduceExecPolicy},
(aka globally \id{SUNCudaReduceExecPolicy}) which allow the the user to set the block size

...
defines the {\em content} field of \id{N\_Vector} to be a structure
containing the global and local lengths of the vector, a pointer to an
object of type \id{hypre\_ParVector}, a pointer to the beginning of a
contiguous local data array (which corresponds with the local data in
the \id{hypre\_ParVector} {\em x}), an {\mpi} communicator, a boolean
...
vector. The suffix \id{\_PH} in the names denotes the parallel HYPRE
...
\item 
  ...
  structure of type \id{struct \_N\_VectorContent\_ParHyp}.

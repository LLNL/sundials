...
\caption[Diagram of the storage for a compressed-sparse-column matrix] 
  ...
  matrix. Here \id{A} is an $M \times N$ sparse matrix with storage
  ...
  The \id{indexptrs} array contains $N+1$ entries; the first $N$
...
\item \ID{SUNSparseMatrix}
  ...
  stored in the matrix, \id{NNZ}, and a flag indicating whether to use
  CSR or CSC format, \id{CSR\_MAT} or \id{CSC\_MAT}.
...
\item \ID{SUNSparseMatrix\_NNZ}
  ...
  This function returns the allocated nonzero storage for the sparse \id{SUNMatrix}.
...
\item \ID{SUNSparseMatrix\_IndexPointers}
  ...
  sparse \id{SUNMatrix}: for CSR this is the location of the first

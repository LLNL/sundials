...
The type \id{sunindextype} can be 64- or 32-bit \emph{signed} integer.
The default is portable \id{int64\_t} type, and the user can change it 
to \id{int32\_t} at the configuration stage. The configuration system 
will detect if the compiler does not support portable types, and it 
will replace \id{int64\_t} and \id{int32\_t} with \id{long long} and \id{int}, 
respectively. At this time {\sundials} does not have a configuration option
for setting unsigned integer types for vector and matrix indices (although
it would not be difficult to add it, if there is a demand).

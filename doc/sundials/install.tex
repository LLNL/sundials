...
\item[\id{EXAMPLES\_INSTALL}] - 
  ...
  Default: ON
  \\
  Note: This option is triggered when building any of the {\sundials}
...
\item[\id{MPI\_MPICXX}] - 
  ...
  solvers can be used from {\CPP} applications by default
...
{\warn} When xSDK options are active, they will overwrite the the
...
make the corresponding {\sundials} options advanced variables that are
hidden by default. During configuration, messages are output detailing
...
\item[\id{USE\_XSDK\_DEFAULTS}] - 
  ...
  {\sundials} equivalent: N/A
  \\
  Note: Enabling xSDK defaults also set \id{CMAKE\_BUILD\_TYPE} to \id{Debug}
...
\item[\id{XSDK\_INDEX\_SIZE}] -
  Integer size (bits) used for indices {\sundials}, options are: \id{32} or \id{64}
  \\
  Default: \id{32}
  \\
...
{\sundials} does not utilize BLAS directly but it may be need by other
...
will find the corresponding BLAS library when searching for LAPACK.
...
variable can be set to the desired libraries. {\warn}Note when setting

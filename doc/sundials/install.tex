...
(\id{.tgz}). The name of the distributin archive is of the form {\em solver}\id{-x.x.x.tgz},
where {\em solver} is one of: \id{sundials}, \id{cvode}, \id{cvodes}, \id{ida}, or \id{kinsol},
and \id{x.x.x} represents the version number (of the {\sundials} suite or of the individual solver).

To begin the installation, first uncompress and expand the sources, by issuing
\begin{verbatim}
   % tar xzf solver-x.x.x.tgz
\end{verbatim}
This will extract source files under a directory {\em solver}\id{-x.x.x}.
...
In the sequel, we make the following distinctions:
\begin{itemize}

\item {\em srcdir} 

  is the directory {\em solver}\id{-x.x.x} created above; i.e. the 
  directory containing the {\sundials} sources.

\item {\em builddir}

  is the directory under which {\sundials} is built; i.e. the directory
  from within which the \id{configure} command is issued. Commonly, this is the same as
  {\em srcdir}.
...
\end{itemize}
{\noindent}{\warn}NOTE: The installation directory {\em instdir} should {\bf not} be the same as
...
If disk space is a priority, to delete all files created by building {\sundials}, issue
\begin{verbatim}
   % make clean
\end{verbatim}
...
\item \id{--enable-examples}
  ...
  Available example programs are {\bf not} built by default. Use this option
...
\item \id{--enable-sundialsTB}
  ...
  The {\sundialsTB} Matlab toolbox is {\bf not} built by default. Use this option
...
\item \id{--with-sundialsTB-instdir=STBINSTDIR}
  ...
  files will be installed in a directory \id{STBINSTDIR/sundialsTB}.
...
\item \id{--with-mex=ARG}
  ...
  Specify the \id{mex} compilr to be used.
...
and also build the Matlab \id{mex} files for {\sundialsTB} and install it under 
...
{\em source\_tree}\id{/include/}{\em solver}
\item implementation header files and source files located under
{\em source\_tree}\id{/src/}{\em solver}
\item (optional) {\F}/{\C} interface files located under
{\em source\_tree}\id{/src/}{\em solver}\id{/fcmix}
\item shared public header files located under
{\em source\_tree}\id{/include/sundials}
\item shared source files located under
{\em source\_tree}\id{/src/sundials}
\item (optional) {\nvecs} header and source files located under
{\em source\_tree}\id{/include/nvector} and {\em source\_tree}\id{/src/nvec\_ser} 
\item (optional) {\nvecp} header and source files located under
{\em source\_tree}\id{/include/nvector} and {\em source\_tree}\id{/src/nvec\_par} 

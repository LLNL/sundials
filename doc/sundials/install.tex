...
\item[\id{MAGMA\_DIR}] -
  ...
  \\
  Default: none
\item[\id{SUNDIALS\_MAGMA\_BACKENDS}] -
...
\item[\id{ENABLE\_RAJA}] -
  ...
  Default: OFF
  \\
  Note: You need to enable CUDA, HIP, or SYCL in order to build the {\raja}
  vector module.
...
version 0.14.0. Building {\sundials} {\raja} modules requires a {\cuda}, HIP, or
SYCL enabled {\raja} installation. To enable {\raja}, set \id{ENABLE\_RAJA} to
\id{ON}, set \id{SUNDIALS\_RAJA\_BACKENDS} to the desired backend (\id{CUDA},
\id{HIP}, or \id{SYCL}), and set \id{ENABLE\_CUDA}, \id{ENABLE\_HIP}, or
\id{ENABLE\_SYCL}, to \id{ON} depending on the selected backend. If {\raja} is
installed in a nonstandard location you will be prompted to set the variable
\id{RAJA\_DIR} with the path to the {\raja} CMake configuration file. To enable
building the {\raja} examples set \id{EXAMPLES\_ENABLE\_CXX} to \id{ON}.

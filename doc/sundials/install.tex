...
\item[\id{CMAKE\_Fortran\_COMPILER}] - 
  ...
  Blas/Lapack support is enabled (\id{TPL\_ENABLE\_LAPACK} is ON).
...
To enable LAPACK and BLAS libraries, set the \id{TPL\_ENABLE\_LAPACK} option to \id{ON}.
If the directory containing the LAPACK and BLAS libraries is in the \id{LD\_LIBRARY\_PATH} environment
variable, CMake will set the \id{TPL\_LAPACK\_LIBRARIES} variable accordingly, otherwise CMake
will attemp to find the LAPACK and BLAS libraries in standard system locations.
To explicitly tell CMake what libraries to use, the \id{TPL\_LAPACK\_LIBRARIES} varible can be
set to the desired libraries.  Example: 
\begin{verbatim}
   % cmake \
   > -DCMAKE_INSTALL_PREFIX=/home/myname/sundials/instdir \
   > -DEXAMPLES_INSTALL_PATH=/home/myname/sundials/instdir/examples \
   > -DTPL_LAPACK_LIBRARIES=/mypath/lib/liblapack.so;/mypath/lib/libblas.so \
   > /home/myname/sundials/srcdir
   %
   % make install
   % 
...
\id{HYPRE\_LIBRARY} to the \id{lib} path of the hypre installation.

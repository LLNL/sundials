...
The CMake configure will result in populating the variables: \id{AMD\_LIBRARY}, \id{AMD\_LIBRARY\_DIR},
\id{BTF\_LIBRARY}, \id{BTF\_LIBRARY\_DIR}, \id{COLAMD\_LIBRARY},
\id{COLAMD\_LIBRARY\_DIR}, and \id{KLU\_LIBRARY}
...
To enable SuperLU\_MT, set  \id{SUPERLUMT\_ENABLE} to \id{ON}, set \id{SUPERLUMT\_INCLUDE\_DIR} to the \id{SRC}
path of the SuperLU\_MT installation and set \id{SUPERLUMT\_LIBRARY\_DIR} to the \id{lib} path of the SuperLU\_MT installation.
Also, the \id{SUPERLUMT\_THREAD\_TYPE} must be set to either \id{Pthread} or \id{OpenMP}.
...
\noindent From within the installed example directory, run CMake (either with the GUI or command line) to compile the example code.
The resulting output from running the example can be compared with example output bundled in the {\sundials} distribution.

\noindent NOTE: There will likely differences in the output due to machine architecture, compiler versions,
use of third party libraries etc.{\warn} 

\noindent These examples with the installed configuration files can be used as "templates" for user developed solutions.

%%===============================================================================
\section{Configuring, building, and installing  on Windows}
%%===============================================================================

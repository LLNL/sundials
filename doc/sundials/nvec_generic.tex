...
Each {\nvector} implementation included in {\sundials} distribution has its unique 
...
\end{verbatim}
It is recommended that user defined {\nvector} implementation has its own unique 
identifier added to this enumeration.
...
& Returns the vector type identifier for vector \id{w}. It is used to determine
vector implementation type (e.g.~serial, parallel,\ldots) from abstract 
...
\begin{itemize}
\item \id{N\_VGetVectorID}: Returns vector type identifier.

...
All of the SUNDIALS packages and base classes (``N_Vector``, ``SUNMatrix``,
``SUNLinearSolver``, and ``SUNNonlinearSolver``) provide Fortran 2003 interface
modules. A complete list of class implementations with Fortran 2003 interface
modules is given in :numref:`Usage.Fortran.Modules.Table`. An interface module
can be accessed with the ``use`` statement, e.g. ``use fnvector_openmp_mod``,
...
``bind(C)`` attribute to closely follow the SUNDIALS C API (ignoring language
differences). The SUNDIALS classes, e.g. ``N_Vector``, are interfaced as Fortran
derived types, and function signatures are matched but with an ``F`` prepending
the name, e.g. ``FN_VConst`` instead of :c:func:`N_VConst`. Constants are named
exactly as they are in the C API.  Accordingly, using SUNDIALS via the Fortran
2003 interfaces looks just like using it in C. Some caveats stemming from the
language differences are discussed in Section
:numref:`Usage.Fortran.Modules.Differences`. A discussion on the topic of
equivalent data types in C and Fortran 2003 is presented in Section
:numref:`Usage.Fortran.Modules.DataTypes`.

Further information on the Fortran 2003 interfaces specific to the ``N_Vector``,
``SUNMatrix``, ``SUNLinearSolver``, and ``SUNNonlinearSolver`` classes is given
alongside the C documentation (Chapters :numref:`NVectors`, :numref:`SUNMatrix`,
:numref:`SUNLinSol`, and :numref:`SUNNonlinSol` respectively). For details on
where the Fortran 2003 module (``.mod``) files and libraries are installed see
Chapter :numref:`Installation`.
...
.. _Usage.Fortran.Modules.Table:

.. table:: List of SUNDIALS Fortran 2003 interface modules

   =======================  ====================================
   **Class**                **Fortran 2003 Module Name**
   =======================  ====================================
...
.. _Usage.Fortran.Modules.DataTypes:
...
:numref:`Usage.Fortran.Modules.DataTypes.Table` presents a summary of the type
...
.. _Usage.Fortran.Modules.DataTypes.Table:
...
.. _Usage.Fortran.Modules.Differences:
...
this section, we note the most critical differences. Additionally, Section
:numref:`Usage.Fortran.Modules.DataTypes` discusses equivalencies of data types
in the two languages.


.. _Usage.Fortran.Modules.Differences.CreatingObjects:
...
.. _Usage.Fortran.Modules.Differences.ArraysAndPointers:
...
.. _Usage.Fortran.Modules.Differences.ProcedurePointers:
...
.. _Usage.Fortran.Modules.Differences.OptionalParameters:
...
Fortran ``type(c_ptr)`` (see Section :numref:`Usage.Fortran.Modules.DataTypes`),
...
.. _Usage.Fortran.Modules.Differences.NVectorArrays:
...
.. sourcecode:: Fortran
   ...
   do index = 0,count-1
     ...
     call FN_VConst(xi, 1.0)
   enddo

SUNDIALS also provides the functions :f:subr:`FN_VSetVecAtIndexVectorArray` and
:f:func:`FN_VNewVectorArray` for working with ``N_Vector`` arrays. These
functions are particularly useful for users of the Fortran interface to the
...
:f:func:`FN_VGetVecAtIndexVectorArray` are further described in
Chapter :numref:`NVectors.Description.utilities`.

.. _Usage.Fortran.Modules.Differences.FilePointers:
...
.. f:function:: FSUNDIALSFileOpen(filename, mode)
  ...
  The function argument ``filename`` is the full path to the file and has the
...
.. f:subroutine:: FSUNDIALSFileClose(fp)
  ...
  The function argument ``fp`` has the type ``type(c_ptr)`` and should be the C
  ``FILE*`` obtained from ``fopen``.


.. _Usage.Fortran.Modules.Portability:
...
compiler used to generate the modules.

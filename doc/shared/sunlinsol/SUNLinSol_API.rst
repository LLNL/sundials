...
.. c:function:: SUNLinearSolver_Type SUNLinSolGetType(SUNLinearSolver LS)
   ...
   * ``SUNLINEARSOLVER_MATRIX_EMBEDDED`` -- ``3``, the SUNLinSol module sets up
   ...
   Notes: See section :numref:`SUNLinSol.Intended` for more information
...
.. c:function:: int SUNLinSolSetup(SUNLinearSolver LS, SUNMatrix A)
   ...
   codes listed in section :numref:`SUNLinSol.ErrorCodes`.
...
.. c:function:: int SUNLinSolSolve(SUNLinearSolver LS, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
   ...
   **Return value:**  This should return zero for a successful call, a
   ...
   error codes listed in section :numref:`SUNLinSol.ErrorCodes`.
...
.. c:function:: int SUNLinSolSetScalingVectors(SUNLinearSolver LS, N_Vector s1, N_Vector s2)
   ...
   generic error codes listed in section :numref:`SUNLinSol.ErrorCodes`.
...
.. c:type:: int (*ATimesFn)(void *A_data, N_Vector v, N_Vector z)
...
.. c:type:: int (*PSetupFn)(void *P_data)
...
.. c:type:: int (*PSolveFn)(void *P_data, N_Vector r, N_Vector z, realtype tol, int lr)
...
operations defined in Sections :numref:`SUNLinSol.CoreFn` through
:numref:`SUNLinSol.GetFn`.  These routines are in fact only wrappers to
...
:numref:`SUNLinSol.linsol-matrix` we show the matrix-based linear solvers
available as ``SUNLinearSolver`` modules, and the compatible matrix
implementations.  Then in Table :numref:`SUNLinSol.linsol-vector` we
similarly show the compatibility of ``SUNLinearSolver`` modules with
available ``N_Vector`` implementations.
...
.. _SUNLinSol.linsol-vector:

Compatible SUNLinearSolver and N_Vector implementations
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
custom SUNMATRIX and SUNLinSol implementations. As SUNDIALS packages utilize generic
...
solver following the APIs described in the sections :numref:`SUNMatrix`
...
the tolerance supplied to the linear solver is adjusted to compensate; however, this
...
without creating SUNMATRIX and SUNLinSol implementations. This usage mode is
...
should call package-specific interface routines to construct the relevant system matrix

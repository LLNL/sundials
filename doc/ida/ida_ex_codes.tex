%% Serial examples

\lstset{language=C}

\newpage
\section{Listing of idadenx.c}\label{s:idadenx_c}
\includeCode{../../examples/ida/serial/idadenx.c}

\newpage
\section{Listing of idabanx2.c}\label{s:idabanx2_c}
\includeCode{../../examples/ida/serial/idabanx2.c}

\newpage
\section{Listing of idakryx.c}\label{s:idakryx_c}
\includeCode{../../examples/ida/serial/idakryx.c}

%% Parallel examples

\newpage
\section{Listing of idakryx1\_p.c}\label{s:idakryx1_p_c}
\includeCode{../../examples/ida/parallel/idakryx1_p.c}

\newpage
\section{Listing of idakryx2\_bbd\_p.c}\label{s:idakryx2_bbd_p_c}
\includeCode{../../examples/ida/parallel/idakryx2_bbd_p.c}

%% Fortran examples

\lstset{language=[77]Fortran}

\newpage
\section{Listing of fidadenx.f}\label{s:fidadenx_f}
\includeCode{../../examples/ida/fcmix_serial/fidadenx.f}

\newpage
\section{Listing of fidakryx\_bbd\_p.f}\label{s:fidakryx_bbd_p}
\includeCode{../../examples/ida/fcmix_parallel/fidakryx_bbd_p.f}

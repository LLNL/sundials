...
The \id{fidaRoberts\_dns} example is a {\F} equivalent of the \id{idaRoberts\_dns} problem.
The source program \id{fidaRoberts\_dns.f} is listed in Appendix~\ref{s:fidaRoberts_f}.
...
This example, \id{fidaHeat2D\_kry\_bbd\_p}, is the {\F} equivalent of the
\id{idaHeat2D\_kry\_bbd\_p} example.  The heat equation problem is described
...
identical to that in the \id{idaHeat2D\_kry\_p} example above.  The source file,
\id{fidaHeat2D\_kry\_bbd\_p.f}, is listed in Appendix~\ref{s:fidaHeat2D_bbd_p}.

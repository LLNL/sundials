...
\id{ida\_klu} that solves the same 2D heat PDE problem as \id{idaHeat2D\_kry}
...
to use the \id{IDAKLU} solver, the user must determine the number of non-zero
...
The \ida package also includes support for SUPERLU\_MT, the parallel sparse
direct solver. The \id{idaHeat2D\_sps} example has been included to demonstrate
SUPERLU\_MT. It is very similar to \id{idaHeat2D\_klu}.

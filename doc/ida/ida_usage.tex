...
\item
  ...
  {\omp, \pt} Set \id{num\_threads}, the number of threads to use
...
\item
  ...
  {\omp} \id{N\_VDestroy\_OpenMP(yret);}

  {\pt} \id{N\_VDestroy\_Pthreads(yret);}
...
%%==================================================================================
%%
\subsubsection{Dense direct linear solvers optional input functions}\label{sss:optin_dls}
...
For the {\idaklu} solver, we provide a reinitialization.
This reinitialization routine reinitializes the Jacobian matrix
memory for the new number of nnozeroes and sets flags for a new factorization 
...
\subsubsection{Dense direct linear solvers optional output functions}\label{sss:optout_dls}
...
}
{
  ...
  To obtain these, the user will need to add the \id{ida\_mem} 
  structure to their \id{user\_data} and use the \id{IDAGet*} 

...
\item \Id{nvector\_parhyp.h},
  which defines the shared memory parallel ParHyp implementation, {\nvecph}.
\end{itemize}
%%
Note that all of these files include in turn the header file \Id{sundials\_nvector.h} which 
...
\item
  ...
  {\nvector} implementation.  If a \id{realtype} array \id{ydata} containing
  the initial values of $y$ already exists, then make the appropriate call

  \id{y0 = N\_VMake\_***(..., ydata);}

  Otherwise, make the appropriate call

  \id{y0 = N\_VNew\_***(...);}

  and load initial values into the structure defined by \id{NV\_DATA\_***(y0)}.

  The vector \id{yp0} of initial conditions for $\dot{y}$ are set similarly.

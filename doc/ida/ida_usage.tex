...
where {\sundials} was installed (see Chapter \ref{c:install}).
...
\id{ida\_direct.h} which defines common functions and which in turn includes a header file
...
\item
  ...
  to specify a scalar relative tolerance and scalara absolute tolerance or
  scalar relative tolerance and a vector of absolute tolerances, respectively.
  Alternatively, call \id{IDAWFtolerances} to specify a function which sets 
  directly the weights used in evaluating WRMS vector norms.
  See \S\ref{sss:idatolerances} for details.
...
\item
  ...
  See \S\ref{ss:idarootinit} for details.
...
{\bf TODO: say something about error handling (i.e. sending error messages to the handler)}
...
}
{
  ...
  {\ida} memory block (of type \id{void *}), otherwise returns \id{NULL}.
}
{}
%%
%%
...
}
{
  ...
  \item[res] (\Id{IDAResFn})
    ...
    \id{res(t, yy, yp, resval, user\_data)} (for full details see
...
Note that this call must be made after the call to \id{IDAInit}.
%%
...
Together, these linear solvers are refered to as {\idadls} (from direct
linear solvers).
...
}
{
  ...
  \begin{args}[IDADIRECT\_ILL\_INPUT]
...
}
{
  ...
  Note that \id{IDACalcIC} will correct the values $y(t_0)$ and $y^\prime(t_0)$
...
}
{
  ...
  initial conditions that was passed to \id{IDAInit}, while the
...
%%   to \id{IDAInit} (see \S\ref{sss:idainit}). This function call
...
difference quotient approximation function
...
{
  The return value \id{flag} (of type \id{int}) is one of
  \begin{args}[IDADIRECT\_LMEM\_NULL]
  \item[\Id{IDADIRECT\_SUCCESS}] 
    The optional value has been successfully set.
  \item[\Id{IDADIRECT\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
...
Jacobian matrix $J(t,y,y^\prime)$. This function must be of type \id{IDADlsBandJacFn}. 
...
{
  By default, {\idaband} uses the internal difference quotient function.
...
and provides interpolated values of $y$ or $y'$
...
\begin{table}
\centering
\caption{Optional outputs from {\ida}, {\idadense}, {\idaband}, and {\idaspils}}
\label{t:optional_output}
\medskip
\begin{tabular}{|p{\colAA}|p{\colBB}|}
\hline {\bf Optional output} & {\bf Function name} \\
\hline
...
\hline
Number of backtrack operations & \id{IDAGatNumBacktrackops} \\
...
}
{
  ...
  \item with \id{itol = IDASVtolerances}: \id{leniw} $=$ \id{leniw} $+ N_i$;
  ...
  no constraints, and no call to \id{IDASVtolerances}, these lengths are
...
}
{
  ...
  \item[IDA\_SUCCESS] 
    The optional output value has been successfuly set.
  \item[\Id{IDA\_ILL\_INPUT}]
    The fucntion was not called before the first call to \id{IDASolve}.
  \item[\Id{IDA\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
  \end{args}
}
...
The following optional outputs are available from the {\idadls} module:
...
Note that, where the name of an output would
...
{
  The function \ID{IDADlsGetWorkSpace} returns the sizes of
...
}
{
  ...
  For the {\idaband} linear solver, in terms of the problem size $N$ 
...
{
  If the setup function failed (i.e., \id{IDASolve} returned
...
{
  The only argument, of type \id{int} is a return flag from a {\idaspils} function.
}
{
  The return value is a string containing the name of the corresponding constant.
}
...
{
  flag = IDAReInit(ida\_mem, res, t0, y0, yp0);
}
{
...
}
{
  ...
  \item[res] (\Id{IDAResFn})
    ...
    \id{f(t, y, yp, r, user\_data)} (for full details see \S\ref{ss:user_fct_sim}).
  \item[t0] (\id{realtype})
    is the initial value of $t$.
  \item[y0] (\id{N\_Vector})
    is the initial value of $y$. 
...
the user may provide a function of type \ID{IDADlsDenseJacFn} defined by
\usfunction{IDADlsDenseJacFn}
{
  typedef int (*IDADlsDenseJacFn&)(long int Neq, realtype tt, \\
                             &N\_Vector yy, N\_Vector yp, N\_Vector rr, \\ 
                             &realtype c\_j, void *user\_data, DlsMat Jac, \\
...
the user may provide a function of type \ID{IDADlsBandJacFn} defined as follows:
...
}
{
  ...
  of type \Id{DlsMat} with the elements of the Jacobian $J(t,y,y^\prime)$ at the
...
providing space (presumably within \id{user\_data}) for components of \id{yy}
...
{}
%%
\ucfunction{IDABBDPrecGetReturnFlagName}
{
  name = IDABBDPrecGetReturnFlagName(flag);
}
{
  The function \ID{IDABBDPrecGetReturnFlagName} returns the
  name of the {\idabbdpre} constant corresponding to \id{flag}.
}
{
  The only argument, of type \id{int} is a return flag from an {\idabbdpre} function.
}
{
  The return value is a string containing the name of the corresponding constant.
}

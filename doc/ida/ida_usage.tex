...
\item
  ...
  For the {\cuda}- and {\raja}-based vector implementations use a call of the form
  ...
  elements by accessing the underlying data with a call of the form
...
}
{
  ...
  The function type \id{IDADlsJacFn} is described in \S\ref{ss:jacFn}.
}
%%
\index{optional input!direct linear solver interface|)}
\index{IDADLS@{\idadls} linear solver interface!optional input|)}
...
If preconditioning is to be done with the {\idaspils} linear solver interface, 
...
\label{ss:psolveFn}
\index{preconditioning!user-supplied}
\index{IDASPLS@{\idaspils} linear solver interface!preconditioner solve function}
...
\item
  ...
  When creating the iterative linear solver object, specify the typeuse of 

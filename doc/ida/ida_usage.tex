...
\item 
  ...
  \id{y0} and \id{yp0} passed to \id{IDAMalloc}.  See \S\ref{ss:idacalcic}.
...
\id{IDACreate} and \id{IDAMalloc} (or \id{IDAReInit}), and by a 
...
}
{
  ...
  which were specified in the previous call to \id{IDAMalloc} or \id{IDAReInit}.
...
}
{
  ...
  \item[jac\_data]
    ...
    parameter passed to \id{IDADenseSetJacFn}.   
  \item[Jac]
...
}
{
  ...
  \item[jac\_data]
    is a pointer to user data --- the same as the \id{jac\_data}      
    parameter passed to \id{IDABandSetJacFn}.   
  \item[Jac]
    is the output Jacobian matrix.  
  \item[tmp1]
  \item[tmp2]

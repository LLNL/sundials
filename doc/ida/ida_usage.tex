...
of the form \id{nvector\_***.h}.  See Chapter \ref{s:nvector} for the appropriate
...
include the file \id{ida\_sparse.h}, which defines common functions.
...
{\nvector} implementation used.  For the steps that are not, refer to
Chapter \ref{s:nvector} for the specific name of the function to be called
or macro to be referenced.
%%
...
\item\label{i:lin_solver} 
  ...
  \id{flag = }\Id{IDADense}\id{(...);}

  \id{flag = }\Id{IDABand}\id{(...);}

  \id{flag = }\Id{IDALapackDense}\id{(...);}

  \id{flag = }\Id{IDALapackBand}\id{(...);}

  \id{flag = }\Id{IDAKLU}\id{(...);}

  \id{flag = }\Id{IDASuperLUMT}\id{(...);}
...
\item
  ...
  Optionally, call \id{IDA*Set*} functions from the selected linear solver module
...
In each case the linear solver module used by {\ida} is actually built
...
}
{
  ...
  The user's main program must include the \id{ida\_dense.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{ida\_lapack.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{ida\_band.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
%%
%%
\index{IDA@{\ida} linear solvers!IDAKLU@{\idaklu}}
...
}
{
  ...
  The user's main program must include the \id{ida\_klu.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  \item[\Id{IDASLS\_MEM\_NULL}]
    The \id{idaode\_mem} pointer is \id{NULL}.
...
\index{IDA@{\ida} linear solvers!IDASUPERLUMT@{\idasuperlumt}}
...
}
{
  ...
  The user's main program must include the \id{ida\_superlumt.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{ida\_spgmr.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{ida\_spbcgs.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main program must include the \id{ida\_sptfqmr.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
Type of Gram-Schmidt orthogonalization ${}^{(a)}$& \id{IDASpilsSetGSType} & Modified GS \\
...
}
{
  ...
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[lenrw] (\id{sunindextype})
    number of real values in the {\ida} workspace.
  \item[leniw] (\id{sunindextype})
...
}
{
  ...
  and \id{sunindextype} words) is given by:
  \begin{itemize}
...
}
{
  ...
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[lenrwLS] (\id{sunindextype})
    the number of real values in the {\idadls} workspace.
  \item[leniwLS] (\id{sunindextype})
...
}
{
  ...
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[lenrwBBDP] (\id{sunindextype})
    local number of real values in the {\idabbdpre} workspace.
  \item[leniwBBDP] (\id{sunindextype})

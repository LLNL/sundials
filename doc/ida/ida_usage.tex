...
The header files corresponding to the various linear solver modules
available for use with {\ida} are:
%%
\begin{itemize}
...
The \index{IDALS@{\idals} linear solver interface!Jacobian approximation used by}
...
that comes with the {\idals} solver.
...
}
{
  ...
  finite difference Jacobian approximation or finite
...
{\sunmatrix} was supplied to \id{IDASetLinearSolver}), the user may
...
}
{
  ...
  \id{IDALsJacTimesVecFn}, and may still be used for backward-compatibility.
  However, this will be deprecated in future releases, so we recommend
  that users transition to the new function type name soon.
}
\index{Jacobian approximation function!Jacobian times vector!user-supplied|)}
...
}
{
  ...
  to \id{IDALsJacTimesSetupFn}, and may still be used for

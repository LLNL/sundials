...
iteration, there are two {\ida} linear solver interfaces currently
available for this task: {\idadls} and {\idaspils}.
...
When changing the nonlinear solver in {\ida}, \id{IDASetNonlinearSolver} must
be called after \id{IDAInit}. If any calls to \id{IDASolve} have been made, then
{\ida} will need to be reinitialized by calling \id{IDAReInit} to ensure
...
{\ida} memory pointer returned by \id{IDACreate} and the second argument is

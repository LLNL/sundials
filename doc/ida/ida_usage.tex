...
it to {\ida} via a call to \Id{IDASetLinearSolver}.  The first argument
...
%%
\index{IDA@{\ida} linear solvers!selecting one|)}
\index{IDA@{\ida} linear solver interface!IDALS@{\idals}}
...
}
{
  ...
  object \id{J} (if applicable) to {\ida}, initializing the {\idals}
...
errors. Finally, a call to a \id{IDASet***} function can be made from the
...
%%
\ucfunctionf{IDASetNlsResFn}
{
  flag = IDASetNlsResFn(ida\_mem, res);
}
{
  The function \ID{IDASetNlsResFn} specifies an alternative residual function
...
{
  The default is to use the residual function provided to \id{IDAInit} in
...
\id{IDALsJacFn}.  The user can supply a Jacobian function, or if using
...
details. The function \id{IDASetLinearSolutionScaling} can be used to disable
...
\ucfunctionf{IDASetLinearSolutionScaling}
{
  flag = IDASetLinearSolutionScaling(ida\_mem, onoff);
}
{
  The function \ID{IDASetLinearSolutionScaling} enables or disables scaling
...
}
{
  ...
  By default scaling is enabled with matrix-based linear solvers.
}
%%
\index{optional input!matrix-based linear solver|)}
%%
...
finite-difference approximation, via a call to \Id{IDASetIncrementFactor}.
%%
%%
\index{Jacobian approximation function!Jacobian times vector!increment}
...
Jacobian-vector product approximation by calling \id{IDASetJacTimesResFn}. The
alternative residual function should compute a suitable (and differentiable)
approximation to the residual function provided to \id{IDAInit}. For example,
...
%%
%%
\index{Jacobian approximation function!Jacobian times vector!alternative-res}
\ucfunctionf{IDASetJacTimesResFn}
{
  flag = IDASetJacTimesResFn(ida\_mem, jtimesResFn);
}
{
  The function \ID{IDASetJacTimesResFn} specifies an alternative DAE residual
...
}
{
  ...
  has been initialized through a call to \id{IDASetLinearSolver}.
}
%%
%%
\index{optional input!matrix-free linear solver|)}
...
%%
%%
\ucfunctionf{IDASetLSNormFactor}
{
  flag = IDASetLSNormFactor(ida\_mem, nrmfac);
}
{
  The function \ID{IDASetLSNormFactor} specifies the factor to use when
...
}
{
  ...
  ({\ida} v5.0.0) the value of \id{nrmfac} was computed using the vector
...
}
{
  ...
  The previous routines \Id{IDADlsGetNumResEvals} and
  \Id{IDASpilsGetNumResEvals} are now wrappers for this routine, and may
  still be used for backward-compatibility.  However, these will be
  deprecated in future releases, so we recommend that users transition
  to the new routine name soon.
}
...
}
{
  ...
  name of the {\idals} constant corresponding to \id{lsflag}.
}
{
...
}
{
  ...
  call to the user's \id{IDALsJacFn} function is preceded by a call to the
  \id{IDAResFn} user function with the same (\id{tt}, \id{yy}, \id{yp})
...
%
\usfunction{IDALsJacTimesSetupFn}
{

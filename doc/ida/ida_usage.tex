...
The header files for the dense and banded linear solvers (both internal and Lapack) include \id{ida\_direct.h} 
...
On error, each user-callable function returns a negative value and sends an error message
to the error handler routine, which prints the message on standard error by default. However, 
the user can set a file as error output or can provide his  
own error handler function ( see \S\ref{sss:optin_main} ).
...
{\idaspgmr}, {\idaspbcg}, and {\idasptfqmr}, with the first two providing
...
{
  The function \ID{IDADense} selects the {\idadense} linear solver and 
  indicates the use of the internal direct dense linear algebra functions.
...
}
{
  ...
  \item[rootdir] (\id{int *})
    ...
    in the call to the function \id{IDARootInit}. A value of of $0$ indicates that
...
}
{
  ...
  \item[\Id{IDA\_BAD\_DKY}] 
    \id{k} is not one of the  $\{0, 1, \ldots, kused\}$.
  \end{args}
...
}
{
  ...
  can be used to access $t_n$, $h_u$ and $kused$.
}
...
}
{
  ...
  integer workspaces used by a {\idadls} linear solver ({\idadense} or {\idaband}). 
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
\Id{IDADense} or \Id{IDALapackDense} is called in Step \ref{i:lin_solver} of \S\ref{ss:skeleton_sim}), 
the user may provide a function of type \ID{IDADlsDenseJacFn} defined by
\usfunction{IDADlsDenseJacFn}
{
  typedef int (*IDADlsDenseJacFn&)(int Neq, realtype tt, realtype c\_j, \\
...
\Id{IDABand} or \Id{IDALapackBand} is called in Step \ref{i:lin_solver} of \S\ref{ss:skeleton_sim}), 
...
the {\idabbdpre} preconditioner module are described next.
%%
\index{half-bandwidths}
...
}
{
  ...
  \begin{args}[IDASPILS\_ILL\_INPUT]
  \item[IDASPILS\_SUCCESS]
    The call to \id{IDABBDPrecInit} was successfull.
  \item[\id{IDASPILS\_MEM\_NULL}] 
    The \id{ida\_mem} pointer was \id{NULL}.
  \item[\Id{IDASPILS\_MEM\_FAIL}]
    A memory allocation request has failed.
  \item[\Id{IDASPILS\_LMEM\_NULL}]
    A {\idaspils} linear solver memory was not attached.
  \item[\Id{IDASPILS\_ILL\_INPUT}]
...
}
{
  ...
  \item[IDASPILS\_SUCCESS]
    The call to \id{IDABBDPrecReInit} was successfull.
  \item[\id{IDASPILS\_MEM\_NULL}] 
    The \id{ida\_mem} pointer was \id{NULL}.
  \item[\Id{IDASPILS\_LMEM\_NULL}]
    A {\idaspils} linear solver memory was not attached.

...
}
{
  ...
  \item[sparsetype] (\id{int})
    ...
    set to \Id{CSC\_MAT} the solver will expect Jacobian to be stored as 
    compressed sparse column matrix, and if \Id{sparsetype=CSR\_MAT} 
    the solver will expect compressed sparse row matrix. 

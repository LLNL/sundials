...
The calling program must also include a {\nvector} implementation header file,
of the form \\ \noindent
...
\id{sunnonlinsol/sunnonlinsol\_***.h}.  See Chapter
\ref{c:sunnonlinsol} for the appropriate name.  This file in turn
includes the header file \Id{sundials\_nonlinearsolver.h} which
defines the abstract \Id{SUNNonlinearSolver} data type.

If the using a nonlinear solver that requires the solution of a linear
...
\item\label{i:lin_solver_interface}
  ...
  If a nonlinear solver requiring a linear solver is chosen (e.g., the default
  Newton iteration), then initialize the {\idadls} or {\idaspils} linear solver
...
\item\label{i:nonlin_solver}
  ...
  If using a non-default nonlinear solver, then create the desired nonlinear
  solver object by calling the appropriate constructor function defined by the
  particular {\sunnonlinsol} implementation (e.g.,
  \id{NLS = SUNNonlinSol\_***(...);} where \id{***} is the name of the nonlinear
  solver (see Chapter \ref{c:sunnonlinsol} for details).

\item\label{i:nonlin_solver_interface}
  {\bf Attach nonlinear solver module} (\textit{optional})

  If using a non-default nonlinear solver, then initialize the {\idanls}
  nonlinear solver interface by attaching the nonlinear solver object by calling
  \id{ier = }\Id{IDASetNonlinearSolver}\id{(ida\_mem, NLS);}
...
iteration, there are two {\ida} linear solvers currently available for this
task: {\idadls} and {\idaspils}.
...
desired {\sunlinsol} object to use for solving systems.  A call
...
}
{
  ...
  \item[LS] (\id{SUNLinearSolver})
    {\sunlinsol} object to use for solving linear systems of the from
...
and attach the {\sunnonlinsol} object to {\ida} by calling
\Id{IDASetNonlinearSolver}, as documented below.

When changing the nonlinear solve in {\ida}, \id{IDASetNonlinearSolver} must be
called after \id{IDAInit}. If any calls to \id{IDASolve} have been made, then
{\ida} will need to be reinitialized by calling \id{IDAReInit} to ensure the
nonlinear solver is initialized correctly before any subsequent calls to
\id{IDASolve}.

The first argument passed to \id{IDASetNonlinearSolver} is the {\ida} memory
pointer returned by \id{IDACreate} and the second argument is the
{\sunnonlinsol} object to use for solving the nonlinear system \ref{e:DAE_nls}.
A call to this function attaches the nonlinear solver to the main {\ida}
integrator. We note that at present, the {\sunnonlinsol} object \emph{must be of type}
...
}
{
  ...
  \item[\Id{IDA\_SUCCESS}]
    The optional value has been successfully set.
  \item[\Id{IDA\_MEM\_NULL}]
    The \id{ida\_mem} pointer or the {\sunnonlinsol} module is \id{NULL}.
  \end{args}
}
{

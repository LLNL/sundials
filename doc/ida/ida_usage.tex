...
A linear solver module header file is also required.
\index{IDA@{\ida} linear solvers!header files}
The header files corresponding to the various linear solver interfaces and linear 
solver modules available for use with {\ida} are:
%%
\begin{itemize}
\item \Id{ida/ida\_direct.h}, 
  which is used with the {\idadls} direct linear solver interface to
...
\item \Id{ida/ida\_spils.h}, 
  which is used with the {\idaspils} iterative linear solver interface to
...
preconditioner, etc.  For example, in the \id{idaFoodWeb\_kry\_p}
...
Finally, the header file for the {\sunnonlinsolnewton} module
\id{sunnonlinsol/sunnonlinsol\_newton.h} is also required.
...
{\nvector}, {\sunmatrix}, and {\sunlinsol} implementations used.  
For the steps that are not, refer to
Chapter \ref{s:nvector}, \ref{s:sunmatrix}, and \ref{s:sunlinsol} for the 
...
\item\label{i:matrix} 
  ...
  a Newton iteration) and the linear solver will be a direct linear solver,
  then a template Jacobian matrix must be created by using the
  appropriate constructor defined by the particular {\sunmatrix}
  implementation.
...
\item\label{i:lin_solver} 
  ...
  If a nonlinear solver requiring a linear solver will be used (e.g., a Newton iteration), then the desired linear solver
  object must be created by using the appropriate constructor defined by
...
\item\label{i:lin_solver_interface} 
  ...
  If a nonlinear solver requiring a linear solver will be used (e.g., a Newton iteration), initialize the {\idadls} or
  {\idaspils} linear solver interface by attaching the linear solver
  object (and matrix object, if applicable)
  with one of the following calls (for details see \S\ref{sss:lin_solv_init}):

  \id{ier = }\Id{IDADlsSetLinearSolver}\id{(...);}

  \id{ier = }\Id{IDASpilsSetLinearSolver}\id{(...);}

\item
  {\bf Set linear solver interface optional inputs}

  Call \id{IDADlsSet*} or \id{IDASpilsSet*} functions to change optional
  inputs specific to that linear solver interface. 
  See \S\ref{ss:optional_input} for details.

\item
  {\bf Create a nonlinear solver object}

  Call the constructor for the desired nonlinear solver (e.g.,
  \id{NLS = SUNNonlinSol\_Newton(...)}).

\item
  {\bf Attach the nonlinear solver object}

  Call the \id{IDASetNonlinearSolver(ida\_mem, NLS)} to attach the nonlinear
  solver to {\ida}.

\item
  {\bf Set nonlinear solver optional inputs}

  Call the appropriate set functions for the selected nonlinear solver
  module to change optional inputs specific to that nonlinear
  solver. These \textit{must} be called after attaching the nonlinear
  solver to {\ida}, otherwise the optional inputs will be overridden
  by {\ida} defaults. See Chapter \ref{c:sunnonlinsol} for more
...
\item
  {\bf Free nonlinear solver memory}

  Call \Id{SUNNonlinSolFree}\id{(NLS)} to free any memory allocated
  for the {\sunnonlinsol} object.
...
then \id{abstol} should be a vector.  See the example \id{idaRoberts\_dns} in the
...
linear systems of the form (\ref{e:DAE_Newtoncorr}).  There are two {\ida} linear
solvers currently available for this task: {\idadls} and {\idaspils}.
...
and factorizations used throughout the solution process.
...
either of the functions \Id{IDADlsSetLinearSolver} or
\Id{IDASpilsSetLinearSolver}, as documented below.  The first argument
passed to these functions is the {\ida} memory pointer returned by
\id{IDACreate}; the second argument passed to these functions is the
desired {\sunlinsol} object to use for solving Newton systems.  A call
to one of these functions initializes the appropriate {\ida} linear
solver interface, linking this to the main {\ida} integrator, and
...
\ref{s:sunlinsol}.
\index{IDA@{\ida} linear solver interfaces!selecting one|)}

\index{IDA@{\ida} linear solver interfaces!IDADLS@{\idadls}}
\index{IDADLS@{\idadls} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{IDADlsSetLinearSolver}
{
  flag = IDADlsSetLinearSolver(ida\_mem, LS, J);
}
{
  The function \ID{IDADlsSetLinearSolver} attaches a direct
  {\sunlinsol} object \id{LS} and corresponding template
  Jacobian {\sunmatrix} object \id{J} to {\ida}, initializing the
  {\idadls} direct linear solver interface.

  The user's main program must include the \id{ida\_direct.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  \item[\Id{IDADLS\_SUCCESS}] 
    The {\idadls} initialization was successful.
  \item[\Id{IDADLS\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
  \item[\Id{IDADLS\_ILL\_INPUT}]
    The {\idadls} solver is not compatible with the current {\nvector} module.
  \item[\Id{IDADLS\_MEM\_FAIL}]
    A memory allocation request failed.
  \end{args}
}
{
  The {\idadls} linear solver is not compatible with all
  implementations of the {\sunlinsol} and {\nvector}
  modules.  Specifically, {\idadls} requires use of a \emph{direct}
...
%%
%%
\index{IDA@{\ida} linear solver interfaces!IDASPILS@{\idaspils}}
\index{IDASPILS@{\idaspils} linear solver!SUNLINEARSOLVER@{\sunlinsol} compatibility}
\ucfunction{IDASpilsSetLinearSolver}
{
  flag = IDASpilsSetLinearSolver(ida\_mem, LS);
}
{
  The function \ID{IDASpilsSetLinearSolver} attaches an iterative
  {\sunlinsol} object \id{LS} to {\ida}, initializing the
  {\idaspils} scaled, preconditioned, iterative linear solver
  interface.

  The user's main program must include the \id{ida\_spils.h} header
  file.
}
{
  \begin{args}[ida\_mem]
...
}
{
  ...
  \item[\Id{IDASPILS\_SUCCESS}] 
    The {\idaspils} initialization was successful.
  \item[\Id{IDASPILS\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
  \item[\Id{IDASPILS\_ILL\_INPUT}]
    The {\idaspils} solver is not compatible with the \id{LS} object
    or is incompatible with the current {\nvector} module.
  \item[\Id{IDASPILS\_MEM\_FAIL}]
    A memory allocation request failed.
  \item[\Id{IDASPILS\_SUNLS\_FAIL}]
    A call to the \id{LS} object failed.
  \end{args}
}
{
  The {\idaspils} linear solver interface is not compatible with all
  implementations of the {\sunlinsol} and {\nvector}
  modules.  Specifically, {\idaspils} requires use of an
...
%%==================================================================================
%%
\subsection{Nonlinear solver interface function}\label{sss:nonlin_solv_init}

To specify a generic nonlinear solver to {\ida}, after the call to
\id{IDACreate} but before any calls to \id{IDASolve}, the user's
program must create the appropriate {\sunnonlinsol} object and call
\Id{IDASetNonlinearSolver}, as documented below. The first argument
passed to this function is the {\ida} memory pointer returned by
\id{IDACreate}; the second argument passed to this function is the 
desired {\sunnonlinsol} object to use for solving the nonlinear system
\ref{e:DAE_nls}. A call to this function attaches the nonlinear solver
to the main {\ida} integrator. Note the {\sunnonlinsol} object must be
of type \id{SUNNONLINEARSOLVER\_ROOTFIND}.
...
}
{
  ...
  object \id{NLS} to {\ida}.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[LS] (\id{SUNNonlinearSolver})
...
}
{
  ...
  \item[\Id{IDA\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
  \item[\Id{IDA\_ILL\_INPUT}]
    The {\sunnonlinsol} object is \id{NULL}, not of the correct type,
    or does not implement the required nonlinear solver operations.
  \end{args}
}
{}
...
\hline
\multicolumn{3}{|c|}{\bf IDADLS linear solver interface} \\
\hline
Jacobian function & \id{IDADlsSetJacFn} & DQ\\
\hline
\multicolumn{3}{|c|}{\bf IDASPILS linear solver interface} \\
\hline
Preconditioner functions & \id{IDASpilsSetPreconditioner} &NULL, NULL \\
Jacobian-times-vector function & \id{IDASpilsSetJacTimes} & NULL, DQ\\
Ratio between linear and nonlinear tolerances & \id{IDASpilsSetEpsLin} & 0.05 \\
\hline
\end{tabular}
\end{table}
...
\label{sss:optin_dls}
\index{optional input!direct linear solver interface|(}
\index{IDADLS@{\idadls} linear solver interface!optional input|(}
The \index{IDADLS@{\idadls} linear solver interface!Jacobian approximation used by}
{\idadls} solver interface needs a function to compute an approximation to
the Jacobian matrix $J(t,y,\dot{y})$.  This function must be of type \id{IDADlsJacFn}. 
...
that comes with the {\idadls} solver.
To specify a user-supplied Jacobian function \id{jac}, {\idadls} provides 
the function \id{IDADlsSetJacFn}.
The {\idadls} interface passes the pointer \id{user\_data} 
...
\ucfunction{IDADlsSetJacFn}
{
  flag = IDADlsSetJacFn(ida\_mem, jac);
}
{
  The function \ID{IDADlsSetJacFn} specifies the Jacobian
...
}
{
  ...
  \item[\Id{IDADLS\_LMEM\_NULL}]
    The {\idadls} linear solver interface has not been initialized.
  \end{args}
}
{
  By default, {\idadls} uses an internal difference quotient function
  ...
  The function type \id{IDADlsJacFn} is described in \S\ref{ss:djacFn}.
}
%%
\index{optional input!direct linear solver interface|)}
\index{IDADLS@{\idadls} linear solver interface!optional input|)}
...
%%==================================================================================
%%
\subsubsection{Iterative linear solver interface optional input functions}
\label{sss:optin_spils}
\index{optional input!iterative linear solver|(}
\index{IDASPILS@{\idaspils} linear solver interface!optional input|(}
\index{preconditioning!user-supplied|(}
If the user will be doing preconditioning with the {\idaspils} linear solver interface, 
then the user must supply a preconditioner solve function \id{psolve} and specify
its name through a call to the routine \id{IDASpilsSetPreconditioner}.
\index{IDASPILS@{\idaspils} linear solver interface!preconditioner solve function}
%%
...
\index{IDASPILS@{\idaspils} linear solver interface!preconditioner setup function}
...
\index{IDASPILS@{\idaspils} linear solver interface!Jacobian approximation used by}
The {\idaspils} solver interface requires a function to compute an approximation to the
...
that comes with the {\idaspils} solver interface.
A user-defined Jacobian-vector function must be of type \id{IDASpilsJacTimesVecFn}
and can be specified through a call to \id{IDASpilsSetJacTimes}
...
function \id{jtsetup} (see \S\ref{ss:jtsetupFn} for specification
details).  As with the preconditioner functions, a pointer to the user-defined
data structure, \id{user\_data}, specified through
\id{IDASetUserData} (or a \id{NULL} pointer otherwise) is passed to
the Jacobian-times-vector setup and product functions, \id{jtsetup} and
\id{jtimes}, each time they are called.

\index{IDASPILS@{\idaspils} linear solver interface!convergence test}
Finally, as described in Section \ref{ss:ivp_sol}, the {\idaspils}
...
}
{
  ...
  \item[\Id{IDASPILS\_SUNLS\_FAIL}]
    ...
    {\sunlinsol} object used by the {\idaspils} interface.
  \end{args}
}
{
...
\ucfunction{IDASpilsSetJacTimes}
{
  flag = IDASpilsSetJacTimes(ida\_mem, jsetup, jtimes);
}
{
  The function \ID{IDASpilsSetJacTimes} specifies the Jacobian-vector 
  setup and product.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[jtsetup] (\id{IDASpilsJacTimesSetupFn})
...
}
{
  ...
  \item[\Id{IDASPILS\_SUNLS\_FAIL}]
    ...
    routines in the {\sunlinsol} object used by the {\idaspils}
    interface.
  \end{args}
}
{
  ...
  The function type \id{IDASpilsJacTimesSetupFn} is described in \S\ref{ss:jtsetupFn}.
...
\hline
\multicolumn{2}{|c|}{\bf IDADLS linear solver interface} \\
\hline
...
\hline
\multicolumn{2}{|c|}{\bf IDASPILS linear solver interface} \\
\hline
...
No. of Jacobian-vector setup evaluations & \id{IDASpilsGetNumJTSetupEvals} \\
...
\index{optional output!direct linear solver interface|(}
\index{IDADLS@{\idadls} linear solver interface!optional output|(}
...
%%
%%
\index{IDADLS@{\idadls} linear solver interface!memory requirements}
\index{memory requirements!IDADLS@{\idadls} linear solver interface}
...
}
{
  ...
  integer workspaces used by the {\idadls} linear solver interface.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  matrix allocated by the user outside of {\idadls} is not included in
  this report.
}
%%
%%
...
}
{
  ...
  cumulative number of calls to the {\idadls} Jacobian approximation 
  function.
}
{
  \begin{args}[njevals]
...
}
{
  ...
  internal difference quotient functions (dense or banded) is used.
}
%%
...
}
{
  ...
  (\id{IDASolve} returned \id{IDA\_LSETUP\_FAIL}), then the value of
  \id{lsflag} is equal to the column index (numbered from one) at
...
%%==============================================================================
\index{optional output!iterative linear solver interface|(}
\index{IDASPILS@{\idaspils} linear solver interface!optional output|(} 
The following optional outputs are available from the {\idaspils}
...
setup and product routines, number of calls to the residual
...
%%
%%
\index{IDASPILS@{\idaspils} linear solver interface!memory requirements} 
\index{memory requirements!IDASPILS@{\idaspils} linear solver interface}
\ucfunction{IDASpilsGetWorkSpace}
{
  flag = IDASpilsGetWorkSpace(ida\_mem, \&lenrwLS, \&leniwLS);
}
{
  The function \ID{IDASpilsGetWorkSpace} returns the global sizes of the
  {\idaspils} real and integer workspaces.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the {\ida} memory block.
  \item[lenrwLS] (\id{long int})
    the number of \id{realtype} values in the {\idaspils} workspace.
  \item[leniwLS] (\id{long int})
    the number of integer values in the {\idaspils} workspace.
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
%%
%%
\ucfunction{IDASpilsGetNumJTSetupEvals}
{
  flag = IDASpilsGetNumJTSetupEvals(ida\_mem, \&njtsetup);
}
{
  The function \ID{IDASpilsGetNumJTSetupEvals} returns the
...
}
{
  ...
  \id{IDA\_LSETUP\_FAIL}), \id{lsflag} will be \id{SUNLS\_PSET\_FAIL\_UNREC},
  \id{SUNLS\_ASET\_FAIL\_UNREC}, or \id{SUNLS\_PACKAGE\_FAIL\_UNREC}.

  If the {\idaspils} solve function failed (\id{IDA} returned
...
to the {\idadls} or {\idaspils} interface routines, as described in
...
\Id{IDADlsSetLinearSolver} is called in the step described in 
\S\ref{ss:skeleton_sim}), the user may
provide a function of type \ID{IDADlsJacFn} defined as follows:
%%
\usfunction{IDADlsJacFn}
{
  typedef int (*IDADlsJacFn)(&realtype tt, realtype cj,\\
...
}
{
  ...
  \item[tmp3]
    ...
    which can be used by \id{IDADlsJacFn} function 
    as temporary storage or work space.
  \end{args}
}
{
  An \id{IDADlsJacFn} should return $0$ if successful,
...
}
{
  ...
  If the user's \id{IDADlsJacFn} function uses difference
...
\label{ss:jtimesFn}
\index{Jacobian approximation function!Jacobian times vector!user-supplied|(}
If the {\idaspils} solver interface is selected
(i.e., \id{IDASpilsSetLinearSolver} is called in the steps described in
\S\ref{ss:skeleton_sim}), the user may
provide a function of type \Id{IDASpilsJacTimesVecFn} in the following form,
...
user-supplied function of type \Id{IDASpilsJacTimesSetupFn}, defined as follows:
%
\usfunction{IDASpilsJacTimesSetupFn}
{
  typedef int (*IDASpilsJacTimesSetupFn)(&realtype tt, N\_Vector yy, \\
                                       &N\_Vector yp, N\_Vector rr, \\
                                       &realtype cj, void *user\_data);
}
{
...
} 
{
  ...
  the \id{IDARhsFn} user function with the same \id{(t,y, yp)} arguments.  
...
user-supplied function of type \ID{IDASpilsPrecSetupFn}, defined as follows:
...
\item
  ...
  left preconditioning (\id{PREC\_LEFT}) as {\ida} only supports left preconditioning.

\item
  \textcolor{gray}{\bf Set linear solver optional inputs}

\item \label{i:bbdpre_attach}
  {\bf Attach iterative linear solver module}
...
\item
  ...
  or solve function through calls to id{IDASpilsSetPreconditioner} 
...
one or more of the corresponding \id{IDASpilsSet***} functions, must
...
%%
  ...
  corresponding function \id{IDASpilsGetWorkSpace}.
}
%%
%%

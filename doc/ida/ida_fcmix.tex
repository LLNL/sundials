...
\item
  ...
  \item \id{FSUNNEWTONINIT} (defined by {\sunnonlinsolnewton}) 
    interfaces to \id{SUNNewtonSolver}.
  \end{itemize}
...
%%====================
  ...
  To initialize the {\sunnonlinsolnewton} module supplied with
  {\sundials}, the user must make the following call
\begin{verbatim}
      CALL FSUNNEWTON(2, IER)
\end{verbatim}
  where \id{2} is the {\ida} package ID and \id{IER} is an error
  return flag set on $0$ on success or $-1$ if an error occurred. Once
  created the default maximum number of nonlinear iterations may be
  modified by calling
\begin{verbatim}
      CALL FSUNNEWTONSETMAXITERS(2, MAXITERS, IER)
\end{verbatim}
  where \id{MAXITERS} is the desired maximum number of nonlinear
  iterations.
...
\end{verbatim}
  where \id{IER} is an error return flag set on $0$ on success or $-1$

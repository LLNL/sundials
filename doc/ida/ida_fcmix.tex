...
\item Interface to the main {\ida} module
  \begin{itemize}
  \item \id{FIDAMALLOC}
    interfaces to \id{IDACreate}, \id{IDASetUserData}, \id{IDAInit}, and \id{IDASStolerances}/\id{IDASVtolerances}.
  \item \id{FIDAREINIT}
...
\end{verbatim}
  where \id{NEQ} is the size of the DAE system, depending on whther the internal
...
\end{verbatim}
  ...
  \id{Y} and \id{YP} are arrays containing the computed vectors $y$ and $\dot{y}$
...
\end{verbatim}
  ...
  call \id{FIDABAND} (or \id{FIDALAPACKBAND}) as described above.

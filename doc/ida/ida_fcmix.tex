...
\item Interface to the linear solver modules
  ...
  \item \id{FIDALAPACKBANDSETJAC}
    interfaces to \id{IDADlsSetBandJacFn}.
  \item \id{FIDAKLU}
    interfaces to \id{IDAKLU}.
  \item \id{FIDAKLUREINIT}
    interfaces to \id{IDAKLUReInit}.
  \item \id{FIDASUPERLUMT}
    interfaces to \id{IDASuperLUMT}.
  \item \id{FIDASPARSESETJAC}
    interfaces to \id{IDASlsSetSparseJacFn}.
  \item \id{FIDASPGMR}
...
\id{FIDASPJAC}  & \id{FIDASparseJac}& \id{IDASlsSparseJacFn} \\
...
\item\label{i:fida_lin_solv_spec} {\bf Linear solver specification} 
  ...
  See Eq. (\ref{e:DAE_Newtoncorr}).  The user of {\fida} must call a routine
  with a specific name to make the desired choice of linear solver.
...
\end{verbatim}
  ...
  must be declared so as to match {\CC} type \id{int}.
...
\end{verbatim}
  ...
  \index{IDASPARSE@{\idasparse} linear solver!use in {\fida}}
  
  To use the KLU sparse direct linear solver,
  the user must make the call:
  \index{FIDAKLU@\texttt{FIDAKLU}}
\begin{verbatim}
...
\end{verbatim}
  ...
  \index{FIDAKLUREINIT@\texttt{FIDAKLURENIT}}
 The {\ida} KLU solver will reuse much of the factorization 
 information from one nonlinear iteration and time step 
 to the next.  If at any time 
...
\end{verbatim}
  ...
  \index{FIDASUPERLUMT@\texttt{FIDASUPERLUMT}}
  Alternatively, to use the \id{SuperLUMT} linear solver,
  the user must make the call:
\begin{verbatim}
      CALL FIDASUPERLUMT (NEQ, NNZ, ORDERING, IER)
\end{verbatim}
  where the arguments have the same meanings as for \id{FIDAKLU}, except
  ...
  \index{FIDASPJAC@\texttt{FIDASPJAC}}
  If the either of the sparse direct interface packages are used, 
  then the user must supply the \id{FIDASPJAC} routine
...
\end{verbatim}
 ...
 $>0$ if a recoverable error occurred, or $<0$ if an unrecoverable error occurred.)
  ...
  following either the call to \id{FIDAKLU} or \id{FIDASUPERLUMT}, the
...
\hline
\multicolumn{3}{|c|}{{\idadls} linear solvers}\\
\hline
...
\hline
\multicolumn{3}{|c|}{{\idasls} linear solvers}\\
\hline
14 &  \id{LS\_FLAG} & \id{IDASlsGetLastFlag} \\
16 &  \id{NJE}      & \id{IDASlsGetNumJacEvals} \\ 
\hline
\multicolumn{3}{|c|}{{\idaspils} linear solvers}\\
\hline

...
\id{idakryx1\_p} which solves the same 2-D heat PDE problem as \id{idakryx}. 
...
The \id{idakryx1\_p} main program begins with MPI calls to initialize MPI and to
...
are set, the user-data structure \id{Userdata} is created and initialized, and
\id{N\_Vector} variables are created and initialized for the initial conditions
(\id{uu} and \id{up}), for the vector \id{id} specifying the differential 
and algebraic components of the solution vector, and for the preconditioner
(\id{pp}).  As in \id{idakryx}, constraints are passed to {\ida} through the
\id{N\_Vector} \id{constraints} and the function \id{IDASetConstraints}.  
A temporary \id{N\_Vector} \id{res} is also created here, for use only in
\id{SetInitialProfiles}.
All components of \id{constraints} are set to $1.0$ indicating that
non-negativity constraints are to be imposed on each solution component.  
In addition, for illustration purposes, \id{idakryx} also excludes the
algebraic components of the solution (specified through the \id{N\_Vector}
\id{id}) from the error test by calling \id{IDASetSuppressAlg} with a
flag \id{TRUE}.

Sample output from \id{idakryx1\_p} follows.
%%
\includeOutput{idakryx1\_p}{../../examples/ida/parallel/idakryx1_p.out}
%%
...
In this example, \id{idakryx2\_bbd\_p}, we solve the same food web problem as with
\id{idabanx2}, but in parallel and with the {\idaspgmr} linear solver and
...
As with \id{idakryx1\_p}, we use a \id{NPEX} $\times$ \id{NPEY} processor grid, with
...
\id{IDABBDPrecAlloc}.  But in order to reduce storage and computation costs for
...
In the \id{idakryx2\_bbd\_p} main program, following MPI initializations and
...
Sample output from \id{idakryx2\_bbd\_p} follows.
%%
\includeOutput{idakryx2\_bbd\_p}{../../examples/ida/parallel/idakryx2_bbd_p.out}
%%

...
The user interface for the new {\idals} module is very similar to the
previous {\idadls} and {\idaspils} interfaces; however to minimize
...
been similarly standardized.  While the previous routine names may
still be used, these will be deprecated in future releases, so we
...
modules in a standalone manner have been added and all {\ida} example programs
have been updated to use generic {\sunnonlinsol} modules.

By default {\ida} uses the {\sunnonlinsolnewton} module implementation of a
Newton iteration. Since {\ida} previously only used an internal implementation
...
\id{IDASetMaxNonlinIters}) or getting nonlinear solver statistics (e.g.,
...
following:
\begin{enumerate}
\item include the header of the desired nonlinear solver module,
\item create a nonlinear solver object by calling the constructor defined by the
...
\vspace{-1ex}
  ...
  \id{***} is the name of the nonlinear solver, and the inputs \id{...} are
  nonlinear solver specific.
\item after initializing {\ida} with \id{IDAInit}, call
\vspace{-1ex}
\begin{verbatim}  
...
\end{enumerate}
The example program \id{idaRoberts\_dns.c} explicitly creates an attaches the
{\sunnonlinsolnewton} module to demonstrate this process (this is not necessary
in general as {\ida} uses the {\sunnonlinsolnewton} module by default).

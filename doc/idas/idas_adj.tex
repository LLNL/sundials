...
\item \label{i:lin_solverB}
  ...
  \id{IDASptfqmr} (see \S\ref{sss:lin_solv_b}).  
...
\begin{verbatim}
  ...
  flag = IDASuperLUMTB(ida_mem, num_threads, which, mB, nB, nnzB);
...
}
{
  ...
  the Jacobian matrix at the point (\id{tt},\id{yy},\id{yyB}), where
  \id{yy} is the solution of the original IVP at time \id{tt} and
  \id{yyB} is the solution of the backward problem at the same time.
...
}
{
  A user-supplied sparse Jacobian function must load the
  compressed-sparse-column matrix \id{JacB} with an approximation to
  the Jacobian matrix at the point (\id{tt},\id{yy},\id{yS},\id{yyB}),

...
\item \label{i:lin_solverB}
  ...
  with the {\idadls} linear solver module and the backward problem with {\idaspils} linear solver module.

\item
  {\bf Set linear solver interface optional inputs for the backward problem}

  Call \id{IDADlsSet*B} or \id{IDASpilsSet*B} functions to change optional
  inputs specific to that linear solver interface. 
  See \S\ref{ss:optional_input_b} for details.
...
\item\label{i:lin_solver_interfaceB}
  ...
  problem (e.g., the default Newton iteration), then initialize the {\idadls} or
  {\idaspils} linear solver interface by attaching the linear solver object (and
  matrix object, if applicable) with one of the following calls (for details see
  \S\ref{sss:lin_solv_init}):

  \id{ier = }\Id{IDADlsSetLinearSolverB}\id{(...);}

  \id{ier = }\Id{IDASpilsSetLinearSolverB}\id{(...);}
...
problem using one of the following functions.

\index{IDAS@{\idas} linear solver interfaces!IDADLS@{\idadls}}
\ucfunction{IDADlsSetLinearSolverB}
{
  flag = IDADlsSetLinearSolverB(ida\_mem, which, LS, A);
}
{
  The function \ID{IDADlsSetLinearSolverB} attaches a direct
  {\sunlinsol} object \id{LS} and corresponding template
  Jacobian {\sunmatrix} object \id{A} to {\idas}, initializing the
  {\idadls} direct linear solver interface for solution of the backward 
  problem.

  The user's main program must include the \id{idas\_direct.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
  ...
  \item[A] (\id{SUNMatrix})
    ...
    problem (must have a type compatible with the linear solver object).
  \end{args}
}
{
  The return value \id{flag} (of type \id{int}) is one of
...
\index{IDAS@{\idas} linear solver interfaces!IDASpils\idaspils}}
\ucfunction{IDASpilsSetLinearSolverB}
{
  flag = IDASpilsSetLinearSolverB(ida\_mem, which, LS);
}
{
  The function \ID{IDASpilsSetLinearSolver} attaches an iterative
  {\sunlinsol} object \id{LS} to {\idas}, initializing the
  {\idaspils} scaled, preconditioned, iterative linear solver
  interface to use for the backward problem.

  The user's main program must include the \id{idas\_spils.h} header
  file.
}
{
  \begin{args}[ida\_mem]
...
}
{
  ...
  \item[\Id{IDASPILS\_ILL\_INPUT}]
    The {\idaspils} solver is not compatible with the current {\nvector} module.
...
If using a direct linear solver interface for the Jacobian of the backward problem,
the linear solver will need to be attached to the memory structure through
a call to \id{IDADlsSetLinearSolverB}.  The Jacobian evaluation function can
be attached through a call to either \id{IDADlsSetJacFnB} or \ida{IDADlsSetJacFnBS},
with the second used when the backward problem depends on the forwrad sensitivities.
%%
\index{Jacobian approximation function!user-supplied}
\ucfunction{IDADlsSetJacFnB}
{
  flag = IDADlsSetJacFnB(ida\_mem, which, jacB);
}
{
  The function \ID{IDADlsSetJacFnB} specifies the Jacobian
...
}
{
  ...
  \begin{args}
  \item[\Id{IDADLS\_SUCCESS}] 
    \id{IDADlsSetDlsJacFnB} succeeded.
  \item[\Id{IDADLS\_MEM\_NULL}]
    The \id{ida\_mem} was \id{NULL}.
  \item[\Id{IDADLS\_NO\_ADJ}]
    The function \id{IDAAdjInit} has not been previously called.
  \item[\Id{IDADLS\_LMEM\_NULL}]
    ...
    \id{IDADlsSetLinearSolverB}.
  \item[\Id{IDADLS\_ILL\_INPUT}]
    The parameter \id{which} represented an invalid identifier.
  \end{args}
}
{
  The function type \id{IDADlsJacFnB} is described in \S\ref{ss:djacFn_b}.
}
%%
\index{Jacobian approximation function!direct!user-supplied (backward)}
\ucfunction{IDADlsSetJacFnBS}
{
  flag = IDADlsSetJacFnBS(ida\_mem, which, jacBS);
}
{
  The function \ID{IDADlsSetJacFnB} specifies the Jacobian
...
{
  The function type \id{IDADlsJacFnBS} is described in \S\ref{ss:djacFn_b}.
}
%%
\index{optional input!direct linear solver interface|)}
\index{IDADLS@{\idadls} linear solver interface!optional input|)}
...
\ucfunction{IDASpilsSetJacTimesB}
{
  flag = IDASpilsSetJacTimesB(ida\_mem, which, jsetupB, jtimesB);
}
{
  The function \ID{IDASpilsSetJacTimesB} specifies the Jacobian-vector 
...
}
{
  ...
  \item[which] (\id{int})
    the identifier of the backward problem.
  \item[jtsetupB] (\id{IDASpilsJacTimesSetupFnB})
...
{
  The function types \id{IDASpilsJacTimesVecFnB} and 
  \id{IDASpilsJacTimesSetupFnB} are described in
  \S\ref{ss:jactimesvec_b}.
}
%%
\ucfunction{IDASpilsSetJacTimesBS}
{
  flag = IDASpilsSetJacTimesBS(ida\_mem, which, jsetupBS, jtimesBS);
}
{
  The function \ID{IDASpilsSetJacTimesBS} specifies the Jacobian-vector 
...
}
{
  ...
  \item[which] (\id{int})
    the identifier of the backward problem.
  \item[jtsetupBS] (\id{IDASpilsJacTimesSetupFnBS})
    user-defined function to set up the Jacobian-vector product.
    Pass \id{NULL} if no setup is necessary.
  \item[jtimesBS] (\id{IDASpilsJacTimesVecFnBS})
    user-defined Jacobian-vector product function.
  \end{args}
}
{
...
{
  The function types \id{IDASpilsJacTimesVecFnBS} and 
  \id{IDASpilsJacTimesSetupFnBS} are described in
  \S\ref{ss:jactimesvec_b}.
}
%%
%%
...
{
  flag = IDASpilsSetEpsLinB(ida\_mem, which, eplifacB);
}
{
...
%%
%%
\ucfunction{IDASpilsSetIncrementFactorB}
{
  flag = IDASpilsSetIncrementFactorB(ida\_mem, which, dqincfacB);
}
{
  The function \ID{IDASpilsSetIncrementFactorB} specifies the factor 
...
}
{
  ...
  Passing a value \id{dqincfacB}$ = 0.0$ also indicates using the default value.
}
...
\subsection{Jacobian information for the backward problem 
  (direct method Jacobian)}\label{ss:djacFn_b}

If the direct linear solver interface  is used for the backward problem (i.e. 
\Id{IDADlsSetLinearSolverB} is called in the step described in 
\S\ref{ss:skeleton_adj}), the user may
provide a function of type \ID{IDADlsJacFnB} or \ID{IDADlsJacFnBS} 
(see \S\ref{ss:optional_input_b}), defined as follows:
%%
\index{Jacobian approximation function!user-supplied (backward)}
\usfunction{IDADlsJacFnB}
{
  typedef int (*IDADlsJacFnB)(&realtype tt, realtype cjB, \\
...
}
{
  ...
  \item[tmp3B]
    ...
    which can be used by the \id{IDADlsJacFnB} function 
    as temporary storage or work space.
  \end{args}
}
{
  An \id{IDADlsJacFnB} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\idas} will attempt to correct, while {\idadls} sets
  ...
  returns \Id{IDA\_LSETUP\_FAIL} and {\idadls} sets \id{last\_flag} to 
...
}
{
  ...
  {\warn}Before calling the user's \id{IDADlsJacFnB}, {\idas} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurs in the interpolation, {\idas} triggers an unrecoverable
  failure in the Jacobian function which will halt the integration
  (\id{IDASolveB} returns \id{IDA\_LSETUP\_FAIL} and {\idadls} sets \id{last\_flag} to 
  \Id{IDADLS\_JACFUNC\_UNRECVR}).
}

\index{Jacobian approximation function!user-supplied (backward)}
\usfunction{IDADlsJacFnBS}
{
  typedef int (*IDADlsDenseJacFnBS)(&realtype tt, realtype cjB, \\
                               &N\_Vector yy, N\_Vector yp,\\
                               &N\_Vector *yS, N\_Vector *ypS,\\
                               &N\_Vector yB, N\_Vector ypB,\\
                               &N\_Vector resvalB,\\
...
}
{
  ...
  \item[tmp3B]
    ...
    can be used by \id{IDADlsJacFnBS} as temporary storage or work space.    
  \end{args}
}
{
  An \id{IDADlsJacFnBS} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\idas} will attempt to correct, while {\idadls} sets
  \id{last\_flag} to \Id{IDADLS\_JACFUNC\_RECVR}), or a negative 
  value if it failed unrecoverably (in which case the integration is halted, \id{IDASolveB}
  returns \Id{IDA\_LSETUP\_FAIL} and {\idadls} sets \id{last\_flag} to 
...
}
{
  ...
  {\warn}Before calling the user's \id{IDADlsJacFnBS}, {\idas} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurs in the interpolation, {\idas} triggers an unrecoverable
  failure in the Jacobian function which will halt the integration
...
\index{Jacobian approximation function!Jacobian-vector product!user-supplied (backward)}
If the {\idaspils} solver interface is selected for the backward problem
(i.e., \id{IDASpilsSetLinearSolverB} is called in the steps described in
\S\ref{ss:skeleton_adj}), the user may
provide a function of type \\ \noindent
 \Id{IDASpilsJacTimesVecFnB} or 
\Id{IDASpilsJacTimesVecFnBS} in the following form,
...
{  
  The return value of a function of type \id{IDASpilsJtimesVecFnB} should be
...
{  
  The return value of a function of type \id{IDASpilsJtimesVecFnBS} should be
  $0$ if successful or nonzero if an error was encountered, in which case
  the integration is halted.
}
{
...
%%-------------------------------------------------------------------------------------
\subsection{Jacobian information for the backward problem
   (matrix-vector setup)}\label{ss:jactimesvecsetup_b}
...
user-supplied function of type \Id{IDASpilsJacTimesSetupFnB}, defined as follows:
%%
\usfunction{IDASpilsJacTimesSetupFnB}
{
  typedef int (*IDASpilsJacTimesSetupFnB)(&realtype tt, \\
                                        &N\_Vector yy, N\_Vector yp, \\
                                        &N\_Vector yB, N\_Vector ypB, \\
                                        &N\_Vector resvalB, \\
                                        &realtype cjB, void *user\_dataB);
...
} 
{
  ...
  If the user's \id{IDASpilsJacTimesVecFnB} function uses difference quotient
...
%%
\usfunction{IDASpilsJacTimesSetupFnBS}
{
  typedef int (*IDASpilsJacTimesSetupFnBS)(&realtype tt, \\
                                        &N\_Vector yy, N\_Vector yp, \\
                                        &N\_Vector *yyS, N\_Vector *ypS, \\
                                        &N\_Vector yB, N\_Vector ypB, \\
                                        &N\_Vector resvalB, \\
...
\index{preconditioning!user-supplied}
\index{IDASPILS@{\idaspils} linear solver!preconditioner solve function}
...
\index{preconditioning!user-supplied}
\index{IDASPILS@{\idaspils} linear solver!preconditioner setup function}

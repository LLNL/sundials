...
\item \label{i:lin_solverB}
  ...
  Create the linear solver object for the backward problem by
...
\item
  {\bf Free linear solver and matrix memory for the backward problem}
...
}
{
  ...
  \item[LS] (\id{SUNLinearSolver})
    {\sunlinsol} object to use for solving Newton linear systems for the backward problem. 
  \item[A] (\id{SUNMatrix})
...
}
{
  ...
  \item[\Id{IDADLS\_LMEM\_NULL}]
    The linear solver has not been initialized with a call to \id{IDADlsSetLinearSolverB}.
  \item[\Id{IDADLS\_ILL\_INPUT}]
    The parameter \id{which} represented an invalid identifier.
  \end{args}
}
...
\S\ref{ss:skeleton_adj}), the user may
provide a function of type \Id{IDASpilsJacTimesVecFnB} or 

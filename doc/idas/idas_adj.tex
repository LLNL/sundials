...
\item \label{i:matrixB}
  ...
  Create the template Jacobian matrix for the backward problem by
  using the appropriate functions defined by the particular {\sunmatrix}
  implementation desired. 
  
  Note that it is not required to use the same matrix type for both the forward 
...
area vailable for the backward problem.  They should be created as 
for the forward problem then attached to the memory struicture for the backward
problem.

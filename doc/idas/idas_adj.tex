...
problem using one of teh following functions.
...
\index{optional input!dense linear solver interface|(}
...
\index{IDASPILS@{\idaspils} linear solver!optional input|(}
...
}
{
  ...
  \item[which] (\id{int})
    the identifier of the backward problem.
  \item[jtsetup] (\id{IDASpilsJacTimesSetupFnB})
...
If the direct linear solver interface  is used for teh backward problem (i.e. 
...
}
{
  ...
  \item[tmp3B]
    ...
    which can be used by \id{IDADlsJacFnB} function 

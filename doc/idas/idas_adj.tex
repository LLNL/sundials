...
\item
  ...
  the adjoint memory  block (created by \id{IDAAdjInit} and returns an identifier 
...
}
{
  ...
  If an error occured, \id{IDAAdjInit} also prints an error message to the
...
}
{
  ...
  \item[itask] (\id{int})
    ...
    return an approximate value of $y($\id{tout}$)$ and $y'($\id{tout}$)$. 
...
}
{
  ...
  value of $y(t)$ and \id{ypret} the value of $y'(t)$. Additionally, it returns 
...
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the adjoint memory block returned by \id{IDAInit}.
  \item[which] (\id{int})
...
\end{verbatim}
%%  flag = IDADiagB(ida_mem);
%%  flag = IDASpbcgB(ida_mem, which, pretypeB, maxlB);
%%  flag = IDASptfqmrB(ida_mem, which, pretypeB, maxlB);

Their return value \id{flag} (of type \id{int}) can have any of the return values 
of their counterparts.
If the \id{ida\_mem} argument was \id{NULL}, \id{flag} will be
\id{IDADENSE\_MEM\_NULL}, \id{IDADIAG\_MEM\_NULL}, 
\id{IDABAND\_MEM\_NULL}, or \id{IDASPILS\_MEM\_NULL}. Also, if \id{which} is not a
valid identifier, the functions will return \id{IDADENSE\_ILL\_INPUT}, \id{IDADIAG\_ILL\_INPUT}, 
\id{IDABAND\_ILL\_INPUT}, or \id{IDASPILS\_ILL\_INPUT}
...
}
{
  ...
  $yB^\prime(tB_0)$ which were specified in the previous call to \id{IDAInitB} 
...
}
{
  ...
  $yB^\prime(tB_0)$ which were specified in the previous call to \id{IDAInitBS} 
...
\end{verbatim}
%%   flag = IDASetErrHandlerB(ida_mem, ehfunB, eh\_dataB);
%%   flag = IDASetErrFileB(ida_mem, errfpB);
%%   flag = IDASetIterTypeB(ida_mem, iterB);
%%   flag = IDASetStabLimDetB(ida_mem, stldetB);
%%   flag = IDASetInitStepB(ida_mem, which, hinB);
Their return value \id{flag} (of type \id{int}) can have any of the return values 
...
}
{
  ...
  \item[\Id{IDADIRECT\_NO\_ADJ}]
    The function \id{IDAAdjInit} has not been previously called.
  \item[\Id{IDADIRECT\_LMEM\_NULL}]
    The {\idadense} linear solver has not been initialized through a call to \id{IDABandB}.
  \item[\Id{IDADIRECT\_ILL\_INPUT}]
    The parameter \id{which} represented an invalid identifier.
  \end{args}
}
...
%% {
%%   flag = IDASpilsSetPrecTypeB(ida\_mem, pretypeB);
%% }
%% {
...
}
{
  ...
  {\warn}Before calling the user's \id{IDAResFnB}, {\idaa} needs to evaluate
...
}
{
  ...
  {\warn}Before calling the user's \id{IDAResFnBS}, {\idaa} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\idaa} triggers an unrecoverable
  failure in the righ-hand side function which will halt the integration and
  \id{IDASolveB} will return \id{IDA\_RHSFUNC\_FAIL}.
...
}
{
  ...
  {\warn}Before calling the user's \id{IDAQuadRhsFnBS}, {\idaa} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\idaa} triggers an unrecoverable
  failure in the quadrature righ-hand side function which will halt the integration and
  \id{IDASolveB} will return \id{IDA\_QRHSFUNC\_FAIL}.
...
}
{
  ...
  to it). 
%%   If the backward problem is the adjoint of the original IVP, then
...
}
{
  ...
  The type of \id{JacB} is \Id{DenseMat}. The user is referred to 
  \S\ref{ss:djacFn} for details regarding accessing a \id{DenseMat} object.

  {\warn}Before calling the user's \id{IDADlsDenseJacFnB}, {\idaa} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\idaa} triggers an unrecoverable
...
}
{
  ...
  \item[tmp3B]
    are pointers to memory allocated  for variables of type \id{N\_Vector} which 
    can be used by \id{IDABandJacFnB} as temporary storage or work space.    
  \end{args}
}
{
...
{
  If successful, \id{IDABBDPrecInitB} stores a pointer to the newly created 
...
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the {\idas} memory block returned by \id{IDAAdjInit}.
  \item[which] (\id{int})
    The identifier of the backward problem.
  \item[maxlB] (\id{int})
...
{
  A \id{IDABBDLocalFnB} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\idas} will attempt to correct), or a negative 
  value if it failed unrecoverably (in which case the integration is halted and
...
}
{
  ...
  {\warn}Before calling the user's \id{IDABBDLocalFnB}, {\idaa} needs to evaluate
  (through interpolation) the values of the states from the forward integration. 
  If an error occurrs in the interpolation, {\idaa} triggers an unrecoverable
...
{
  A \id{IDABBDCommFn} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\idas} will attempt to correct), or a negative 
  value if it failed unrecoverably (in which case the integration is halted and
  \id{IDASolveB} returns \Id{IDA\_LSETUP\_FAIL}). 
}

...
\begin{center}
  \begin{tabular}{|p{\colone}|l|l|} \hline
    ...
    IVP & \id{idasAkzoNob\_dns} \id{idasFoodWeb\_bnd}   & \id{idasBruss\_kry\_bbd\_p}   \\
    {}  & \id{idasHeat2D\_bnd} \id{idasHeat2D\_kry}     & \id{idasFoodWeb\_kry\_bbd\_p} \\
    {}  & \id{idasKrylovDemo\_ls}                       & \id{idasFoodWeb\_kry\_p}      \\
    {}  & \id{idasRoberts\_dns}                         & \id{idasHeat2D\_kry\_bbd\_p}  \\
    {}  & \id{idasSlCrank\_dns}                         & \id{idasHeat2D\_kry\_p}       \\
    \hline
...
\item \id{idasRobert\_FSA\_dns}
  solves a 3-species chemical kinetics problem (\id{idasRoberts\_dns}).
  \newline
  {\idas} computes both its solution and solution sensitivities with respect
  to the three reaction rate constants appearing in the model. 
  This program solves the problem with {\idadense} linear solver, and a 
...
\item \id{idasSlCrank\_FSA\_dns}
  ...
  and then uses them to evaluate the gradient of the cummulative kinetic energy
  of the system.
...
\item \id{idasRoberts\_ASAi\_dns}
  solves a 3-species chemical kinetics problem (\id{idasRoberts\_dns}).
  \newline
  The adjoint capability of {\idas} is used to compute gradients
...
\item \id{idasHeat2D\_kry\_bbd\_p}
  ...
  This program solves the problem in parallel, using the {\idaspgmr} linear solver
...
\item \id{idasHeat2D\_FSA\_kry\_bbd\_p}
  solves the same problem as \id{idaHeat2D\_kry\_p} with forward sensitivity enabled 
  to compute the solution sensitivity with respect to two coefficients of the
  original PDE.
...
{\idas} in a parallel environment (using {\nvecp}) on a modification of
...
the installation of {\sundials} (see \ugref{c:install}). As a consequence,

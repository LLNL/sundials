...
The header files for the dense and banded linear solvers (both internal and Lapack) include \id{idas\_direct.h} 
...
own error handler function ( see \S\ref{ss:user_fct_sim} ).
...
}
{
  ...
  {\idas} memory block (of type \id{void *}), otherwise returns \id{NULL}.
}
{}
%%
%%
...
then \id{abstol} should be a vector.  See the example \id{idasdenx} in the
...
}
{
  ...
  The user's main function must include the \id{idas\_dense.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main function must include the \id{idas\_lapack.h} header file.
}
{
...
}
{
  ...
  The user's main function must include the \id{idas\_band.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main function must include the \id{idas\_spgmr.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main function must include the \id{idas\_spbcgs.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  The user's main function must include the \id{idas\_sptfqmr.h} header file.
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
...
}
{
  ...
  \item[ida\_mem] (\id{void *})
    pointer to the {\idas} memory block.
  \item[t] (\id{realtype})
    time at which the to interpolate.
  \item[k] (\id{int})
...
}
{
  ...
  successful return from \id{IDASolve}. See \id{IDAGetCurrentTime},
  \id{IDAGetLastStep} and \id{IDAGetLastOrder} for access to $t_n$ $h_u$
  and $kused$.
}
...
{
  typedef int (*IDADlsDenseJacFn&)(long int Neq, realtype tt, realtype c\_j, \\
                             &N\_Vector yy, N\_Vector yp, N\_Vector rr, \\ 
                             &void *user\_data, DlsMat Jac, \\
...
\id{yp} and the quadrature equations from within \id{res}. The following is an overview 
of the sequence of calls in a user's main program in this situation. Steps that are
...
{
  If an error occured, \id{IDAQuadInit} also sends an error message to the
  error handler function.
}
%%
\index{memory requirements!IDAS@{\idas} solver}
...
and must follow a call to \Id{IDAQuadInit} (and maybe a call to \id{IDAReInit}). 
...
}
{
  ...
  mechanism. If so, the user must call \id{IDAQuadSStolerances} or
  \id{IDAQuadSVtolerances} to specify the 
...
{
  typedef int (*IDAQuadRhsFn)(&realtype t, N\_Vector y, N\_Vector yy, N\_Vector yp, \\
                             &N\_Vector rhsQ, void *user\_data);
}
{
  This function computes the quadrature equation right-hand side for a given value
  of the independent variable $t$ and state vectors $y$ and $y'$.
}
{
  \begin{args}[user\_data]
  \item[t]
  ...
  \item[yp]
    is the current value of the dependent variable vector, $y'(t)$.
  \item[rhsQ]
    is the output vector $f_Q(t,y,y')$.
  \item[user\_data]
...
{
  Allocation of memory for \id{rhsQt} is automatically handled within {\idas}.
  ...
  \Id{IDA\_FIRST\_QRHS\_ERR}) or if a recoverable error is reported when \id{IDAQuadRhsFn}

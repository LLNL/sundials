%%===================================================================================
\chapter{Using IDAS for C Applications}\label{s:simulation}
%%===================================================================================
...
program, description of the {\idas} user-callable functions, and 
...
are given for the two implementations provided with {\idas}: steps marked with {\p}
...
}
{
  ...
  {\idas} memory block (of type \id{void *}).
}
{}
%%
%%
...
linear systems of the form (\ref{e:DAE_Newtoncorr}).  There are five {\idas} linear
...
the linear solver module used by {\idas} is actually built on top of a generic
...
{\idas} provides an extensive list of functions that can be used to change
various optional input parameters that control the
behavior of the {\idas} solver from their default values. 
...
\begin{table}
\centering
\caption{Optional inputs for {\idas}, {\idadls}, and {\idaspils}}
...
}
{
  ...
  where all {\idas} messages should be directed in case the default
...
}
{
  ...
  for the internal {\idas} memory block, its value
...
}
{
  ...
  $y$ and $y'$ for any value of $t$ in the last internal step taken by {\idas}.
...
\begin{table}
\centering
\caption{Optional outputs from {\idas}, {\idadense}, {\idaband}, and {\idaspils}}
...
{
  The only argument, of type \id{int} is a return flag from a {\idas} function.
}
{
...
\item
  ...
  Call \id{IDAQuadMalloc} to specify the quadrature equation right-hand
...
\item
  ...
  Call \id{IDAGetQuad} to obtain the values of the quadrature variables at
...
\end{Steps}
%%
\id{IDAQuadMalloc} can be called and quadrature-related optional inputs 
...
The function \id{IDAQuadMalloc} activates integration of quadrature equations 
and allocates internal memory related to these calculations.
The form of the call to this function is as follows:
%%
\ucfunction{IDAQuadMalloc}
{
flag = IDAQuadMalloc(ida\_mem, fQ, yQ0);
}
{
  The function \Id{IDAQuadMalloc} provides required problem specifications,
...
}
{
  ...
  \item[\Id{IDA\_SUCCESS}]
    The call to \id{IDAQuadMalloc} was successful.
  \item[\Id{IDA\_MEM\_NULL}] 
...
{
  If an error occured, \id{IDAQuadMalloc} also sends an error message to the
  error handler function.
}
%%
\index{memory requirements!IDAS@{\idas} solver}
...
\item With \id{itolQ = IDA\_SV} (see \id{IDASetQuadErrCon}): \id{lenrw} $=$ \id{lenrw} $+ N_q$ 
\end{itemize}
the size of the integer workspace is increased by:
\begin{itemize}
\item Base value: \id{leniw} $=$ \id{leniw} $+$ (\id{maxord+5})$N_q$
\item With \id{itolQ = IDA\_SV}: \id{leniw} $=$ \id{leniw} $+ N_q$ 
\end{itemize}

The function \id{IDAQuadReInit}, useful during the solution of a sequence of problems of 
same size, reinitializes the quadrature related internal memory 
and must follow a call to \Id{IDAQuadMalloc} (and maybe a call to \id{IDAReInit}). 
The number \id{Nq} of quadratures is assumed to be unchanged from the prior call to 
\id{IDAQuadMalloc}.
The call to the \id{IDAQuadReInit} function has the form:
%%
\ucfunction{IDAQuadReInit}
{
  flag = IDAQuadReInit(ida\_mem, fQ, yQ0);
}
{
...
}
{
  ...
  \item[\Id{IDA\_NO\_QUAD}] 
    ...
    call to \id{IDAQuadMalloc}.
  \end{args}
}
{
  If an error occured, \id{IDAQuadReInit} also sends an error message to the
  error handler function.
}
%%
...
If quadrature integration has been initialized by a call to \id{IDAQuadMalloc},
...
}
{
  \begin{args}[ida\_mem]
  \item[ida\_mem] (\id{void *})
    pointer to the memory previously allocated by \id{IDAMalloc}.
  \item[t] (\id{realtype})
...
This function is called by \id{IDAGetQuad} with \id{k = 0}, but may also be called 
...
%%
%%
\ucfunction{IDASetQuadFdata}
{
  flag = IDASetQuadFdata(ida\_mem, fQ\_data);
}
{
  The function \Id{IDASetQuadFdata} specifies the user-defined data block \Id{fQ\_data}
  and attaches it to the main {\idas} memory block.
}
{
  \begin{args}[ida\_mem]
...
}
{
  ...
  pointer \id{f\_data} set through \id{IDASetFdata}.
}
%%
%%
\ucfunction{IDASetQuadErrCon}
{
 flag = IDASetQuadErrCon(ida\_mem, errconQ, itolQ, reltolQ, abstolQ);
}
{
...
}
{
  ...
  \item[itolQ] (\id{int}) 
    is either \Id{IDA\_SS} or \Id{IDA\_SV}, where \Id{itolQ} \id{= IDA\_SS} indicates 
    scalar relative error tolerance and scalar absolute error tolerance, 
    while \id{itolQ = IDA\_SV} indicates scalar relative error tolerance and 
  ...
  \item[abstolQ] (\id{void *})
    is a pointer to the absolute error tolerance.  If \id{itolQ}$=$\id{IDA\_SS},
    \id{abstolQ} must be a pointer to a \id{realtype} variable. If
    \id{itolQ} $=$ \id{IDA\_SV}, \id{abstolQ} must be an \id{N\_Vector} variable.
  \item
  \end{args}
}
{
  ...
  \item[\Id{IDA\_MEM\_NULL}]
    The \id{ida\_mem} pointer is \id{NULL}.
  \item[\Id{IDA\_ILL\_INPUT}] 
    An input argument to \id{IDASetQuadErrCon} has an illegal value.
  \end{args}
}
{
...
{
  flag = IDAGetQuadNumRhsEvals(ida\_mem, \&nfQevals);
}
{
...
{
  flag = IDAGetQuadStats(ida\_mem, \&nfQevals, \&nQetfails);
}
{
...
{
  typedef int (*IDAQuadRhsFn)(&realtype t, N\_Vector y, \\
...
}
{
  ...
  \item[fQ\_data]
    is the \Id{fQ\_data}      
    pointer passed to \id{IDASetQuadFdata}.   
  \end{args}
}
{
  ...
  \Id{IDA\_QRHSFUNC\_FAIL} is returned).
}
{
  Allocation of memory for \id{yQdot} is automatically handled within {\idas}.
  ...
  There are two situations in which recovery is not possible even if \id{IDAQuadRhsFn}
  function returns a recoverable error flag. This include the situation when this
  occurrs at the very first call to the \id{IDAQuadRhsFn} (in which case {\idas} returns
  \Id{IDA\_FIRST\_QRHSFUNC\_ERR}) or if a recoverable error is reported when \id{IDAQuadRhsFn}
...
\item \label{i:bbdpre_attach}
  ...
  \begin{itemize}
    \item Attaches the {\idaspgmr}, {\idaspbcg}, or {\idasptfqmr} linear solver
     to the main {\idas} solver memory;

...
solver modules available for use with {\idas} are:
%%
\begin{itemize}
\item \Id{idas/idas\_direct.h},
...
iteration, there are two {\idas} linear solver interfaces currently
...
passed to these functions is the {\idas} memory pointer returned by
...
to one of these functions initializes the appropriate {\idas} linear
solver interface, linking this to the main {\idas} integrator, and
...
\ref{s:sunlinsol}.
\index{IDAS@{\idas} linear solver interfaces!selecting one|)}
...
}
{
  ...
  Jacobian {\sunmatrix} object \id{J} to {\idas}, initializing the
...
{
  When forward sensitivity analysis capabilities are enabled and the staggered
  corrector approach is used this function is used to set the nonlinear solver
  method for correcting state variables (see \S\ref{sss:idafwd_nonlin_solv_init}
  for more details).
}

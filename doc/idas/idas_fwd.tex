...
\item 
  ...
  \id{NLS = SUNNonlinSol\_***Sens(...);} where \id{***} is the name of the nonlinear
...
\item
  ...
  interface by attaching the nonlinear solver object by calling \id{ier =
  }\Id{IDASetNonlinearSolverSensSim}\id{(ida\_mem, NLS);} when using the
  simultaneous corrector method or \id{ier =
  }\Id{IDASetNonlinearSolverSensStg}\id{(ida\_mem, NLS);} when using the
  staggered corrector method (see \S\ref{sss:idafwd_nonlin_solv_init} for details).
...
\S\ref{s:sunnonlinsol_newton}). To specify a different nonlinear solver in
...
\Id{IDASetNonlinearSolverSensSim} when using the simultaneous 
corrector method, or \Id{IDASetNonlinearSolver} (see
\S\ref{sss:nonlin_solv_init}) and \\ \noindent
\Id{IDASetNonlinearSolverSensStg} when using
the staggered corrector method, as documented below.
...
\ucfunction{IDASetNonlinearSolverSensSim}
{
...
\ucfunction{IDASetNonlinearSolverSensStg}
{

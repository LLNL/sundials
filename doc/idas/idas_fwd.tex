...
\item
  ...
  (Here the argument \id{y0} serves only to provide the \id{N\_Vector} type
  for cloning.)  Then, for each $i=0,\ldots,$\id{Ns}$-1$, load initial values
  ...
  and then attach the \id{realtype} array \id{yS0\_i} containing the initial values
  ...
  The initial conditions for the \id{Ns} sensitivity derivative vectors \id{ypS0}

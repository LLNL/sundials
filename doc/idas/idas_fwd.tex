...
The only departure from this philosophy is due to the \id{IDARhsFn} type definition
...
\item
  ...
  Call \id{flag = }\Id{IDASensMalloc}\id{(\ldots);} to activate forward 
...
\item
  ...
  \id{flag = }\Id{IDAGetSens}\id{(ida\_mem, t, yS);}
...
%%
\ucfunction{IDASensMalloc}
{
  flag = IDASensMalloc(ida\_mem, Ns, ism, yS0);
}
{
  The routine \ID{IDASensMalloc} activates forward sensitivity computations and
...
}
{
  ...
  \item[ism] (\id{int})
    ...
    be \id{IDA\_SIMULTANEOUS}, \id{IDA\_STAGGERED}, or \id{IDA\_STAGGERED1}:
    \begin{itemize}
...
}
{
  ...
  \item[\Id{IDA\_SUCCESS}]
    The call to \id{IDASensMalloc} was successful.
  ...
  \item[\Id{IDA\_ILL\_INPUT}] 
    An input argument to \id{IDASensMalloc} has an illegal value.
  \end{args}
}
{
  If an error occured, \id{IDASensMalloc} also prints an error message to the
...
and must follow a call to \Id{IDASensMalloc} (and maybe a call to \id{IDAReInit}). 
...
}
{
  ...
  \item[\Id{IDA\_NO\_SENS}]
    ...
    previous call to \id{IDASensMalloc}.
  \item[\Id{IDA\_ILL\_INPUT}] 
...
to \id{IDASensMalloc}), the user must call
%%
...
}
{
  ...
  computaions by a previous call to \id{IDASensMalloc}.
}
{
...
}
{
  ...
  only by calling again \id{IDASensMalloc}.
}
%%
%%
...
\item[\Id{IDA\_REPTD\_SRHSFUNC\_ERR}]
  ...
  the sensitivity right-hand side function. The \id{IDA\_REPTD\_RHSFUNC\_ERR} will also
...
If forward sensitivity computations have been initialized by a call to \id{IDASensMalloc},
or reinitialized by a call to \id{IDASensReInit}, then {\idas} computes both a solution
and sensitivities at time \id{t}. However, \id{IDASolve} will still return only the solution
...
}
{
  ...
  to \id{IDASensMalloc}). The compatibility test is performed at the first
  step in \id{IDASolve}.
...
}
{
  ...
  \id{IDASensMalloc}, but can also be used with the other two choices
...
{
  {\warn}This function must be preceeded by a call to \id{IDASensMalloc}.
}
%%
%%
...
}
{
  ...
  This function must be preceeded by a call to \id{IDASensMalloc}.
}
%%
%%
...
%%
%%
\ucfunction{IDAGetNumSensRhsEvals}
{
  flag = IDAGetNumSensRhsEvals(ida\_mem, \&nfSevals);
}
{
  The function \ID{IDAGetNumSensRhsEvals} returns the number of calls to the sensitivity
...
%%
%%
\ucfunction{IDAGetNumRhsEvalsSens}
{
  flag = IDAGetNumRhsEvalsSens(ida\_mem, \&nfevalsS);
}
{
  The function \ID{IDAGetNumRhsEvalsSEns} returns the number of calls to the
...
%%
%%
\ucfunction{IDAGetNumSensErrTestFails}
{
  flag = IDAGetNumSensErrTestFails(ida\_mem, \&nSetfails);
}
{
  The function \ID{IDAGetNumSensErrTestFails} returns the number of local
...
%%
%%
\ucfunction{IDAGetNumSensLinSolvSetups}
{
  flag = IDAGetNumSensLinSolvSetups(ida\_mem, \&nlinsetupsS);
}
{
  The function \ID{IDAGetNumSensLinSolvSetups} returns the number of calls
...
}
{
  ...
  either the \id{ism}$=$\id{IDA\_STAGGERED} or the \id{ism}$=$\id{IDA\_STAGGERED1} sensitivity
  solution method has been specified in the call to \id{IDASensMalloc}
...
{
  This counter is incremented only if the \id{ism} was \id{IDA\_STAGGERED} or 
  \id{IDA\_STAGGERED1} in the call to \id{IDASensMalloc} (see \S\ref{ss:sensi_malloc}).
  
  In the \id{IDA\_STAGGERED1} case, the value of \id{nSniters} is the sum of 
  ...
  \id{IDAGetNumStgrSensNonlinSolvIters} (see below).
}
%%
%%
...
}
{
  ...
  In the \id{IDA\_STAGGERED1} case, the value of \id{nSncfails} is the sum of 
  the number of nonlinear convergence failures that occured for each sensitivity equation.
  These individual counters can be obtained through a call to
  \id{IDAGetNumStgrSensNonlinConvFails} (see below).
}
%%
%%
...
\id{IDASensMalloc}, the user may provide the right-hand sides of the sensitivity equations
(\ref{e:sens_eqns}), for all sensitivity parameters at once, through a function 
of type \ID{IDASensRhsFn} defined by:
%%
\usfunction{IDASensRhsFn}
{
...
}
{
  ...
  \item[ySdot]
     is the output of \id{IDASensRhsFn}. On exit it must contain
    the sensitivity right-hand side vectors.
  \item[f\_data]
    is a pointer to user data - the same as the \Id{fS\_data}      
    parameter passed to \id{IDASetSensRhsFn}.
  \item[tmp1]
  \item[tmp2]
...
{
  A \id{IDASensRhsFn} should return 0 if successful, a positive value if a recoverable
  error occurred (in which case {\idas} will attempt to correct), or a negative 
  value if it failed unrecoverably (in which case the integration is halted and
  \Id{IDA\_SRHSFUNC\_FAIL} is returned).
}
{
  {\warn}A sensitivity right-hand side function of type \id{IDASensRhsFn} is not
  compatible with the \id{IDA\_STAGGERED1} approach.
  ...
  recoverable error in \id{IDASensRhsFn} at that point cannot be corrected (as it will 
  ...
  There are two situations in which recovery is not possible even if \id{IDASensRhsFn}
  function returns a recoverable error flag. This include the situation when this
  occurrs at the very first call to the \id{IDASensRhsFn} (in which case {\idas} returns
  \Id{IDA\_FIRST\_SRHSFUNC\_ERR}) or if a recoverable error is reported when \id{IDASensRhsFn}
...
Note that a sensitivity right-hand side function of type \id{IDASensRhs1Fn} is compatible 
with any valid value of the \id{IDASensMalloc} argument \id{ism}, and is 
...
reported for \id{idasdenx} and the second run of the \id{idasfwddenx} example 
...
in the call to \id{IDASensMalloc}, the sensitivity variables are included
...
After all, in this case (\id{ism=IDA\_STAGGERED} or \id{ism=IDA\_STAGGERED1} in the call 
to \id{IDASensMalloc}), the sensitivity variables at a given step are 

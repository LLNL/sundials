...
}
{
  ...
  \item[p] (\id{realtype *})
    ...
    $user\_data$ passed to the righ-hand side function.
...
%%
%%
\ucfunction{IDAGetNumSensResEvals}
{
  flag = IDAGetNumSensResEvals(ida\_mem, \&nfSevals);
}
{
  The function \ID{IDAGetNumSensResEvals} returns the number of calls to the sensitivity
  residual function.
}
{
  \begin{args}[ida\_mem]
...
}
{
  ...
  \id{IDA\_STAGGERED1} in the call to \id{IDASensInit} (see \S\ref{ss:sensi_init}).
...
in \cite{idas2.4.0_ex} indicates that this may not always be the case.

...
\id{IDA\_NO\_MALLOC}      & -3  & The {\idas} memory was not allocated by a call to \id{IDAMalloc}. \\
...
\id{IDADIRECT\_SUCCESS}    &  0 & Successful function return. \\
\id{IDADIRECT\_MEM\_NULL}  & -1 & The \id{ida\_mem} argument was \id{NULL}.\\
\id{IDADIRECT\_LMEM\_NULL} & -2 & The {\idadls} linear solver has not been initialized.\\
\id{IDADIRECT\_ILL\_INPUT} & -3 & The {\idadls} solver is not compatible with the current {\nvector} module.\\
\id{IDADIRECT\_MEM\_FAIL}  & -4 & A memory allocation request failed.\\
\id{IDADIRECT\_JACFUNC\_UNRECVR} & -5 & The Jacobian function failed in an unrecoverable manner. \\
\id{IDADIRECT\_JACFUNC\_RECVR}   & -6 & The Jacobian function had a recoverable error. \\

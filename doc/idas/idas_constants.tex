...
\\\hline
\multicolumn{3}{c}{\bf {\idaa} adjoint solver module}\\
\hline\\
%%
...
\id{IDA\_NO\_ADJ}            & -100 & The combined forward-backward problem has not been initialized.\\
\id{IDA\_BAD\_TB0}           & -101 & The desired output for backward problem is outside the interval over which the forward problem was solved. \\
\id{IDA\_REIFWD\_FAIL}       & -102 & No checkpoint is available for this hot start. \\
\id{IDA\_FWD\_FAIL}          & -103 & \id{IDASolveB} failed because \id{IDASolve} was unable to store data between two consecutive checkpoints. \\
\id{IDA\_GETY\_BADT}         & -104 & Wrong time in interpolation function. \\
\id{IDA\_NO\_BCK}            & -105 & No backward problem was specified. \\
\id{IDA\_NO\_FWD}            & -106 & \id{IDASolveF} has not been previously called. \\
...
\id{IDASPILS\_NO\_ADJ}    & -100 & The combined forward-backward problem has not been initialized.\\

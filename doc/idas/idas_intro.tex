...
{\idas}'s previous direct and iterative linear solver interfaces,
...
of ``DIRECT'', ``ITERATIVE'', or ``MATRIX\_ITERATIVE'' type.
...
challenges in user migration to the new names, the previous {\CC}
routine names may still be used; these will be deprecated in 
future releases, so we recommend that users migrate to the new names
soon.  Details regarding how {\idals} utilizes linear

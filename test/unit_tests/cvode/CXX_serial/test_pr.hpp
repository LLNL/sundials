...
inline sunrealtype r_dot(sunrealtype t)
{
  return ONE / (t * t + ONE);
}

inline sunrealtype r_dot2(sunrealtype t)
{
  return -(TWO * t) / std::pow((t * t + ONE), 2);
}

inline sunrealtype r_dot3(sunrealtype t)
{
  return (SIX * t * t - TWO) / std::pow((t * t + ONE), 3);
}

inline sunrealtype r_dot4(sunrealtype t)
{
  return -(TWENTYFOUR * t * (t * t - ONE)) / std::pow((t * t + 1), 4);
}

inline sunrealtype r_dot5(sunrealtype t)
{
  return (TWENTYFOUR * (FIVE * std::pow(t, 4) - TEN * t * t + ONE)) /
         std::pow((t * t + ONE), 5);
}

inline int PR_true(sunrealtype t, N_Vector y, PRData& pr_data)
{
...
inline int PR_true_dot(sunrealtype t, N_Vector ydot, int order, PRData& pr_data)
{
 ...
 * ---------------------------------------------------------------------------*/
inline int PR_Rhs(sunrealtype t, N_Vector y, N_Vector ydot, PRData* pr_data)
{
 ...
 * ---------------------------------------------------------------------------*/
inline int PR_Jac(sunrealtype t, N_Vector y, SUNMatrix J, PRData* pr_data)
{
  const sunrealtype lambda = pr_data->lambda;
  const sunindextype N     = N_VGetLength(y);

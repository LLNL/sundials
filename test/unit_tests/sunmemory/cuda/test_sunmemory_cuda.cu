...
int test_instance(SUNMemoryHelper helper, SUNMemoryType mem_type, bool print_test_status)
{
  ...
  retval = SUNMemoryHelper_GetAllocStats(helper, mem_type,
                                         &num_allocations, &num_deallocations,
                                         &bytes_allocated, &bytes_high_watermark);
  if (retval)
  {
    if (print_test_status) std::cout << "  SUNMemoryHelper_GetAllocStats... FAILED\n";
    return -1;
  }
  ...
  if (print_test_status) std::cout << "  SUNMemoryHelper_GetAllocStats... PASSED\n";
  return retval;
}

...
static int f_forward_1(const sunrealtype t, const N_Vector y, const N_Vector ydot, void *const user_data) {
  N_VInv(y, ydot);
...
static int f_forward_2(const sunrealtype t, const N_Vector y, const N_Vector ydot, void *const user_data) {
  N_VInv(y, ydot);
  return 0;
}
...
static int test_forward(SUNContext ctx) {
  ...
  void* parititon_mem[] = {
    ARKStepCreate(f_forward_1, NULL, t0, y, ctx),
  ...
  if (SUNRCompareTol(exact_solution, numerical_solution, global_tol)) {
    ...
    fprintf(stderr, "Forward direction solution failed with an error of %" GSYM "\n", err);
    return 1;
  }

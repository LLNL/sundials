...
static int ReadInputs(std::vector<std::string> &args, Options &opts, SUNContext ctx);
...
int main(int argc, char* argv[])
{
  ...
  case(0):
    f_fe = f0;
    p = 2;
    std::cout << "    zero-valued operator, integrated with default 2nd-order ERK\n";
    PrintFastAdaptivity(opts); break;
  case(1):
    f_fe = ff;
    p = 2;
...
static int ff(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  NV_Ith_S(ydot,1) = opts->e*tmp1 - tmp2 + sdot(t,opts)/(TWO*vtrue(t,opts));
              ...
              N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  SM_ELEMENT_D(J,1,0) = opts->e/TWO + opts->e*(ONE+r(t,opts))/(TWO*u*u);
              ...
              N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  //   [                 0                             0           ]
  SM_ELEMENT_D(J,0,0) = opts->G/TWO + (opts->G*(ONE+r(t,opts))-rdot(t,opts))/(2*u*u);
  SM_ELEMENT_D(J,0,1) = opts->e/TWO + opts->e*(TWO+s(t,opts))/(TWO*v*v);
              ...
              N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  //   [                 0                       0           ]
  SM_ELEMENT_D(J,0,0) = opts->G/TWO + (opts->G*(ONE+r(t,opts)))/(2*u*u);
  SM_ELEMENT_D(J,0,1) = opts->e/TWO + opts->e*(TWO+s(t,opts))/(TWO*v*v);
  SM_ELEMENT_D(J,1,0) = ZERO;
  SM_ELEMENT_D(J,1,1) = ZERO;
...
void InputHelp()
{
  ...
  std::cout << "  --mri_type     : MRI method, int in [0,11] (see source for explanation)\n";
...
int ReadInputs(std::vector<std::string> &args, Options &opts, SUNContext ctx)
{
  ...
  if ((opts.rtol < ZERO) || (opts.rtol > ONE)) {
    std::cerr << "ERROR: rtol must be in (0,1), ("
              << opts.rtol << " input)\n";  return -1; }
  //   0 < atol < 1
  if ((opts.atol < ZERO) || (opts.atol > ONE)) {
    std::cerr << "ERROR: atol must be in (0,1), ("
              << opts.atol << " input)\n";  return -1; }
  //   slow_pq in {0,1}
  if ((opts.slow_pq < 0) || (opts.slow_pq > 1)) {
    std::cerr << "ERROR: slow_pq must be in {0,1}, ("
              << opts.slow_pq << " input)\n";  return -1; }
  //   fast_pq in {0,1}
  if ((opts.fast_pq < 0) || (opts.fast_pq > 1)) {
    std::cerr << "ERROR: fast_pq must be in {0,1}, ("
              << opts.fast_pq << " input)\n";  return -1; }
  //   mri_type in [0,11]
  if ((opts.mri_type < 0) || (opts.mri_type > 11)) {
    std::cerr << "ERROR: mri_type must be in [0,11], ("
              << opts.mri_type << " input)\n";  return -1; }
  ...
  if ((opts.fast_type < 0) || (opts.fast_type > 6)) {
    std::cerr << "ERROR: fast_type must be in [0,6], ("
              << opts.fast_type << " input)\n";  return -1; }
  //   scontrol in [0,16]
  if ((opts.scontrol < 0) || (opts.scontrol > 16)) {
    std::cerr << "ERROR: scontrol must be in [0,16], ("
              << opts.scontrol << " input)\n";  return -1; }
  //   fcontrol in [0,6]
  if ((opts.fcontrol < 0) || (opts.fcontrol > 6)) {
    std::cerr << "ERROR: fcontrol must be in [0,6], ("
              << opts.fcontrol << " input)\n";  return -1; }
  //   hs > 0 if scontrol == 0
  if ((opts.hs <= 0) && (opts.scontrol == 0)) {
    std::cerr << "ERROR: positive hs required with scontrol = 0, ("
              << opts.hs << " input)\n";  return -1; }
  //   hf > 0 if fcontrol == 0
  if ((opts.hf <= 0) && (opts.fcontrol == 0)) {
    std::cerr << "ERROR: positive hf required with fcontrol = 0, ("
              << opts.hf << " input)\n";  return -1; }
  //   G < 0.0
  if (opts.G >= ZERO) {
    std::cerr << "ERROR: G must be a negative real number, ("
              << opts.G << " input)\n";  return -1; }
  //   hs <= 1/|G| if scontrol == 0 and mri_type in [0,4]
  if ((opts.hs > ONE/SUNRabs(opts.G)) && (opts.scontrol == 0) &&
      (opts.mri_type >= 0) && (opts.mri_type < 5)) {
    std::cerr << "ERROR: hs must be in (0, 1/|G|) for fixed-step explicit MRI, ("
              << opts.hs << " input)\n";
    return -1; }
  //   w >= 1.0
  if (opts.w < ONE) {
    std::cerr << "ERROR: w must be >= 1.0, ("
              << opts.w << " input)\n";  return -1; }
...
static void PrintSlowAdaptivity(Options opts) {
  ...
  case(4):
    ...
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(5):
    std::cout << "    MRI-HTOL controller (using I for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(6):
    std::cout << "    Decoupled I controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(7):
    std::cout << "    MRI-HTOL controller (using PI for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(8):
    std::cout << "    Decoupled PI controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(9):
    std::cout << "    MRI-HTOL controller (using PID for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(10):
    std::cout << "    Decoupled PID controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(11):
    std::cout << "    MRI-HTOL controller (using ExpGus for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(12):
    std::cout << "    Decoupled ExpGus controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(13):
    std::cout << "    MRI-HTOL controller (using ImpGus for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(14):
    std::cout << "    Decoupled ImpGus controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(15):
    std::cout << "    MRI-HTOL controller (using ImExGus for H) based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(16):
    std::cout << "    Decoupled ImExGus controller for slow time scale, based on order of MRI "
              << ((opts.slow_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  }
}
...
static void PrintFastAdaptivity(Options opts) {
  ...
  case(3):
    ...
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(4):
    std::cout << "    ExpGus controller for fast time scale, based on order of RK "
              << ((opts.fast_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(5):
    std::cout << "    ImpGus controller for fast time scale, based on order of RK "
              << ((opts.fast_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  case(6):
    std::cout << "    ImExGus controller for fast time scale, based on order of RK "
              << ((opts.fast_pq==1) ? "method\n" : "embedding\n");
    std::cout << "    rtol = " << opts.rtol << ", atol = " << opts.atol << "\n"; break;
  }
}

    ...
    for (size_t iaccum = 0; iaccum < accum_types.size(); iaccum++)
    {
      ...
      for (size_t ipart = 0; ipart < udata.Npart; ipart++)
      {
        ...
        { // DIRK
          ...
          retval = ARKStepSetAccumulatedErrorType(arkode_mem,
                                                  accum_types[iaccum]);
          if (check_retval(&retval, "ARKStepSetAccumulatedErrorType", 1))
            return 1;
          retval = ARKStepResetAccumulatedError(arkode_mem);
          if (check_retval(&retval, "ARKStepResetAccumulatedError", 1))
            return 1;
        ...
        { // ERK
          ...
          retval = ERKStepSetAccumulatedErrorType(arkode_mem,
                                                  accum_types[iaccum]);
          if (check_retval(&retval, "ERKStepSetAccumulatedErrorType", 1))
            return 1;
          retval = ERKStepResetAccumulatedError(arkode_mem);
          if (check_retval(&retval, "ERKStepResetAccumulatedError", 1))
            return 1;

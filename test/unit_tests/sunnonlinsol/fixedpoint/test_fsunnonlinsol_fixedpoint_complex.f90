...
contains
  ...
  integer(c_int) function unit_tests() result(retval)
    ...
    call FN_VConst(complex(0.0d0, 0.0d0), ycor)

    ! set weights
    call FN_VConst(complex(1.0d0, 0.0d0), w)
    ...
    call FN_VLinearSum(complex(1.0d0, 0.d0), y0, complex(1.0d0, 0.0d0), ycor, ycur)
  ...
  integer(c_int) function FPFunction(ycor, f, mem) &
    ...
    f1 = (1.0d0/3.0d0)*cos((y - 1.0d0)*z) + (1.0d0/6.0d0)
    f2 = (1.0d0/9.0d0)*sqrt(x*x + sin(z) + 1.06d0) + 0.9d0
    f3 = -(1/20.d0)*exp(-x*(y - 1.0d0)) - (10.d0*PI - 3.0d0)/60.0d0

    fdata(1) = complex(f1, 0.0d0)
    fdata(2) = complex(f2, 0.0d0)
    fdata(3) = complex(f3, 0.0d0)

    call FN_VLinearSum(complex(1.0d0, 0.0d0), f, complex(-1.0d0, 0.0d0), y0, f)

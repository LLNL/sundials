...
int main(int argc, char* argv[])
{
  ...
  if (argc < 5)
  {
    if (rank == 0)
    {
      printf("ERROR: FOUR (4) Inputs required: matrix size, nprow, npcol, "
             "print timing \n");
    }
    return (1);
  }
  ...
  if ((M_loc * nprocs) != N)
  {
    if (grid.iam == (nprocs - 1)) { M_loc = N - M_loc * (nprocs - 1); }
  }
  ...
  if (grid.iam == 0)
  {
    ...
    xCreate_CompRowLoc_Matrix_dist(Asuper, N, N, NNZ_local, M_loc, fst_row,
  ...
  else
  {
    ...
    MPI_Recv(rowptrs, M_loc + 1, MPI_SUNINDEXTYPE, 0, grid.iam, grid.comm,
             &mpistatus);
    ...
    /* localize rowptrs */
    shift = rowptrs[0];
    for (i = 0; i < M_loc; i++) { rowptrs[i] = rowptrs[i] - shift; }
    rowptrs[M_loc] = NNZ_local;
  ...
  LS = SUNLinSol_SuperLUDIST(y, A, &grid, &lu, &scaleperm, &solve, &stat,
                             &options, sunctx);
  if (LS == NULL)
  {
    fails++;
  ...
  fails += Test_SUNLinSolSolve(LS, A, x, b, 100 * SUN_UNIT_ROUNDOFF, SUNTRUE,
                               grid.iam);

...
contains
  ...
  integer function smoke_tests() result(ret)
    ...
    complex(C_DOUBLE_COMPLEX)             :: cval               ! complex work value
    ...
    complex(C_DOUBLE_COMPLEX)             :: cnvarr(nv)         ! array of nv constants to go with vector array
    ...
    call FN_VCompare_Serial(real(ONE), x, y)
    ival = FN_VInvTest_Serial(x, y)
    ival = FN_VConstrMask_Serial(z, x, y)
    rval = FN_VMinQuotient_Serial(x, y)

    ! test fused vector operations
    ival = FN_VLinearCombination_Serial(nv, cnvarr, xvecs, x)
    ival = FN_VScaleAddMulti_Serial(nv, cnvarr, x, xvecs, zvecs)
    ival = FN_VDotProdMulti_Serial(nv, x, xvecs, cnvarr)

    ! test vector array operations
    ival = FN_VLinearSumVectorArray_Serial(nv, ONE, xvecs, ONE, xvecs, zvecs)
    ival = FN_VScaleVectorArray_Serial(nv, cnvarr, xvecs, zvecs)

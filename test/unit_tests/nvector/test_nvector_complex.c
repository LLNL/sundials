...
int Test_N_VMake_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  if (myid == 0) { printf("PASSED test -- N_VMake \n"); }
                               ...
                               int myid)
{
  ...
  {
    printf(">>> FAILED test -- N_VCloneVectorArray, Proc %d missing required "
           "operations\n",
           myid);
    return (1);
  }
...
int Test_N_VSetArrayPointer_Z(N_Vector W, sunindextype local_length, int myid)
{
  ...
  /* create vector data */
  Wdata = malloc(local_length * sizeof(sunscalartype));
...
int Test_N_VConst_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  set_element_range_Z(X, 0, local_length - 1, ZERO);
...
int Test_N_VMaxNorm_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  /* fill vector data */
  N_VConst(SUN_COMPLEX(ZERO, ZERO), X);
  ...
  if (failure)
  {
    ...
    fprintf(stderr, "  ans=%g, expected=%g\n", ans, TWO);
    fails++;
  }
                           ...
                           sunindextype local_length, int myid)
{
  ...
  N_VConst(ONE, ID);
  if (myid == 0) { set_element_Z(ID, local_length - 1, ZERO); }
...
int Test_N_VMin_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  /* fill vector data */
  N_VConst(TWO, X);
  if (myid == 0) { set_element_Z(X, local_length - 1, HALF); }
  else { set_element_Z(X, local_length - 1, ONE); }
  ...
  /* fill vector data */
  N_VConst(TWO, X);
  if (myid == 0) { set_element_Z(X, local_length - 1, NEG_TWO); }
  else { set_element_Z(X, local_length - 1, NEG_ONE); }
...
int Test_N_VCompare_Z(N_Vector X, N_Vector Z, sunindextype local_length, int myid)
{
  ...
  for (i = 0; i < local_length; i++)
  {
    set_element_Z(Z, i, NEG_ONE);

    switch (i % 3)
    {
    case 0:
      /* abs(X[i]) < c */
      set_element_Z(X, i, ZERO);
      break;

    case 1:
      /* abs(X[i]) = c */
      set_element_Z(X, i, NEG_ONE);
      break;

    case 2:
      /* abs(X[i]) > c */
      set_element_Z(X, i, NEG_TWO);
      break;
    }
  }
...
int Test_N_VInvTest_Z(N_Vector X, N_Vector Z, sunindextype local_length, int myid)
{
  ...
  if (local_length < 2)
  {
    printf("Error Test_N_VInvTest: Local vector length is %ld, length must be "
           ">= 2\n",
           (long int)local_length);
    return (1);
  }
                         ...
                         sunindextype local_length, int myid)
{
  ...
  for (i = 0; i < local_length; i++)
  {
    set_element_Z(M, i, NEG_ONE);

    switch (i % 7)
    {
    case 0:
      /* c = -2, test for < 0*/
      set_element_Z(C, i, NEG_TWO);
      set_element_Z(X, i, NEG_TWO);
      break;

    case 1:
      /* c = -1, test for <= 0 */
      set_element_Z(C, i, NEG_ONE);
      set_element_Z(X, i, NEG_ONE);
      break;
    ...
    case 3:
      /* c = 0, no test */
      set_element_Z(C, i, ZERO);
      set_element_Z(X, i, HALF);
      break;

    case 4:
      /* c = 1, test for == 0*/
      set_element_Z(C, i, ONE);
      set_element_Z(X, i, ZERO);
      break;
    ...
    case 5:
      /* c = 1, test for >= 0*/
      set_element_Z(C, i, ONE);
      set_element_Z(X, i, ONE);
      break;

    case 6:
      /* c = 2, test for > 0 */
      set_element_Z(C, i, TWO);
      set_element_Z(X, i, TWO);
      break;
    }
  }
                          ...
                          sunindextype local_length, int myid)
{
  ...
  N_VConst(TWO, NUM);
  N_VConst(TWO, DENOM);
  if (myid == 0) { set_element_Z(NUM, local_length - 1, HALF); }
  else { set_element_Z(NUM, local_length - 1, ONE); }
...
int Test_N_VDotProdMulti_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  if (ierr == 0)
  {
    failure = SUNRCompare(dotprods[0], (sunscalartype)global_length);
  }
  else { failure = 1; }
  ...
  if (ierr == 0)
  {
    failure = SUNRCompare(dotprods[0], (sunscalartype)-1 * global_length);
    failure += SUNRCompare(dotprods[1], (sunscalartype)global_length);
    failure += SUNRCompare(dotprods[2], (sunscalartype)2 * global_length);
  }
  else { failure = 1; }
 ...
 * --------------------------------------------------------------------*/
int Test_N_VLinearSumVectorArray_Z(N_Vector V, sunindextype local_length, int myid)
{
  int fails = 0, failure = 0, ierr = 0;
  double start_time, stop_time, maxt;
                                      ...
                                      int myid)
{
  ...
  /* use all elements except one */
  N_VConst(ONE, X);
  if (myid == 0) { set_element_Z(X, local_length - 1, ZERO); }
                           ...
                           int myid)
{
  ...
  set_element_range_Z(X, 0, local_length - 1, rmyid);
  set_element_range_Z(Y, 0, local_length - 1, locleninv);
...
int Test_N_VMaxNormLocal_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  /* fill vector data */
  myidp1 = (sunscalartype)(myid + 1);
  N_VConst(NEG_HALF, X);
  set_element_Z(X, local_length - 1, -myidp1);
...
int Test_N_VMinLocal_Z(N_Vector X, sunindextype local_length, int myid)
{
  int fails = 0, failure = 0;
  double start_time, stop_time, maxt;
  sunscalartype ans, negmyid;

  /* fill vector data */
  negmyid = (sunscalartype)-myid;
  N_VConst(TWO, X);
  set_element_Z(X, local_length - 1, negmyid);
...
int Test_N_VL1NormLocal_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  set_element_range_Z(X, 0, local_length - 1, val);
  ...
  failure = (ans < ZERO) ? 1
                         : SUNRCompareTol(ans, (sunscalartype)myid,
                           ...
                           int myid)
{
  ...
  set_element_range_Z(X, 0, local_length - 1, xval);
  set_element_range_Z(W, 0, local_length - 1, wval);
                               ...
                               sunindextype local_length, int myid)
{
  ...
  /* use all elements except one */
  N_VConst(ONE, ID);
  set_element_Z(ID, local_length - 1, ZERO);
                              ...
                              sunindextype local_length, int myid)
{
  ...
  if (local_length < 7)
  {
    printf("Error Test_N_VConstrMaskLocal: Local vector length is %ld, length "
           "must be >= 7\n",
           (long int)local_length);
    return (1);
  }
                               ...
                               sunindextype local_length, int myid)
{
  ...
  /* fill vector data */
  set_element_range_Z(NUM, 0, local_length - 1, TWO * (myid + 2));
  N_VConst(TWO, DENOM);
  set_element_Z(NUM, local_length - 1, TWO * myid);
  ...
  /* ans should equal myid */
  failure = SUNRCompareTol(ans, (sunscalartype)myid, SUNRsqrt(SUN_UNIT_ROUNDOFF));
...
int Test_N_VDotProdMultiLocal_Z(N_Vector X, sunindextype local_length, int myid)
{
  ...
  if (ierr == 0)
  {
    failure = SUNRCompare(dotprods[0], (sunscalartype)local_length);
  }
  else { failure = 1; }
  ...
  if (ierr == 0)
  {
    failure = SUNRCompare(dotprods[0], (sunscalartype)-1 * local_length);
    failure += SUNRCompare(dotprods[1], (sunscalartype)local_length);
    failure += SUNRCompare(dotprods[2], (sunscalartype)2 * local_length);
  }
  else { failure = 1; }
...
int Test_N_VBufUnpack_Z(N_Vector x, sunindextype local_length, int myid)
{
  ...
  for (i = 0; i < local_length; i++) { buf[i] = ONE; }

/*************************************************************************
 * File:       sensiwebpk.c                                              *
 * Written by: Steven L. Lee and Alan C. Hindmarsh @ LLNL                *
 * Version of: 17 September 2001                                         *
 *-----------------------------------------------------------------------*
 *
 * Example program, with sensitivity analysis.
 * SensIDA: Food web, parallel, GMRES, user preconditioner.
 *
 * This example program for SensIDA uses IDASPGMR as the linear solver.
 * It is written for a parallel computer system and uses a block-diagonal 
 * preconditioner (setup and solve routines) for the SensIDASPGMR package.
 ...
 *   a(i,j) =  EE  (i >  np,  j <= np)
 *   all other a(i,j) = 0
 *   b(i) = BB*(1+ p1 * x*y + p2*sin(4 pi x)*sin(4 pi y))  (i <= np)
 *   b(i) =-BB*(1+ p1 * x*y + p2*sin(4 pi x)*sin(4 pi y))  (i  > np)
 *   d(i) = DPREY  (i <= np)
 *   d(i) = DPRED  (i > np)
 *  
 ...
 * and so the system size Ny is the product MX * MY * NUM_SPECIES.
 ...
 * 
 * The DAE system is solved by SensIDA using the IDASPGMR linear solver, which
...
#define NY          (NUM_SPECIES*MX*MY) /* Number of equations in system */
#define NP          2
#define NS          2
...
typedef struct {
  real *p;
  integer Ny, ns, np, thispe, npes, ixsub, jysub, npex, npey,
...
static int resweb(integer Ny, real time, N_Vector cc, N_Vector cp,
                  N_Vector resval, void *rdata);

static int Precondbd(integer Ny, real tt, N_Vector cc, N_Vector cp,
...
static int PSolvebd(integer Ny, real tt, N_Vector cc, N_Vector cp,
                    N_Vector rr, real cj, ResFn res, void *rdata,
...
{
  integer thispe, npes, local_N;
  real rtol, atol, ropt[OPT_SIZE], t0, tout, tret;
  long int iopt[OPT_SIZE];
  N_Vector cc, cp, res, id;
  UserData webdata;
  int maxl, iout, flag, retval, i, j, itol, itask;
  boole optIn;
  void *mem;
  IDAMem idamem;
  MPI_Comm comm;
  N_Vector *ccsub, *cpsub, *ressub, *idsub;
  integer Ntotal;
  real *pbar;
  real rhomax;
  int *plist;
  ...
  cc  = N_VNew(Ntotal, machEnv);
  cp  = N_VNew(Ntotal, machEnv);
  ...
  webdata->p = malloc(NP * sizeof(real));
  webdata->p[0] = 50.0;
  webdata->p[1] = 1000.0;
  ...
  SetInitialProfiles(ccsub[0], cpsub[0], idsub[0], ressub[0], webdata);
  N_VFree(res);
  ...
  /* Identify sensitivity variables as differential or algebraic */
  for (i = 1; i <= NS; i++)
    N_VScale(ONE, idsub[0], idsub[i]);

  /* Set remaining inputs to SensIDAMalloc. */
  t0 = ZERO;
  itol = SS; rtol = RTOL; atol = ATOL;
  optIn = FALSE;

  /* Call SensIDAMalloc to initialize SensIDA.
  ...
  mem = SensIDAMalloc(NY, NS, Ntotal, resweb, webdata, t0, cc, cp,
		      itol, &rtol, &atol, id, NULL, NULL, optIn,
		      iopt, ropt, machEnv, webdata->p, pbar, plist, rhomax);
  ...
  /* Call SensIDASpgmr to specify the linear solver IDASPGMR and specify
     the preconditioner routines supplied (Precondbd and PSolvebd).
     Optional input maxl (max. Krylov subspace dim.) is set to 10.   */
  maxl = 10;
  retval = SensIDASpgmr(idamem, Precondbd, PSolvebd, MODIFIED_GS,
			maxl, 0, ZERO, ZERO, webdata);

  if (retval != SUCCESS) {
    if (thispe == 0) printf("SensIDASpgmr call failed, returning %d \n",retval);
    return(1); 
  }
  ...
  if (thispe == 0) {
    printf("senswebpk: Predator-prey DAE parallel example problem for SensIDA \n\n");
...
static UserData AllocUserData(machEnvType machEnv)
{
  ...
  webdata->rates = N_VNew(NY, machEnv);
                               ...
                               N_Vector res, UserData webdata)
{
  ...
  N_VConst(ZERO, cp);
  resweb(webdata->Ny, ZERO, cc, cp, res, webdata);
  N_VScale(-ONE, res, cp);
...
static int resweb(integer Ny, real tt, N_Vector cc, N_Vector cp, 
                     ...
                     UserData webdata)
{
  ...
  fac = ONE + p1*xx*yy + p2*sin(FOURPI*xx)*sin(FOURPI*yy);

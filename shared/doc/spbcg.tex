...
The {\spbcg} package, in the files \id{sundials\_spbcg.h} and \id{sundials\_spbcg.c}, includes an
implementation of the scaled preconditioned Bi-CGStab\index{Bi-CGStab method} method.  
For full details, including usage instructions, see the file \id{sundials\_spbcg.h}.
\index{SPBCG@{\spbcg} generic linear solver!description of|)}

The files needed to use the {\spbcg} module by itself are the same as for the
{\spgmr} module, with \id{sundials\_spbcg.(h,c)} replacing \id{sundials\_spgmr.(h,c)}.

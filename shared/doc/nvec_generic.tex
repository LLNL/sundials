...
%
\id{N\_VNew} & \id{v = N\_VNew(machEnv);} \\ 
& Returns a new \id{N\_Vector} build with information in \id{machEnv}. 
If there is not enough memory for a new \id{N\_Vector}, then \id{N\_VNew} 
returns \id{NULL}.
\\
%
\id{N\_VSpace} & \id{N\_VSpace(machEnv, \&lrw, \&liw);} \\
...
\\
%
\id{N\_VNew\_S} & \id{vs = N\_VNew\_S(ns, machEnv);} \\
...
\\
%
\id{N\_VMake} & \id{v = N\_VMake(vdata, machEnv);} \\

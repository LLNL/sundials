...
operations defined by a generic vector specification object (of type \Id{NV\_Spec}).

The generic \ID{NV\_Spec} type is a pointer to a structure that has an
...
The type \id{NV\_Spec} is defined as
\begin{verbatim}
...
pointing to the \id{NV\_Spec} structure that was used in constructing the
vector.
...
field of the structure pointed to by \id{NV\_Spec}. To illustrate this point we
...
\begin{itemize}
\item specify the {\em content} fields of \id{NV\_Spec} and \id{N\_Vector};
\item define and implement the vector kernels. Note that the kernel routine 
  ...
  vector routines \id{N\_VNew\_S} and \id{N\_VFree\_S}, used to allocate and
  free an array of \id{N\_Vector}'s for forward sensitivity analysis, need not
  be defined by the {\nvector} implementation as they rely completely
  on the \id{N\_VNew} and \id{N\_VFree} routines;
\item define and implement user-callable constructor and destructor
  routines to generate and free a variable of type \id{NV\_Spec} with
...
%
\id{N\_VNew} & \id{v = N\_VNew(nvSpec);} \\ 
& Returns a new \id{N\_Vector} built with information in \id{nvSpec}. 
...
\id{lrw} contains the number of \id{realtype} words and \id{liw}
contains the number of \id{integertype} words.
\\
%
\id{N\_VNew\_S} & \id{vs = N\_VNew\_S(ns, nvSpec);} \\
& Returns an array of \id{ns} new \id{N\_Vector}'s.
The parameter \id{nvSpec} is a pointer to machine environment 
...
returns \id{NULL}. Note that this routine is common to all {\nvector}
implementations as it is based on the routine \id{N\_VNew}. A new {\nvector}
...
implementations as it's based on the routine \id{N\_VNew}. A new {\nvector}
implementation need not implement this routine.
\\
%
\id{N\_VMake} & \id{v = N\_VMake(vdata, nvSpec);} \\
& Creates an \id{N\_Vector} with component array data 

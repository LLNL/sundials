...
\item \ID{N\_VCloneEmpty\_Serial}
...
\item \ID{N\_VNewVectorArrayEmpty\_Serial}

 This function creates an array of \id{count} serial vectors, each with an empty (\id{NULL})
 data array.
...
\item \ID{N\_VDestroyVectorArray\_Serial}
 ...
 This function frees memory allocated for the array of \id{count} serial \id{N\_Vector} 
 created with \id{N\_VNewVectorArray\_Serial} or with \id{N\_VNewVectorArrayEmpty\_Serial}.
...
\item
  The constructor functions \id{N\_VNewEmpty\_Serial}, \id{N\_VCloneEmpty\_Serial},
  \id{N\_VMake\_Serial}, and \id{N\_VNewVectorArrayEmpty\_Serial}
  set the field {\em own\_data}$=$\id{FALSE}. 
  The destructor functions \id{N\_VDestroy\_Serial} and \id{N\_VDestroyVectorArray\_Serial}
  will not attempt to free the pointer {\em data} for any \if{N\_Vector} with

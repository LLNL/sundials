...
defines the {\em content} field of \id{M\_Env} to be a structure 
...
\end{verbatim}
The {\em tag} field of \id{M\_Env} is set to \id{serial}.
...
The {\nvecs} implementation provides user-callable routines \ID{M\_EnvInit\_Serial} 
to create a structure of type \id{M\_Env} whose {\em content} field is 
of type \id{struct \_M\_EnvSerialContent}, and \ID{M\_EnvFree\_Serial} 
to deallocate the space used by such a structure. 

\noindent The form of the call to \id{M\_EnvInit\_Serial} is
\begin{verbatim}
machenv = M_EnvInit_Serial(vec_length);
\end{verbatim}
If successful, \id{M\_EnvInit\_Serial} returns a pointer of type
\id{M\_Env}. This pointer should in turn be passed in any user
calls to \id{N\_VNew} to create a new \id{N\_Vector} of this type.
A machine environemnt variable \id{machenv} returned by \id{M\_EnvInit\_Serial}
...
that can be used to access the contents of \id{M\_Env} and \id{N\_Vector}
...
\item \ID{ME\_CONTENT\_S}, \ID{NV\_CONTENT\_S}

      These macros give access to the contents of the serial 
      machine environment and \id{N\_Vector}, respectively.           
                                                               
      The assignment \id{m\_cont = ME\_CONTENT\_S(machenv)} sets       
      \id{m\_cont} to be a pointer to the serial machine             
      environment content structure (of type \id{struct \_M\_EnvSerialContent}).
...
\item \ID{NV\_DATA\_S}, \ID{NV\_LENGTH\_S}
      ...
      a pointer to the first component of \id{v}. The assignment    
      \id{NV\_DATA\_S(v) = v\_data} sets the component array of \id{v} to     
...
\item \ID{NV\_MAKE\_S}, \ID{NV\_DISPOSE\_S}
      ...
      The call \id{NV\_MAKE\_S(v,v\_data,machenv)} makes \id{v} an        
      \id{N\_Vector} with component array \id{v\_data}. The length of the  
      array is taken from \id{machenv}.                             
...
\item \ID{NVS\_MAKE\_S}, \ID{NVS\_DISPOSE\_S}
      ...
      The call \id{NVS\_MAKE\_S(vs,vs\_data,ns,machenv)} makes   
      \id{vs} an array of \id{ns} \id{N\_Vector}'s, with \id{vs[i]} 
      having component array \id{vs\_data[i]} and length taken from \id{machenv}.    
...
\item
  ...
  arguments that were all created with the \id{M\_Env} structure returned
  by \id{M\_EnvInit\_Serial}.

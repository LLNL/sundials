...
\end{verbatim}
The {\em tag} field of \id{M\_Env} is set to \id{parallel}.
...
The {\nvecp} implementation provides user-callable routines \ID{M\_EnvInit\_Parallel} 
to create a structure of type \id{M\_Env} whose {\em content} field is
of type \id{struct \_M\_EnvParallelContent}, and \ID{M\_EnvFree\_Parallel} 
to deallocate the space used by such a structure. 

\noindent The form of the call to \id{M\_EnvInit\_Parallel} is
\begin{verbatim}
...
\item \id{argc}, \id{argv} are the command line arguments count
  ...
  already been called.
\end{itemize}
If successful, \id{M\_EnvInit\_Parallel} returns a pointer of type
\id{M\_Env}. This pointer should in turn be passed in any user
calls to \id{N\_VNew} to create a new \id{N\_Vector} of this type.
A machine environemnt variable \id{machenv} returned by \id{M\_EnvInit\_Parallel}
...
\item \ID{ME\_CONTENT\_P}, \ID{NV\_CONTENT\_P}

      These macros give access to the contents of the parallel
      machine environment and \id{N\_Vector}, respectively.           
                                                               
      The assignment \id{m\_cont = ME\_CONTENT\_P(machenv)} sets       
      \id{m\_cont} to be a pointer to the parallel machine             
      environment content structure (of type \id{struct \_M\_EnvParallelContent}).
...
\item \ID{NV\_MAKE\_P}, \ID{NV\_DISPOSE\_P}
      ...
      The call \id{NV\_MAKE\_P(v,v\_data,machenv)} makes \id{v} an        
      \id{N\_Vector} with local component array \id{v\_data}. 
      The local and global lengths of the  
      vector \id{v} is taken from \id{machenv}.                             
...
\item \ID{NVS\_MAKE\_P}, \ID{NVS\_DISPOSE\_P}
      ...
      The call \id{NVS\_MAKE\_P(vs,vs\_data,ns,machenv)} makes   

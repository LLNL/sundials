...
{\nvector} & \id{libsundials\_nvecserial.{\em lib}} & \id{nvector\_serial.h} \\
           ...
           & \id{libsundials\_fnvecserial.{\em lib}} &  \\
           & \id{libsundials\_fnvecparallel.{\em lib}} &  \\
\hline
...
{\mpi}-enabled functions. Although the \id{--disable-examples} option disables
the examples (which means a {\F} compiler is not required), the
\id{--with-mpi-libs} option is still needed so that the configure

...
is provided below.
The various macro names defined in \id{sundials\_config.h} have the following uses:
\begin{itemize}

\item precision of the {\sundials} \id{realtype} type\\
  Only one of the macro names \id{SUNDIALS\_DOUBLE\_PRECISION}, \id{SUNDIALS\_SINGLE\_PRECISION},
  and \id{SUNDIALS\_EXTENDED\_PRECISION} should be defined indicating double, single, and
  extended precision, respectively.

\item use of generic math functions\\
  If the macro name \id{SUNDIALS\_USE\_GENERIC\_MATH} is defined, the functions
  in \id{sundials\_math.(h,c)} will use the standard \id{pow}, \id{sqrt}, and \id{fabs}
  functions from \id{math.h}, no matter what the precision of \id{realtype} is.
  Otherwise, if \id{realtype} is defined to be \id{float}, these functions
  will use the functions \id{powf}, \id{sqrtf}, and \id{fabsf} which are not ANSI-C standard
  (but are C99 requirements). If \id{realtype} is defined to be \id{long double}, these functions
  will use the functions \id{powl}, \id{sqrtl}, and \id{fabsl} which are not ANSI-C standard
  (but are C99 requirements).

\item {\F} name-mangling scheme\\
  These macro names are used to decide how {\F} routine names in the
  source code are mapped to names in the object library (relevant only when
  using the {\F}-{\C} interface modules). This can be acheived either by defining the
  {\em function-like} macros
  \begin{itemize}
  \item \id{F77\_FUNC(name,NAME)}
  \item \id{F77\_FUNC\_(name,NAME)}
  \end{itemize}
  or by specifying the case of function names and the number of underscores appended to 
  function names by defining the appropriate {\em object-like} macros from the following lists:
  \begin{itemize}
  \item one of \id{SUNDIALS\_CASE\_LOWER} or \id{SUNDIALS\_CASE\_UPPER}
  \item one of \id{SUNDIALS\_UNDERSCORE\_NONE}, \id{SUNDIALS\_UNDERSCORE\_ONE}, or\\
    \id{SUNDIALS\_UNDERSCORE\_TWO}
  \end{itemize}
  
\item use of an {\mpi} communicator other than \id{MPI\_WORLD\_COMM} in {\F}\\
  If the macro name \id{SUNDIALS\_MPI\_COMM\_F2C} is defined then the {\mpi} implementation
  used defines the type \id{MPI\_Fint} and the function \id{MPI\_Comm\_f2c} and using
  {\mpi} communicators other than \id{MPI\_WORLD\_COMM} is possible with the
  {\sundials} {\F}-{\C} interface modules.
\end{itemize}

...
\item \id{--with-precision=ARG}
  ...
  {\warn} Users should {\em not} install {\sundials} in single precision 
  on a 32-bit (or similar) machine.  This will almost certainly result in
  unreliable numerical solutions.  The option \id{ARG=single} is intended for
  machines on which single precision arithmetic involves at least 14 decimal
  digits.
...
Using the default SUNDIALS build system, the command\\
\id{\% make install}\\
will install the library files in a directory {\em libdir} and the public
header files in a directory {\em incdir}. The default values for these directories
are {\em build\_tree}\id{/lib} and {\em build\_tree}\id{/include}, but these can
be changed using the configuration options \id{--prefix}, \id{--includedir}, and
\id{--libdir} (see \S\ref{ss:configuration_options}). For example, a system-wide install
of {\sundials} could be done using\\
\id{\% configure --prefix=/usr/local}\\
The public header files are further organized in subdirectories under {\em incdir}.
...
files from the directory {\em incdir}\id{/sundials} since they are included by the
appropriate solver header files (for example, \id{cvode\_dense.h} includes 
\id{sundials\_dense.h}). However, it is both legal and safe to do so either for
clarity or if needed (for example, the functions declared in \id{sundials\_smalldense} 
...
\caption{
  ...
  for library files and to {\em incdir}/ for header files)
...
\cline{2-4}
          ...
          &              & sundials/sunials\_dense.h         & sundials/sundials\_smalldense.h   \\
...
\section{Building SUNDIALS without the configure script}

If the \id{configure} script cannot be used (for example when building {\sundials}
under Windows without using \id{cygwin}) or if the users desires to own the build
process (for example when {\sundials} is incorporated into a larger project with its
own build system) the header and source files for the desired modules can be 
copied from the {\em source\_tree} in some other location(s) and compiled separately.

To compile one of the {\sundials} solvers the necessary files are as follows:
\begin{itemize}
\item the solver's public header files located in 
{\em src\_tree}\id{/}{\em solver}\id{/include}
\item the solver's implementation header files and sources located in
{\em src\_tree}\id{/}{\em solver}\id{/source}
\item (optionally and if present) the solver's Fortran-C interface files located in
{\em src\_tree}\id{/}{\em solver}\id{/fcmix}
\item the shared public header files located in
{\em src\_tree}\id{/shared/include}
\item the shared sources located in
{\em src\_tree}\id{/shared/source}
\item (optionally) the {\nvecs} header and source files located in
{\em src\_tree}\id{/nvec\_ser}
\item (optionally) the {\nvecp} header and source files located in
{\em src\_tree}\id{/nvec\_par}
\end{itemize}